// app/api/cron/plays-live/cfbPlaysAll.test.mjs - CFB_PLAYS_ALL, composed.
//
...
// the alert sender is stubbed, so this can neither spend the real budget nor
// send mail. DEV holds no live game of its own (checked 25 Sep 2026); the
...
install();
const ALERTS = stubPath('__alerts_stub.mjs');
writeFileSync(ALERTS, 'export const sent = []; export async function maybeAlert(sql, a) { sent.push(a); return { sent: false }; }\n');
...
after(async () => {
  ...
  try { unlinkSync(ALERTS); } catch { /* gone */ }
});
...
test('FLAG OFF: main\'s scope - no board, no poll, no CFBD call, no plays', async () => {
  ...
  const { liveBoardGames } = await import('../../../../lib/pollers/playsScope.js');
