        ...
        <section className="mod mod--entered">
          ...
          {roster.length ? (
            <>
              ...
              <div>
                {(card?.rows ?? roster.map((r) => ({ ...r, key: r.ffc ?? r.id, counting: true, state: null }))).map((r) => (
                  <div className={`row${r.counting ? '' : ' row--dropped'}`} key={r.key} data-game={r.state?.kind}>
                    <span>
                      <span className="slot-tag">R{r.round}</span> {r.name}
                      {/* THE SIX THAT COUNT ARE MARKED, not merely the two
                          that do not. A reader should not have to infer
                          membership from the absence of dimming. */}
                ...
                {/* .row .r is white-space:nowrap so the numbers never break;
                    this line is prose, and without r--wrap it ran off the
                    right edge of a phone instead of wrapping. */}
