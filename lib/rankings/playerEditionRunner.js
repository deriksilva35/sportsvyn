  ...
  const inserted = await sql`
    ...
    new_entries AS (
      INSERT INTO ranking_entries (
        ...
        output_score, impact_score, impact_scored_against_fingerprint
      )
      SELECT (SELECT id FROM new_ed), 'player',
             ...
             NULLIF(e->>'impact_fp', 'null')::int   -- Phase 3 stamp
...
// ============================================================================
// loadPriorByPlayerId -- Map<player_id, {rank, score, impact_score,
// impact_scored_against_fingerprint}> from the current is_current edition's
// ranking_entries. Used by the scheduler to compute real movement on ed2+
// AND to enable the impact-cache freshness check.
//
// Returns an empty Map when no prior edition exists (ed1 case).
//
// Phase 3 fix note: the impact_scored_against_fingerprint column is the
// Phase 3 addition (migration 036). A NULL fingerprint means the prior
// edition predates the fix and its cached impact must be re-scored fresh
// (the safe direction -- never reuse an unstamped cache).
export async function loadPriorByPlayerId({ sql, listSlug, leagueSlug }) {
  ...
  for (const r of rows) {
    map.set(r.player_id, {
      ...
      impact_scored_against_fingerprint: r.impact_fp,
    });
  }
  return map;
}
