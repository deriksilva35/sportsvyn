...
//
// IDEMPOTENT ON THE INPUT WEEK, NOT ON THE DAY. An edition is identified by
// the week it is computed FROM - the NFL's newest completed REG week, the AP's
// newest poll week for CFB. Publishing again for the SAME week REPLACES that
// edition, keeps its number, and recomputes movement against the edition
// BEFORE it. A new edition number appears only when the week advances.
//
// WHY THE WEEK AND NOT THE DAY, which is what this used to key on. The cron
// fires weekly but a board gets republished for other reasons - a retry, a
// fixed editor file, a corrected constant - and those can land on a different
// UTC day than the first attempt. Keyed on the day, a Tuesday re-run of
// Monday's board became edition 2 and reported movement against itself: every
// arrow a zero, for a week in which no game had been played. Keyed on the week,
// the same board is the same edition however many times and on whatever days it
// is written, and an arrow can only mean a week's worth of football.
//
// THE KEY IS STORED, NOT INFERRED. ranking_editions has no column for it, so
// forWeek rides in the notes blob (the same place every World Cup edition keeps
// its parameters) and loadPriorComputedEdition parses it back. An edition
// written before this existed has no forWeek, and those fall back to the old
// same-UTC-day comparison rather than being treated as a different week - which
// would have duplicated them once and then settled down.
//
...
/**
 * EVERY RANK-TO-SCORE CONVERSION USES THE PUBLISHED FIELD SIZE. Not the length
 * of the list the rank came from - the size of the board the score is going
 * to be compared ON.
 *
 * THIS WAS THE BUG THE FIRST DRY RUN FOUND, and it is worth stating plainly
 * because the wrong answer is the intuitive one. A rank of 20 in a 25-team
 * list scored 2.46 over a field of 25 - a whisker off the floor, because it is
 * near the bottom OF THAT LIST. But the result dimension it gets averaged with
 * is a z-score over all 138 teams, where the same team scores 5.9. So a team
 * the editor deliberately ranked 20th finished 97th of 138, BELOW teams nobody
 * ranked at all: being on the list was worse than being off it. A #25 team is
 * in the top 18% of the country and the list-length curve scored it as nearly
 * the worst thing in football.
 *
 * Both conversions - the AP rank and the editor's rank - therefore take the
 * PUBLISHED field size, which is 138 for CFB and 32 for the NFL. The same rank
 * scores identically through both, because it is the same curve over the same
 * field asking the same question; the only difference is who wrote the rank
 * down. publishGridironEdition.test.mjs pins both halves of that, including
 * that neither curve is ever handed a list length again.
 *
 * It is DERIVED, not a constant, so a league with a different number of teams
 * is right by construction rather than by somebody remembering.
 */
export function publishedFieldSize(teams = []) {
  return (teams ?? []).filter((t) => String(t?.classification ?? '').toLowerCase() !== 'fcs').length;
}
...
/** The anchor is a rank-to-score conversion too, so it takes the same field. */
export function anchorElo(apRank, fieldSize) {
  const score = normalizeRankToScore(apRank, fieldSize);
 ...
 */
export async function buildApAnchors(league, fieldSize) {
  ...
  const out = new Map();
  for (const r of rows) {
    const elo = anchorElo(Number(r.rank), fieldSize);
...
export async function loadPriorComputedEdition(listSlug) {
  const [ed] = await sql`
    SELECT rd.id, rd.edition_number, rd.published_at, rd.notes
...
/**
 * The week an edition was computed from, out of its notes blob. NULL for an
 * edition written before the key existed, and null on anything unparseable -
 * notes is free text and a reader of it must never throw over a board.
 */
export function parseForWeek(notes) {
  if (notes == null) return null;
  try {
    const j = typeof notes === 'string' ? JSON.parse(notes) : notes;
    const w = j?.forWeek;
    if (w?.season == null || w?.week == null) return null;
    return { season: Number(w.season), week: Number(w.week) };
  } catch { return null; }
}

/** Same season, same week. Null on either side is never a match. */
export function sameWeek(a, b) {
  if (a?.season == null || a?.week == null || b?.season == null || b?.week == null) return false;
  return Number(a.season) === Number(b.season) && Number(a.week) === Number(b.week);
}

/**
 * WHICH EDITION SLOT THIS PUBLISH TAKES. PURE, so the ruling is testable
 * without a database and so the two branches can be read side by side.
 *
 * @returns { replaces, editionNumber, reason }
 *   replaces true  -> delete the prior edition, reuse its number, and take
 *                     movement from the edition BEFORE it
 *   replaces false -> a fresh number, movement from the prior edition
 */
export function editionSlotFor({ prior = null, forWeek = null, lastNumber = -1, now = new Date() } = {}) {
  if (!prior) return { replaces: false, editionNumber: lastNumber + 1, reason: 'first' };
  if (sameWeek(prior.forWeek, forWeek)) {
    return { replaces: true, editionNumber: prior.editionNumber, reason: 'same-week' };
  }
  // THE COMPATIBILITY BRANCH, and it is narrow on purpose: only an edition with
  // NO recorded week falls back to the day, and only for as long as one exists.
  // Without it the first publish after this change would duplicate a board that
  // is plainly the same board.
  if (prior.forWeek == null && prior.publishedAt != null
      && utcDay(prior.publishedAt) === utcDay(now)) {
    return { replaces: true, editionNumber: prior.editionNumber, reason: 'same-day-legacy' };
  }
  return { replaces: false, editionNumber: lastNumber + 1, reason: 'week-advanced' };
}
...
/**
 * THE WEEK THIS EDITION IS FOR, for a league with no poll to name it.
 *
 * THE DEFECT THIS CLOSES. The editor file was keyed off the AP poll's season
 * and week - fine for CFB, and SILENTLY WRONG for the NFL, which has no poll:
 * usesAp is false, so ap.season and ap.week are null, editorFilePath() returns
 * null, and publish('nfl') read no editor file at all. The file would have been
 * committed, ignored, and nothing would have said so - the board would simply
 * have published on the model alone and looked fine.
 *
 * THE NEWEST FULLY-FINAL REG WEEK, which is the week the board actually grades.
 * Not the nearest upcoming week: lib/gridiron/leagueWeek.js resolveLeagueWeek()
 * answers "where is the league now" for a header and would say week 3 while
 * week 3 has not been played. A week with one game still in flight is not a
 * week this board can be about, so a week is only complete when NOTHING in it
 * is unfinal - which is the same gate lib/pickem/settle.js uses on a board.
 *
 * A LEAGUE WITH A POLL KEEPS USING THE POLL'S WEEK, and that is not an
 * inconsistency. The AP's week number is forward-looking by construction - week
 * 4's poll is published before week 4 is played - so for CFB the poll week IS
 * the label the board carries, and cfb-2026-w4.md is named for the poll that
 * fed it. Changing that would orphan a file and a published edition.
 */
export async function completedWeek(league) {
  const [row] = await sql`
    SELECT m.season_year AS season, m.week
      FROM matches m
      JOIN leagues l ON l.id = m.league_id AND l.slug = ${league}
     WHERE m.season_phase = 'REG' AND m.week IS NOT NULL
     GROUP BY m.season_year, m.week
    HAVING count(*) FILTER (WHERE m.status <> 'final') = 0
     ORDER BY m.season_year DESC, m.week DESC
     LIMIT 1`;
  return row ? { season: Number(row.season), week: Number(row.week) } : { season: null, week: null };
}
 ...
 */
function rankBand(rows, startAt) {
  const sorted = [...rows].sort((a, b) => (b.score - a.score) || (a.teamId - b.teamId));
  let rank = startAt; let seen = startAt; let last = null;
...
/**
 * THE EDITOR'S LIST IS A GATE, not a nudge.
 *
 * The twenty-five teams on it take ranks 1-25, ordered among THEMSELVES by
 * composite; everybody else ranks from 26 down, ordered among themselves by
 * composite. Ties share inside each band exactly as they did before.
 *
 * WHY A GATE AND NOT A BLEND, given the composite already carries the editor's
 * score. Because a blend can be overwhelmed. With the field-size fix the
 * editor's 25 all score high, but a team nobody listed and the model loves -
 * James Madison, Duke, Vanderbilt, none of them ranked by the editor OR the AP
 * - could still finish third, seventh and ninth on a strong Elo. That is the
 * model overruling the editor on the only question the editor was asked. The
 * gate makes the list mean what a list means: these are the twenty-five, and
 * the order inside them is where the model gets its say.
 *
 * THE MODEL IS NOT SILENCED, it is relocated: the highest team the editor left
 * off is named under the board as "The model's case", by ELO, so the argument
 * against the gate is published beside it every week.
 *
 * NO LIST IS NO GATE. The NFL has no editor file, so every team is "unlisted"
 * and the whole board ranks from 1 by composite - identical to the behaviour
 * before this existed.
 */
export function rankBySort(rows, { listed = null } = {}) {
  if (!listed || listed.size === 0) return rankBand(rows, 1);
  const inList = rows.filter((r) => listed.has(r.teamId));
  const rest = rows.filter((r) => !listed.has(r.teamId));
  // THE SECOND BAND STARTS AT listed.length + 1, not at "one past whatever
  // rank the first band happened to end on". A tie inside the top band ends it
  // early - two teams sharing 24 leaves 25 unused - and the unlisted field
  // must still start at 26, or a tie among the editor's 25 would promote an
  // unlisted team INTO the list's range.
  return [...rankBand(inList, 1), ...rankBand(rest, inList.length + 1)];
}
...
export function buildEntries({
  table, teams = [], apByTeam = new Map(), editorByTeam = new Map(), fieldSize = null, cfg,
}) {
  ...
  // THE PUBLISHED FIELD. Every rank-to-score conversion below uses it - see
  // publishedFieldSize()'s header for the defect that taught us why.
  const N = fieldSize ?? rated.length;
  ...
  for (const r of rated) {
    ...
    const apScore = apRank == null ? null : normalizeRankToScore(apRank, N);
    scorerResults.push({
  ...
  const rows = composed.map((c) => {
    ...
    return {
      ...
      inputs: {
        ...
        delta3: d.delta3,
        last3: d.last3,
        // THE FIELD EVERY CURVE ON THIS ROW WAS TAKEN OVER. Stored, not
        // described in prose, because the panel shipped a caption reading
        // "curved over a 25-team field" for one deploy AFTER the ruling moved
        // it to 138 - a sentence that had been true and quietly stopped being
        // so. A number the row carries cannot go stale against the row.
        field: N,
  ...
  return rankBySort(rows, { listed: new Set(editorByTeam.keys()) });
}
...
export async function publish({ league, apply = false, now = new Date() } = {}) {
  ...
  const fieldSize = publishedFieldSize(teams);
  const anchors = await buildApAnchors(league, fieldSize);
  const table = runElo({ games, k: cfg.k, hfa: cfg.hfa, regress: cfg.regress, anchors });
  // THE EDITOR'S LIST, keyed to the edition being published. A league WITH a
  // poll is named by the poll's week; a league without one is named by the
  // newest week it has finished playing. See completedWeek() for why those are
  // different questions and why reading the poll for both published the NFL
  // board with no editor layer and no complaint.
  const forWeek = cfg.usesAp ? { season: ap.season, week: ap.week } : await completedWeek(league);
  const editorUrl = editorFilePath(league, forWeek.season, forWeek.week);
  const editorText = editorUrl && existsSync(editorUrl) ? readFileSync(editorUrl, 'utf8') : null;
  const editor = buildEditorLayer({ text: editorText, teams, fieldSize });
  const prior = await loadPriorComputedEdition(cfg.listSlug);
  const rows = applyMovement(
    buildEntries({ table, teams, apByTeam: ap.byTeam, editorByTeam: editor.byTeam, fieldSize, cfg }),
    prior);
  ...
  const summary = {
    ...
    weights: { editorial: cfg.editorialWeight, sites: cfg.sitesWeight },
    prior: prior
      ? { editionNumber: prior.editionNumber, forWeek: prior.forWeek, isToday: prior.isToday }
      : null,
    // WHAT WOULD HAPPEN IF THIS WERE APPLIED, computed on the dry run too, so
    // the decision is visible before it is taken rather than only after.
    slot: editionSlotFor({ prior, forWeek, lastNumber: -1, now }).reason,
    applied: false,
  };
  ...
  // IDEMPOTENT ON THE WEEK. Republishing the same week deletes that edition and
  // REUSES ITS NUMBER, so a retry or a corrected editor file does not leave two
  // editions for one week of football. Movement was already taken from `prior`,
  // which in that case IS the edition being replaced - it would report every
  // team as having moved zero places since this morning - so it is recomputed
  // against the edition BEFORE it.
  const slot = editionSlotFor({
    prior, forWeek, lastNumber: await loadLastEditionNumber(cfg.listSlug), now,
  });
  const editionNumber = slot.editionNumber;
  summary.slot = slot.reason;
  if (slot.replaces) {
  ...
  const [edition] = await sql`
    ...
    VALUES (${list.id}, ${editionNumber}, ${editionLabel(league, forWeek, now)}, ${METHODOLOGY_VERSION},
            ${cfg.editorialWeight}, ${cfg.sitesWeight}, 0.00, 'published', false, now(),
            ${JSON.stringify({
    // forWeek IS THE EDITION'S IDENTITY and is written first because
    // everything else in this blob is a parameter, not a key.
    forWeek,
    k: cfg.k, hfa: cfg.hfa, regress: cfg.regress, games: games.length,
    ap: summary.ap, editor: summary.editor,
  })})
    RETURNING id`;
...
/**
 * THE LABEL NAMES THE WEEK FIRST, because the week is what the edition IS.
 * "Week 2 · computed 2026-09-22". It used to lead with the day, which stopped
 * being the identity the moment idempotency moved onto the week: a Wednesday
 * re-run of week 2 is still week 2, and a label reading "Computed 2026-09-23"
 * on edition 1 invited the reader to think a week had passed. Both facts are
 * kept - the week it is about, and the day it was last computed.
 */
export function editionLabel(league, forWeek = null, now = new Date()) {
  const day = utcDay(now);
  const week = forWeek?.week == null
    ? null
    : (LEAGUE_CONFIG[league]?.usesAp ? `AP week ${forWeek.week}` : `Week ${forWeek.week}`);
  return week ? `${week} · computed ${day}` : `Computed ${day}`;
}
