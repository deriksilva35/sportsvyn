...
export const DEFAULT_PARAMS = Object.freeze({
  w_production:     0.60,
  w_impact:         0.40,
  // C': scale 0.9 + cap 10 keeps production on the same 0-10 axis as impact
  // (so the 0.60/0.40 blend stays meaningful) while the cap only binds at
  // raw ~11 (above any realistic Golden Boot) — the old 2.6/9.5 clipped every
  // 4-goal scorer to 9.5 and let impact alone order the elite tier.
  ...
  w_assist:         0.6,
  w_red:            0.5,
  // Match-stakes weight: a goal/assist in a BOTH-SECURED dead rubber (both
  // teams already through to the knockouts at kickoff) counts at this factor.
  // Knockout + live group strikes are full weight (1.0). See lib/rankings/
  // deadRubbers.js for the structural derivation. 0.7 = shade modestly, not halve.
  dead_factor:      0.7,
  candidate_pool:   50,
});
...
// ============================================================================
export async function rollupProductionForLeague({ sql, leagueSlug, deadMatchIds = [], candidatePool = DEFAULT_PARAMS.candidate_pool, params = DEFAULT_PARAMS }) {
  // Per-event match-stakes weighting: each event carries a `stakes` factor —
  // dead_factor when its match is a both-secured dead rubber, else 1.0. We sum
  // the stakes per event type (weighted contributions) for production, and ALSO
  // keep the raw integer counts for display (goals/assists shown on the board).
  const dead = deadMatchIds.length ? deadMatchIds : [-1]; // ANY([]) is invalid; -1 never matches
  const rows = await sql`
    WITH wc_events AS (
      SELECT me.*,
             CASE WHEN me.match_id = ANY(${dead}::int[]) THEN ${params.dead_factor}::numeric ELSE 1.0 END AS stakes
    ...
    goals_cards AS (
      SELECT p.id AS player_id, p.full_name, p.known_as, p.position,
             ...
             COALESCE(SUM(e.stakes) FILTER (WHERE e.event_type='Goal' AND e.detail NOT IN ('Own Goal','Missed Penalty','Goal cancelled','Penalty')), 0)::float AS open_play_weight,
             COALESCE(SUM(e.stakes) FILTER (WHERE e.event_type='Goal' AND e.detail='Penalty'), 0)::float                                                       AS penalty_weight
        FROM wc_events e
    ...
    assists AS (
      SELECT p.id AS player_id,
             ...
             COALESCE(SUM(e.stakes), 0)::float AS assists_weight
        FROM wc_events e
    ...
    SELECT g.player_id, g.full_name, g.known_as, g.position, g.team_id, g.team_name,
           ...
           g.open_play_weight, g.penalty_weight,
           COALESCE(a.assists, 0) AS assists,
           COALESCE(a.assists_weight, 0) AS assists_weight
      FROM goals_cards g
  ...
  const hydrated = rows.map((r) => {
    // production_raw uses STAKES-WEIGHTED contributions (open_play_weight etc.);
    // reds stay a raw count (discipline, not a stakes-shaded production event).
    const production_raw = computeProductionRaw(
      { open_play_goals: Number(r.open_play_weight),
        penalty_goals:   Number(r.penalty_weight),
        assists:         Number(r.assists_weight),
        reds:            Number(r.reds) },
      params,
    );
    return { ...r, production_raw, production_score: computeProductionScore(production_raw, params) };
  });
