...
import { getDeadRubberMatchIds } from './deadRubbers.js';
...
export async function publishPlayerEditionDaily({
  ...
  while (attempt < MAX_DRIFT_ITERATIONS) {
    ...
    // Stakes weighting: both-secured dead-rubber match ids (derived structurally
    // from standings; see lib/rankings/deadRubbers.js). Goals/assists in these
    // matches are shaded by params.dead_factor inside the production rollup.
    const deadMatchIds = await getDeadRubberMatchIds({ sql, leagueSlug });
    const candidates = await rollupProductionForLeague({
      sql, leagueSlug, deadMatchIds, candidatePool: CANDIDATE_POOL, params,
    });
