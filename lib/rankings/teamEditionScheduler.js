...
export const TEAM_PARAMS = Object.freeze({
  // Outer blend — sites layer removed (external FIFA/ESPN ranks no longer
  // influence the composite). The score IS the editorial composite.
  w_editorial: 1.0,
  w_sites:     0.0,
  // Editorial dim weights — Scheme A: RESULT-led, SQUAD demoted to a
  // supporting prior. Sum to 1.0; re-normalized over present dims when a dim
  // is held/null (e.g. momentum pre/early-tournament).
  w_squad:     0.20,
  w_coherence: 0.15,
  w_result:    0.35,
  w_process:   0.15,
...
export async function rollupTeamRecordsAndMatches({ sql, leagueSlug, asOfTimestamp = null }) {
  const rows = asOfTimestamp
    ? await sql`
        SELECT m.id AS match_id, m.kickoff_at, m.stage,
               m.external_ids->>'api_sports' AS api_sports,
  ...
  // Fixture-level dedup (belt-and-suspenders): the stage guard already drops
  // stage-NULL orphans, but if two final rows ever share one api_sports
  // fixture id (a future fork that BOTH carry a stage), count it once. Rows
  // are ordered by kickoff; the first occurrence wins. Rows without an
  // api_sports id are never collapsed (each stays its own match).
  const seenFixtures = new Set();
  for (const m of rows) {
    if (m.api_sports != null) {
      if (seenFixtures.has(m.api_sports)) continue;
      seenFixtures.add(m.api_sports);
    }
    for (const side of [
...
export async function publishTeamEdition({
  ...
  // is_current flip. Demote EVERY current edition for this list, then promote
  // the new one, inside ONE transaction so the partial UNIQUE index
  // idx_ranking_editions_one_current never transiently sees two current rows.
  //
  // The prior form did both in a single CASE UPDATE. A partial unique index is
  // enforced per row as the UPDATE walks them, so when the promote (new -> true)
  // landed before the demote (old -> false) the statement hit a duplicate and
  // aborted nondeterministically -- leaving the new edition published but not
  // current. That is exactly how ed#13 (2026-07-02) never became current.
  // Demote-first-then-promote in a transaction removes the transient duplicate
  // (mirrors lib/blurbs.js publishBlurb, same partial-unique problem).
  await sql.transaction([
    sql`UPDATE ranking_editions SET is_current = false, updated_at = now()
         WHERE ranking_list_id = ${list_id}::int AND is_current = true`,
    sql`UPDATE ranking_editions SET is_current = true, updated_at = now()
         WHERE id = ${new_ed_id}::int`,
  ]);

  // Post-publish assertion: exactly one current edition for this list and it is
  // the one we just published. Fail LOUDLY rather than leave a silent split or
  // orphaned-not-current edition (the failure mode this repair addresses).
  const currentCheck = await sql`
    SELECT id FROM ranking_editions
     WHERE ranking_list_id = ${list_id}::int AND is_current = true
  `;
  if (currentCheck.length !== 1 || currentCheck[0].id !== new_ed_id) {
    throw new Error(
      `team-power is_current invariant violated for list ${list_id}: `
      + `expected exactly [${new_ed_id}], got [${currentCheck.map((r) => r.id).join(', ') || 'none'}]`,
    );
  }
...
async function driftSnapshot({ sql, leagueSlug }) {
  const r = await sql`
    SELECT
      (SELECT count(*)::int FROM matches m JOIN leagues lg ON lg.id=m.league_id
        WHERE lg.slug=${leagueSlug} AND m.status='final'
          AND m.stage IS NOT NULL) AS finals,
...
// Cumulative bracket-final counts for the 104-match WC 2026 format:
//   Group 72 (MD1 24, MD2 48, MD3 72) -> R32 +16 = 88 -> R16 +8 = 96
//   -> QF +4 = 100 -> SF +2 = 102 -> (3rd 103, Final 104).
// The prior map jumped 72 -> 80 (R16) -> 84 (QF): it OMITTED the Round of 32
// entirely, so every post-group label landed ~16 finals early (that is how a
// mid-R32 edition at finals 83 got labelled "After Quarterfinals" on 07-02).
// finalsCount here is the bracket-final count (stage IS NOT NULL — see the
// snapshot query), so a stageless stray match cannot shift the label.
function deriveTeamMatchdayLabel(finalsCount) {
  ...
  if (finalsCount <= 88) return 'After Round of 32';
  if (finalsCount <= 96) return 'After Round of 16';
  if (finalsCount <= 100) return 'After Quarterfinals';
  if (finalsCount <= 102) return 'After Semifinals';
  return 'After Final';
}
...
export async function publishTeamEditionDaily({
  ...
  // Next number = global MAX(edition_number) + 1, NOT is_current + 1. Deriving
  // from the current edition collides whenever a newer edition already exists
  // but is not current (e.g. the orphaned ed#13 left by the old flip bug:
  // is_current ed#12 -> 12+1 = 13, colliding with the UNIQUE (list, number)).
  // MAX + 1 is robust to any such split.
  const maxNumRow = await sql`
    SELECT COALESCE(MAX(ed.edition_number), 0)::int AS mx
      FROM ranking_editions ed
      JOIN ranking_lists rl ON rl.id = ed.ranking_list_id
      JOIN leagues lg       ON lg.id = rl.league_id
     WHERE rl.slug = ${listSlug} AND lg.slug = ${leagueSlug}
  `;
  const editionNumber = maxNumRow[0].mx + 1;
  ...
  // Paced loop with retry-on-rate-limit. The first live publish queued
  // 0/10 drafts to a transient Anthropic 429 right after the 48-call
  // process phase; the same calls retried at ~2s spacing succeeded 10/10.
  // 1.5s spacing + one 5s-wait retry on rate-limit keeps the cron
  // self-healing without meaningfully extending the run (10 calls *
  // 1.5s = 15s added to a ~9 min orchestrator).
  ...
  const isRateLimitErr = (err) => {
    const s = String(err?.message ?? err ?? '').toLowerCase();
    return s.includes('rate') || s.includes('429') || s.includes('too many requests');
  };
  async function draftOnceWithRetry(entryId) {
    try {
      return await draftRankingRowBlurb({ rankingEntryId: entryId });
    } catch (err) {
  ...
  for (let i = 0; i < newEntries.length; i++) {
    ...
    try {
      const r = await draftOnceWithRetry(e.id);
      const action = r.ok ? 'drafted' : (r.skipped ? 'skipped' : 'error');
      // Retain the error STRING in draft_details. Bare counts hid the
      // first live publish's failure mode; future runs surface the
      // actual issue (validation_failed list, throw message, etc.).
      const errStr = (!r.ok && !r.skipped)
        ? (r.issues ? `validation_failed: ${r.issues.join('; ')}` : (r.reason ?? 'unknown'))
        : null;
      draftResults.push({
        entry_id: e.id,
        rank:     e.rank,
        action,
        blurb_id: r.blurb_id ?? r.existing_blurb_id ?? null,
        reason:   r.reason ?? null,
        error:    errStr,
      });
    } catch (err) {
      draftResults.push({
        entry_id: e.id, rank: e.rank, action: 'error',
        error: String(err?.message ?? err),
      });
    }
    if (i < newEntries.length - 1) await sleep(1500);
  }
