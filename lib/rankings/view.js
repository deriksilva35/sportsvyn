...
import { apTop25, nflPower, ourTop25, groupTable, seasonLeaders, tdLeaders, ourVsAp, modelCase, STAT_KEYS } from './reads.js';
...
async function teamsView({ league, userId, seasonYear, followed }) {
  ...
  if (league === 'nfl') {
    // The ranking is read FIRST so its number one can name the default
    // division for a reader who follows nobody.
    const power = await empty([])(nflPower({ limit: 8 }));
    const division = await empty(null)(
      groupTable('nfl', seasonYear, group, { defaultTeamId: power[0]?.teamId ?? null }),
    );
    return { kind: 'nfl', power, division };
  }
  const [ap, ours, theCase] = await Promise.all([
    empty(null)(apTop25()),
    empty([])(ourTop25({ limit: 5 })),
    // THE ARGUMENT AGAINST THE GATE, published beside it.
    empty(null)(modelCase('cfb-top25')),
  ]);
  const conference = await empty(null)(
    groupTable('cfb', seasonYear, group, { defaultTeamId: ap?.rows?.[0]?.teamId ?? null }),
  );
  ...
  return { kind: 'cfb', ap, ours: oursWithGap, conference, modelCase: theCase };
}
  ...
  if (pickem) {
    ...
    mods.push({
      ...
      rows: pickem.top.map((r) => ({
        rank: r.rank ?? null, name: r.name,
        sub: `${r.boardsPlayed} board${r.boardsPlayed === 1 ? '' : 's'} · ${r.correct}-${r.played - r.correct}`,
