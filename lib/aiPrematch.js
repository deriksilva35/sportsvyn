...
- Never use em dashes or en dashes. Use hyphens (-) only. This applies to every field: subtitle, watch_summary, and both preview paragraphs.
...
// ============================================================================
// Em / en dash normalization. House style is hyphens (-) only. The system
// prompt forbids em (U+2014) and en (U+2013) dashes, but models leak them
// reliably enough that we treat the prompt as a soft signal and the
// normalize step as the load-bearing fix. Applied to EVERY model-produced
// text field (preview paragraphs, watch_summary, all 5 justifications)
// before validation runs.
//
// Behavior:
//   "word — word"   ->  "word - word"   (spaced em becomes spaced hyphen)
//   "word—word"     ->  "word - word"   (jammed em becomes spaced hyphen,
//                                        the readable form)
//   "1986–2026"     ->  "1986 - 2026"   (en treated identically)
//   "co-host"       ->  "co-host"       (real hyphens untouched)
// Then collapses runs of horizontal whitespace so " word  -  word " does
// not leak through the spaced-replacement. Newlines preserved (paragraph
// body splits on \n\n).
// ============================================================================
...
// Apply normalizeDashes to every model-produced text field on the parsed
// analyst-pass result. Mutates and returns the same object for caller
// ergonomics; idempotent (re-running over normalized text is a no-op).
export function normalizeAnalystPassText(parsed) {
  if (!parsed || typeof parsed !== 'object') return parsed;
  parsed.preview_paragraph_1 = normalizeDashes(parsed.preview_paragraph_1);
  parsed.preview_paragraph_2 = normalizeDashes(parsed.preview_paragraph_2);
  parsed.watch_summary       = normalizeDashes(parsed.watch_summary);
  if (parsed.justifications && typeof parsed.justifications === 'object') {
    for (const k of ['stakes', 'quality', 'narrative', 'drama', 'moment']) {
      parsed.justifications[k] = normalizeDashes(parsed.justifications[k]);
    }
  }
  return parsed;
}
...
// Belt-and-suspenders dash-leak check. Run AFTER normalizeAnalystPassText,
// so any hit here means the normalize step missed something — surface as a
// validation issue (advisory at this layer, same as the other gates) so a
// regression in the normalize logic gets logged loudly.
function findDashLeaks(parsed) {
  const fields = {
    preview_paragraph_1: parsed.preview_paragraph_1,
    preview_paragraph_2: parsed.preview_paragraph_2,
    watch_summary:       parsed.watch_summary,
    'justifications.stakes':    parsed.justifications?.stakes,
    'justifications.quality':   parsed.justifications?.quality,
    'justifications.narrative': parsed.justifications?.narrative,
    'justifications.drama':     parsed.justifications?.drama,
    'justifications.moment':    parsed.justifications?.moment,
  };
  const leaks = [];
  for (const [name, text] of Object.entries(fields)) {
    if (!text) continue;
    if (/[—–]/.test(text)) leaks.push(name);
  }
  return leaks;
}
...
export function validateAnalystPass(parsed, envelope) {
  ...
  // 6. Em / en dash leak check. The caller is expected to have run
  // normalizeAnalystPassText already; any residue here is a regression
  // in the normalize logic and worth logging loudly.
  const dashLeaks = findDashLeaks(parsed);
  for (const f of dashLeaks) {
    issues.push(`dash leak (em/en) in ${f}`);
  }
...
export async function runAnalystPassForMatch(matchDbId) {
  ...
  // House style: hyphens only. Normalize em (U+2014) and en (U+2013)
  // dashes BEFORE validation so the row written downstream is clean
  // regardless of whether the model honored the prompt. validateAnalystPass
  // re-checks post-normalize as belt-and-suspenders.
  normalizeAnalystPassText(gen.parsed);
