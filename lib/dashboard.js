...
import { getModelBoard, getTotalsBoard } from './matchProbability.js';
import { getLedger } from './marketLedger.js';
...
// =============================================================================
// getMarketPanelData()
// Lean feed for the /my Market panel: ONLY the rows carrying a non-fair tag
// (generous / rich / wide) across upcoming scheduled matches, from the shipped
// model readers — 1x2 selections (getModelBoard) and totals TAIL lines
// (getTotalsBoard, kind='tail'). Each row is normalized to a single shape with
// a humanized selection label, team ids + flags (for volt on followed teams),
// and the market/model/gap/tag read. Tournament-wide (no followedIds).
//
// Also returns the ledger stats (getLedger) but ONLY once at least one tagged
// entry has graded — a pre-whistle ledger has nothing honest to show.
// =============================================================================
const MARKET_NON_FAIR = new Set(['generous', 'rich', 'wide']);

export async function getMarketPanelData() {
  const [board, totals, ledger] = await Promise.all([
    getModelBoard(),
    getTotalsBoard(),
    getLedger(),
  ]);

  const oneX2 = board.filter((r) => MARKET_NON_FAIR.has(r.tag));
  const tails = totals.filter((r) => r.kind === 'tail' && MARKET_NON_FAIR.has(r.tag));

  // getModelBoard / getTotalsBoard carry abbrs + names but not team ids or
  // flags; the panel needs ids (volt-on-followed) and flags (the row). One
  // supplemental query keyed by match_id.
  const matchIds = [...new Set([...oneX2, ...tails].map((r) => r.match_id))];
  const flagByMatch = new Map();
  if (matchIds.length > 0) {
    const frows = await sql`
      SELECT m.id,
             ht.id AS home_id, ht.flag_svg_path AS home_flag,
             at.id AS away_id, at.flag_svg_path AS away_flag
        FROM matches m
        JOIN teams ht ON ht.id = m.home_team_id
        JOIN teams at ON at.id = m.away_team_id
       WHERE m.id = ANY(${matchIds})
    `;
    for (const f of frows) flagByMatch.set(f.id, f);
  }

  const shapeRow = (r, meta) => {
    const f = flagByMatch.get(r.match_id) ?? {};
    return {
      key: meta.key,
      match_id: r.match_id,
      kickoff_at: r.kickoff_at,
      ...
      home_id: f.home_id ?? null,
      away_id: f.away_id ?? null,
  ...
  const rows = [
    ...oneX2.map((r) => shapeRow(r, {
      key: `mw-${r.match_id}-${r.selection}`,
      market_type: 'match_winner',
      selection_label: r.selection === 'home' ? `${r.home_name} to win`
        : r.selection === 'away' ? `${r.away_name} to win`
          : 'Draw',
    })),
    ...tails.map((r) => shapeRow(r, {
      key: `tot-${r.match_id}-${r.line}-${r.selection}`,
      market_type: 'total',
      selection_label: r.selection === 'over' ? `Over ${r.line} goals` : `Under ${r.line} goals`,
    })),
  ];
  ...
  rows.sort((a, b) => {
    ...
    if (a.market_type !== b.market_type) return a.market_type === 'match_winner' ? -1 : 1;
    return 0;
  });

  // Ledger line only once something has graded (pre-whistle → nothing honest).
  const ledgerStats = ledger?.stats && ledger.stats.graded > 0 ? ledger.stats : null;

  return { rows, ledger: ledgerStats };
}
