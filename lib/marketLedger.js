...
import { computeMatchProbabilities, evaluatePrice } from './matchProbability.js';
...
async function freezeSweep(leagueSlug) {
  ...
  // Live/final matches with a closing 1X2 line and NO ledger row yet.
  const rows = await sql`
    ...
    WHERE lg.slug = ${leagueSlug}
      ...
      AND NOT EXISTS (SELECT 1 FROM market_tag_ledger l WHERE l.match_id = m.id)
    ORDER BY m.id
  `;
...
async function gradeSweep(leagueSlug) {
  ...
  const pending = await sql`
    ...
    WHERE lg.slug = ${leagueSlug}
      AND m.status = 'final'
      AND l.regulation_result IS NULL
  `;
  ...
  for (const p of pending) {
    ...
    // Grade every ungraded row for this match by gap direction.
    const rows = await sql`
      SELECT id, selection_label, gap
      FROM market_tag_ledger
      WHERE match_id = ${p.match_id} AND regulation_result IS NULL
    `;
    for (const r of rows) {
      const selectionHit = r.selection_label === regulationResult;
      // gap>0 (generous / wide-positive) -> hit on HIT; gap<0 (rich /
      // wide-negative) -> hit on MISS.
      const result = ((Number(r.gap) > 0) === selectionHit) ? 'hit' : 'miss';
      const upd = await sql`
        UPDATE market_tag_ledger
           SET regulation_result = ${regulationResult}, result = ${result}, graded_at = now()
         WHERE id = ${r.id} AND regulation_result IS NULL
        RETURNING id
      `;
