...
async function freezeSweep(leagueSlug) {
  ...
  const rows = await sql`
    ...
    WHERE lg.slug = ${leagueSlug}
      ...
      -- Freeze ONLY at the scheduled->live transition or shortly after: a
      -- kickoff within the last 6h. The window exists to survive a missed tick,
      -- NEVER to reach back into history. Without it the sweep would freeze
      -- every historical final retroactively (with current-edition ratings =
      -- hindsight). A match already final before the ledger saw it scheduled
      -- has a kickoff far in the past and is correctly excluded.
  ...
  const totalRows = await sql`
    ...
    WHERE lg.slug = ${leagueSlug} AND m.status IN ('live', 'final')
      AND m.kickoff_at >= now() - interval '6 hours' -- same 6h catch-up bound as 1X2 (never reach history)
    ORDER BY m.id
  `;
