...
test('EVERY COMPONENT THAT RENDERS A BOARD ROW RENDERS THE MARKER', () => {
  for (const f of [
    // GAMES v3: the board rows moved out of app/games/page.js and into the
    // pane that draws them. The marker and its stylesheet moved with them -
    // the law is that whatever renders a board row renders the marker, and
    // that is now LobbyV3.
    '../../components/games/LobbyV3.js',
