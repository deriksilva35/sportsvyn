...
import { saveRunPick, runBoardOf } from '../run/entry.js';
...
export async function fileRun(sql, { userId, personaKey, contest, now = new Date() }) {
  ...
  const used = await runUsedPlayers(userId, contest.season_year, { excludeContestId: contest.id });
  const lineup = pickRun(personaKey, flat, { board, used, now }, seedFor(personaKey, contest.id));
  if (!lineup) return skip('no method on this game');
  if (!Object.keys(lineup).length) return skip('no eligible players');
  ...
  for (const slot of RUN_SLOTS) {
    ...
    // A LOCKED ROUND STOPS THE WHOLE FILING, not just this slot: the lock is
    // the round's, so if one pick is late they all are, and the rest would
    // fail identically.
    if (r.reason === 'round_locked') return ok({ filed, of: RUN_ROSTER, late: true });
    return { ok: false, reason: r.reason, slot };
  }
  // A SHORT NINE IS FILED SHORT AND WILL DNF. The house takes the same
  // consequence a reader does rather than being handed a pool nobody else has.
  return ok({ filed, of: RUN_ROSTER, short: filed < RUN_ROSTER });
}
