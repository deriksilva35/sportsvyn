...
import { saveRunPick, runBoardOf, isRunPreview, panelFor } from '../run/entry.js';
import { runPool, roundMatches, roundGames, lineupsByClub, probablesByClub } from '../run/pool.js';
...
export async function fileRun(sql, { userId, personaKey, contest, now = new Date() }) {
  ...
  // THE HOUSE IS OFFERED WHAT A READER IS OFFERED - the panel's cut: starter
  // arms only (never a reliever), the posted card's bats once it is up.
  const matchRows = await roundMatches(contest).catch(() => []);
  const raw = await runPool(contest).catch(() => null);
  const lineups = await lineupsByClub(contest, matchRows).catch(() => new Map());
  const pool = panelFor(raw, lineups, probablesByClub(matchRows)).pool;
