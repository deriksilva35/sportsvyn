...
function fill(pool, ctx, choose) {
  ...
  for (const slot of SLOTS) {
    ...
    // THE CLUB RIDES ALONG, the same as lib/house/pickRun.js's does. It was the
    // one field this fill() dropped, and the card's slot sub-line reads it
    // ("PIT · 7:40 PM"): the first house rows filed on PROD showed a blank club
    // beside every name because the pool had the abbreviation and this object
    // threw it away.
    if (p) {
      lineup[slot] = {
        playerId: String(p.playerId), matchId: Number(p.matchId),
