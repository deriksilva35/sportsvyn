...
// is the gate, called for every candidate, so the house cannot exceed two from
// a game, cannot re-use a burned player and cannot pick from a started game.
// A persona that got its own relaxed copy of the rules would not be playing
// the game the reader is playing.

import { slotsFor, refuseReason } from '../october/rules.js';
...
function fill(pool, ctx, choose) {
  const lineup = {};
  // THE DAY'S OWN SLOTS, not always five - a two-game day has four and a
  // World Series day has two (lib/october/rules.js cardSizeFor). The house
  // must field the card the reader is fielding.
  for (const slot of slotsFor(ctx?.board ?? [])) {
...
/** THE CHALK. Highest PPG at every slot, inside the burn. */
