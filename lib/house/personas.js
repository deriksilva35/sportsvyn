...
export const PERSONAS = Object.freeze([
  {
    ...
    // OCTOBER: the same policy it has on the Weekly, which is the whole
    // point of "existing policies" - highest PPG at every slot, inside the
    // rules everyone plays by (one arm, four bats, two from a game, and the
    // burn). The Chalk's Daily objection does not apply here: October's
    // constraints BIND - a burned player is gone for the tournament - so the
    // obvious pick is a strategy again rather than the answer key.
    plays: Object.freeze(['pickem', 'weekly', 'draft', 'october', 'run']),
    method: Object.freeze({
      october: 'Highest PPG at every slot, inside the burn.',
