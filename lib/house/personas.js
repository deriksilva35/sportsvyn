...
export const PERSONAS = Object.freeze([
  {
    ...
    // OCTOBER: the same policy it has on the Weekly - highest PPG at every
    // slot, inside the rules everyone plays by (one arm, four bats, and the
    // day's cap).
    //
    // AND THE ARGUMENT FOR ITS ROW HERE IS WEAKER THAN IT WAS. This comment
    // used to say October's constraints BIND because a burned player is gone
    // for the tournament, so the obvious pick was a strategy rather than the
    // answer key. THE BURN IS GONE (lib/october/rules.js), and what is left
    // binding is only the per-game cap - which on a fifteen-game slate barely
    // binds at all. That is the same objection that keeps The Chalk off the
    // Daily, and it is now live here. It keeps its row for today because five
    // slots from one day's games is still a narrower pool than a twelve-card
    // Daily board, but this is a judgment worth revisiting rather than a
    // settled one.
    plays: Object.freeze(['pickem', 'weekly', 'draft', 'october', 'run']),
    method: Object.freeze({
      october: 'Highest PPG at every slot the day\'s cap allows.',
      run: 'Highest PPG at every slot, inside the burn and the club cap.',
  ...
  },
  {
    ...
    plays: Object.freeze(['daily', 'pickem', 'weekly', 'draft', 'october', 'run']),
    method: Object.freeze({
      daily: 'Weighted random over the top tier at every slot.',
      october: 'Weighted random over the top tier at every slot.',
      run: 'Weighted random over the top tier at every slot.',
  ...
  },
  {
    ...
    method: Object.freeze({
      ...
      // ITS TEAMS ARE FOOTBALL TEAMS, and it says so rather than pretending.
      // HOMER_TEAMS is a gridiron set; in October it has nobody of its own
      // most days and falls to the chalk, which is the honest reading of "its
      // own teams first" for a persona whose teams are not in this sport.
      october: 'Its own teams where today\'s games have them, the chalk everywhere else.',
      run: 'Its own teams where the round has them, the chalk everywhere else.',
