...
test('DAILY / ONLY GUT AND HOMER PLAY IT - chalk, fade and book file nothing', () => {
  assert.equal(pickDaily('fade', DAILY, Math.random), null, 'no line to fade');
  assert.equal(pickDaily('book', DAILY, Math.random), null, 'parked');
  // THE CHALK WAS REMOVED FOR BEING TOO GOOD. Over the six boards that
  // existed when this shipped, the greedy scored 99.5% and was the outright
  // optimum on three - on a twelve-card board the one-player-per-team
  // constraint almost never binds, so it is the answer key, not a persona.
  assert.equal(pickDaily('chalk', DAILY, Math.random), null);
  // chalkDaily() itself stays: it is The Homer's fallback.
  assert.equal(chalkDaily(DAILY).length, DAILY_SLOTS.length);
});
