...
//
// SLOT ORDER IS THE BOARD'S OWN (boardShape.js SLOTS). Walking it in a
// different order would be a different strategy wearing the same name.

import { SLOTS, eligibleForSlot } from '../daily/boardShape.js';
...
export function pickDaily(personaKey, board, rng) {
  switch (personaKey) {
    case 'chalk': return chalkDaily(board);
    case 'gut': return gutDaily(board, rng);
    case 'homer': return homerDaily(board);
    // fade and book have no method on this board and do not enter it (R1).
    default: return null;
  }
}
