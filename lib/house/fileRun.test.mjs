...
test('IT PLAYS THE SAME RULES, through the same door', () => {
  ...
  // THE LOCK IS THE CLUB'S: a started club is one refused slot and the rest
  // are still filed - never a stop for the whole nine.
  assert.doesNotMatch(fn, /round_locked/);
  assert.match(fn, /if \(r\.reason === 'game_started'\) \{ refused\.push\(\{ slot, reason: r\.reason \}\); continue; \}/);
  // And the picker sees the round's games, so it reaches for no started club.
  assert.match(fn, /pickRun\(personaKey, flat, \{ board, used, now, games \}/);
  // A SHORT NINE IS FILED SHORT - the same consequence a reader takes, not a
  // private exemption.
