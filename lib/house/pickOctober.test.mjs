...
test('THERE IS NO BURN TO BIND THE HOUSE - a used set changes nothing', () => {
  // THIS TEST USED TO ASSERT THE OPPOSITE: a spent arm pushed the Chalk to its
  // second choice. October has no burn, so the house takes the best arm again
  // today whatever it took yesterday - the same card a reader now gets.
  const ctx = { ...CTX, used: new Set(['1', '3']) };
  const c = chalkOctober(POOL, ctx);
  assert.equal(c.arm.playerId, '1', 'the best arm, not the next one');
  assert.ok(Object.values(c).some((p) => p.playerId === '3'), 'and a "spent" bat is back');
  // Byte for byte the same card as one built with no used set at all: the option
  // is inert, which is the only way to be sure the rule cannot creep back.
  assert.deepEqual(c, chalkOctober(POOL, CTX));
  // THE CAP STILL BINDS IT, though - that rule did not go anywhere.
