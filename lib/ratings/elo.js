...
export function runElo({
  ...
  const table = new Map();
  const seat = (key) => {
    if (!table.has(key)) table.set(key, { elo: START_ELO, games: 0, history: [] });
  ...
  for (const g of usable) {
    ...
    if (season != null && g.season !== season && regress > 0) {
      for (const row of table.values()) {
        row.elo = START_ELO + (row.elo - START_ELO) * (1 - regress);
      }
    }
    season = g.season;

    const H = seat(g.homeKey); const A = seat(g.awayKey);
