...
test('the file is a REQUIRED --file argument, read from disk; the script builds no markup and names no default', () => {
  const t = stripComments(script);
  assert.match(t, /args\.indexOf\('--file'\)/, '--file is parsed');
  assert.match(t, /if \(!FILE_ARG\) throw/, 'no --file refuses');
  assert.doesNotMatch(t, /docs', 'email', 'launch-email-sep8/, 'no default file baked in');
  assert.match(t, /readFileSync\(HTML_FILE\)/, 'the file is read from disk');
  assert.match(t, /emailMeta\(HTML_TEMPLATE\)/, 'subject and preheader come from the file');
  assert.match(t, /includes\('\[\[COPY'\)/, 'a [[COPY placeholder refuses at load');
...
// ---------------------------------------------------------------------------
// THE CAMPAIGN KEY (8 Sep 2026): the send-once check is per campaign.
// The launch email skipped 76 of 222 as "already sent" because the predicate
// matched source + userId and nothing named which broadcast - the 76 were
// August's recipients. These pin the rule that replaces it.
// ---------------------------------------------------------------------------
import { sentForCampaign } from './broadcastRules.js';

const AUG = 'aug-sha-0000';   // the 19 Aug send, retro-keyed or not
const SEP = 'ae8602ffad2c741203229dc90dfb46d0df175820951876fcd32d3424fb8d3fd0';
const NEXT = 'next-campaign-sha-9999';
const NOW = new Date('2026-09-08T22:00:00Z');

test('a user with an August row and no September row IS a recipient of September', () => {
  const rows = [{ campaign: null, outcome: 'sent', started_at: '2026-08-19T19:23:50Z' }];
  assert.equal(sentForCampaign(rows, SEP, { now: NOW }), false, 'an unkeyed historic row never matches');
  const keyedAug = [{ campaign: AUG, outcome: 'sent', started_at: '2026-08-19T19:23:50Z' }];
  assert.equal(sentForCampaign(keyedAug, SEP, { now: NOW }), false, 'a row keyed to another campaign never matches');
});

test("a user with today's keyed row is SKIPPED for today's campaign", () => {
  const rows = [
    { campaign: null, outcome: 'sent', started_at: '2026-08-19T19:23:50Z' },
    { campaign: SEP, outcome: 'sent', started_at: '2026-09-08T21:19:30Z' },
  ];
  assert.equal(sentForCampaign(rows, SEP, { now: NOW }), true);
});

test('a future campaign with a new sha sends to BOTH', () => {
  const augOnly = [{ campaign: null, outcome: 'sent', started_at: '2026-08-19T19:23:50Z' }];
  const sepOnly = [{ campaign: SEP, outcome: 'sent', started_at: '2026-09-08T21:19:30Z' }];
  assert.equal(sentForCampaign(augOnly, NEXT, { now: NOW }), false);
  assert.equal(sentForCampaign(sepOnly, NEXT, { now: NOW }), false);
});

test('an in-flight send counts only for its own campaign and only inside the stuck window', () => {
  const fresh = [{ campaign: SEP, outcome: 'sending', started_at: '2026-09-08T21:55:00Z' }];
  const stale = [{ campaign: SEP, outcome: 'sending', started_at: '2026-09-08T21:00:00Z' }];
  const other = [{ campaign: NEXT, outcome: 'sending', started_at: '2026-09-08T21:55:00Z' }];
  assert.equal(sentForCampaign(fresh, SEP, { now: NOW }), true, 'in flight 5 min ago: skip, do not double-send');
  assert.equal(sentForCampaign(stale, SEP, { now: NOW }), false, 'stuck an hour: retry');
  assert.equal(sentForCampaign(other, SEP, { now: NOW }), false);
  assert.throws(() => sentForCampaign([], null), /campaign key is required/);
});

test('the script keys every ledger write, decides via sentForCampaign, and confirms the SEND count', () => {
  const t = stripComments(script);
  assert.match(t, /const CAMPAIGN = HTML_SHA256;/, 'the key is the html sha');
  assert.match(t, /sentForCampaign\(rows, CAMPAIGN/, 'alreadySent decides per campaign');
  assert.doesNotMatch(t, /SELECT 1 FROM sync_runs\s+WHERE source = \$\{SOURCE\} AND \(summary->>'userId'\)/, 'the unkeyed predicate is gone');
  // recordStart + 4 recordFinish paths + owner-verify = 6 keyed ledger writes; the
  // 7th `campaign: CAMPAIGN` is the click link, which signs with the same key
  const ledger = (t.match(/campaign: CAMPAIGN/g) ?? []).length - (t.match(/clickUrlFor\(\{ campaign: CAMPAIGN/g) ?? []).length;
  assert.equal(ledger, 6, 'every ledger write carries the key');
  assert.match(t, /clickUrlFor\(\{ campaign: CAMPAIGN, userId, to \}\)/, 'click links are signed with the campaign key');
  assert.match(t, /Type the recipient count \(\$\{toSend\.length\}\)/, 'the typed count is the send count');
  assert.match(t, /typed\.trim\(\) !== String\(toSend\.length\)/);
  assert.match(t, /RECIPIENTS: \$\{toSend\.length\}/, 'the dry run prints what will be sent');
});
