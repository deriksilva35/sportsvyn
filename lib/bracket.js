...
//   winner_of/loser_of : parent match status='final' AND decided in regulation
    ...
    for (const side of ['home', 'away']) {
      ...
      if (slot.type === 'group_winner') {
        ...
        if (parent.home_score === parent.away_score) {
          skipped.push({
            match_number: matchNumber, side,
            reason: `parent ${slot.match_number} ended ${parent.home_score}-${parent.away_score}; penalty winner not encoded`,
          });
          continue;
        }
        const homeWon = parent.home_score > parent.away_score;
...
export async function resolveKnockoutBracket(leagueSlug = WC_LEAGUE_SLUG, { dryRun = true } = {}) {
  ...
  if (dryRun) return { plan, skipped, positionClinch, writes: 0 };

  if (plan.length === 0) return { plan, skipped, positionClinch, writes: 0 };
  ...
  return { plan, skipped, positionClinch, writes: plan.length };
}
