...
// Per-KO-round completion for the homepage tournament-progress strip.
// Returns an object keyed by bracket stage -> { total, final, live }. Only
// BRACKET rows count: `stage IN (...)` excludes any stage=null stray (e.g. the
// misfiled czechia-mexico friendly), same principle as the edition-label fix.
// The prior strip hardcoded R32/R16 to 0 and had no QF/SF tiers, so both rounds
// read "0 completed" once played; this wires the real counts for the 104-match
// format (R32 16, R16 8, QF 4, SF 2, final 1).
export async function getKnockoutRoundProgress(leagueSlug = WC_LEAGUE_SLUG) {
  const rows = await sql`
    SELECT m.stage,
           count(*)::int                                 AS total,
           count(*) FILTER (WHERE m.status = 'final')::int AS final,
           count(*) FILTER (WHERE m.status = 'live')::int  AS live
      FROM matches m
      JOIN leagues lg ON lg.id = m.league_id
     WHERE lg.slug = ${leagueSlug}
       AND m.stage IN ('round_of_32','round_of_16','quarter','semi','third_place','final')
  ...
  `;
  const by = {};
  for (const r of rows) by[r.stage] = { total: r.total, final: r.final, live: r.live };
  return by;
}
  ...
  for (const [matchNumber, m] of knockoutBracket) {
    for (const side of ['home', 'away']) {
      ...
      if (slot.type === 'group_winner') {
        ...
        // Seeded slot metadata stores the parent under `match`; tolerate the
        // legacy `match_number` key too. (Reading only match_number meant the
        // parent was never found, so winner_of/loser_of never advanced.)
        const parentMn = slot.match ?? slot.match_number;
        const parent = knockoutBracket.get(parentMn);
        ...
        if (parent.home_score > parent.away_score)      homeWon = true;
          ...
          if (hp != null && ap != null && hp !== ap) {
            ...
            skipped.push({
              match_number: matchNumber, side,
              reason: `parent ${parentMn} level ${parent.home_score}-${parent.away_score}; shootout ${hp ?? '–'}-${ap ?? '–'} undecided`,
            });
            continue;
          }
        }
...
// ---------------------------------------------------------------------------
// getTournamentChampion — the winner of the completed Final, read from the data
// (never hardcoded). Returns { name, slug } once stage='final' is status='final'
// with a decided score; null while the tournament is unfinished (or a draw score
// with no recorded winner, which we treat as undecided). Drives the homepage's
// completed-tournament state.
// ---------------------------------------------------------------------------
export async function getTournamentChampion() {
  const rows = await sql`
    SELECT m.status, m.home_score, m.away_score,
           h.name AS home_name, h.slug AS home_slug,
           a.name AS away_name, a.slug AS away_slug
      FROM matches m
      JOIN leagues l ON l.id = m.league_id AND l.slug = 'fifa-wc-2026'
     ...
     LIMIT 1`;
  const f = rows[0];
  if (!f || f.status !== 'final' || f.home_score == null || f.away_score == null) return null;
  if (f.home_score === f.away_score) return null; // undecided by score (PK winner not recorded here)
  const homeWon = f.home_score > f.away_score;
  return { name: homeWon ? f.home_name : f.away_name, slug: homeWon ? f.home_slug : f.away_slug };
}
