...
export function deriveReadTimeMin(bodyLen) {
...
// =============================================================================
// The league(s) a Reads query is scoped to. Defaulted to the World Cup so any
// caller written before the homepage moved to football keeps its exact previous
// behaviour; the homepage now passes the football slugs explicitly.
export const WC_READS_SLUGS = ['fifa-wc-2026'];
export const FOOTBALL_READS_SLUGS = ['nfl', 'cfb'];

export async function getTodaysReads({ ptDay = null, limit = 4, leagueSlugs = WC_READS_SLUGS } = {}) {
  let day = ptDay;
  if (!day) {
    const r = await sql`SELECT to_char((now() AT TIME ZONE 'America/Los_Angeles')::date, 'YYYY-MM-DD') AS d`;
    day = r[0].d;
  }
  // An empty list would silently match nothing, which reads on the page as "no
  // reads today" rather than "you asked for no leagues". Refuse it instead.
  const slugs = Array.isArray(leagueSlugs) ? leagueSlugs.filter(Boolean) : [];
  ...
  const rows = await sql`
    SELECT a.id, a.slug, a.title, a.subtitle, a.type, a.score_type, a.published_at,
     ...
     WHERE a.status = 'published'
       ...
       AND (
         (a.type = 'preview'
          AND lg_m.slug = ANY(${slugs}::text[])
          AND (m.kickoff_at AT TIME ZONE 'America/Los_Angeles')::date = ${day}::date)
         OR
         -- 'feature' is what a published topic draft becomes (migration 043
         -- added it to articles_type_check; this list predates that and was
         -- never extended). Without it the homepage was the ONE editorial
         -- surface that could not see an AI-drafted, editor-published article:
         -- /articles already lists them, since the archive excludes only
         -- 'preview'. A feature published this morning simply never appeared.
         (a.type IN ('essay','edge','profile','rankings','recap','newsletter','feature')
          AND lg_a.slug = ANY(${slugs}::text[])
