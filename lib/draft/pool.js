 ...
 * - see the caller.
 *
 * IT MEASURES THE DRAFTABLE COUNT, NOT THE ROW COUNT. A ranked roster is
 * QB/RB/WR/TE plus a FLEX with no bench, so every DEF and PK in the snapshot
 * is a row the room would refuse to draft (engine.canRoster clause (d)).
 * Today that is 5 of 78. A candidate that carries `draftable` is measured on
 * it; one that carries only `rows` is measured on that, so a caller with a
 * K/DST-slotted config is not silently held to a skill-only floor.
 *
 * @param {Array<{snapshotDate: *, rows: number, draftable?: number}>} candidates any order
 * @param {number} demand the config's real (non-synthetic) pick demand
 * @returns {{snapshotDate: *, rows: number, draftable?: number}|null}
 */
export function chooseSnapshot(candidates, demand) {
  const need = Number(demand);
  if (!Number.isFinite(need) || need <= 0) return null;
  const sizeOf = (c) => (Number.isFinite(Number(c?.draftable)) ? Number(c.draftable) : Number(c?.rows));
  const fits = (candidates ?? [])
    .filter((c) => c?.snapshotDate != null && sizeOf(c) >= need)
