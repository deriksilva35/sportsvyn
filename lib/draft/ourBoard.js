...
// PRIOR, and measured football takes the board over as it accumulates:
//
//     weight = min(games in the window, 6) / 6
//     board rank = (1 - weight) * prior rank + weight * VOR rank
//
// GAMES IN THE WINDOW, NOT GAMES THIS SEASON (ruled). The prior exists to price
// the men we have no measurement of - a rookie, a man who changed teams in
// March, a back returning from a lost year. A veteran with a full 2025 is not
// one of those: we have seventeen games of him, and holding his board rank at
// 5/6 of an August guess until he has played six more Sundays would be
// deferring to the market about the one player it has the least to add on. So
// the weight counts the whole window. A man with no window at all is exactly
// the man the market is for, and he gets weight 0 - the prior, entire.
//
...
/**
 * How much measured football owns the board: 0 at no games, 1 at six.
 *
 * `games` is GAMES IN THE WINDOW (2025 REG + 2026 REG finals), not games this
 * season - see the header. Six is the threshold at which a per-game average
 * stops being an anecdote, and it does not matter which season those six came
 * from: the window is one measurement.
 */
...
export async function ourBoard({
  ...
  const rows = ident.map((r) => {
    ...
    return {
      ...
      ppg: st?.ppg ?? null,
      // `games` IS THE BLEND'S INPUT, and it is the window's count. The
      // season's own count rides along beside it for display and diagnosis -
      // nothing orders the board on it.
      games: st?.gp ?? 0,
      seasonGames: st?.games ?? 0,
