...
test('EIGHT PICKS FEED A BEST-SIX - the draft is larger than the lineup', () => {
  // Ruled, and pinned so the 6-vs-8 question does not resurface. The lineup
  // grammar is SCORING law; the draft size is not. Rounds seven and eight are
  // where a construction decision costs something, which is the game.
  if (!row) return;
  const rounds = Object.values(row.roster_slots).reduce((a, b) => a + Number(b), 0);
  assert.equal(rounds, 8, 'the ranked draft is eight rounds');
  assert.ok(rounds > 6, 'and deliberately larger than the six it scores');
});
