...
// activePool, firstKickoff, tuesdayBefore and easternLocalToUtc went with
// ensureDraftWeek - they were its imports and nothing else here used them.
// Removing them also removes the import cycle back into lib/weekly/create.js
// that the header above used to have to explain.
...
// ensureDraftWeek() WAS HERE AND IS GONE. It was a SECOND creator for the
// draft contest row, and nothing in the tree ever called it - lib/weekly/
// create.js ensureWeek() writes the weekly and the draft rows in one
// transaction under its own "BOTH ROWS OR NEITHER" ruling, and that is the
// only path the crons take.
//
// A dormant second creator for a row that must exist exactly once is a trap
// waiting for somebody to find it and call it, which is why it is deleted
// rather than commented. The behaviour it had lives in ensureWeek.
