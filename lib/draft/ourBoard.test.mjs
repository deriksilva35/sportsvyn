...
// ---------------------------------------------------------------------------
test('the weight is min(games in the window, 6) / 6', () => {
...
test('six games of evidence can overtake the market, and one game cannot', () => {
  // `games` is the WINDOW's count (2025 REG + 2026 REG finals), not this
  // season's - a veteran with a full prior year is measured, not a guess.
...
//
// WHAT IT PINS AND WHAT IT DOES NOT. It pins the PROPERTIES - no quarterback in
// round 1, at most two in round 2, and at least eighteen of the first
// twenty-four are backs and receivers - never a position for a named man,
// because the blend is designed to MOVE him. A test reading "Allen at 13"
// would go red the week a rookie back outplays him, which is the design
// working rather than the design breaking.
...
test('the real board puts backs and receivers in front of quarterbacks', async () => {
  ...
  const seats = DRAFT_CONFIG.teamsCount;
  const roundOne = board.rows.slice(0, seats);
  const roundTwo = board.rows.slice(seats, seats * 2);
  const topTwoRounds = board.rows.slice(0, seats * 2);

  // NO QUARTERBACK IN ROUND 1. A one-week best-ball roster starts exactly one,
  // and the thirteenth-best is nearly free - spending 1.01 on the position is
  // the mistake the whole board exists to stop.
  const qb1 = roundOne.filter((r) => r.position === 'QB');
  assert.equal(qb1.length, 0,
    `no QB in round 1; found ${qb1.map((q) => `${q.name} @${q.adp}`).join(', ')}`);

  // AT MOST TWO IN ROUND 2. Not zero: a quarterback far enough above the
  // replacement IS worth a second-round pick, and a board that could never say
  // so would be a rule dressed as a measurement.
  const qb2 = roundTwo.filter((r) => r.position === 'QB');
  assert.ok(qb2.length <= 2,
    `at most two QBs in round 2; found ${qb2.length}: ${qb2.map((q) => `${q.name} @${q.adp}`).join(', ')}`);

  const skillTop = topTwoRounds.filter((r) => ['RB', 'WR'].includes(r.position));
  assert.ok(skillTop.length >= 18,
    `at least 18 of the first 24 are RB/WR; found ${skillTop.length}`);
});
...
test('the blend counts the window, not the season alone', () => {
  const src = readFileSync(new URL('./ourBoard.js', import.meta.url), 'utf8');
  // gp is the window's game count; gp_now is this season's. The row the
  // ordering reads must carry the former.
  assert.match(src, /games: st\?\.gp \?\? 0,/, 'the blend input is the window count');
  assert.match(src, /seasonGames: st\?\.games \?\? 0,/, 'the season count rides along, unused by the order');
  const order = src.slice(src.indexOf('export function orderBoard'), src.indexOf('/** \'YYYY-MM-DD\''));
  assert.doesNotMatch(order, /seasonGames/, 'nothing orders the board on this season alone');
});
