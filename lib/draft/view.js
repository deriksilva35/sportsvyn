 ...
 */
/** Seat s's pick number in round r (1-indexed both) of a snake draft. */
  ...
  return Array.from({ length: n }, (_, i) => ({
    ...
    note: i === 0 ? 'First overall, longest wait'
      : i === n - 1 ? 'Back-to-back at the turn'
        : null,
    // round1Pick/round2Pick: the seat-copy math (relay 2a item 7, "Seat {n}
    // picks {a}th and {b}th") - real snake math, not the note above (which
    // is deliberately null for a middle seat; this is never null).
    round1Pick: n ? snakePick(i + 1, 1, n) : null,
    round2Pick: n ? snakePick(i + 1, 2, n) : null,
  }));
}
...
export function draftSettledView({ contest, entry, board }) {
  ...
  // TEAM COMES FROM THE BOARD, NOT THE ROSTER (relay 2b-fix-2 item 1).
  // bridgeRoster() writes {id, ffc, name, pos, round} onto the entry - it
  // has no team to write, since it bridges from the FFC pool. The board
  // snapshot does carry it (activePool selects teams.abbreviation), and
  // this lookup is already open for `points`, so the team rides along.
  const roster = (entry?.meta?.roster ?? []).map((r) => {
    const p = byId.get(String(r.id));
    return { ...r, team: p?.team ?? null, points: p ? Number(p.points) : null };
  });
  const started = new Set(Object.values(entry?.lineup ?? {}).map(String));
  // THE ONE ROUND scoreLineup DROPPED AS WORST, not the ones bestBall never
  // started at all - both read as "not counted" (relay 2b item 3's "dropped-
  // worst rows struck"), but only this one earns the label; a benched pick
  // was never a candidate to begin with.
  const droppedId = entry?.lineup?.[entry?.meta?.droppedSlot];
  ...
  return {
    ...
    roster: roster.map((r) => ({
      ...r, label: `R${r.round}`,
      started: started.has(String(r.id)),
