...
/** The settled view: your best-ball six against the perfect six. */
...
export function draftHomeView({ contest = null, entry = null, draft = null, now = new Date() } = {}) {
  ...
  if (state === 'drafting') return { ...base, state: 'drafting', rounds, locksAt: contest.locks_at };
  if (state === 'waiting') {
    return { ...base, state: 'waiting', picks: entry?.meta?.roster?.length ?? 0, locksAt: contest.locks_at };
  }
