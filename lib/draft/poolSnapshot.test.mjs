...
test('RANKED ONLY: the ranked start passes the flag, the tracker start does not', () => {
  ...
  // THE RANKED BRANCH NO LONGER REACHES BACK FOR AN FFC SNAPSHOT - it drafts
  // our own board (lib/draft/ourBoard.js), which is built from every identity
  // the pool has ever carried and so cannot be too small to seat a room. What
  // this test was always about survives unchanged: the ranked start is GATED
  // ON THE FLAG, and the unranked path still takes the latest snapshot.
  // chooseSnapshot and its own tests stay in lib/draft/pool.js - the fallback
  // is still correct, it is simply no longer on this path.
  assert.ok(fn.indexOf('if (ranked) {') < fn.indexOf('ourBoard('), 'gated on ranked');
  assert.doesNotMatch(fn, /chooseSnapshot\(/, 'the reach-back is off the ranked path');
...
test('THE REFUSAL SURVIVES: pool_too_small when nothing fits', () => {
  ...
  // TWO, NOT THREE. The third was the re-check after loading a chosen
  // snapshot - "it fitted a moment ago and does not now" - which only a
  // snapshot that can change under us needs. Our board is computed in the same
  // call that measures it. The refusal itself survives on both paths.
  assert.equal((body.match(/reason: 'pool_too_small'/g) ?? []).length, 2,
    'the ranked board and the unranked snapshot each refuse a room they cannot seat');
});
