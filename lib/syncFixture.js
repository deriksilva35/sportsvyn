...
import { soccerLiveState } from './soccer/liveChip.js';
...
async function doSync(fixtureApiId, capture) {
  ...
  const liveState = soccerLiveState(status, f.fixture.status);
  const homeScore = f.goals.home ?? null;
  const awayScore = f.goals.away ?? null;
  // Penalty-shootout score (API-Sports: f.score.penalty.{home,away}). Null for
  // any match not decided by a shootout. Captured alongside the regulation/ET
  // score — home_score/away_score stay the end-of-ET score (STATUS_MAP keeps
  // PEN -> 'final' unchanged); the shootout tally lives in *_penalties.
  const homePenalties = f.score?.penalty?.home ?? null;
  const awayPenalties = f.score?.penalty?.away ?? null;
  ...
  // MATCH-BY-API-ID (kills the slug-fork). The upsert below keys on slug, but
  // SEEDED rows — e.g. WC knockout rows with structural slugs like
  // 'wc-2026-ko-73' — carry an api_sports id whose generated team-name slug
  // does NOT match, so keying on slug alone FORKS a parallel row (the Czech-
  // fork class, m_id 12674/16671). So: if a row already OWNS this fixture id,
  // UPDATE that row BY id (live-result fields only — never its slug, stage,
  // metadata, or seeded teams); otherwise fall back to the UNCHANGED slug-
  // upsert. Group/import rows carry the id AND a matching slug, so both paths
  // resolve to the same row and write the same fields — behavior is identical
  // there. league_id is reused from the owning row (was `existingLeague`).
  ...
  `;
  if (owners.length > 1) {
    // Never guess which twin to update (the orphan-fork rows still exist).
    throw new Error(
      `syncFixture: fixture ${fixtureApiId} is owned by ${owners.length} rows (${owners.map((o) => o.id).join(',')}) — refusing to update an ambiguous twin`,
    );
  }
  const existing = owners[0] ?? null;
  const leagueId = existing ? existing.league_id : await upsertFriendliesLeague();
  ...
  const rows = existing
    ? await sql`
        UPDATE matches SET
          ...
          venue = ${venue},
          -- WHERE IN THE MATCH. A FLAT top-level key, replaced wholesale on
          -- every poll and NULL whenever the match is not live, so a stale
          -- clock cannot outlive its match. Flat by construction means the
          -- shallow || merge cannot reach any nested sibling (the 14 Aug law).
          metadata = COALESCE(matches.metadata, '{}'::jsonb)
                     || ${JSON.stringify({ live_state: liveState })}::jsonb,
          external_ids = ${externalIds}::jsonb,
          data_provider_synced_at = now(),
          updated_at = now()
        WHERE id = ${existing.id}
        RETURNING id, slug, status, home_score, away_score, home_penalties, away_penalties, kickoff_at
      `
    : await sql`
        INSERT INTO matches (
