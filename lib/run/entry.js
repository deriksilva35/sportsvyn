...
import { SLOTS, refuseReason, progress, rosterState, roundPips, isLocked, roundLockAt, ROSTER_SIZE, DNF } from './rules.js';
import { scoreRoster, boxForRound, clubStateFrom } from './settle.js';
import { usedPlayers, runPool, clubStarters, lineupsByClub, roundMatches, roundGames } from './pool.js';
...
export async function saveRunPick(userId, contestId, slot, player, { now = new Date() } = {}) {
  ...
  // THE DOOR READS THE SAME LOCK THE CARD DOES, or the only thing holding a
  // roster open past a called-off opener is a browser tab.
...
export async function clearRunPick(userId, contestId, slot, { now = new Date() } = {}) {
  ...
  const games = roundGames(await roundMatches(contest).catch(() => []));
  if (isLocked(runBoardOf(contest), now, { games })) return { ok: false, reason: 'round_locked' };
  await sql`
...
export async function runView(userId, contest, { now = new Date(), withPool = true } = {}) {
  ...
  const [rows, series, used, pool, done, matchRows] = await Promise.all([
    ...
    // THE ROUND'S OWN MATCHES, once. The lock reads their statuses and their
    // LIVE first pitches (roundLockAt) and the panel reads their posted cards,
    // off the same rows.
    roundMatches(contest).catch(() => []),
  ]);
  const lineups = await lineupsByClub(contest, matchRows).catch(() => new Map());
  const panel = panelFor(pool, lineups);
  // A POSTPONED GAME NEITHER LOCKS THIS ROUND EARLY NOR HOLDS IT OPEN PAST THE
  // NEXT EARLIEST ONE - see roundLockAt(). Without these the round's lock was
  // meta.firstPitch forever, frozen at creation, so a called-off opener sealed
  // every roster at a first pitch nobody threw.
  const games = roundGames(matchRows);
  const lockOpts = { games };
  const lockAt = roundLockAt(board, lockOpts);
  ...
  const state = rosterState(lineup, board, now, lockOpts);
  ...
  return {
    ...
    contest: {
      ...
      // THE LOCK THE CARD PRINTS AND COUNTS DOWN TO is the effective one, not
      // the frozen one: a reader watching a clock tick to a called-off game's
      // first pitch is watching a deadline that will not arrive.
      locksAt: Number.isFinite(lockAt) ? new Date(lockAt).toISOString() : board.firstPitch,
      ...
      msToLock: Number.isFinite(lockAt)
        ? Math.max(0, lockAt - new Date(now).getTime())
        : null,
    },
    ...
    slots: card.slots.map((s) => ({
      ...
      // "NOT STARTING - SWAP", and only while the round is still open. The Run
      // locks as a WHOLE at the round's first pitch, so `locked` is the round's
      // state and not the slot's - unlike October, where each slot locks at its
      // own game. Same function either way.
 ...
 * this shape.
 */
function panelFor(pool, lineups = new Map()) {
  const posted = {};
  if (!pool?.byClub) return { pool, posted };
  const byClub = {};
  for (const [key, rows] of Object.entries(pool.byClub)) {
    const cut = clubStarters(rows ?? [], { posted: lineups.get(String(key)) ?? null });
    byClub[key] = cut.rows;
