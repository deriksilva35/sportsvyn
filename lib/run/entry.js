...
import { usedPlayers, runPool, clubStarters, lineupsByClub } from './pool.js';
...
export async function runView(userId, contest, { now = new Date(), withPool = true } = {}) {
  ...
  const [rows, series, used, pool, done, lineups] = await Promise.all([
    boxForRound(contest),
    ...
    lineupsByClub(contest).catch(() => new Map()),
  ]);
