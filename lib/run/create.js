...
export function boardFor({ round, series = [], byeClubs = [], seeds = new Map(), probablesByKey = new Map() }) {
  ...
  const first = series
    .flatMap((s) => s.games.map((g) => new Date(g.kickoffAt).getTime()))
 ...
 */
export async function ensureRunRound({ round, season, now = new Date() } = {}) {
  ...
  if (statsApiEnabled(process.env)) {
    ...
    for (const d of days) {
      const byKey = await fetchProbablesByMatch(d).catch(() => new Map());
...
/** The round a reader is looking at: the open one, else the last settled. */
export async function currentRunRound({ now = new Date() } = {}) {
  const [c] = await sql`
    SELECT id, season_year, week, board, meta, opens_at, locks_at, settled, settled_at
      FROM contests
     WHERE game_type = 'run' AND sport = 'mlb' AND opens_at <= ${new Date(now).toISOString()}
     ORDER BY settled ASC, week DESC
     LIMIT 1`;
  return c ?? null;
}
