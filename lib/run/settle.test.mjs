...
import { scoreRoster, clubStateFrom, runTotal, settleGate } from './settle.js';
import { readFileSync } from 'node:fs';
import { SLOTS, DNF, rosterState } from './rules.js';
...
// 23 SEP, 06:00Z: settleDueRun read its rounds without meta, every preview
// round looked like a real round with no series, and "no undecided series" was
// true of an empty list. All four preview rounds settled, three of them days
// before a pitch was thrown.
test('NOTHING DECIDED IS NOT EVERYTHING DECIDED - the gate refuses an empty reading', () => {
  assert.equal(settleGate({ meta: undefined, series: [] }).ok, false, 'no meta');
  assert.equal(settleGate({ meta: {}, series: [] }).ok, false, 'no round');
  assert.equal(settleGate({ meta: { round: 'division' }, series: [] }).ok, false, 'no series');
  assert.equal(settleGate({ meta: { round: 'division' }, series: [{ key: 'a', winner: null }] }).ok, false);
  assert.equal(settleGate({ meta: { round: 'division' }, series: [{ key: 'a', winner: 7 }] }).ok, true);
});

test('A PREVIEW SETTLES ONLY WHEN EVERY GAME ON ITS BOARD IS FINAL', () => {
  const meta = { preview: true, round: 'wild_card' };
  assert.equal(settleGate({ meta, previewComplete: null }).ok, false);
  assert.equal(settleGate({ meta, previewComplete: { complete: false, remaining: 12 } }).ok, false);
  // A preview never falls through to the series branch, even with series handed in.
  assert.equal(settleGate({ meta, series: [{ key: 'a', winner: 1 }], previewComplete: { complete: false } }).ok, false);
  assert.equal(settleGate({ meta, previewComplete: { complete: true, remaining: 0 } }).ok, true);
});

test('settleDueRun READS META - the column whose absence settled the preview', () => {
  const src = readFileSync(new URL('./settle.js', import.meta.url), 'utf8');
  const due = src.split('export async function settleDueRun')[1].split('\n}\n')[0];
  assert.match(due, /SELECT[^`]*\bmeta\b[^`]*FROM contests/);
});

// THE LOCK IS THE CLUB'S, AND A POINT IS NEVER BACKFILLED (ruling of 24 Sep).
test('A LATE-ADDED BAT SCORES GAME 2, NOT GAME 1', () => {
  // TB played game 1 on the 29th; the bat went on the nine that night, before
  // game 2. Game 1's 12 points happened without him on anybody's roster.
  const rows = [
    { ...g(100, 1, { at_bats: 4, hits: 2, doubles: 1, rbi: 2 }), kickoff_at: '2026-09-29T14:05:00Z' },   // 12
    { ...g(101, 1, { at_bats: 3, hits: 1, home_runs: 1, runs: 1 }), kickoff_at: '2026-09-30T14:05:00Z' }, // 12
  ];
  const late = { bat1: { ...pick(1, 1), at: '2026-09-29T20:00:00Z' } };
  const bat1 = scoreRoster(late, rows, CLUBS).slots.find((s) => s.slot === 'bat1');
  assert.equal(bat1.games, 1, 'game 2 only');
  assert.equal(bat1.points, 12);
  assert.equal(bat1.line, '1-3 · HR · 1 R', "the sub-line is the games he was on the nine for");
  // Made before game 1, he scores both - the stamp only ever takes away what
  // happened before the pick.
  const early = { bat1: { ...pick(1, 1), at: '2026-09-29T10:00:00Z' } };
  assert.equal(scoreRoster(early, rows, CLUBS).slots.find((s) => s.slot === 'bat1').points, 24);
  // A pick with no stamp predates the rule and scores every game, as before.
  assert.equal(scoreRoster({ bat1: pick(1, 1) }, rows, CLUBS).slots.find((s) => s.slot === 'bat1').points, 24);
});

test('SIX EMPTY AT ROUND END ARE DNF - AND THE THREE FILLED STILL SCORE', () => {
  const three = { arm1: pick(10, 1), bat1: pick(1, 1), bat3: pick(3, 3) };
  const rows = [
    g(100, 1, { at_bats: 4, hits: 2, doubles: 1, rbi: 2 }),                  // 12
    g(100, 3, { at_bats: 4, hits: 1, home_runs: 1, rbi: 1, runs: 1 }),       // 13
    g(100, 10, { outs_recorded: 18, strikeouts_pitched: 6, earned_runs: 2 }),
  ];
  const card = scoreRoster(three, rows, CLUBS);
  const v = rosterState(three, { final: true });
  assert.equal(v.state, 'short', 'a partial nine is not a DNF round');
  assert.equal(v.dnfSlots, 6);
  assert.ok(card.total > 0, 'it scores what it scored');
  assert.equal(card.filled, 3);
  assert.equal(card.slots.filter((s) => s.state === 'empty').length, 6);
  // The settle's own sum: a DNF round would be 0, a short one keeps its total.
  const points = v.state === DNF ? 0 : card.total;
  assert.equal(points, card.total);
});

test('THE SETTLE DECIDES DNF WITH final: true, and boxForRound carries each kickoff', () => {
  const src = readFileSync(new URL('./settle.js', import.meta.url), 'utf8');
  assert.match(src, /rosterState\(e\.lineup \?\? \{\}, \{ final: true \}\)/);
  assert.match(src, /JOIN matches m ON m\.id = s\.match_id/);
});
