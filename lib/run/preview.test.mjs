...
import { clubStarters, probablesByClub } from './pool.js';
...
// THE PANEL'S ARMS ARE STARTERS (hotfix 24 Sep).
const arm = (name, position, ppg, extra = {}) => ({ playerId: name, name, kind: 'arm', team: 'NYY', position, ppg, ...extra });
const bat = (name, ppg) => ({ playerId: name, name, kind: 'bat', team: 'NYY', position: 'OF', ppg });

test('ARMS ARE SPs OR THE ANNOUNCED PROBABLE - no RP is ever offered', () => {
  const rows = [arm('Gerrit Cole', 'SP', 18), arm('David Bednar', 'RP', 4), arm('Luke Weaver', 'RP', 5),
    arm('Cam Schlittler', 'SP', 24), bat('Aaron Judge', 11), bat('Juan Soto', 13)];
  // No probable: the SPs by PPG, then the bats by PPG. Neither RP.
  const none = clubStarters(rows).rows.map((r) => r.name);
  assert.deepEqual(none, ['Cam Schlittler', 'Gerrit Cole', 'Juan Soto', 'Aaron Judge']);
  // An opener LISTED AS RP who is today's probable is offered - first, flagged.
  const opener = clubStarters(rows, { probable: { name: 'Luke Weaver' } }).rows;
  assert.equal(opener[0].name, 'Luke Weaver');
  assert.equal(opener[0].probable, true);
  assert.ok(!opener.some((r) => r.name === 'David Bednar'), 'the other RP still is not');
  // A SP probable leads the SPs.
  const cole = clubStarters(rows, { probable: { name: 'Gerrit Cole' } }).rows.filter((r) => r.kind === 'arm');
  assert.deepEqual(cole.map((r) => [r.name, r.probable]), [['Gerrit Cole', true], ['Cam Schlittler', false]]);
});

test('BATS BY PPG, each carrying his batting order once the card is up', () => {
  const rows = [bat('Aaron Judge', 11), bat('Juan Soto', 13), bat('Benched Guy', 20)];
  const posted = [{ name: 'Aaron Judge', order: 2 }, { name: 'Juan Soto', order: 3 }];
  const cut = clubStarters(rows, { posted }).rows;
  assert.deepEqual(cut.map((r) => [r.name, r.order]), [['Juan Soto', 3], ['Aaron Judge', 2]]);
});

test('THE PROBABLE IS READ OFF EACH CLUB\'S NEXT UNFINISHED GAME', () => {
  const m = (id, ko, home, away, status, probables) => ({ id, kickoff_at: ko, home_team_id: home, away_team_id: away, status, probables });
  const by = probablesByClub([
    m(2, '2026-09-25T23:05:00Z', 10, 11, 'scheduled', { home: { name: 'Gerrit Cole' }, away: { name: 'X' } }),
    m(1, '2026-09-24T23:05:00Z', 10, 11, 'final', { home: { name: 'Old Guy' }, away: { name: 'Y' } }),
    m(3, '2026-09-24T23:05:00Z', 12, 13, 'scheduled', null),
  ]);
  assert.equal(by.get('10').name, 'Gerrit Cole', 'the final game is skipped');
  assert.equal(by.get('11').name, 'X');
  assert.equal(by.get('12'), null, 'not announced yet');
});
