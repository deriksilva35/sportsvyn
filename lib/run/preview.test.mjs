...
import { refuseReason, SLOTS, MAX_PER_CLUB, reopenPreview, roundLockAt, isLocked } from './rules.js';
...
// THE PREVIEW'S MERCY, ruled 24 Sep. Round 23 had settled by mistake and was
// unsettled after its 12:35 ET lock had passed; it reopens on the games still
// ahead of it.
const RG = (id, iso, home, away, status = 'scheduled') =>
  ({ matchId: String(id), kickoffAt: iso, status, homeTeamId: home, awayTeamId: away });
const R23 = {
  meta: { preview: true, firstPitch: '2026-09-24T16:35:00.000Z', round: 'wild_card' },
  clubs: [24, 25, 26, 27, 28, 29].map((id) => ({ teamId: id, abbr: `T${id}`, bye: false })),
  games: [
    RG(1, '2026-09-24T16:35:00Z', 24, 25, 'live'),      // 12:35 ET - under way at 13:00
    RG(2, '2026-09-24T22:05:00Z', 26, 27),              // 18:05 ET - the next first pitch
    RG(3, '2026-09-24T23:05:00Z', 28, 29),
  ],
};
const AT_1300_ET = '2026-09-24T17:00:00Z';
const bat = (teamId) => ({ playerId: `p${teamId}`, teamId, kind: 'bat' });

test('A ROUND REOPENED AT 13:00 ET WITH ONE GAME UNDER WAY LOCKS AT THE NEXT GAME', () => {
  const r = reopenPreview({ ...R23, now: AT_1300_ET });
  assert.equal(r.ok, true);
  assert.equal(r.meta.firstPitch, '2026-09-24T22:05:00.000Z');
  const board = { ...R23.meta, ...r.meta, clubs: r.clubs };
  const games = R23.games;
  // The lock the card prints and the door reads - the started game's 16:35Z
  // does not drag it back.
  assert.equal(roundLockAt(board, { games }), Date.parse('2026-09-24T22:05:00Z'));
  assert.equal(isLocked(board, AT_1300_ET, { games }), false, 'open at 13:00 ET');
  assert.equal(isLocked(board, '2026-09-24T22:05:00Z', { games }), true, 'locked at the next first pitch');
});

test('...AND REFUSES PICKS FROM THE STARTED ONE, on the board and at the door', () => {
  const r = reopenPreview({ ...R23, now: AT_1300_ET });
  const board = { ...R23.meta, ...r.meta, clubs: r.clubs };
  // THE BOARD: both clubs of the started game are gone from the pool.
  assert.deepEqual(r.clubs.map((c) => c.teamId), [26, 27, 28, 29]);
  assert.deepEqual(r.meta.reopen.startedMatchIds, ['1']);
  // THE DOOR: named, not a generic "not in this round".
  const opts = { board, now: AT_1300_ET, games: R23.games };
  assert.equal(refuseReason({}, 'bat1', bat(24), opts), 'game_started');
  assert.equal(refuseReason({}, 'bat1', bat(25), opts), 'game_started');
  assert.equal(refuseReason({}, 'bat1', bat(26), opts), null);
});

test('THE MERCY IS PREVIEW-ONLY, frozen at the reopen, and needs a game left', () => {
  assert.equal(reopenPreview({ ...R23, meta: { ...R23.meta, preview: false }, now: AT_1300_ET }).reason, 'not-preview');
  assert.equal(reopenPreview({ ...R23, now: '2026-09-25T00:00:00Z' }).reason, 'nothing-left');
  // Frozen: once game 2 starts the round is locked - the lock does not slide on
  // to game 3.
  const r = reopenPreview({ ...R23, now: AT_1300_ET });
  const board = { ...R23.meta, ...r.meta, clubs: r.clubs };
  assert.equal(isLocked(board, '2026-09-24T22:30:00Z', { games: R23.games }), true);
  // A postponed game is not the next first pitch.
  const pp = reopenPreview({ ...R23, games: [R23.games[0], { ...R23.games[1], status: 'postponed' }, R23.games[2]], now: AT_1300_ET });
  assert.equal(pp.meta.firstPitch, '2026-09-24T23:05:00.000Z');
  assert.ok(pp.clubs.some((c) => c.teamId === 26), 'a postponed game\'s clubs have not started');
});
