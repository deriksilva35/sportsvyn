...
import { fetchRoster, fetchSeasonStats, ppgOf, kindOfPosition, batWithOrder, orderIndex } from '../october/pool.js';
...
export async function runPool(contest, { rebuild = false } = {}) {
  ...
  const pool = { builtAt: new Date().toISOString(), byClub, incomplete: failed > 0 };
  if (failed === 0) {
    await sql`
 ...
 */
export async function lineupsByClub(contest) {
