...
export async function settleRunRound(contest, { now = new Date() } = {}) {
  ...
  const round = contest.meta?.round ?? null;
  ...
  if (preview) {
    ...
    const r = await previewRoundComplete(contest);
    if (!r.complete) {
      return { contestId: contest.id, settled: false, remaining: r.remaining, preview: true };
    }
  } else {
    series = await seriesFor(round, contest.season_year);
    const undecided = series.filter((s) => s.winner == null);
    if (undecided.length) {
      return { contestId: contest.id, settled: false, remaining: undecided.length,
...
export async function settleDueRun({ now = new Date() } = {}) {
  const due = await sql`
    SELECT id, board, week, season_year FROM contests
