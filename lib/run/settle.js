...
export function scoreRoster(lineup = {}, rows = [], clubs = []) {
  ...
  const slots = SLOTS.map((slot) => {
    ...
    // NO BACKFILL. A pick scores the games that BEGIN at or after it was made
    // (pick.at, stamped by saveRunPick). A game already under way when he was
    // added is not his, however it went. A pick with no stamp predates the rule
    // and scores every game, as it always did.
    const since = pick.at ? new Date(pick.at).getTime() : NaN;
    const games = (byPlayer.get(String(pick.playerId)) ?? []).filter((g) => {
      if (!Number.isFinite(since)) return true;
      const ko = new Date(g.kickoff_at ?? NaN).getTime();
      return Number.isFinite(ko) && ko >= since;
    });
...
export async function boxForRound(contest) {
  ...
  // THE KICKOFF RIDES WITH EVERY ROW - scoreRoster needs it to know which of a
  // club's games began after a pick was made.
  return sql`
    SELECT s.match_id, s.bdl_player_id, s.at_bats, s.hits, s.doubles, s.triples, s.home_runs,
           s.rbi, s.runs, s.walks, s.stolen_bases,
           s.outs_recorded, s.strikeouts_pitched, s.wins, s.earned_runs, s.hits_allowed, s.walks_allowed,
           m.kickoff_at
      FROM mlb_player_game_stats s JOIN matches m ON m.id = s.match_id
     WHERE s.match_id = ANY(${ids})`.catch(() => []);
}
...
export async function settleRunRound(contest, { now = new Date() } = {}) {
  ...
  // DNF IS DECIDED HERE, when the round is - per slot. A partial nine scores
  // what it scored and its empty slots are the DNF; only a nine with nothing
  // in it is a DNF round.
  let dnf = 0;
  for (const e of entries) {
    const { state, dnfSlots } = rosterState(e.lineup ?? {}, { final: true });
    ...
    run: { state, filled: card.filled, dnfSlots, out: card.out, raw: card.total, preview },
  })}::jsonb,
