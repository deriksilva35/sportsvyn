 ...
 */
export async function runBoard(season, { memberIds = null, preview = false, now = new Date(), limit = 50 } = {}) {
  // `memberIds != null`, NEVER A FALSY SHORTCUT - an EMPTY league is an EMPTY
  // board, not the world. `[]` is truthy in JS, so a truthy test happens to be
  // right while reading as though it were not; the explicit null check is the
  // house rule (lib/leagues/core.test.mjs) and lib/october/board.js's twin.
  const rows = memberIds != null
    ? await sql`
      SELECT e.user_id, u.handle, u.name, u.is_house, c.week, c.settled,
             COALESCE(c.week, (c.meta->>'roundIndex')::int) AS round_index,
  ...
  for (const r of rows) {
    ...
    // A PREVIEW ROUND HAS NO WEEK - its column comes from meta.roundIndex.
    const round = ROUNDS[Number(r.round_index) - 1];
    if (!round) continue;
