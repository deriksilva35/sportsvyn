...
test('generate-briefs is restricted to leagues whose event data exists', () => {
  // The rule has never been "no football". It is that a league is briefed only
  // once the rows its envelope reads actually exist.
  //
  // For the NFL they now do. lib/gridiron/gameBrief.js assembles from
  // gridiron_game_events and gridiron_player_lines - real scoring plays and
  // real player lines - so 'nfl' is in on the same evidence every soccer slug
  // is in on, and the cron routes it to that envelope rather than to the soccer
  // one.
  //
  // CFB IS STILL OUT, and this is the assertion that keeps it out: the college
  // feed serves no scoring plays, so its envelope would still be a score, two
  // names and three empty arrays - the thin brief, on 934 final games.
  assert.ok(Array.isArray(BRIEF_LEAGUE_SLUGS) && BRIEF_LEAGUE_SLUGS.length > 0);
  assert.ok(!BRIEF_LEAGUE_SLUGS.includes('cfb'),
    'cfb must not be briefed until it has play data');
  assert.ok(BRIEF_LEAGUE_SLUGS.includes('nfl'), 'nfl has play data and its own envelope');
  assert.match(cron, /generateGameBrief/, 'and it is actually routed to it');
...
test('the NFL is briefed only when somebody TURNS IT ON', () => {
  // Being in the allowlist makes a league eligible. An environment variable
  // decides whether it is eligible today. Shipping the writer and enabling it
  // are two decisions, and only one of them is a deploy.
  // Source-checked, like everything else here: a route handler cannot be
  // imported under node --test.
  assert.match(cron, /GRIDIRON_BRIEFS_ENABLED === '1'/);
  assert.match(cron, /const GRIDIRON_SLUGS = new Set\(\['nfl'\]\)/);
  // OFF BY DEFAULT. An unset variable must never mean "start writing", so the
  // comparison is to '1' rather than to a falsy check, and the filter removes
  // the gridiron slugs rather than adding them.
  assert.match(cron,
    /return enabled \? BRIEF_LEAGUE_SLUGS : BRIEF_LEAGUE_SLUGS\.filter\(\(s\) => !GRIDIRON_SLUGS\.has\(s\)\)/);
  assert.ok(!/GRIDIRON_SLUGS\.has\(s\)\s*\)\s*:\s*BRIEF_LEAGUE_SLUGS/.test(cron),
    'the branches must not be the wrong way round');
});
...
test('the allowlist is applied in the candidate query, not filtered afterwards', () => {
  ...
  assert.match(cron, /WHERE l\.slug = ANY\(\$\{slugs\}::text\[\]\)/);
  assert.match(cron, /const slugs = activeLeagueSlugs\(gridironBriefsEnabled\(\)\)/,
    'and `slugs` is the allowlist after the enablement switch, not a loose variable');
...
test('both codes can be WRITTEN ABOUT; only one of them can be BRIEFED', () => {
  // Easy to read as a contradiction later. Topic drafts run off rankings,
  // season records and research, all of which exist for both gridiron leagues.
  // Tier 1 briefs run off play-by-play, which exists for one of them.
  for (const g of ['nfl', 'cfb']) {
    assert.ok(TOPIC_DRAFT_LEAGUE_SLUGS.includes(g), `${g} can be written about`);
  }
  assert.ok(BRIEF_LEAGUE_SLUGS.includes('nfl'), 'the NFL feed serves scoring plays');
  assert.ok(!BRIEF_LEAGUE_SLUGS.includes('cfb'), 'the college feed does not');
});
