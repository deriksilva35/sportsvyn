 ...
 *
 * AND THE LEAGUE'S HOLDINGS ARE NOT ON THE BOARD, the same rule and the SAME MAP
 * the NFL half uses. A devy player already on somebody's minors shelf is not
 * draftable, and the first college board shipped without this: 18 held players
 * sat on it, Kewan Lacy at placement 10000 and Ryan Wingo - on the importer's
 * own shelf - at 10088.
 *
 * `held` IS playerInfo's STATUS MAP, NOT THE MINORS SHELF, and that is the point.
 * Measured on the live league: all 35 MINORS roster items appear in playerInfo
 * with a held status, so the map the NFL half already builds covers the college
 * half exactly, with no second source of truth. Reading draft_configs.teams[]
 * .minors instead would have been worse twice over - it is written AFTER the
 * pool in the same import, so it describes the PREVIOUS run, and measured
 * tonight its id set already disagreed with the live rosters.
 */
export function toCollegePoolRows(adp, crosswalk, { snapshotDate, scoringFormat, teamsCount, slots = null, exclude = null }) {
  const allowed = slots ? draftablePositions(slots) : null;
  const skipped = [];
  const candidates = [];
  let excluded = 0;
  for (const a of adp ?? []) {
    ...
    const position = toPoolPosition(a.pos ?? x?.position, { isTeamRow });
    if (allowed && !allowed.has(position)) { skipped.push({ reason: `undraftable position ${position}`, id }); continue; }
    // AFTER the position gate, so the arithmetic the importer prints is about
    // players this league could otherwise have drafted - the same ordering the
    // NFL path uses for the same reason.
    const status = exclude?.get(id) ?? null;
    if (status) { excluded += 1; skipped.push({ reason: `rostered (${status})`, id, name }); continue; }
...
export async function importFantraxLeague(sql, { userId, leagueId, now = new Date(), fetchers = api, rookieFixture = null, myOwner = null }) {
  const summary = { leagueId, userId, poolWritten: 0, poolCandidates: 0, poolDraftable: 0, poolExcluded: 0, collegeWritten: 0, collegeSkipped: 0, collegeExcluded: 0, keepers: 0, minors: 0, skipped: [] };
  ...
  //
  // EXCLUDED BY THE SAME `held` MAP AS THE NFL ROWS. An earlier version of this
  // comment claimed playerInfo could not describe a devy player. That was wrong,
  // and measuring it is what corrected it: all 35 MINORS roster items are in
  // playerInfo with a held status, so one map covers both halves.
  const { rows: collegeRows, skipped: collegeSkipped, excluded: collegeExcluded } = toCollegePoolRows(ncaafAdp, ncaaf, {
    snapshotDate, scoringFormat: scoring.format, teamsCount, slots, exclude: held,
  });
  ...
  summary.collegeExcluded = collegeExcluded;
