...
export async function importFantraxLeague(sql, { userId, leagueId, now = new Date(), fetchers = api, rookieFixture = null, myOwner = null }) {
  const summary = { leagueId, userId, poolWritten: 0, poolCandidates: 0, poolDraftable: 0, poolExcluded: 0, keepers: 0, minors: 0, skipped: [] };
  ...
  const [leagues, info, results, playerIds, ncaafIds, adp, rosters] = await Promise.all([
    ...
    fetchers.getAdp(), fetchers.getTeamRosters(leagueId),
  ]);
  ...
  for (const k of keepers) {
    await sql`
      INSERT INTO draft_config_keepers (config_id, team_slot, round, pick_in_round,
                                        fantrax_player_id, player_name, position, adp, team)
      VALUES (${cfg.id}, ${k.team_slot}, ${k.round}, ${k.pick_in_round},
              ${k.fantrax_player_id}, ${k.player_name}, ${k.position}, ${k.adp}, ${k.team})
      ON CONFLICT (config_id, round, pick_in_round) DO NOTHING`;
    summary.keepers += 1;
  }
