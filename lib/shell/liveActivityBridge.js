...
//
//   window.postMessage({ type:'startLiveActivity', matchId, url, league, state }, '*')
...
export function startMessage({ matchId, url, league = null, state } = {}) {
  ...
  //
  // league IS TOP LEVEL TOO, and static like the url: the match's league slug
  // ('nfl', 'cfb', 'mlb') off its league row - the same value that builds the
  // /<league>/game/ path, handed in rather than parsed back out of the url.
  // null, not absent, when the caller has none, so the key is always there.
  const lg = typeof league === 'string' && league.trim() ? league.trim() : null;
  return { type: 'startLiveActivity', matchId: id, url, league: lg, state: contentState(state) };
}
 ...
 */
export function startLiveActivity({ matchId, url, league = null, state } = {}) {
  return post(startMessage({ matchId, url, league, state }));
}
