...
//
//   window.postMessage({ type:'startLiveActivity', matchId, state, url }, '*')
...
export function startMessage({ matchId, state, url } = {}) {
  ...
  return { type: 'startLiveActivity', matchId: id, state: contentState(state), url };
}
...
/**
 * Start - or, for a matchId already running, update. The one-at-a-time rule is
 * the NATIVE side's to keep (a start for a different match ends the current
 * one first; a start for the same match updates it in place), so the web side
 * sends the same message either way and does not track what is running.
 * @returns {boolean} whether the message was posted.
 */
export function startLiveActivity({ matchId, state, url } = {}) {
  return post(startMessage({ matchId, state, url }));
}
