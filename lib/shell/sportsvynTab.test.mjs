...
test('the sign-in guard precedes the data reads on both pages', () => {
  // Same v0.6 note: the fantasy board has no gate to precede its reads.
  for (const [p, read] of [
    ['app/scores/page.js', 'getSlateByDate'],
  ]) {
    // Same anchor correction as signedOut.test.mjs: /scores' body is now the
    // exported ScoresView, mounted by both the network route and the
    // league-pinned one, with a two-line default export below it. The rule is
    // unchanged - the redirect must not pay for the reads - so the slice
    // starts at whichever export actually holds the guard.
    const whole = stripComments(src(p));
    const guardAt = whole.indexOf('requireSignInInShell(');
    const exps = [...whole.matchAll(/^export (default )?async function/gm)].map((m) => m.index);
    const t = whole.slice(exps.filter((i) => i < guardAt).pop() ?? whole.indexOf('export default'));
