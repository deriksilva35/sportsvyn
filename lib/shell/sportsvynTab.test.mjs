...
test('ONE "TODAY" IN THE SHELL, and it is the app bar\'s', () => {
  // The collision this rename exists to prevent, asserted from both sides so
  // it cannot come back by editing either list alone.
  assert.equal(TABS.some((t) => t.label === 'Today'), false, 'the segment has no Today');
  assert.equal(appTabs.APP_TABS.filter((t) => t.label === 'Today').length, 1);
  assert.equal(appTabs.APP_TABS.find((t) => t.label === 'Today').href, '/nfl');
});
