...
// /nfl/fantasy left this list in v0.6: as a league route it no longer carries a
// shell sign-in gate, so a test asserting the gate's SHAPE there has nothing to
// inspect. lib/shell/signedOut.test.mjs asserts the absence instead.
const PAGES = ['app/scores/page.js'];
const appTabs = await import('./appTabs.js');
...
test('the web cross-nav is the inverse gate - web only', () => {
  for (const p of PAGES) {
    ...
    // THE CLAIM IS THE GATE, and !isShell must be the first term of it. Further
    // conditions may follow - /scores adds `&& !pinned`, because the league
    // wearing of that board (/nfl/scores) has the league header for its
    // cross-navigation and does not want the network one - but nothing may
    // render this link inside the shell.
    assert.match(t, /\{!isShell &&(?: [^<]*&&)? <(a|Link) className="gi-cross"/,
      `${p}: cross-nav must be gated on !isShell - the shell's hop is the segment`);
  }
});
...
test('NEITHER SPORTSVYN SURFACE HAS A GUARD LEFT TO PRECEDE ITS READS', () => {
  // WHAT THIS USED TO CLAIM. It walked from the export holding
  // requireSignInInShell( down to the first data read and asserted the guard
  // came first - the redirect must not pay for the reads. That claim needed a
  // guard to exist. /nfl/fantasy lost its guard in v0.6 (a league route), and
  // /scores lost its own once the gate was re-measured: a redirect() inside a
  // streamed Suspense boundary serves 200 with a <meta http-equiv="refresh">,
  // so the status sweep that reported the board "already open" was blind to
  // the redirect it was firing. Re-ruled to open, like /market beside it.
  //
  // The ordering rule is NOT weakened - it still holds everywhere a guard
  // lives, and signedOut.test.mjs enforces it there. What is asserted here is
  // that these two pages are out of that set, which is the stronger statement:
  // a guard that does not exist cannot be mis-ordered.
  for (const p of ['app/scores/page.js', 'app/nfl/fantasy/page.js']) {
    const t = stripComments(src(p));
    assert.doesNotMatch(t, /requireSignInInShell/, `${p} must stay open in the shell`);
    assert.match(t, /getSlateByDate|FantasyBoard|fantasy/i, `${p}: still renders its board`);
  }
});
...
test('PROFILE is off the bar, and the Sportsvyn surfaces stay reachable', () => {
  // THE BAR WENT TO FOUR (Play / Scores / Rankings / You) and the 'sportsvyn'
  // tab went with the fifth slot. What that tab was FOR has to survive, so
  // this now pins the reachability instead of the tab key: the segment that
  // carries Home, Fantasy and Market renders on /scores, which IS on the bar.
  // Pinning the key would have gone on passing while the surfaces went dark.
  ...
  assert.match(scores, /isShell && <SportsvynSegment \/>/,
    'the segment is the only route to Home/Fantasy/Market in the shell');
  const barHrefs = appTabs.APP_TABS.map((x) => x.href);
  assert.ok(barHrefs.includes('/scores'), 'and the page carrying it must be on the bar');
  for (const t2 of TABS) {
    assert.ok(t2.href, `${t2.key} still names a destination`);
  }
});
...
// ---------------------------------------------------------------------------
// FOUR TABS (Home | Scoreboard | Fantasy | Market)
// ---------------------------------------------------------------------------

test('four tabs, in reading order, with the ruled targets', () => {
  // "TODAY" BECAME "HOME" in v0.6: the app bar gained a Today tab of its own
  // pointing at /nfl, and one label on two tabs at two levels going to two
  // places is a thing a reader has to learn for no reason. This one points at
  // "/", the Daily Card, and Home is what that is.
...
test('NO LABEL APPEARS ON BOTH THE BAR AND THE SEGMENT', () => {
  // THE RULE, NOT ONE INSTANCE OF IT. This started life pinning that only the
  // app bar said "Today" - the v0.6 rename existed because one label on two
  // tabs at two levels going to two places is a thing a reader has to learn
  // for no reason. The four-tab bar has no Today at all now, which satisfies
  // that pin by deleting its subject and would have left the general case
  // unguarded. So the general case is what is asserted: the two lists may
  // never share a word, whatever either of them becomes.
  //
  // AND THE RULE IS ABOUT DESTINATIONS, NOT WORDS. The defect it was written
  // for was "Today" on both lists going to TWO DIFFERENT PLACES - a reader had
  // to learn which Today was which. When both lists carry a label that leads to
  // the SAME page, there is nothing to learn: the app bar's Market and the
  // segment's Market are one destination reachable at two levels, which is what
  // a section tab is for. The query is not the destination - ?tab=props picks a
  // view inside /market, exactly as the page's own strip does - so the paths are
  // compared and the queries are not.
  const bar = appTabs.APP_TABS.map((t) => t.label);
  const seg = TABS.map((t) => t.label);
  const route = (href) => String(href).split(/[?#]/)[0];
  const segByLabel = new Map(TABS.map((t) => [t.label, route(t.href)]));
  const conflicting = appTabs.APP_TABS
    .filter((t) => segByLabel.has(t.label) && segByLabel.get(t.label) !== route(t.href))
    .map((t) => `${t.label} -> ${route(t.href)} vs ${segByLabel.get(t.label)}`);
  assert.deepEqual(conflicting, [],
    `one label, two destinations: ${conflicting.join('; ')}`);
  // A label shared across the two lists must therefore agree, and Market is the
  // case that proves it rather than an exception carved out for it.
  assert.equal(segByLabel.get('Market'), '/market');
  assert.equal(route(appTabs.APP_TABS.find((t) => t.key === 'market').href), '/market');
  // And neither list may carry the same label twice within itself.
  assert.equal(new Set(bar).size, bar.length, 'the bar repeats a label');
  assert.equal(new Set(seg).size, seg.length, 'the segment repeats a label');
});
