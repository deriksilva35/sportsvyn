...
export function resumeDecision({ gapMs = null, dataTab = null, deepLinkPending = false, pathname = '' } = {}) {
  ...
  // THE SIGN-IN SURFACE IS EXEMPT: mount now evaluates on every document, and
  // a signed-out reader ON /signin has no stamp - the cold-start branch would
  // yank them off the form mid-OTP into the guard's bounce. Sign-in completes
  // to /games via callbackUrl already; this rule has no business there.
  if (String(pathname).startsWith('/signin')) return null;
