...
// it is made.
// FIVE TABS, IN THIS ORDER: PLAY · SCORES · MARKET · RANKINGS · YOU.
// Today is still off the bar (YOU TAB v1, Q5): it is reachable from the Scores
// tab's league pills and from the site header, and a front door is a logo, not
// a seat.
//
// MARKET SITS THIRD, BETWEEN SCORES AND RANKINGS, because the three reading
// surfaces belong together in the middle - what the games are doing, what the
// books think, how the teams stack up - with Play at the front and You at the
// end. It used to be reachable only by lighting the SCORES tab and finding a
// link, which is how a whole section stays unfound.
//
// ITS HREF CARRIES ?tab=props ON PURPOSE. Props is the surface the Market page
// was rebuilt around and the one a reader arriving from a bottom bar wants;
// Lines is one tap away on the page's own tab strip, and nothing is hidden.
//
...
export const APP_TABS = [
  ...
  // AN EMOJI, LIKE THE THREE AROUND IT. The Y-monogram on YOU is the one
  // exception and is the brand's own mark; a second non-emoji glyph here would
  // read as a second brand rather than as a section.
  { key: 'market', label: 'Market', icon: '📈', href: '/market?tab=props' },
...
export function activeTabFor(pathname, override = null) {
  ...
  // MARKET IS TESTED BEFORE THE LEAGUE ROUTES, and it has to be. A league's
  // market page is /cfb/market, which starts with /cfb - left below, the league
  // branch would swallow it and light SCORES on the very page the MARKET tab
  // points at. Longest prefix first is the rule of this function, and this is
  // the case it exists for. /market covers the player card at
  // /market/props/<slug> by the same prefix, so a card lights the tab it
  // belongs to rather than nothing.
  // THE BOUNDARY IS A SEGMENT, NOT A PREFIX. startsWith('/market') also claims
  // /marketing and anything else that merely begins with those characters -
  // the same class of mistake the cookie check at the top of this file carries
  // a comment about. A path owns a tab when the SEGMENT matches.
  if (/^\/(nfl|cfb|epl)\/market(?=[/?#]|$)/.test(p)) return 'market';
  if (/^\/market(?=[/?#]|$)/.test(p)) return 'market';
  ...
  // /scores serves all three at once.
