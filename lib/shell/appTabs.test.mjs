...
test('the tabs, in order - MOCK AND TRACKER CAME OFF, RANKINGS CAME ON', () => {
  ...
  assert.deepEqual(APP_TABS.map((t) => t.key), ['games', 'scores', 'rankings', 'today', 'sportsvyn']);
  assert.deepEqual(APP_TABS.map((t) => t.href), ['/games', '/scores', '/rankings', '/nfl', '/']);
  assert.deepEqual(APP_TABS.map((t) => t.label), ['Play', 'Scores', 'Rankings', 'Today', 'Sportsvyn']);
  // Neither retired key survives anywhere in the list.
  assert.equal(APP_TABS.some((t) => t.key === 'practice' || t.key === 'tracker'), false);
});
...
test('surfaces outside the app light NOTHING rather than guessing', () => {
  ...
  // one.
  assert.equal(activeTabFor('/'), 'sportsvyn');
  for (const p of ['/membership', '/privacy']) {
    assert.equal(activeTabFor(p), null, `${p} must light nothing`);
  }
  // ABSENT INPUT IS NOT THE FRONT DOOR. null and undefined collapse to an
  // empty string inside the resolver, and lighting Sportsvyn for them would
  // put the bar in a state no route produced.
...
test('the override still wins when it names a tab that exists', () => {
  ...
  assert.equal(activeTabFor('/games', 'scores'), 'scores');
});
...
test('FIVE TABS, one for playing and four for reading', () => {
  const labels = APP_TABS.map((t) => t.label);
  assert.deepEqual(labels, ['Play', 'Scores', 'Rankings', 'Today', 'Sportsvyn']);
  // Play is what you DO and it keeps the first slot; Scores, Rankings, Today
  // and Sportsvyn are what you READ, and they sit together after it.
  assert.equal(labels.indexOf('Play'), 0);
  assert.equal(APP_TABS.find((t) => t.key === 'today').href, '/nfl');
  assert.equal(APP_TABS.find((t) => t.key === 'rankings').href, '/rankings');
});

test('TODAY OWNS THE LEAGUE ROUTES, SCORES OWNS THE NETWORK ONES', () => {
  ...
  assert.equal(activeTabFor('/'), 'sportsvyn', 'the front door has a tab now');
});
