...
test('the tabs, in order - PLAY · SCORES · MARKET · RANKINGS · YOU', () => {
  ...
  //
  // MARKET JOINED THIRD. The order is named here rather than counted, because
  // a count is what a future tab quietly breaks and a name is what it has to
  // argue with: the three reading surfaces sit together in the middle, Play at
  // the front, You at the end.
  assert.deepEqual(APP_TABS.map((t) => t.key), ['games', 'scores', 'market', 'rankings', 'you']);
  assert.deepEqual(APP_TABS.map((t) => t.href),
    ['/games', '/scores', '/market?tab=props', '/rankings', '/you']);
  assert.deepEqual(APP_TABS.map((t) => t.label),
    ['Play', 'Scores', 'Market', 'Rankings', 'You']);
  // MARKET SITS BETWEEN SCORES AND RANKINGS - stated as a relationship, so a
  // reordering has to break a sentence rather than shuffle an index.
  const keys = APP_TABS.map((t) => t.key);
  assert.equal(keys.indexOf('market'), keys.indexOf('scores') + 1);
  assert.equal(keys.indexOf('market'), keys.indexOf('rankings') - 1);
  assert.equal(keys[keys.length - 1], 'you', 'You is still last');
...
test('THE MARKET TAB OPENS ON PROPS, and Lines is one tap away on the page', () => {
  const t = APP_TABS.find((x) => x.key === 'market');
  assert.equal(t.label, 'Market');
  assert.equal(t.href, '/market?tab=props');
  // AN EMOJI, like the three tabs around it. The Y-monogram on YOU is the one
  // exception and is the brand's own mark.
  assert.equal(t.icon, '📈');
  assert.notEqual(t.icon, 'Ȳ', 'only the brand tab carries the monogram');
});
...
test('NEITHER OF THE BROWSE ROUTES GOES DARK - but they light different tabs now', () => {
  ...
  for (const p of ['/scores', '/scores?date=2026-09-10']) {
    assert.equal(activeTabFor(p), 'scores', `${p} must light SCORES`);
  }
  // /market HAS ITS OWN TAB NOW. It lit SCORES only because it had nowhere
  // else to go; the rule that mattered - never dark on a browse surface - is
  // unchanged, and the owner moved.
  assert.equal(activeTabFor('/market'), 'market');
...
test('FIVE TABS: play, watch, price, compare, yourself', () => {
  // The sentence gained a word rather than a number: MARKET is "price", and it
  // belongs beside watch and compare because all three are reading.
  const labels = APP_TABS.map((t) => t.label);
  assert.deepEqual(labels, ['Play', 'Scores', 'Market', 'Rankings', 'You']);
  assert.equal(labels.indexOf('Play'), 0, 'what you DO keeps the first slot');
  assert.equal(labels[labels.length - 1], 'You', 'and yourself is last');
...
test('SCORES OWNS THE LEAGUE ROUTES AND THE NETWORK ONES', () => {
  ...
  for (const p of ['/nfl', '/nfl/scores', '/nfl/wire', '/nfl/standings',
                   '/nfl/rankings', '/nfl/fantasy',
                   '/cfb', '/cfb/scores', '/cfb/wire', '/epl', '/epl/standings']) {
    assert.equal(activeTabFor(p), 'scores', p);
  }
  // /nfl/market LEFT THIS LIST. A league's market page is a MARKET surface, and
  // it starts with /nfl - so the market test has to run first or the league
  // branch swallows the very page the tab points at.
  assert.equal(activeTabFor('/nfl/market'), 'market');
  // A league route is where a reader is inside ONE code; /scores serves all
  // three. The tab it lights is called SCORES, which is the name of the thing.
  assert.equal(activeTabFor('/scores'), 'scores');
...
test('every tab points at a route that exists', () => {
  ...
  for (const t of APP_TABS) {
    // THE QUERY IS NOT PART OF THE ROUTE. Market's href carries ?tab=props, so
    // the path has to be taken alone - otherwise this looked for a directory
    // literally named "market?tab=props" and failed a tab that resolves fine.
    const route = t.href.split(/[?#]/)[0];
    const file = path.join(REPO, 'app', route.replace(/^\//, ''), 'page.js');
    assert.ok(existsSync(file), `${t.label} -> ${t.href} has no route`);
  }
  // And a tab that carries a query must still name a real one: the Market tab
  // opens the props view, which is a value the page's own reader accepts.
  const market = APP_TABS.find((t) => t.key === 'market');
  assert.match(market.href, /^\/market\?tab=props$/);
});
