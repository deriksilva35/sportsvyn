...
test('shell detected via the ?shell=sim-app param, not just the cookie', () => {
...
test('ALREADY PURCHASED maps to purchased - they own it', () => {
  // Non-consumable: reinstall, restore, or a second device all land here.
  // "Nothing was charged" would be false; they already paid.
  assert.deepEqual(mapPurchaseError(rcError('6')), { ok: true, state: 'purchased' });
});
