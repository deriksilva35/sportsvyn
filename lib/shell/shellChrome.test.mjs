...
test('THE APP HEADER: the mark, the profile chip, and nothing else', () => {
  ...
  // chip to /account. The mark itself stays a non-link (home is a tab), and
  ...
  assert.equal(links.length, 1, 'exactly one link: the profile chip');
  assert.match(s, /<Link href="\/account" className="gh-app-me"/);
