...
test('the app wordmark is DRAFTVYN, sized from ITS OWN measurements', () => {
  ...
  assert.match(mark, /height: '1\.71em'/, 'sized from the measured 0.4167 cap ratio');
...
test('THE CAP HEIGHT IS PRESERVED ACROSS THE SWAP, which is the point of 1.71em', () => {
  // Arithmetic, pinned so a future resize cannot quietly change the type size:
  // sportsvyn 133/336 x 1.8 == draftvyn 125/300 x 1.71, to four decimals.
  const before = (133 / 336) * 1.8;
  const after = (125 / 300) * 1.71;
  assert.equal(before.toFixed(4), after.toFixed(4), `${before} vs ${after}`);
});
