...
//
// The p2 floor is context-aware: rich-context matches (bracket / standings /
// win_probability / watch_score) keep the 100-word floor; low-stakes matches
// (friendlies, undecided early group games) drop to 60 — forcing 100 there
// manufactures filler rather than meaning.
function gateWordCounts(parsed, envelope) {
  const richContext = !!(
  ...
  const p2Floor = richContext ? 100 : 60;
  ...
  if (p1 < 100 || p1 > 150) issues.push(`paragraph_1 ${p1} words (need 100-150)`);
...
function collectSourceMinutes(envelope) {
  const set = new Set();
  for (const e of envelope.events ?? []) {
    if (typeof e.minute === 'number') set.add(e.minute);
  }
  return set;
}
...
function gateHallucination(parsed, envelope) {
  const sourceTokens = collectSourceTokens(envelope);
  const sourceMinutes = collectSourceMinutes(envelope);
  ...
  for (const min of findReferencedMinutes(text)) {
    if (!sourceMinutes.has(min)) issues.push(`minute ${min}' not in source events`);
  }
