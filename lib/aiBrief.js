...
export function tokenizeName(name, into) {
  if (!name) return;
...
function collectSourceTokens(envelope) {
  ...
  // Player lines (gridiron). The gridiron envelope has no lineups - the
  // provider serves no start XI for football - and its named players arrive in
  // stat groups instead. Without this walk, every rusher and receiver the model
  // correctly named from the data would read as a hallucination, and the brief
  // would fall back to template on a game it had described accurately.
  for (const side of envelope.player_lines ?? []) {
    tokenizeName(side.team, tokens);
    for (const p of side.players ?? []) tokenizeName(p.name, tokens);
  }
  return tokens;
}
...
export const SENTENCE_START_OK = new Set([
...
export const COMMON_PROSE_CAPS = new Set([
...
export function findReferencedNames(text) {
  const out = new Set();
  ...
  while ((m = fullName.exec(text)) !== null) {
    // Strip trailing possessive 's / ' from the multi-word capture so
    // two-word team/country possessives match their base name in source.
    // The fullName regex includes ' in its wordChar class, so it captures
    // "Czech Republic's" / "Costa Rica's" / "Saudi Arabia's" / "South
    // Korea's" — all common at the WC — with the apostrophe-s intact.
    // Stripping here lets downstream grounding checks compare against the
    // bare token set (which has "republic", "rica", "arabia", "korea").
    // The single-word `poss` regex below already strips 's at capture
    // time; this brings the multi-word path into line. Apostrophe
    // variants — straight or curly. Safe for the Brief gate too (its
    // inline strip just becomes a no-op on an already-stripped string).
    out.add(m[1].replace(/['']s?$/, ''));
  }
...
export function gateHallucination(parsed, envelope) {
  ...
  for (const name of findReferencedNames(text)) {
    // THE CHECKER USES THE COLLECTOR'S OWN TOKENIZER. It did not, and the two
    // disagreed about hyphens: tokenizeName SPLITS ("Mercedes-Benz" ->
    // mercedes, benz) while the checker STRIPPED ("mercedesbenz"), a
    // concatenation that cannot match either half. Every hyphenated proper
    // noun was therefore unmatchable, however plainly it sat in the source.
    //
    // On 14 Aug that rejected both attempts at the Denver-Atlanta brief over
    // "Mercedes-Benz Stadium" - a venue the envelope itself had handed the
    // model - and published the deterministic template instead. Amon-Ra St.
    // Brown, JuJu Smith-Schuster and Nick Westbrook-Ikhine fail the same way.
    //
    // One tokenizer, used by both sides, or the sides drift again.
    const nameTokens = new Set();
    tokenizeName(name, nameTokens);
    const checkToks = [...nameTokens].filter((t) => t.length >= 3);
...
// ============================================================================
// Gate 3b — COUNT CLAIMS (gridiron only)
// ============================================================================
// The gates check names and minutes. They do not check ARITHMETIC, and the
// briefs kept getting arithmetic wrong in exactly one way: the model tallying
// the event list by hand instead of reading a count it was handed.
//
//   14 Aug, Miami at Washington: "20 consecutive points across the second
//   quarter". Washington scored 17 in the second and 3 in the fourth.
//   Same game, after the quarter fix: "all three Washington touchdowns".
//   Washington scored two. Three was the count for BOTH teams combined.
//
// That second one is the shape of the problem: the number existed in the data,
// just not for the team it was attached to. So this gate resolves each claim
// against the narrowest scope the sentence names — a quarter if it names one, a
// team if it names one — and only falls back to the whole game when the
// sentence commits to neither.
//
// IT ONLY RUNS ON GRIDIRON. Soccer envelopes carry no scoring_counts, and a
// gate that silently no-ops is better than one that invents a rule for a sport
// it was not written for.
const NUMBER_WORDS = {
  one: 1, two: 2, three: 3, four: 4, five: 5,
  six: 6, seven: 7, eight: 8, nine: 9, ten: 10,
};
// A number followed by one of these is a MEASUREMENT, not a count: "a 41-yard
// field goal" is not a claim that anyone kicked forty-one of them.
const MEASURE_WORDS = new Set([
  'yard', 'yards', 'yd', 'yds', 'yarder', 'minute', 'minutes', 'second', 'seconds',
  'foot', 'feet', 'game', 'games', 'week', 'weeks', 'carry', 'carries', 'catch',
  'catches', 'attempt', 'attempts', 'reception', 'receptions', 'target', 'targets',
  'completion', 'completions', 'pass', 'passes', 'play', 'plays', 'drive', 'drives',
]);
const QUARTER_WORDS_ORD = { first: 'First', second: 'Second', third: 'Third', fourth: 'Fourth' };
// Which derived tally answers each noun.
const COUNT_NOUNS = [
  { re: /^touchdowns?$/i, kind: 'TD' },
  { re: /^(?:td|tds)$/i, kind: 'TD' },
  { re: /^points?$/i, kind: 'POINTS' },
];
const FIELD_GOAL_KIND = 'FG';

const splitSentences = (t) => String(t ?? '').split(/(?<=[.!?])\s+/).filter(Boolean);

// BLOCKING OR ADVISORY. One constant, deliberately at the top where it is
// visible rather than buried in the return. See the note at the end of
// gateCountClaims for the evening of evidence behind `false`.
export const COUNT_CLAIMS_BLOCKING = false;

export function gateCountClaims(parsed, envelope) {
  const counts = envelope?.match?.scoring_counts;
  if (!counts) return { pass: true, reason: '' };

  const byTeam = counts.by_team ?? {};
  const byQuarter = counts.points_by_quarter_by_team ?? {};
  const teams = Object.keys(byTeam);
  const teamTokens = teams.map((t) => {
    const set = new Set(); tokenizeName(t, set); return { team: t, tokens: set };
  });

  // Every count a claim could legitimately be making, per scope.
  const teamCount = (team, kind) => (kind === 'POINTS'
    ? Object.values(byQuarter).reduce((a, q) => a + (q[team] ?? 0), 0)
    : (byTeam[team]?.[kind] ?? 0));
  const gameTotals = (kind) => {
    const per = teams.map((t) => teamCount(t, kind));
    return new Set([...per, per.reduce((a, b) => a + b, 0)]);
  };

  const issues = [];
  // BLOCKS ARE SCANNED SEPARATELY, not joined. A headline does not reliably end
  // in a period, so joining ran it into paragraph_1 as one "sentence": the team
  // names in the headline then made every paragraph_1 claim look game-wide, and
  // a stray "seven" in the headline reached a "touchdowns" four words into the
  // paragraph. Both were false readings produced purely by the concatenation.
  const blocks = [parsed.headline, parsed.paragraph_1, parsed.paragraph_2, parsed.paragraph_3 ?? '']
    .filter((s) => typeof s === 'string' && s.trim());

  for (const sentence of blocks.flatMap(splitSentences)) {
    const words = sentence.split(/\s+/);
    // Scope: the narrowest thing this sentence names.
    const lower = sentence.toLowerCase();
    const quarter = Object.entries(QUARTER_WORDS_ORD)
      .find(([w]) => new RegExp(`\\b${w}\\b`).test(lower) && /\bquarter\b/.test(lower))?.[1] ?? null;
    const sentTokens = new Set();
    for (const w of words) tokenizeName(w, sentTokens);
    const namedTeams = teamTokens
      .filter(({ tokens }) => [...tokens].some((t) => t.length >= 4 && sentTokens.has(t)))
      .map(({ team }) => team);

    for (let i = 0; i < words.length; i++) {
      const raw = words[i];
      // "20-7" is a scoreline, not a count. "41-yard" is handled below.
      if (/^\d+[-–]\d+/.test(raw)) continue;
      const bare = raw.replace(/^[^0-9a-zA-Z]+|[^0-9a-zA-Z]+$/g, '');
      const hyphenTail = /^(\d+|[a-z]+)-([a-z]+)/i.exec(raw);
      let n = null;
      if (/^\d+$/.test(bare)) n = Number(bare);
      else if (NUMBER_WORDS[bare.toLowerCase()] != null) n = NUMBER_WORDS[bare.toLowerCase()];
      else if (hyphenTail && NUMBER_WORDS[hyphenTail[1].toLowerCase()] != null) n = NUMBER_WORDS[hyphenTail[1].toLowerCase()];
      else if (hyphenTail && /^\d+$/.test(hyphenTail[1])) n = Number(hyphenTail[1]);
      if (n == null) continue;
      // Measurement, not a count — either "41-yard ..." or "41 yards ...".
      if (hyphenTail && MEASURE_WORDS.has(hyphenTail[2].toLowerCase())) continue;
      const next = (words[i + 1] ?? '').replace(/[^a-zA-Z]/g, '').toLowerCase();
      if (MEASURE_WORDS.has(next)) continue;

      // Look ahead a short window for the noun this number is counting. The
      // scan STOPS at another number, because the nearer one owns the noun -
      // otherwise "seven ... two touchdowns" reads as a claim of seven.
      let kind = null;
      for (let j = i + 1; j <= Math.min(i + 4, words.length - 1); j++) {
        const w = words[j].replace(/[^a-zA-Z]/g, '');
        const rawJ = words[j].replace(/^[^0-9a-zA-Z]+|[^0-9a-zA-Z]+$/g, '');
        if (/^\d+$/.test(rawJ) || NUMBER_WORDS[rawJ.toLowerCase()] != null) break;
        if (!w) continue;
        if (/^field$/i.test(w) && /^goals?$/i.test((words[j + 1] ?? '').replace(/[^a-zA-Z]/g, ''))) { kind = FIELD_GOAL_KIND; break; }
        const hit = COUNT_NOUNS.find((c) => c.re.test(w));
        if (hit) { kind = hit.kind; break; }
      }
      if (!kind) continue;

      // Resolve against the narrowest scope the sentence committed to.
      let allowed; let scope;
      if (kind === 'POINTS' && quarter) {
        const q = byQuarter[quarter] ?? {};
        const vals = namedTeams.length
          ? namedTeams.map((t) => q[t] ?? 0)
          : [...Object.values(q), Object.values(q).reduce((a, b) => a + b, 0)];
        allowed = new Set(vals); scope = `${quarter} quarter${namedTeams.length ? ` / ${namedTeams.join(', ')}` : ''}`;
      } else if (namedTeams.length === 1) {
        allowed = new Set([teamCount(namedTeams[0], kind)]); scope = namedTeams[0];
      } else {
        allowed = gameTotals(kind); scope = 'game';
      }
      if (!allowed.has(n)) {
        issues.push(`"${n} ${kind === 'FG' ? 'field goals' : kind === 'TD' ? 'touchdowns' : 'points'}" for ${scope} — derived count is ${[...allowed].join('/')}`);
      }
    }
  }

  if (issues.length) {
    // DISTINCT TAG so the rate is measurable in the logs without parsing prose
    // reasons. While advisory this is the ENTIRE output of the gate.
    console.warn(`[brief-gate] count_claims ${COUNT_CLAIMS_BLOCKING ? 'REJECTED' : 'ADVISORY'}`,
      JSON.stringify({ issues: issues.slice(0, 6) }));
  }
  const reason = [...new Set(issues)].slice(0, 6).join(' | ');
  // ADVISORY, NOT BLOCKING - flipped 15 Aug after one evening of live traffic.
  //
  // Two of three briefs that night were CORRECT PROSE replaced by a mechanical
  // template by this gate, on three distinct false-positive classes:
  //
  //   "cut the lead to four with a 54-yard field goal"  a SCORE MARGIN counted
  //   "Keenum ... two touchdowns ... the Bears' offense" a PLAYER line scoped
  //                                                      to the team
  //   "added four extra points"                         a COMPOUND NOUN read
  //                                                      as points
  //
  // Sports prose is dense with numbers adjacent to scoring nouns that are not
  // team totals, and patching three holes would have found a fourth in
  // September. The working defence against a miscount is the DERIVED INPUTS -
  // scoring_counts and the numeric_claim_rule in the envelope - which is what
  // actually fixed the "20 points across the second quarter" error.
  //
  // The gate stays wired so its [brief-gate] lines accumulate a real
  // false-positive/true-positive ledger against live traffic. Blocking gets
  // revisited with that data rather than with an argument.
  //
  // THE COST IS ACCEPTED WITH EYES OPEN: "all three Washington touchdowns"
  // would publish again today. Flip this constant to restore blocking.
  return { pass: COUNT_CLAIMS_BLOCKING ? issues.length === 0 : true, reason, advisory: !COUNT_CLAIMS_BLOCKING };
}
...
function runAllGates(parsed, envelope) {
  ...
  results.push({ name: 'count_claims', ...gateCountClaims(parsed, envelope) });
...
/**
 * @param {object} matchData  the envelope, pre-prune
 * @param {object} [opts]
 * @param {(envelope: object) => object} [opts.fallback]
 *   The deterministic last resort. Defaults to the soccer template below, which
 *   writes scorer lines as "K. Mbappe 32'" - correct for a code measured in
 *   minutes and nonsense for one measured in quarters. A caller whose envelope
 *   has a different shape supplies its own rather than accepting prose about
 *   a touchdown in the 2nd minute.
 */
export async function generateBrief(matchData, { fallback = templatedFallback } = {}) {
