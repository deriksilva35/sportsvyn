...
export function gateCountClaims(parsed, envelope) {
  ...
  if (issues.length) {
    // DISTINCT TAG so the fallback rate attributable to arithmetic is
    // measurable in the logs without parsing prose reasons.
    console.warn('[brief-gate] count_claims REJECTED', JSON.stringify({ issues: issues.slice(0, 6) }));
  }
  return { pass: issues.length === 0, reason: [...new Set(issues)].slice(0, 6).join(' | ') };
}
