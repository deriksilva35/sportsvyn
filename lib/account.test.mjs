...
// PER-RUN MARKER (test-iso relay): this wipe walks users and deletes their
// sessions, accounts, follows and tokens, so a shared pattern meant one run
// could tear down another run's account mid-assertion.
const MARK = fixtureMark('acctdel');
async function wipe() {
  // best-effort cleanup on failure: cascade from users, plus the no-FK tables.
  const users = await sql`SELECT id, email FROM users WHERE email LIKE ${MARK.like}`;
...
before(async () => { await sweepStale(sql, MARK); await wipe(); });
after(wipe);
