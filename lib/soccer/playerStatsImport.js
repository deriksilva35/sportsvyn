...
export function mapStatLine(st) {
  if (!st) return null;
  return {
    minutes_played: num(st.games?.minutes),
    // `substitute: false` means they were in the XI. The provider has no
    // "started" flag of its own.
    started: st.games?.substitute == null ? null : !st.games.substitute,
    ...
    yellow_cards: num(st.cards?.yellow),
    red_cards: num(st.cards?.red),
  };
}
