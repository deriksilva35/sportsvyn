...
test('THE GRIDIRON CARD IS UNTOUCHED - its instruments all still render', () => {
  const t = src('components/gridiron/Scoreboard.js');
  // ANCHOR ON THE DECLARATION, NOT THE PARAMETER LIST. This slice used to be
  // pinned to `function Card({ g })`; threading a records prop through the
  // card renamed nothing and broke nothing, but indexOf returned -1 and the
  // slice quietly became the wrong text. A guard that can be disarmed by
  // adding a prop is not guarding the thing it names.
  ...
  const card = t.slice(t.indexOf('function Card('), t.indexOf('function SoccerCard'));
  ...
  // THE DORMANT DEMO IS GONE (TODAY PAGE DATA FIXES item 3). It was a hidden,
  // aria-hidden block carrying hardcoded KC / 2nd & 6 / OPP 34 values, and it
  // shipped inside every page that mounts this shell - including /nfl and
  // /cfb while real games were live. The live drive strip readers actually
  // see is the Gamecast DriveStrip on the game pages and ScoresV2's own
  // sv2-strip on the Scores tab; neither is this block.
  assert.equal(/<DriveStrip yardsToEndzone=/.test(t), false, 'no hardcoded demo strip');
  assert.equal(/<div hidden aria-hidden/.test(t), false, 'and no hidden demo block at all');
