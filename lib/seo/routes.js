...
export const NOINDEX_PREFIXES = [
  ...
  '/you',    // the You tab: handle, streak, ranks, follows, alert state
  ...
  '/join',   // /join/{code}: an invite link into a league (085) - signed-in claim screen, one code each
  // /market/props/{slug}?match={id}: one player's priced props for ONE game.
  // A PRICE PAGE IS TRUE UNTIL KICKOFF AND MEANINGLESS AFTER, and its whole
  // subject is a line that moves - there is nothing here for a crawler to
  // rank a week later, and a stale prop page ranking for a player's name
  // would be worse than no page. /market itself stays indexable; only the
  // per-player, per-game cards under it are out.
  '/market/props',
];
...
export const STATIC_ROUTES = [
  ...
  // /market is ALIVE AGAIN as of the Market Phase A relay. It spent the
  // post-World-Cup weeks serving a dead tournament at this same priority,
  // which is the entry a sitemap should never carry; it now covers NFL, CFB
  // and EPL and genuinely changes every 15 minutes.
