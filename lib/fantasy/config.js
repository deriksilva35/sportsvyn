...
// The (scoringFormat, teamsCount) pairs the daily ADP snapshot fetches — one per
// launch preset in draft_configs (Standard 12 PPR / 10-Team Half / 8-Team Casual /
// 12-Team 2QB, ids 1-4). Held HERE rather than read from draft_configs on purpose:
// snapshotPool() refuses more than 4 pairs (gentle-client discipline toward FFC),
// so a fifth preset row must be a deliberate edit to this list and its cap, not a
// silent widening of our upstream call volume. Consumed by the adp-snapshot cron
// and scripts/sim-snapshot.mjs so both agree on what "the pool" means.
export const LAUNCH_PRESET_PAIRS = [
...
// FREE FOR THE 2026 SEASON: this was 12, with 14/16 behind the Pass.
//
// LIFTED TO TEAMS_MAX RATHER THAN DELETED, and the reason is what reads it.
// configLocks() below computes `oversize` from this constant, and StartForm
// uses that to caption a custom room with "16 teams - ADP maps to the nearest
// market pool" - a POOL-PROVENANCE notice, not a paywall. Deleting the constant
// means unpicking configLocks, its callers and its tests to remove a warning
// that is still true. Setting it to the ceiling makes `oversize` naturally
// false while leaving that machinery intact, and makes a future reprice a
// one-line change rather than a re-derivation.
export const FREE_TEAMS_MAX = TEAMS_MAX;

// Slot keys the console can set, and their per-slot count bounds. SUPERFLEX is a
// free like everything else; it renders through roster.js's arbitrary-key path.
export const SLOT_KEYS = ['QB', 'RB', 'WR', 'TE', 'FLEX', 'SUPERFLEX', 'DST', 'K', BENCH];

// POSITIONS WHOSE SECOND COPY HAS ZERO MARGINAL VALUE: one starting slot, no
// flex path, waiver-replaceable most weeks. The suggestion machinery treats a
// filled singleton as CAPPED - excluded, not merely sunk. The law is "no
// copies beyond startable slots," never a hardcoded 1: a console roster built
// with K: 2 caps at 2. Defined here, in one place, so the engine's floors,
// the tracker's best-available panel and the MY TEAM sort cannot drift apart
// on which positions this covers.
export const SINGLETON_POSITIONS = ['K', 'DST'];

// The starter console cells, DERIVED from SLOT_KEYS rather than retyped.
// Two consoles render this list (the Mock's StartForm and the Tracker's
// setup); a hand-written copy in either is how one grows a slot the other
// cannot read - the preset drift lesson, applied to UI.
const CELL_LABEL = { FLEX: 'FLX', SUPERFLEX: 'SFLX' };
export const ROSTER_CELLS = SLOT_KEYS
  .filter((k) => k !== BENCH)
  .map((k) => ({ k, label: CELL_LABEL[k] ?? k }));
...
/**
 * HOW MANY REAL PLAYERS A ROOM WILL ASK THE POOL FOR.
 *
 * Only K and DST are synthetically backfillable, so the demand that has to FIT
 * is every other seat: teams x rounds, less the K and DST slots every team
 * will fill from nowhere.
 *
 * IT LIVES HERE BECAUSE THREE ENTRANCES NEED IT AND ONE HAD IT. The custom and
 * tracker paths computed this inline in resolveCustomPool and refused a room
 * that could not be seated; the PRESET path checked only `if (!pool.length)`,
 * so a preset whose rounds outran the snapshot passed the guard and failed
 * deeper, where the reader sees a raw token instead of a sentence. One
 * definition, three callers - a second copy of this arithmetic is how the
 * paths drift.
 */
export function realDemandFor({ rosterSlots = {}, teamsCount = 0 } = {}) {
  const rounds = deriveRounds(rosterSlots);
  const synthPerTeam = (Number(rosterSlots.K) || 0) + (Number(rosterSlots.DST) || 0);
  return Number(teamsCount) * rounds - Number(teamsCount) * synthPerTeam;
}
