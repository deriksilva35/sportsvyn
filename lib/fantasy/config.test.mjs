...
import {
  validateConfig, deriveRounds, starterCount, rosterTokens, configLocks, realDemandFor,
  nearestPoolPair, SCORING_FORMATS, LAUNCH_PRESET_PAIRS, TEAMS_MIN, FREE_TEAMS_MAX,
} from './config.js';
...
test('configLocks: OVERSIZE NEVER FIRES NOW, superflex still reports', () => {
  // 14 teams used to be `oversize: true` - a member unlock. FREE_TEAMS_MAX is
  // the ceiling for the 2026 season, so nothing is oversize. The lock is kept
  // rather than deleted because StartForm uses it to caption a POOL-PROVENANCE
  // notice ("ADP maps to the nearest market pool"), which is true at any price.
  assert.deepEqual(configLocks(base), { oversize: false, superflex: false });
  assert.deepEqual(configLocks({ ...base, teamsCount: 14 }), { oversize: false, superflex: false });
  assert.deepEqual(configLocks({ ...base, teamsCount: 16 }), { oversize: false, superflex: false });
...
// ---------------------------------------------------------------------------
// LAUNCH_PRESET_PAIRS — what the daily adp-snapshot cron fetches.
// ---------------------------------------------------------------------------

test('LAUNCH_PRESET_PAIRS stays inside snapshotPool\'s 4-pair cap', () => {
  // snapshotPool() throws above 4 (gentle-client discipline toward FFC). A fifth
  // pair added here without raising that cap would fail the cron every night, so
  // pin the count: growing the pool must be a deliberate two-file change.
  assert.equal(LAUNCH_PRESET_PAIRS.length, 4);
});

test('LAUNCH_PRESET_PAIRS: every pair is a valid, distinct config axis', () => {
  const seen = new Set();
  for (const p of LAUNCH_PRESET_PAIRS) {
    assert.ok(SCORING_FORMATS.includes(p.scoringFormat), `bad format ${p.scoringFormat}`);
    assert.ok(Number.isInteger(p.teamsCount), `teamsCount not an int for ${p.scoringFormat}`);
    // Presets are the free tier, so they must sit inside the free team ceiling.
    assert.ok(p.teamsCount >= TEAMS_MIN && p.teamsCount <= FREE_TEAMS_MAX,
      `teamsCount ${p.teamsCount} outside ${TEAMS_MIN}..${FREE_TEAMS_MAX}`);
    const key = `${p.scoringFormat}/${p.teamsCount}`;
    assert.ok(!seen.has(key), `duplicate pair ${key} would double-fetch the same feed`);
    seen.add(key);
  }
});

test('LAUNCH_PRESET_PAIRS matches the four launch presets exactly', () => {
  // Mirrors draft_configs ids 1-4 (Standard 12 PPR / 10-Team Half / 8-Team Casual /
  // 12-Team 2QB). Hand-written, not read off the constant.
  assert.deepEqual(LAUNCH_PRESET_PAIRS, [
    { scoringFormat: 'ppr', teamsCount: 12 },
    { scoringFormat: 'half-ppr', teamsCount: 10 },
    { scoringFormat: 'standard', teamsCount: 8 },
    { scoringFormat: '2qb', teamsCount: 12 },
...
test('every launch pair resolves to itself via nearestPoolPair', () => {
  // The pool the cron writes must be the pool a preset draft reads — an exact hit
  // on both axes, never a "nearest" fallback to a different feed.
  for (const p of LAUNCH_PRESET_PAIRS) {
    const r = nearestPoolPair(p, LAUNCH_PRESET_PAIRS);
    assert.deepEqual(r.pair, p);
    assert.equal(r.scoringExact, true, `${p.scoringFormat}: scoring not exact`);
    assert.equal(r.exact, true, `${p.scoringFormat}/${p.teamsCount}: teams not exact`);
  }
});

// ---------------------------------------------------------------------------
// THE DEMAND RULE, SHARED BY ALL THREE ENTRANCES (Mock Part B, filed defect)
// ---------------------------------------------------------------------------

test('realDemandFor counts every seat the pool must actually fill', () => {
  // 12 teams x 15 rounds = 180 seats, less the 12 K and 12 DST every team
  // backfills synthetically = 156 real players.
  assert.equal(realDemandFor({
    teamsCount: 12,
    rosterSlots: { QB: 1, RB: 2, WR: 2, TE: 1, FLEX: 1, K: 1, DST: 1, BN: 6 },
  }), 156);
  // The Weekly Six: six rounds, no K, no DST - every seat is real.
  assert.equal(realDemandFor({
    teamsCount: 12, rosterSlots: { QB: 1, RB: 1, WR: 1, TE: 1, FLEX: 2 },
  }), 72);
  // Two kickers a team takes two more off the real demand, not none.
  assert.equal(realDemandFor({ teamsCount: 10, rosterSlots: { QB: 1, K: 2 } }), 10);
});

test('realDemandFor is defensive about the shapes a config row can hold', () => {
  assert.equal(realDemandFor(), 0);
  assert.equal(realDemandFor({}), 0);
  assert.equal(realDemandFor({ teamsCount: 12 }), 0);
  // strings off a jsonb column count as numbers, not as concatenation
  assert.equal(realDemandFor({ teamsCount: '2', rosterSlots: { QB: '1', RB: '1' } }), 4);
});

test('IT IS THE SAME ARITHMETIC resolveCustomPool used inline, not a near-copy', async () => {
  // The custom path's guard was teams*rounds - teams*(K+DST). If this ever
  // stops matching, the preset path and the custom path start refusing
  // different rooms - which is the drift this function exists to prevent.
  const cfg = { teamsCount: 8, rosterSlots: { QB: 1, RB: 2, WR: 2, TE: 1, K: 1, DST: 1, BN: 3 } };
  const rounds = deriveRounds(cfg.rosterSlots);
  const synthPerTeam = (cfg.rosterSlots.K || 0) + (cfg.rosterSlots.DST || 0);
  assert.equal(realDemandFor(cfg), cfg.teamsCount * rounds - cfg.teamsCount * synthPerTeam);
});

test('the grade scale is ONE list, best first, and nobody keeps a second copy', async () => {
  // getDraftsSummary picks "best grade" by position in this array. A second
  // list of these nine letters anywhere is a scale that can disagree with
  // itself between two screens.
  const { GRADE_ORDER } = await import('./calibration.js');
  assert.deepEqual(GRADE_ORDER, ['A', 'A-', 'B+', 'B', 'B-', 'C+', 'C', 'D', 'F']);
  assert.equal(GRADE_ORDER[0], 'A', 'best first - the finder takes the first held');
