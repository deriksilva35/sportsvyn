...
//
//   valueScore_i = par - adp_i
//       (positive = the player has FALLEN past his ADP to you -> attracts;
//        negative = drafting him now is a REACH -> repels)
//   par = max(currentOverallPick, adp of the board's #1 available)
//
//   BOARD PAR. Until 2 Sep 2026 par was the raw pick number, and on a board
//     that is not drained by keepers the two are the same thing to within a
//     pick or two: the best player left almost always sits at or below the
//     pick (FFC ppr/12, preset 1: the #1's ADP exceeds the pick at 6% of
//     picks, never by more than 1.4). A keeper-drained board is different in
//     kind. In THE LONGEST YARD (config 6661, 41 keepers off the board before
//     any AI pick) the best player left at overall 24 is Zay Flowers at ADP
//     35.65, so EVERY candidate's value was negative, and with every logit
//     below zero the term that wins is not the smallest reach but the
//     flattest one - value*nw/T rewards a LOW need weight and a HIGH
//     temperature when value is negative. Lamar Jackson (ADP 47.9, nw 1.45,
//     T 3.19) went at 24 in 72.6% of 500 seeds over Flowers (nw 2.80, T 2.50)
//     at 9%. That is a wrong zero, not a preference: the pick number is not
//     the market's price when the market's top of board has already gone.
//     Anchoring par to the board's #1 restores the FFC regime (the #1 sits at
//     value 0, everyone below him is a reach in proportion) with no constant:
//     QB at overall 24 falls to 0.8%, QBs in rounds 1-3 from 3.53 to 2.02
//     against a market top-36 that holds 2, and the undrained board is
//     untouched in every metric (top-3 inside four 91.8%, R1 set overlap
//     99.7% seed for seed). Note what this does NOT fix: for ranks 2+ every
//     value is still negative, so need weight still repels a needed reach
//     harder than an unneeded one. That is the sign asymmetry proper and it
//     is queued with the additive need term (nw out of value, TEMP_MIN 1.0).
//
...
// A COLLEGE ROW IS NOT COVERAGE. The scarcity padding below asks "does this
// pool hold enough kickers to fill every K slot in the league" - and the answer
// has to be about players this league can START. College rows sit at the bottom
// of the board by design and are bench-eligible only (see assignToSlot), so
// counting the 58 college kickers and 80 school defenses the NCAAF feed carries
// would tell a league with no NFL kicker at all that it is fully covered, and
// the synthetic replacements would never be built. The board would then run out
// of legal kickers mid-draft, which is the exact failure this function exists
// to prevent - "no legal pick at overall 193", now with a plausible-looking
// pool behind it.
const isCollege = (p) => p.league === 'ncaaf';
...
export function ensureFillablePool(pool, config) {
  ...
  const counts = {};
  for (const p of pool) {
    if (isCollege(p)) continue;
    const sp = slotPos(p.position); counts[sp] = (counts[sp] ?? 0) + 1;
  }
...
//
// THE ABSENCE FILTER IS numbersOf's (lib/util/scoresOf.js) - the one place a list
// of numbers is read off rows. The arithmetic stays here and UNROUNDED: this
// median feeds the temperature model, not a screen, and the house one decimal
// would quantize it.
function median(nums) {
  const a = numbersOf(nums).sort((x, y) => x - y);
...
export function canRoster(state, team, player, round, ctx = null, opts = {}) {
  ...
  // (e) A COLLEGE PLAYER IS BENCH-ELIGIBLE ONLY - the ONE gate that applies to a
  //     human pick as hard as to an engine one. Everything else in this function
  //     is judgement the engine applies to itself and a person may overrule
  //     (opts.humanPick); this is not judgement, it is the roster shape. Fantrax
  //     gives this league 17 active spots and a college player can start in none
  //     of them, so with no bench open the pick is simply illegal - refused here,
  //     in the same grammar as any other illegal pick, rather than falling
  //     through to assignToSlot and quietly taking a starter's seat.
  if (isCollege(player)) return bnOpen > 0;
...
// A COLLEGE PLAYER IS BENCH-ELIGIBLE ONLY, and that is a roster fact rather than
// a preference. Fantrax says this league has 17 active spots and names the
// positions that fill them; a college player cannot be started in any of them.
// Letting one take the RB or the FLEX would make our roster panel disagree with
// the league's in the one way that misleads - it would show a starter who is not
// one. The bench is what a devy pick costs in the real league, so the bench is
// what it costs here.
//
// NO SILENT FALLBACK. If the bench is full, this does NOT quietly place the
// player somewhere else - canRoster refuses the pick first (same refusal grammar
// as any other illegal pick), and reaching here with a full bench is a bug, so
// it throws rather than corrupting the roster shape.
function assignToSlot(team, player) {
  const p = slotPos(player.position);
  team.posCount[p] = (team.posCount[p] ?? 0) + 1;
  if (isCollege(player)) {
    if (!team.slots.BN || team.slots.BN.filled >= team.slots.BN.cap) {
      throw new Error(`assignToSlot: college player ${player.name} with no open bench slot`);
    }
...
// EXPORTED for lib/house/pickDraft.js. The roster rules - starter slots, the
// K/DST guardrails, bye stacking - live in here, so a seat strategy that
// picked around this list would be a strategy that drafts an illegal roster.
// Everything in the tree that chooses a player goes through it.
export function legalCandidates(state, team, round) {
...
export function aiPick(state, teamIndex, rng) {
  ...
  // Board par, not the raw pick number (see THE PICK MODEL: "BOARD PAR").
  const par = Math.max(state.overallPick, Number(state.available[0].adp));
  const scored = cands.map((c) => {
    const nw = needWeight(state, team, c);
    const value = par - Number(c.adp);
