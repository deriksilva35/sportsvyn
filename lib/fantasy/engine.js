...
export function canRoster(state, team, player, round, ctx = null, opts = {}) {
  ...
  // (b) never a 2nd K / 2nd DST. ENGINE SEATS ONLY - see the note on this
  //     function. A person may stash one on a bench slot that would otherwise
  //     sit empty; the sort ranks it behind skill depth rather than refusing it.
  if (!opts.humanPick && (p === 'K' || p === 'DST') && team.posCount[p] >= 1) return false;
