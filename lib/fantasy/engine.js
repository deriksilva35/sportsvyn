 ...
 *
 * `opts.humanPick` exempts rule (a), the K/DST round floor. That floor exists to
 * keep AI DRAFTERS realistic - engines left to themselves will happily take a
 * kicker in round 3 - and it is a sanity rule about the simulation, not a rule
 * of fantasy football. Imposed on a person it contradicts the whole promise:
 * the read is theirs. A user with an open DST slot in round 8 was refused with
 * "Roster can't fit that pick" over a slot that was visibly empty.
 *
