...
test('a matched QB returns structured REG-only stats', async () => {
  const s = await getPlayerSeasonStats(STAFFORD, SEASON);
...
//
// THE SEASON IS AN ARGUMENT NOW, not a fence. playerStats.js used to carry
// `const SEASON_YEAR = 2025` and every reader inherited it; it takes a season
// from its caller now (a ranked room's contest, else the current REG season),
// so these fixtures state which season they are about. 2025 is what
...
test('a DST aggregates its team defensive rows, per game and for the season', async () => {
  const d = await getPlayerSeasonStats(DENVER, SEASON);
...
test('a matched player with no 2025 line returns null, not a line of zeros', async () => {
  assert.equal(await getPlayerSeasonStats(NOSTATS, SEASON), null);
  const sum = await getPlayerSeasonSummaries([NOSTATS], 'ppr', SEASON);
...
test('an unknown ffc id returns null rather than throwing', async () => {
  ...
  assert.deepEqual(await getPlayerSeasonSummaries(['not-a-real-ffc-id'], 'ppr', SEASON), {});
});
...
test('summaries are keyed per player and never multiplied by snapshot count', async () => {
  const sum = await getPlayerSeasonSummaries([STAFFORD, DENVER], 'ppr', SEASON);
...
// summaries
// ---------------------------------------------------------------------------
test('collapsed-row PPG equals the expanded strip PER GAME (same numbers, no drift)', async () => {
  const s = await getPlayerSeasonStats(STAFFORD, SEASON);
  const sum = await getPlayerSeasonSummaries([STAFFORD], 'ppr', SEASON);
  const strip = seasonSummary(s.games, 'ppr');
  assert.equal(sum[STAFFORD].games, strip.games, 'the same games');

  // WITHIN A TENTH, AND THE TENTH IS FLOATING POINT, NOT A DISAGREEMENT.
  // The strip scores each game and sums the points; the collapsed row sums the
  // COMPONENTS and scores once. scoring.js is linear - per-unit coefficients,
  // no thresholds, no bonuses - so points(sum) equals sum(points) exactly in
  // arithmetic, and the two can only part when a total lands on a .x5 boundary
  // and IEEE rounding sends each side a different way. Stafford's 2025 is such
  // a total (350.3 one way, 350.4 the other). Summing the components first is
  // the MORE accurate of the two, which is why the shared reader does it and
  // why careerIndex() argues the same way.
  assert.ok(Math.abs(sum[STAFFORD].points - strip.points) <= 0.1,
    `${sum[STAFFORD].points} vs ${strip.points}`);
  assert.ok(Math.abs(sum[STAFFORD].ppg - strip.ppg) <= 0.1,
    `${sum[STAFFORD].ppg} vs ${strip.ppg}`);
});
...
test('summaries are format-aware: PPR pays receptions, standard does not', async () => {
  const wr = (await sql`
    SELECT p.ffc_player_id FROM sim_player_pool p
     ...
     LIMIT 1`)[0].ffc_player_id;
  const ppr = await getPlayerSeasonSummaries([wr], 'ppr', SEASON);
  const std = await getPlayerSeasonSummaries([wr], 'standard', SEASON);
...
test('the whole pool resolves: every id either summarises or has no 2025 REG line', async () => {
  ...
  // FFC IDS ONLY (083). ffc_player_id is FFC's id space on source='ffc' rows and
  // Fantrax's on source='fantrax' rows - the column name predates the second
  // source. The stats path matches on FFC identity, so a Fantrax id can never
  // resolve through it; unscoped, the 469 league ids sank the rate to 31.8%
  // the morning after the first import and said nothing about matching.
  const ids = (await sql`SELECT DISTINCT ffc_player_id FROM sim_player_pool WHERE source = 'ffc'`).map((r) => r.ffc_player_id);
  assert.ok(ids.length > 100, `pool looks empty (${ids.length} ids) — did a snapshot land?`);

  const sum = await getPlayerSeasonSummaries(ids, 'ppr', SEASON);
...
test('getPlayerSeasonSummaries([]) short-circuits without a query', async () => {
  assert.deepEqual(await getPlayerSeasonSummaries([], 'ppr', SEASON), {});
  assert.deepEqual(await getPlayerSeasonSummaries(undefined, 'ppr', SEASON), {});
});
  ...
  const k = (await sql`
    SELECT p.ffc_player_id FROM sim_player_pool p
     ...
     WHERE p.position = 'PK'
       AND EXISTS (SELECT 1 FROM nfl_player_game_stats s WHERE s.nfl_player_id = np.id)
     LIMIT 1`)[0].ffc_player_id;
  const s = await getPlayerSeasonStats(k, SEASON);
  ...
  for (const pos of ['QB', 'RB', 'WR', 'TE', 'PK', 'DEF']) {
    const row = (await sql`
      SELECT p.ffc_player_id FROM sim_player_pool p
       ...
       LIMIT 1`)[0];
    const s = await getPlayerSeasonStats(row.ffc_player_id, SEASON);
