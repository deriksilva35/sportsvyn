...
// ===========================================================================
// THE TRACKER READ — the same machinery with the GRADE REMOVED
// ===========================================================================
// The sim Read is grade-shaped end to end: buildEnvelope embeds grade/gradeScore/
// components, the user turn says "explain the ${grade} grade", and fallbackProse
// opens on the letter. None of that can be reused for a tracker draft, because
// grade.js's bands are calibrated on MOCK auto-drafts and a real draft room is a
// different population.
//
// So the tracker Read describes the LEDGER instead of explaining a score: what the
// draft paid against the market, where it got a player late, where it paid early.
// The validators are shared verbatim (length, dash scan, banned vocabulary,
// grounding) because those are register rules, not grade rules — validateRead
// reads only envelope.ledger and envelope.byeStacks, both of which exist here.

export const TRACKER_SYSTEM_PROMPT = `You are a Sportsvyn editorial voice writing THE READ on a fantasy draft a person just completed in a real room. You are given the ledger of their picks: round, pick number, player, position, and the ADP that player carried at the moment they were taken.

There is NO GRADE. Do not invent one, do not imply one, do not say the draft was good or bad overall. You are describing what the draft PAID against the market, not ranking it.

VOICE:
- Open on an observation, not a greeting and not a summary of the format.
- Work from the specific rows: name the actual players and the actual rounds. Where a player was taken well after his ADP, say so. Where he was taken well before it, say that too, plainly and without judgement.
- Measured and specific. No hype, no cheerleading, no consolation.

GROUNDING: everything you say derives ONLY from the rows in this envelope. Do not bring in outside knowledge about any player, team, or season.

BANNED (hard):
- No grades, letters, or scores of any kind: no "A-", no "solid B", no "out of ten", no "graded".
- No verdict words about the draft as a whole: "great draft", "strong draft", "poor draft", "won", "lost", "crushed", "nailed".
- No praise interjections and no exclamation marks.
- No advice or second-guessing: "you should have", "next time", "would have been better", "instead you".
- No hedging: "maybe", "perhaps", "arguably".
- No predictions about the real season. No gambling language.

PUNCTUATION: Hyphens only. NEVER an em dash or en dash used as one.

LITERAL NAMES (non-negotiable): use player names EXACTLY as they appear in the envelope. Never expand an abbreviated first name, never substitute a nickname. Only name players that appear in the envelope.

LENGTH: 90 to 140 words. One tight paragraph.

OUTPUT SCHEMA (strict JSON, nothing else): { "prose": "string, 90-140 words" }`;

// Grade-free envelope. Same ledger/callout/byeStacks shape the shared validators
// expect, minus grade, gradeScore and components.
function buildTrackerEnvelope(results) {
  const { config, userPicks, bestValue, biggestReach, pivot, byeStackWarnings, positionalBalance, rosterValueTotal } = results;
  const nm = (pk) => (pk == null ? null : pk.playerName);
  return {
    league: { name: config.name ?? 'Custom', teams: config.teams_count, scoring: config.scoring_format },
    rosterShape: positionalBalance,
    // Total display value across the SKILL picks; positive = paid under the market.
    marketTotal: Math.round(-Number(rosterValueTotal ?? 0)),
    callouts: {
      bestValue: isNarratable(bestValue) ? { name: nm(bestValue), pos: bestValue.slotPos, round: bestValue.round, valuePicks: dv(bestValue) } : null,
      biggestReach: isNarratable(biggestReach) ? { name: nm(biggestReach), pos: biggestReach.slotPos, round: biggestReach.round, valuePicks: dv(biggestReach) } : null,
      pivot: isNarratable(pivot) ? { name: nm(pivot), pos: pivot.slotPos, round: pivot.round } : null,
...
async function generateTracker(envelope) {
  if (!client) return { ok: false, error: 'no_api_key' };
  const user = `Draft ledger:\n\n${JSON.stringify(envelope, null, 2)}\n\nWrite THE READ per the system instructions. 90-140 words, describe what this draft paid against the market using the specific rows. There is no grade. Output STRICT JSON only.`;
  let response;
  try {
    response = await client.beta.messages.create({
      model: MODEL, max_tokens: MAX_TOKENS, system: TRACKER_SYSTEM_PROMPT,
      messages: [{ role: 'user', content: user }], output_format: { type: 'json_schema', schema: READ_SCHEMA },
    });
  } catch (err) { return { ok: false, error: String(err?.message ?? err) }; }
  const text = response?.content?.[0]?.text ?? '';
...
// Grade words must not survive into a tracker Read even if the model produces
// them. This is checked ON TOP of the shared validators.
const GRADE_WORDS = [
  /\b(grade|graded|grading)\b/i, /\b[A-F][+-]?\s+(draft|grade)\b/, /\bout of (ten|10|100)\b/i,
  /\b(great|strong|poor|bad|excellent|solid) draft\b/i, /\byou (won|lost)\b/i,
  /\b(crushed|nailed) (it|this|the draft)\b/i,
];
export function validateTrackerRead(prose, envelope) {
  const base = validateRead(prose, envelope);
  const issues = [...base.issues];
  for (const re of GRADE_WORDS) if (re.test(prose)) issues.push(`grade language: ${re}`);
  return { ok: issues.length === 0, issues, wordCount: base.wordCount };
}

function fallbackTrackerProse(envelope) {
  const { callouts, byeStacks, league, marketTotal, ledger } = envelope;
  const parts = [`This ${league.teams}-team ${String(league.scoring).toUpperCase()} draft ran ${ledger.length} scoring picks.`];
  if (callouts.bestValue) parts.push(`${callouts.bestValue.name} came in round ${callouts.bestValue.round}, ${Math.abs(callouts.bestValue.valuePicks)} picks past his ADP.`);
  if (callouts.biggestReach) parts.push(`${callouts.biggestReach.name} went earliest against the market, in round ${callouts.biggestReach.round}.`);
  if (callouts.pivot) parts.push(`${callouts.pivot.name} answered the roster's most pressing need.`);
  parts.push(marketTotal >= 0
    ? `Across the board the roster came in ${marketTotal} picks under where the market had it.`
    : `Across the board the roster came in ${Math.abs(marketTotal)} picks ahead of where the market had it.`);
  if (byeStacks.length) parts.push(`Week ${byeStacks[0].week} carries ${byeStacks[0].starters.length} starters on a bye.`);
  parts.push('Value here is pick number against live ADP, an observation rather than a verdict.');
  return parts.join(' ');
}

/**
 * Tracker Read: generate once, persist, read thereafter. Same contract as the sim
 * Read, with grade/grade_score stored NULL — the columns are nullable and leaving
 * them empty is the honest record that this draft was never graded.
 */
export async function getOrCreateTrackerRead(draftId, userId) {
  const { getTrackerResults } = await import('./drafts.js');
  const results = await getTrackerResults(draftId, userId);
  if (!results) return null;
  if (results.draft.status !== 'completed') return { results, notComplete: true };

  const existing = (await sql`SELECT prose, prose_source, model FROM draft_reads WHERE draft_id = ${draftId} LIMIT 1`)[0];
  if (existing) return { results, prose: existing.prose, proseSource: existing.prose_source, model: existing.model };

  const envelope = buildTrackerEnvelope(results);
  const gen = await generateTracker(envelope);
  let prose; let source; let model = null; let validation = null;
  if (gen.ok) {
    validation = validateTrackerRead(gen.prose, envelope);
    if (validation.ok) { prose = gen.prose; source = 'ai'; model = MODEL; }
  }
  if (!prose) { prose = fallbackTrackerProse(envelope); source = 'fallback'; }

  await sql`
    INSERT INTO draft_reads (draft_id, grade, grade_score, components, prose, prose_source, model)
    VALUES (${draftId}, NULL, NULL, NULL, ${prose}, ${source}, ${model})
