...
function buildEnvelope(results) {
  ...
  const nm = (pk) => (pk == null ? null : (pk.synthetic ? `Replacement ${pk.slotPos}` : pk.playerName));
  return {
    ...
    callouts: {
      bestValue: bestValue && { name: nm(bestValue), pos: bestValue.slotPos, round: bestValue.round, valuePicks: dv(bestValue) },
      biggestReach: biggestReach && { name: nm(biggestReach), pos: biggestReach.slotPos, round: biggestReach.round, valuePicks: dv(biggestReach) },
      pivot: pivot && { name: nm(pivot), pos: pivot.slotPos, round: pivot.round },
    },
