...
// test.
// NCAA IS A LEAGUE, NOT A POSITION, and it sits at the end of the same chip row
// because that is where the reader looks to narrow the board. Every other chip
// filters the NFL half; this one is the only way to see the college half - which
// is what makes the board "one board with a chip" rather than two boards.
export const POS_FILTERS = ['ALL', 'QB', 'RB', 'WR', 'TE', 'K', 'DST', 'NCAA'];
export const NCAA_FILTER = 'NCAA';
...
export function filterPlayers(list, { position = 'ALL', team = 'ALL', search = '', cls = 'ALL' } = {}) {
  ...
  // THE COLLEGE HALF IS BEHIND ITS OWN CHIP, INCLUDING UNDER 'ALL'.
  //
  ...
  // position RB, so without this the RB chip would interleave 239 college backs
  // with the NFL ones, priced on a scale that shares nothing with them. 'ALL'
  // is the same argument one step out: 927 rows below the 417 that a reader
  // opening the board came to see.
  //
  // So the chips partition rather than overlap: every NFL chip shows NFL, NCAA
  // shows college, and nothing appears twice. "He filters to NCAA to see them"
  // is the whole interaction.
  const isCollege = (p) => p.league === 'ncaaf';
  return (list ?? []).filter((p) => (
    (position === NCAA_FILTER
      ? isCollege(p)
      : (!isCollege(p) && (position === 'ALL' || displayPosition(p.position) === position)))
 ...
 */
// THE COLLEGE SORTS, and they are a DIFFERENT LIST rather than an addition.
//
// NCAAF ADP IS OFFERED NOWHERE ELSE. It is the true college order and the reason
// the filter exists - but it is a price from another market, so outside this
// filter there is nothing for it to order: every NFL row's ncaafAdp is null, and
// a sort whose metric is null for the entire list is a sort that silently does
// nothing. Offering it on the NFL board would be offering a control that lies.
//
// MY TEAM IS ABSENT HERE. The seat read is a gap against the NFL board and a
// slot a college player cannot fill, so ranking college rows by it would be
// ordering them by a fact that does not apply. The stat sorts are absent for
// the same reason one step down: they read NFL season totals this half has none of.
const COLLEGE = [
  { key: 'adp', label: 'ADP', asc: true, metric: null },
  { key: 'ncaafAdp', label: 'NCAAF ADP', asc: true, metric: null, college: true,
    compare: (a, b) => {
      const av = a?.ncaafAdp ?? null; const bv = b?.ncaafAdp ?? null;
      if (av == null && bv == null) return 0;
      if (av == null) return 1;      // unknown last, never as a zero
      if (bv == null) return -1;
      return av - bv;
    } },
  { key: 'ppg', label: 'PPG', metric: (s) => s?.ppg ?? null },
...
export function sortsFor(filter) {
  if (filter === NCAA_FILTER) return COLLEGE;
