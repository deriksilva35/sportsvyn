...
// ---------------------------------------------------------------- topMovers
//
// THE DEFECT THIS GUARDS: a module that shows "the biggest movers" but really
// shows the head of each column. Interleaving the two ranked lists puts a
// trivial riser above a large faller, so the panel would name the wrong player
// as the day's story while looking perfectly plausible.

test('topMovers ranks by absolute move, mixing directions', () => {
  const card = {
    rising:  [{ ffcPlayerId: 'r1', d3: 4.0 }, { ffcPlayerId: 'r2', d3: 0.4 }],
    falling: [{ ffcPlayerId: 'f1', d3: -9.1 }, { ffcPlayerId: 'f2', d3: -6.0 }],
  };
  assert.deepEqual(topMovers(card, 3).map((p) => p.ffcPlayerId), ['f1', 'f2', 'r1']);
  // The head-of-each-column ordering this replaces would have led with r1.
  assert.notEqual(topMovers(card, 3)[0].ffcPlayerId, 'r1');
});

test('topMovers survives a null card, an empty board and null d3', () => {
  assert.deepEqual(topMovers(null), []);
  assert.deepEqual(topMovers({ rising: [], falling: [] }), []);
  assert.deepEqual(topMovers({ rising: [{ ffcPlayerId: 'x', d3: null }] }), []);
});

test('topMovers caps at n', () => {
  const rising = Array.from({ length: 9 }, (_, i) => ({ ffcPlayerId: `p${i}`, d3: i + 1 }));
  assert.equal(topMovers({ rising }, 5).length, 5);
  assert.equal(topMovers({ rising }, 5)[0].ffcPlayerId, 'p8');
});
