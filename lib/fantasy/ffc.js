...
// pool. FFC fields high/low normalize to adp_high/adp_low. stdev (market
// disagreement, feeds the engine) and bye are stored as of migration 047;
// adp_formatted is still not stored.

import { sql } from '../db.js';

// Single-sourced in attribution.js (no imports there, so client surfaces can read
// it too — this module pulls in the DB driver). Re-exported so server importers
// keep the historical `from '@/lib/fantasy/ffc'` path.
export { FFC_ATTRIBUTION } from './attribution.js';
...
function normalize(p) {
  return {
    ...
    stdev: p.stdev ?? null,    // market disagreement (engine sampling temperature)
    bye: p.bye ?? null,
  };
}
...
//
// THE CONFLICT TARGET IS THE TABLE'S WHOLE UNIQUE KEY, ALL SIX COLUMNS, and it
// has to be spelled out in full: Postgres matches an inference list against a
// unique index EXACTLY, so a shorter list matches nothing at all and the
// statement throws rather than degrading to an insert.
//
// THIS BROKE PRODUCTION ONCE. 083 added `source` to the key and this list kept
// naming four columns, so every run after it threw "there is no unique or
// exclusion constraint matching the ON CONFLICT specification" - measured, the
// 2026-09-02 11:01Z run (sync_runs 18646), which took matchPoolIdentities down
// with it because step 2 is downstream of the throw. 086 then added `league`.
// A key change is a change HERE, and ffc.test.mjs now reads the live key out of
// information_schema and fails the suite if this list drifts from it again.
    ...
    for (const r of rows) {
      await sql`
        INSERT INTO sim_player_pool (
          ...
          adp, adp_high, adp_low, times_drafted, stdev, bye
        ) VALUES (
          ${snapshotDate}, ${scoringFormat}, ${teamsCount}, ${r.ffcPlayerId}, ${r.name}, ${r.position}, ${r.team},
          ${r.adp}, ${r.adpHigh}, ${r.adpLow}, ${r.timesDrafted}, ${r.stdev}, ${r.bye}
        )
        ON CONFLICT (snapshot_date, scoring_format, teams_count, ffc_player_id, source, league) DO UPDATE SET
          ...
          times_drafted = EXCLUDED.times_drafted, stdev = EXCLUDED.stdev, bye = EXCLUDED.bye`;
      n += 1;
    }
