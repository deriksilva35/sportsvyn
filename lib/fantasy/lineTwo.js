 ...
 */
export function lineTwoTokens({ pos, team = null, range = null, quick = null, seatSort = false, seatRead = null, league = 'nfl' }) {
  // A COLLEGE ROW CARRIES NO SEAT READ, and the refusal lives here rather than
  // in each room so the two cannot drift apart. The gap is a market distance
  // measured against the NFL board, and the slot is one a college player is not
  // eligible for (bench only, engine.js) - so both would be true-looking
  // statements about the wrong thing. The tag's `team` is the SCHOOL, which is
  // why "RB·OkSt" reads correctly with no other change.
  const college = league === 'ncaaf';
  const tagText = `${pos}${team ? `·${team}` : ''}${range ? ` · ${range}` : ''}`;
  const out = [{ kind: 'tag', text: tagText }];
  if (!college && seatSort && seatRead) {
