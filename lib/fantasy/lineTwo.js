...
// is where it is. Other sorts keep the original order.
 ...
 *   gap    - "-29.9 at 50"   (seatSort only, when the seat has a next pick)
 *   slot   - "TE · flex"     (seatSort only)
 *   quick  - one per stat, "88 REC"
 */
export function lineTwoTokens({ pos, team = null, range = null, quick = null, seatSort = false, seatRead = null, nextOverall = null }) {
  const tagText = `${pos}${team ? `·${team}` : ''}${range ? ` · ${range}` : ''}`;
  const out = [{ kind: 'tag', text: tagText }];
  if (seatSort && seatRead) {
    if (seatRead.gap != null) {
      out.push({ kind: 'gap', text: `${seatRead.gap > 0 ? '+' : ''}${seatRead.gap} at ${nextOverall}`, gap: seatRead.gap });
    }
    out.push({ kind: 'slot', text: `${pos} · ${seatRead.slot}`, slot: seatRead.slot, muted: Boolean(seatRead.deferred || seatRead.streamer) });
  }
