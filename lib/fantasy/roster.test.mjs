...
// ---- Stage 5: kept players render in their slots before the commit --------
test('kept players fill their position slot, FLEX for an eligible overflow, bench after; never a pick', () => {
  const kept = [
    { overall: 10, round: 1, name: 'Brock Bowers', position: 'TE', slotPos: 'TE', teamSlot: 2 },
    { overall: 27, round: 3, name: 'Joe Burrow', position: 'QB', slotPos: 'QB', teamSlot: 2 },
    { overall: 74, round: 7, name: 'Cameron Skattebo', position: 'RB', slotPos: 'RB', teamSlot: 2 },
    { overall: 146, round: 13, name: 'Rico Dowdle', position: 'RB', slotPos: 'RB', teamSlot: 2 },
    { overall: 150, round: 13, name: 'Third RB', position: 'RB', slotPos: 'RB', teamSlot: 2 },
    { overall: 160, round: 14, name: 'Fourth RB', position: 'RB', slotPos: 'RB', teamSlot: 2 },
    { overall: 170, round: 15, name: 'Kicker', position: 'PK', slotPos: 'K', teamSlot: 2 },
  ];
  const slots = buildRoster([], STANDARD_12_PPR, kept);
  const byLabel = Object.fromEntries(slots.map((s) => [s.label, s.kept?.name ?? null]));
  assert.equal(byLabel.TE, 'Brock Bowers');
  assert.equal(byLabel.QB, 'Joe Burrow');
  assert.equal(byLabel.RB1, 'Cameron Skattebo');
  assert.equal(byLabel.RB2, 'Rico Dowdle');
  assert.equal(byLabel.FLEX, 'Third RB');   // RB is FLEX-eligible
  assert.equal(byLabel.BN1, 'Fourth RB');   // FLEX taken -> bench
  assert.equal(byLabel.K, 'Kicker');        // pool PK arrives as slot K
  assert.ok(slots.every((s) => s.pick === null), 'a kept entry is not a pick');
  // 0 drafted + 7 kept: the pick count readers use stays 0.
  assert.equal(slots.filter((s) => s.pick).length, 0);
  assert.equal(slots.filter((s) => s.kept).length, 7);
  // A committed keeper arrives as a pick with the engine's slot and wins the slot;
  // the room drops his kept entry, so he is never rendered twice.
  const committed = [{ overallPick: 10, rosterSlot: 'TE', playerName: 'Brock Bowers', isKeeper: true }];
  const after = buildRoster(committed, STANDARD_12_PPR, kept.filter((k) => k.overall !== 10));
  const te = after.find((s) => s.label === 'TE');
  assert.equal(te.pick.playerName, 'Brock Bowers');
  assert.equal(te.kept, undefined);
  // Without kept, the output is byte-for-byte what it was.
  assert.deepEqual(buildRoster(committed, STANDARD_12_PPR), buildRoster(committed, STANDARD_12_PPR, []));
});
