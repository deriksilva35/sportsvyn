...
import { seasonSummary, fantasyPoints } from './scoring.js';
import { seasonTotals } from '../weekly/seasonLine.js';

// THE SEASON A STAT LINE MEANS IS NOW AN ARGUMENT, NOT A CONSTANT.
//
// It used to be `const SEASON_YEAR = 2025`, written when 2026 had no weeks and
// "last completed season" was the only honest answer. 2026 has weeks now, and
// the constant outlived its own comment: the draft room showed Kenneth Walker
// at 11.3 ppg (his 2025 line) on the same afternoon the Weekly's panel showed
// him at 34.1 (his 2026 line). Same man, two screens, two seasons, no way for
// a reader to tell which.
//
// THE CALLER KNOWS WHICH SEASON IT MEANS: a ranked room takes its contest's
// season_year, and a practice mock or a tracker takes the current REG season
// (lib/pollers/seasonResolver.js, the same resolver every cron uses). The
// phase never varies - fantasy is a regular-season game.
//
// FINALS ONLY, like every other season read in this product: a game in
// progress is not in an average.
 ...
 */
export async function getPlayerSeasonStats(ffcPlayerId, season) {
  const id = String(ffcPlayerId);
  const ident = (await resolveIdentities([id])).get(id);
  if (!ident) return null;

  const games = ident.is_team_defense
    ? await defenseGames(ident.team_id, season)
    : await playerGames(ident.nfl_player_id, season);
...
async function playerGames(nflPlayerId, season) {
  const rows = await sql`
    SELECT m.week, m.home_team_id, s.team_id,
     ...
     WHERE s.nfl_player_id = ${nflPlayerId}
       AND m.season_phase = ${SEASON_PHASE} AND m.season_year = ${season}
       AND m.status = 'final'
     ORDER BY m.week`;
...
async function defenseGames(teamId, season) {
  const rows = await sql`
 ...
 */
export async function getPlayerSeasonSummaries(ffcPlayerIds, scoringFormat, season) {
  ...
  // THE SKILL PLAYERS COME FROM THE SHARED SCAN. seasonTotals is the same sum
  // over the same finals the Weekly's panel reads, so the two surfaces cannot
  // print different numbers for the same man. Only the SCORING FORMAT differs,
  // and it should: a standard-scoring room must not show PPR points.
  const [skill, defRows] = await Promise.all([
    seasonTotals(season, { playerIds }),
    teamIds.length ? sql`
  ...
  for (const [ffcId, i] of idents) {
    if (i.is_team_defense) {
      const games = byTeam.get(String(i.team_id)) ?? [];
      if (!games.length) continue; // unknown, not zero
      out[ffcId] = { ...seasonSummary(games, scoringFormat), totals: sumStats(games) };
      continue;
    }
    const t = skill.get(Number(i.nfl_player_id));
    if (!t) continue; // unknown, not zero
    // One scorer, one sum: points(sum of games) equals sum(points per game)
    // because scoring.js is linear - the same argument seasonLine.js makes.
    const points = Math.round(fantasyPoints(t.totals, scoringFormat) * 10) / 10;
    out[ffcId] = {
      points,
      ppg: Math.round((points / t.gp) * 10) / 10,
      games: t.gp,
      totals: t.totals,
    };
  }
