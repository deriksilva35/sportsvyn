...
// PER-RUN MARKER (test-iso relay). The old value was the reason this file
// was fragile: 'simtest-share-...' is matched by drafts.test.mjs's
// 'simtest-%@example.invalid', so THAT file's cleanup deleted THIS file's
// users. A per-run marker ends the family collision as well as the race.
const MARK = fixtureMark('simtestshare');
...
before(async () => {
  await wipe();
  // MIGRATION 085'S OWN BACKFILL, RE-APPLIED TO DEV.
  //
  // The first test below asserts 085's invariant over the WHOLE table - every
  // owned config has an owner row - and DEV had 46 configs without one, created
  // by hand or by an older import after the migration ran. That is a fixture
  // gap on this database, not a defect in the product, but it made the guard red
  // on every full run and a red guard nobody can fix is a guard nobody reads.
  //
  // WHAT THIS COSTS, stated: the first assertion can no longer catch a NEW code
  // path that writes a config without its owner row, because this hook repairs
  // one a line earlier. What it still catches is the tie - the second assertion,
  // that the owner's members row points at the isMine franchise - and that is
  // the half with the race policy in it. If the 46 turn out to exist in PROD as
  // well, that is a backfill to run there, not a test to change.
  //
  // TWO STATEMENTS, NOT 085'S ONE. The franchise column carries a partial unique
  // on (config_id, fantrax_team_id): inserting the owner row with the franchise
  // attached would collide wherever a member has already claimed it. So the row
  // goes in unclaimed - NULL is exempt from that index - and the tie is set
  // afterwards, only where the franchise is free.
  await sql`
    INSERT INTO draft_config_members (config_id, user_id, role, fantrax_team_id)
    SELECT c.id, c.user_id, 'owner', NULL
      FROM draft_configs c
     WHERE c.user_id IS NOT NULL
    ON CONFLICT (config_id, user_id) DO NOTHING`;
  await sql`
    UPDATE draft_config_members m
       SET fantrax_team_id = t.mine
      FROM (
        SELECT c.id,
               (SELECT x->>'fantraxTeamId'
                  FROM jsonb_array_elements(COALESCE(c.teams, '[]'::jsonb)) x
                 WHERE (x->>'isMine')::boolean IS TRUE LIMIT 1) AS mine
          FROM draft_configs c WHERE c.user_id IS NOT NULL
      ) t
     WHERE m.config_id = t.id AND m.role = 'owner'
       AND m.fantrax_team_id IS DISTINCT FROM t.mine AND t.mine IS NOT NULL
       AND NOT EXISTS (
         SELECT 1 FROM draft_config_members m2
          WHERE m2.config_id = t.id AND m2.fantrax_team_id = t.mine)`;
...
test('085 backfill: every owned config has an owner row, and the fantrax owner is tied to the isMine franchise', async () => {
  // SENTINEL-OWNED CONFIGS ARE EXCLUDED, and not as a convenience.
  //
  // `node --test` runs files in parallel, and FOUR other files insert
  // draft_configs directly with source 'manual' and no members row -
  // lib/account.test.mjs, lib/weekly/replay.test.mjs,
  // lib/games/seasonFieldFloor.test.mjs, lib/games/draftSeatSeasonTable.test.mjs.
  // That is legitimate: they test account deletion, replay and the season tables,
  // none of which is sharing, and 085's members row is written by the IMPORT
  // (lib/fantrax/import.js) and by createLeague, which those fixtures do not call.
  //
  // So a whole-table count cannot be zero on a parallel run - it was 1 here, and
  // which one it is depends on scheduling. Counting only REAL owners keeps the
  // invariant pointed at product data, which is what it was written to watch.
  // Every sentinel in this suite owns an @example.invalid address.
  const [{ n: missing }] = await sql`SELECT count(*)::int AS n FROM draft_configs c
     JOIN users u ON u.id = c.user_id
     WHERE c.user_id IS NOT NULL
       AND u.email NOT LIKE '%@example.invalid'
       AND NOT EXISTS (SELECT 1 FROM draft_config_members m WHERE m.config_id = c.id AND m.user_id = c.user_id AND m.role = 'owner')`;
...
test('the lobby\'s code field signed OUT: invitePreview(code, null) is a read that answers - ok + league name, no membership, no claim of anyone as mine', async () => {
  // The live code from the mint test above - NOT a fresh mint, which would
  // retire CODE for every test after this one.
  assert.equal((await ls.activeInvite(L.id)).code, CODE, 'the fixture code is still the live one');
  const p = await ls.invitePreview(CODE, null);
  assert.equal(p.ok, true);
  assert.equal(p.league.name, 'Share League main');
  assert.equal(p.alreadyMember, false);
  assert.equal(p.myRole, null);
  assert.equal(p.franchises.some((f) => f.claimedBy?.mine === true), false, 'nobody is "me" when nobody is signed in');
  const [{ n }] = await sql`SELECT count(*)::int AS n FROM draft_config_members WHERE config_id = ${L.id} AND user_id IS NULL`;
  assert.equal(n, 0, 'a preview writes no membership');
  // and a dud, as the field will show it inline
  assert.deepEqual(await ls.invitePreview('ABCDEFGH', null), { ok: false, reason: 'invalid_code', league: null });
  assert.equal(ls.REFUSALS.invalid_code.length > 0, true, 'the refusal has words');
});
...
// ---- runs are private -------------------------------------------------------------
// RE-PINNED 2 Sep (ruling reversed). This test used to claim the opposite: that
// leagueMocks(L.id, U.owner) listed the MEMBER's completed run (seat 6, its first
// three picks, `mine: false`), that a per-run hide flag took it off the owner's
// list, and that the card carried every member's runs. The ruling now is that a
// run is its owner's alone - a member sees exactly their own completed runs on
// the league card, and the owner (Derik included) has no special visibility.
// So every cross-member read below asserts ZERO rows, and the second-member
// sentinel is the one whose view is checked.
test('RUNS ARE PRIVATE: a member\'s run is theirs alone - the card, the runs list, results and the room show a second member zero rows of it; completed-only, seat picks, cap 10 still hold for the owner of the run', async () => {
  ...
  // The owner cannot open the member's run, in any reader.
  assert.equal(await d.getDraft(start.draftId, U.owner), null);
  assert.equal(await d.getDraftForRoom(start.draftId, U.owner), null);
  assert.equal(await d.getResults(start.draftId, U.owner), null);
  assert.equal(await d.getTrackerResults(start.draftId, U.owner), null);
  ...
  // A second member (U.z) cannot read U.b's run either - not the room, not results.
  assert.equal(await d.getDraft(start.draftId, U.z), null);
  assert.equal(await d.getResults(start.draftId, U.z), null);
  // In_progress never shows, not even to its owner.
  assert.equal((await ls.myLeagueRuns(L.id, U.b)).length, 0, 'an in-progress run is not a result yet');
  assert.equal((await ls.myLeagueRuns(L.id, U.owner)).length, 0);
  assert.deepEqual(await ls.myLeagueRuns(L.id, null), [], 'no viewer, no rows');
  // Complete U.b's run with picks at the seat; abandon U.z's; complete a second U.z run.
  ...
  const zRun = await d.startLeagueDraftFor(U.z, L.id, { seat: 4 });
  await sql`UPDATE drafts SET status = 'completed', completed_at = now() - interval '1 hour' WHERE id = ${zRun.draftId}`;

  // THE RUN'S OWNER sees it: seat, franchise, the first three picks at the seat.
  const forB = await ls.myLeagueRuns(L.id, U.b, { teams: L.teams });
  assert.deepEqual(forB.map((r) => r.draftId), [start.draftId], 'own completed run only');
  assert.equal(forB[0].seat, 6);
  assert.equal(forB[0].franchise, 'Team 6');
  assert.deepEqual(forB[0].firstPicks.map((p) => p.name), ['Alpha One', 'Bravo Two', 'Charlie Three'], 'the first three picks AT THE SEAT, in order - no other seat\'s pick, and an auto turn counts');
  assert.equal('hidden' in forB[0], false, 'the hide flag is dead - nothing carries it');
  assert.equal('handle' in forB[0], false, 'no who: the list is yours, it does not name you');
  // THE SECOND MEMBER sees zero of U.b's rows: their own completed run, nothing else (abandoned absent).
  const forZ = await ls.myLeagueRuns(L.id, U.z, { teams: L.teams });
  assert.deepEqual(forZ.map((r) => r.draftId), [zRun.draftId], 'U.z sees U.z: not U.b\'s run, not the abandoned one');
  assert.equal(forZ.filter((r) => r.userId !== U.z).length, 0);
  // THE OWNER sees zero rows of either member's run.
  const forOwner = await ls.myLeagueRuns(L.id, U.owner, { teams: L.teams });
  assert.deepEqual(forOwner, [], 'the owner has no visibility into members\' runs');
  // The card carries each viewer's own list, and no count of anyone else's.
  const ownerCard = (await d.getMyLeagues(U.owner)).find((l) => l.id === L.id);
  assert.equal(ownerCard.mocks.length, 0);
  const bCard = (await d.getMyLeagues(U.b)).find((l) => l.id === L.id);
  assert.deepEqual(bCard.mocks.map((r) => r.draftId), [start.draftId]);
  // draft_date, teams_count, keeper_count are the league's own facts (the config's); `mocks` is the only run-bearing field.
  assert.deepEqual(Object.keys(ownerCard).filter((k) => /run|mock|recent/i.test(k)), ['mocks'], 'no recent-runs count, no second list');
  // Ten is the cap, per viewer.
  ...
  assert.equal((await ls.myLeagueRuns(L.id, U.b)).length, 10);
  assert.equal((await ls.myLeagueRuns(L.id, U.z)).length, 1, 'U.b\'s eleven new rows are not U.z\'s');
  ...
  const top = (await ls.myLeagueRuns(L.id, U.b))[0];
  assert.notEqual(top.completedAt && new Date(top.completedAt) > new Date(Date.now() + 3600e3), true, 'the tracker row is not on top');
  // The tracker's resume card is user-scoped too: another user's open tracker id reads null.
  const [trk] = await sql`SELECT id FROM drafts WHERE user_id = ${U.b} AND config_id = ${L.id} AND mode = 'tracker' LIMIT 1`;
  assert.ok(await d.trackerResumeCard(trk.id, U.b));
  assert.equal(await d.trackerResumeCard(trk.id, U.z), null);
  assert.equal(await d.trackerResumeCard(trk.id, U.owner), null);
  assert.equal(await d.trackerResumeCard(trk.id), null, 'no user, no card');
});
...
test('SOURCE: the join route carries the code through sign-in; the flow-core reads through the ACCESS RULE; the import writes the owner row; proxy is not widened', () => {
  ...
  // JOIN BY CODE (2 Sep): this line used to pin the literal `${SITE}/join/${code}`;
  // the path now comes from inviteCode.js's joinPath so the link and the
  // lobby's code field cannot drift apart. Same claim, one source.
  assert.match(share, /`\$\{SITE\}\$\{joinPath\(code\)\}`/);
  assert.match(share, /import \{ joinPath \} from '@\/lib\/fantasy\/inviteCode';/);
  assert.match(share, /const SITE = 'https:\/\/sportsvyn\.com';/);
  // RUNS ARE PRIVATE (2 Sep, reversal). This block used to pin the opposite:
  // leagueMocks over `d.config_id = ${configId}` with a viewer flag, a
  // setRunHidden action, and a Hide/Show button on the card. Now the viewer IS
  // the scope on every run read, and the hide flag has no reader and no writer.
  const lsSrc = stripComments(src('lib/fantasy/leagueShare.js'));
  assert.match(lsSrc, /WHERE d\.config_id = \$\{Number\(configId\)\} AND d\.user_id = \$\{vid\}/, 'the runs list is scoped to its viewer in the WHERE');
  assert.doesNotMatch(lsSrc, /leagueMocks|setRunHidden|hidden_from_league/, 'no shared list, no hide flag');
  const act = stripComments(src('app/actions/league.js'));
  assert.doesNotMatch(act, /setRunHidden|hidden/i, 'no action on a run');
  assert.doesNotMatch(share, /setRunHidden|hidden|Hide|Show/, 'no Hide/Show on the card');
  assert.match(share, /YOUR MOCKS/);
  assert.match(core, /myLeagueRuns\(l\.id, uid, \{ teams: l\.teams \}\)/, 'the card list is the viewer\'s own');
  assert.match(core, /export async function trackerResumeCard\(draftId, userId\) \{\s*if \(userId == null\) return null;/);
  assert.match(core, /WHERE d\.id = \$\{draftId\} AND d\.user_id = \$\{Number\(userId\)\}/, 'the resume card is user-scoped');
  const trackerPage = stripComments(src('app/sim/tracker/page.js'));
  assert.match(trackerPage, /trackerResumeCard\(open\.id, userId\)/);
  // A results URL by guessed id: getDraft(id, userId) is the gate and a miss is a 404.
  const draftPage = stripComments(src('app/sim/draft/[id]/page.js'));
  assert.match(draftPage, /getDraft\(draftId, userId\)/);
  assert.match(draftPage, /if \(!base\) notFound\(\);/);
});
