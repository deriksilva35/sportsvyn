...
test('085 backfill: every owned config has an owner row, and the fantrax owner is tied to the isMine franchise', async () => {
  // SOURCE = 'fantrax', WHICH IS WHAT 085 IS ABOUT.
  //
  // The invariant was written as "every owned config has an owner row" when every
  // config came from the Fantrax import. It is over-broad now, and a read of PROD
  // says so: 75 owned configs there have no owner row and NOT ONE is a league.
  // They come from two functions, neither of them an import -
  // startCustomDraftFor (source 'manual') and startTrackerDraftFor ('tracker') -
  // which build one reader's own draft room. There is no franchise to claim in
  // one, getMyLeagues filters `c.source = 'fantrax'` so none is ever listed, and
  // both access reads resolve the owner from draft_configs.user_id directly
  // (`CASE WHEN c.user_id = uid THEN 'owner' ELSE m.role END`), so a missing row
  // costs an owner nothing. Every FANTRAX config on PROD has its row, because
  // lib/fantrax/import.js writes it.
  //
  // SCOPED, THE COUNT IS ALSO STABLE UNDER PARALLELISM. Four other files insert
  // configs directly - account, replay, seasonFieldFloor, draftSeatSeasonTable -
  // all with source 'manual', so a whole-table count could never be zero on a
  // parallel run and this one is not affected by them at all.
  //
  // AND A RED HERE IS NOW A REAL SIGNAL: a fantrax config without its owner row
  // means the import path stopped writing one.
  const [{ n: missing }] = await sql`SELECT count(*)::int AS n FROM draft_configs c
     WHERE c.user_id IS NOT NULL AND c.source = 'fantrax'
       AND NOT EXISTS (SELECT 1 FROM draft_config_members m WHERE m.config_id = c.id AND m.user_id = c.user_id AND m.role = 'owner')`;
  assert.equal(missing, 0, 'no shareable (fantrax) config without its owner row');
  // NOT VACUOUS: there are fantrax configs to count.
  const [{ n: scope }] = await sql`SELECT count(*)::int AS n FROM draft_configs WHERE source = 'fantrax' AND user_id IS NOT NULL`;
  assert.ok(scope >= 1, 'at least one fantrax config on DEV to hold the invariant over');
