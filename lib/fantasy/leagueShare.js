...
import { CODE_ALPHABET } from '../leagues/core.js';
...
/** A pasted code: upper-cased, whitespace dropped; null when it cannot be one. */
...
/**
 * The league's last N COMPLETED runs: who, as which franchise, when, their
 * first three picks. "Their" picks are the picks AT THE RUN'S SEAT (snake
 * order from overall_pick, the engine's own geometry), keepers excluded - not
 * picked_by = 'user', because an auto run's picks are all 'ai' and the seat is
 * still the one they played. in_progress and abandoned never appear - a run that is
 * not over is nobody's business yet, and one that was walked away from is not
 * a result. A run its owner hid shows to that owner alone (marked, so the
 * "show" toggle has somewhere to live) and to nobody else.
 */
export async function leagueMocks(configId, viewerId = null, { limit = 10, teams = null } = {}) {
  const vid = viewerId == null ? null : Number(viewerId);
  const rows = await sql`
    SELECT d.id, d.user_id, d.pick_position, d.completed_at, d.hidden_from_league,
           u.handle, u.name AS user_name,
     ...
     WHERE d.config_id = ${Number(configId)} AND d.status = 'completed' AND d.mode = 'sim'
       AND (d.hidden_from_league IS NOT TRUE OR d.user_id = ${vid})
     ORDER BY d.completed_at DESC NULLS LAST, d.id DESC
     LIMIT ${Number(limit)}`;
  return rows.map((r) => {
    const f = (teams ?? []).find((t) => Number(t.slot) === Number(r.pick_position)) ?? null;
    return {
      draftId: r.id, userId: r.user_id, handle: r.handle ?? null, userName: r.user_name ?? null,
      mine: vid != null && r.user_id === vid,
      seat: r.pick_position, franchise: f?.name ?? null, completedAt: r.completed_at,
      hidden: r.hidden_from_league === true, firstPicks: r.first_picks ?? [],
    };
  });
}

/** Hide/show one of YOUR runs on the league list. Never another user's. */
export async function setRunHidden(userId, draftId, hidden) {
  const rows = await sql`UPDATE drafts SET hidden_from_league = ${hidden === true}
                          WHERE id = ${Number(draftId)} AND user_id = ${Number(userId)} RETURNING id`;
  return rows.length ? { ok: true } : { ok: false, reason: 'not_your_run' };
}
