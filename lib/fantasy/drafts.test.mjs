...
// PER-RUN MARKER (test-iso relay), and this one was worse than a race: the old
// pattern 'simtest-%@example.invalid' MATCHES leagueShare.test.mjs's
// 'simtest-share-...' users, so this file's cleanup deleted that file's
// fixtures out from under it. The two live in different chunks today, which is
// the only reason nobody saw it.
const MARK = fixtureMark('simtest');
async function wipe() { await sql`DELETE FROM users WHERE email LIKE ${MARK.like}`; }
async function mkUser(tag) {
  return (await sql`INSERT INTO users (name, email) VALUES (${'SimTest ' + tag}, ${MARK.email(tag)}) RETURNING id`)[0].id;
}
let U = {}, PRESET;
before(async () => {
  await wipe();
...
test('interactive: startDraft persists AI picks to user turn; makePick advances snake', async () => {
  ...
  // THE POOL THIS DRAFT ACTUALLY PLAYS, not an FFC snapshot of its date. A
  // preset now drafts our own board (lib/draft/ourBoard.js) and its
  // pool_snapshot_date is the day the board was COMPUTED, so reconstructing a
  // pool from that date would load a different set of players - or, on a
  // database with no snapshot that day, none at all. poolFor is the one
  // reader that knows the difference, and it is what the room itself calls.
  const dRow = (await sql`SELECT * FROM drafts WHERE id=${draftId}`)[0];
  const dCfg = (await sql`SELECT * FROM draft_configs WHERE id=${dRow.config_id}`)[0];
  const pool = await d.poolFor(dRow, dCfg);
...
test('CUSTOM IS FREE: a non-member gets a real draft, not an entitlement refusal', async () => {
  // INVERTED, NOT DELETED. This asserted a non-member was refused with
  // 'entitlement_custom' and that no rows were written. Custom is free for the
  // 2026 season, so the same call must now SUCCEED and write.
  ...
  assert.equal(res.ok, true, JSON.stringify(res));
  assert.notEqual(res.reason, 'entitlement_custom');
  const after = await sql`SELECT count(*)::int n FROM draft_configs WHERE user_id = ${U.b}`;
  assert.equal(after[0].n, before[0].n + 1, 'the custom config is written now');
});
...
test('TRACKER IS FREE: no membership row, and it still starts', async () => {
  // INVERTED, NOT DELETED. This asserted a user with no membership was refused
  // with 'entitlement_tracker' and that no draft row was written.
  const u = await mkUser('trk-gate');
  const free = await d.startTrackerDraftFor(u, TRACKER_CFG, 1, null);
  assert.equal(free.ok, true, JSON.stringify(free));
  assert.notEqual(free.reason, 'entitlement_tracker');
  const wrote = await sql`SELECT count(*)::int n FROM drafts WHERE user_id = ${u}`;
  assert.equal(wrote[0].n, 1, 'the tracker room is written for a free account');
  // And it stays open, so the second start below resumes rather than duplicating.
  await sql`UPDATE drafts SET status = 'abandoned' WHERE user_id = ${u}`;

  // A pass-holder is unaffected - the entitlement still resolves, it just no
  // longer gates. Two live passes on PROD depend on that read still working.
  await grantPass(u);
