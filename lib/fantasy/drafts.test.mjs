...
test('the K/DST round floor binds AI seats only - a person may draft any legal player', () => {
  ...
  // The exemption lifts ONLY that rule. A second kicker is still illegal for
  // anyone, because two kickers cannot both start.
  const st2 = engine.createDraftState(config, pool, 3);
  engine.applyPick(st2, 2, kicker, 'user');
  const kicker2 = pool.filter((p) => p.position === 'PK')[1];
  assert.equal(engine.canRoster(st2, st2.teams[2], kicker2, 14, null, { humanPick: true }), false,
    'humanPick must not become a licence to break real roster rules');
});
