...
test('the Pick row joins the window with its own separator, and drops both together', () => {
  ...
  // RE-PINNED 2 Sep 2026 (NCAA chip). This read
  //   {sum ? `${approx ? '~' : ''}${fmt1(sum.ppg)}` : '-'}
  // The '~' marks a PARTIALLY scored position (K/DST: no distance tiers, no
  // points allowed) and must not appear on the college board, where the marked
  // positions are filtered out of the view entirely and a '~' would be claiming
  // an imprecision that is not this number's. The '-' branch is unchanged, and
  // it is the part that matters: an unresolved player prints nothing, not 0.0.
...
test('the Fantrax import runs matchPoolIdentities after its pool write and before keepers; the summary reports the match', () => {
  ...
  // The matcher takes the whole pool ACROSS SOURCES - no source predicate - so
  // one call still heals every universe, and the cron's call and the import's
  // call are the same call. That half is unchanged.
  //
  // RE-PINNED 2 Sep 2026. This asserted the SELECT was
  //   SELECT DISTINCT name, position, team FROM sim_player_pool ORDER BY position, name
  // with no predicate at all. It now carries `WHERE league = 'nfl'`, because the
  // pool holds college rows and a college player has no NFL identity: matching
  // one anyway does not blank a cell, it shows another man's stats under his
  // name. Measured on the real 927-row college board before the scope existed:
  // 6 players and 3 school defenses resolved onto NFL ids. The source-agnostic
  // claim above is the part that must not drift; league scoping is the fix.
  const nm = stripComments(src('lib/gridiron/nameMatch.js'));
  assert.match(nm, /SELECT DISTINCT name, position, team FROM sim_player_pool\s*\n\s*WHERE league = 'nfl' ORDER BY position, name/);
  assert.doesNotMatch(nm, /source =/);
  // And the write is scoped the same way, or an NFL identity could still land
  // on a college row sharing the (name, position, team) triple.
  assert.match(nm, /UPDATE sim_player_pool SET matched_player_id[\s\S]*?AND league = 'nfl'/);
});

// ---- 4. the season scoping stays as found -----------------------------------
test('PPG MEANS THE SEASON THE CALLER NAMES - the handoff happened, and the constant went with it', () => {
  // THIS TEST USED TO PIN `const SEASON_YEAR = 2025`. The comment it also
  // pinned said the handoff was 2026 REG week 1, "after which last season and
  // this season to date become a policy choice, not a constant". Week 1 of
  // 2026 has been played; the policy was chosen; the constant is deleted. The
  // rule this test protects is the same one it always protected - a reader
  // must not have a private opinion about which season it is showing - and it
  // is now enforced by the season being an ARGUMENT.
  const ps = src('lib/fantasy/playerStats.js');
  const code = stripComments(ps);
  assert.doesNotMatch(code, /const SEASON_YEAR/, 'no hardcoded year survives');
  assert.match(ps, /^const SEASON_PHASE = 'REG';$/m, 'the phase never varies - fantasy is a REG game');
  assert.match(code, /m\.season_phase = \$\{SEASON_PHASE\} AND m\.season_year = \$\{season\}/,
    'every season read takes the caller\'s season');
  // AND THE ROOM AND THE WEEKLY READ THE SAME SCAN. The agreement itself is
  // asserted against the database in lib/weekly/sameSeason.test.mjs.
  assert.match(code, /import \{ seasonTotals \} from '\.\.\/weekly\/seasonLine\.js'/);
});
