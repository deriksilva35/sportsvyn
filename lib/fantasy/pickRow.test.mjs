...
test('the Pick row joins the window with its own separator, and drops both together', () => {
  ...
  // (stripComments leaves a JSX comment's braces behind - the `\{\s*\}` between.)
  assert.match(room, /\{slot\}\{p\.team \? `·\$\{p\.team\}` : ''\}\s*(\{\s*\}\s*)?\{range && ` · \$\{range\}`\}/,
    'position·team, then " · low-high" only when there is a window');
