...
// ---------------------------------------------------------------------------
// NO PURCHASE SURFACE IS REACHABLE IN THE SHELL
// ---------------------------------------------------------------------------
// The teardown removed the GATES but left the purchase CARDS mounted, and on a
// real device that is what a user actually sees: a $9.99 price, UNLOCK THE
// DRAFT PASS and RESTORE PURCHASE, in front of features that are free. Reads
// were what needed protecting - two live passes still resolve - and a rendered
// paywall is not a read.

test('NO PURCHASE COMPONENT IS MOUNTED ANYWHERE', () => {
  const walk = (dir, out = []) => {
    for (const f of readdirSync(dir)) {
      if (['node_modules', '.next', '.git'].includes(f)) continue;
      const p = path.join(dir, f);
  ...
  for (const dir of ['app', 'components']) {
    for (const file of walk(path.join(REPO, dir))) {
      ...
      // The components themselves may still contain their own markup - they are
      // kept in the tree deliberately. What must not exist is a MOUNT of them.
      if (rel === 'components/sim/MembershipCard.js' || rel === 'components/sim/PassBuy.js') continue;
      const s2 = code(rel);
      if (/<MembershipCard/.test(s2)) mounts.push(`${rel} mounts MembershipCard`);
      if (/<PassBuy/.test(s2)) mounts.push(`${rel} mounts PassBuy`);
    }
  }
  assert.deepEqual(mounts, [], `purchase surfaces still render:\n${mounts.join('\n')}`);
});

test('THE TRACKER GOES STRAIGHT TO SETUP - no wall', () => {
  const s2 = code('components/sim/TrackerStart.js');
  assert.equal(/if \(!entitled\) \{/.test(s2), false, 'no entitlement branch');
  assert.equal(/MembershipCard/.test(s2), false, 'and nothing to render in one');
});

test('THE PRACTICE CONSOLE SHOWS NO LOCK CHIPS', () => {
  const s2 = code('components/sim/StartForm.js');
  assert.equal(/pl lock/.test(s2), false, 'the CUSTOM card wears no MEMBER padlock');
  assert.equal(/copt locked push/.test(s2), false, 'the teams stepper has no UP TO 16 teaser');
  assert.equal(/iapPitch/.test(s2), false, 'the price-pitch flag is gone');
});

test('THE ENTITLEMENT READS SURVIVE - this was never about the reads', () => {
  // lib/membership.js, both webhooks and the memberships table are untouched,
  // and u1/u5 must keep resolving sim:true.
  for (const p of ['lib/membership.js', 'app/api/stripe/webhook/route.js',
    'app/api/revenuecat/webhook/route.js', 'components/sim/PassBuy.js',
