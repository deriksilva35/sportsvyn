  ...
  for (const nextPick of [24, 52]) {
    for (const seat of Object.values(SEATS)) {
      ...
      for (const p of avail) {
        ...
        // streamer rank moved 2 -> 4 under the K/DST-cap law: a capped
        // singleton sorts ABSOLUTE LAST, below even 'full' - exclusion
        // rendered as ranking, per the module's own charter.
        // RE-PINNED (draft 443, 2 Sep 2026). This claimed `flex ? 0` - flex
        // could never defer. That exemption is what let 65 tight ends own
        // the list once every open slot deferred at once; flex now defers by
        // the same test as open.
        const expected = read.slot === 'open' ? (read.deferred ? 1 : 0)
          : read.slot === 'flex' ? (read.deferred ? 1 : 0)
            : read.slot === 'bench' ? (read.streamer ? 4 : 0)
              : 3;
...
test('deferral is decided per position by the pool, not by a round table', () => {
  ...
  // available ADP sits beyond par + 12. Nothing here names a round.
  // RE-PINNED (draft 443, 2 Sep 2026): the horizon was `83 + TEAMS`, the raw
  // pick number; it is now board par, max(pick, board #1 ADP) - here 84, the
  // first RB - plus a round. And flex rows are held to the same test as open.
  const boardTop = Math.min(...avail.map((q) => q.adp));
  const horizon = Math.max(83, boardTop) + TEAMS;
  assert.equal(boardTop, 84, 'precondition: this board\'s #1 sits one past the pick');
  for (const p of avail) {
    const read = seatReadOf(v, p);
    if (read.slot !== 'open' && read.slot !== 'flex') continue;
    const best = Math.min(...avail.filter((q) => q.position === p.position).map((q) => q.adp));
    assert.equal(read.deferred, best > horizon,
      `${p.position} (${read.slot}): deferral must follow bestAdp ${best} vs horizon ${horizon}`);
  }
});
...
test('open and flex share a rank; deferred and full still do not', () => {
  ...
  for (const p of avail) {
    ...
    const key = (r.slot === 'open' || r.slot === 'flex') && r.deferred ? `${r.slot}-deferred` : r.slot;
  ...
  if (kinds.has('flex-deferred')) assert.equal(kinds.get('flex-deferred'), 1);
...
test('(a) a second DST never outranks a skill playable row - but it is still there', () => {
  ...
  assert.equal(slotRank(read), 4, 'capped: absolute last, per the K/DST-cap law');
...
test('(c) the same holds for kickers - symmetric, and nothing else moves', () => {
  ...
  assert.equal(slotRank(seatReadOf(v, k)), 4, 'capped, per the K/DST-cap law');
...
// ---------------------------------------------------------------------------
// BOARD PAR, AND FLEX DEFERS — draft 443, 2 Sep 2026.
//
// Reported mid-mock at pick 50 of a keeper league: with the filter at ALL and
// the sort at MY TEAM, the list was 65 tight ends and then everything else.
// The comparator was working exactly as written. The horizon read the raw pick
// number (50 + 12 = 62) on a board whose #1 available sat at ADP 73.1 - a
// keeper-drained board runs well ahead of the pick number - so the best player
// at EVERY position was "more than a round away", every open slot deferred at
// once, and the one bucket the deferral test never looked at, flex, was left
// alone at rank 0. The seat had just taken a TE, so every TE was flex.
//
// Same defect the engine fixed in f4dadb6: the pick number is not the market.
// Two amendments: the horizon anchors to board par, and flex defers like open.
// The fixture is that room's exact state, read from the draft at pick 50.
// ---------------------------------------------------------------------------
...
const DRAFT_443 = JSON.parse(readFileSync(path.join(HERE, 'fixtures/draft-443-pick-50.json'), 'utf8'));

const draft443 = () => {
  const fx = DRAFT_443;
  const v = computeSeatValuation({
    rosterSlots: fx.rosterSlots, rounds: fx.rounds, allPicks: [], seatPicks: fx.seatPicks,
    available: fx.available, myNextOverall: fx.myNextOverall,
    currentOverall: fx.currentOverall, teamsCount: fx.teamsCount,
  });
  return { fx, v, order: sortedWith(fx.available, v) };
};
const bestAdpOf = (rows, position) => Math.min(...rows.filter((q) => q.position === position).map((q) => q.adp));

test('draft 443 @50: the top of MY TEAM at ALL is not a single position', () => {
  const { fx, v, order } = draft443();
  // NON-VACUOUS: this is the drained board. Under the raw-pick horizon every
  // position's best available sat beyond it, which is the whole failure.
  const rawHorizon = fx.currentOverall + fx.teamsCount;
  for (const pos of ['RB', 'WR', 'QB', 'TE', 'DEF', 'PK']) {
    assert.ok(bestAdpOf(fx.available, pos) > rawHorizon,
      `precondition: ${pos}'s best sits beyond the raw-pick horizon ${rawHorizon}`);
  }
  assert.equal(seatReadOf(v, fx.available.find((p) => p.position === 'TE')).slot, 'flex',
    'precondition: the seat has its TE, so every TE reads as flex');

  const top15 = order.slice(0, 15).map((p) => p.position);
  const kinds = new Set(top15);
  assert.ok(kinds.size > 1, `the first 15 rows were all ${top15[0]} - one position owns the list`);
  // Concretely: the market's own top of board leads, whatever slot it fills.
  assert.equal(order[0].name, 'Jaylen Warren', 'board #1 (RB 73.11, open slot) leads');
  assert.ok(kinds.has('RB') && kinds.has('WR') && kinds.has('QB') && kinds.has('TE'),
    `expected RB, WR, QB and TE in the first 15, got ${top15.join(',')}`);
});

test('draft 443 @50: the horizon is board par plus a round, and K/DST stay deferred', () => {
  const { fx, v } = draft443();
  const boardTop = Math.min(...fx.available.map((p) => p.adp));
  const horizon = Math.max(fx.currentOverall, boardTop) + fx.teamsCount;
  assert.ok(boardTop > fx.currentOverall, 'precondition: the board runs ahead of the pick');
  // Every open or flex row follows the par-anchored test - nothing else.
  for (const p of fx.available) {
    const r = seatReadOf(v, p);
    if (r.slot !== 'open' && r.slot !== 'flex') continue;
    assert.equal(r.deferred, bestAdpOf(fx.available, p.position) > horizon,
      `${p.name} (${p.position} ${r.slot}): deferral must follow the par horizon ${horizon}`);
  }
  // The five-defenses case stays fixed: DEF 106.4 and PK 115.3 are still past
  // the horizon and still wait.
  for (const pos of ['DEF', 'PK']) {
    const best = fx.available.find((p) => p.position === pos && p.adp === bestAdpOf(fx.available, pos));
    assert.equal(seatReadOf(v, best).slot, 'open', `${pos}: the slot is open`);
    assert.equal(seatReadOf(v, best).deferred, true, `${pos} at ${best.adp} is beyond ${horizon} - still deferred`);
  }
  for (const pos of ['RB', 'WR', 'QB', 'TE']) {
    const best = fx.available.find((p) => p.position === pos && p.adp === bestAdpOf(fx.available, pos));
    assert.equal(seatReadOf(v, best).deferred, false, `${pos} at ${best.adp} is inside ${horizon} - its turn has come`);
  }
});

test('draft 443 @50: no K or DST row sits above any undeferred row', () => {
  const { v, order } = draft443();
  const firstKDst = order.findIndex((p) => p.position === 'DEF' || p.position === 'PK');
  const lastPlayable = order.findLastIndex((p) => isPlayableNow(seatReadOf(v, p)));
  assert.ok(firstKDst > lastPlayable, 'a deferred K/DST must still sit below every playable row');
});

test('flex defers by the SAME test as open', () => {
  // One board, two seats: TE slot open on one, filled (so TE reads flex) on the
  // other. The TE's deferral must not depend on which slot he would fill.
  const rows = [];
  let i = 0;
  const add = (position, adp) => { rows.push({ ffcPlayerId: `f${i++}`, name: `${position}${i}`, position, adp, team: 'CIN' }); };
  for (let a = 84; a < 130; a += 3) { add('RB', a); add('WR', a + 1); }
  add('TE', 120); add('TE', 131); // best TE well past 83 + 12 = 95
  for (let a = 150; a < 180; a += 8) { add('DEF', a); add('PK', a + 3); }
  const teOpen = [pick('QB', 'QB'), pick('RB', 'RB'), pick('RB', 'RB'), pick('WR', 'WR'), pick('WR', 'WR')];
  const teFlex = [...teOpen, pick('TE', 'TE')];
  const at = (seat) => computeSeatValuation({
    rosterSlots: ROSTER, rounds: ROUNDS, allPicks: [], seatPicks: seat,
    available: rows, myNextOverall: 95, currentOverall: 83, teamsCount: TEAMS,
  });
  const te = rows.find((p) => p.position === 'TE');
  const open = seatReadOf(at(teOpen), te);
  const flex = seatReadOf(at(teFlex), te);
  assert.equal(open.slot, 'open');
  assert.equal(flex.slot, 'flex');
  assert.equal(open.deferred, true, 'TE at 120 is beyond 84 + 12 for an open slot');
  assert.equal(flex.deferred, true, '...and equally beyond it for the flex slot - same test');
  assert.equal(slotRank(flex), slotRank(open), 'and the two ranks agree');
  assert.equal(slotRank(flex), 1, 'deferred, not playable');

  // The sort agrees: on the flex seat, a deferred TE sits below every
  // undeferred RB/WR flex row, however bad their gaps.
  const v = at(teFlex);
  const order = sortedWith(rows, v);
  const firstTe = order.findIndex((p) => p.position === 'TE');
  const lastUndeferredFlex = order.findLastIndex((p) => { const r = seatReadOf(v, p); return r.slot === 'flex' && !r.deferred; });
  assert.ok(firstTe > lastUndeferredFlex, 'a deferred flex row ranks below every undeferred one');

  // And it expires the same way: bring the TE inside the horizon and he competes.
  const near = rows.map((p) => (p.position === 'TE' && p.adp === 120 ? { ...p, adp: 90 } : p));
  const vNear = computeSeatValuation({
    rosterSlots: ROSTER, rounds: ROUNDS, allPicks: [], seatPicks: teFlex,
    available: near, myNextOverall: 95, currentOverall: 83, teamsCount: TEAMS,
  });
  assert.equal(seatReadOf(vNear, near.find((p) => p.position === 'TE')).deferred, false);
});

test('the horizon anchors to board par: the pick number alone never defers a whole board', () => {
  // A board that has run 40 picks ahead of the pick number (keepers drained the
  // top). With the raw-pick horizon every open slot deferred; with par, the
  // positions priced at the top of the board are in play.
  const rows = [];
  let i = 0;
  const add = (position, adp) => { rows.push({ ffcPlayerId: `k${i++}`, name: `${position}${i}`, position, adp, team: 'CIN' }); };
  for (let a = 90; a < 140; a += 4) { add('RB', a); add('WR', a + 1); add('QB', a + 2); add('TE', a + 3); }
  for (let a = 160; a < 190; a += 8) { add('DEF', a); add('PK', a + 3); }
  const v = computeSeatValuation({
    rosterSlots: ROSTER, rounds: ROUNDS, allPicks: [], seatPicks: SEATS.empty,
    available: rows, myNextOverall: 50, currentOverall: 50, teamsCount: TEAMS,
  });
  const boardTop = 90;
  assert.ok(boardTop > 50 + TEAMS, 'precondition: every position is past the raw-pick horizon');
  const deferredPositions = new Set(rows.filter((p) => seatReadOf(v, p).deferred).map((p) => p.position));
  assert.deepEqual([...deferredPositions].sort(), ['DEF', 'PK'],
    'only the positions past par + a round (102) wait; RB/WR/QB/TE are the market\'s top and are in play');
  const order = sortedWith(rows, v);
  assert.ok(new Set(order.slice(0, 8).map((p) => p.position)).size > 1, 'the top of the board is not one position');
});
