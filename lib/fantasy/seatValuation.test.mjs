  ...
  for (const nextPick of [24, 52]) {
    for (const seat of Object.values(SEATS)) {
      ...
      for (const p of avail) {
        ...
        // The rank is derived from the tag PLUS deferral - 'open' alone no
        // longer places a row, which is the whole point of the round-context fix.
        const expected = read.slot === 'open' ? (read.deferred ? 2 : 0)
          : read.slot === 'flex' ? 1 : 3;
...
test('the deferral expires on its own once the market catches up', () => {
  ...
  // ...and with the skill starters full, DST/K now lead the board.
  const order = sortedWith(avail, late);
  assert.equal(seatReadOf(late, order[0]).slot, 'open');
  assert.ok(['DEF', 'PK'].includes(order[0].position),
    `late board should open with K/DST, got ${order[0].position}`);
});
