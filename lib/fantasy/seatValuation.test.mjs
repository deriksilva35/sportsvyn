  ...
  for (const nextPick of [24, 52]) {
    for (const seat of Object.values(SEATS)) {
      ...
      for (const p of avail) {
        ...
        // The rank follows from the tag PLUS deferral. Note open-and-undeferred
        // and flex share rank 0: both are startable this week, so they settle it
        // on gap rather than on category.
...
test('open and flex share a rank; deferred and full still do not', () => {
  ...
  if (kinds.has('full')) assert.equal(kinds.get('full'), 2);
});
