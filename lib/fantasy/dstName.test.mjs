...
test('the flow-core hands every name to a room through displayName(): pool row, keeper, persisted pick, undo', () => {
  ...
  // The clubs load once per process and retry after a failure. THE LOADER
  // MOVED to lib/fantasy/clubs.js, unchanged, when lib/draft/ourBoard.js
  // needed the same map: ourBoard builds the board that drafts.js loads, so an
  // import the other way would be an ES module cycle. The rule this pins is
  // the caching, not the file it lives in - so it follows the function.
  const clubs = stripComments(src('lib/fantasy/clubs.js'));
  assert.match(clubs, /clubsPromise \?\?= sql`SELECT t\.abbreviation, t\.name FROM teams t\n\s+JOIN leagues l ON l\.id = t\.league_id WHERE l\.slug = 'nfl'`/);
  assert.match(clubs, /\.catch\(\(e\) => \{ clubsPromise = null; throw e; \}\)/);
  assert.match(core, /import \{ nflClubs \} from '\.\/clubs\.js';/, 'and the flow-core imports it rather than keeping a second copy');
  assert.doesNotMatch(core, /let clubsPromise/, 'exactly one cache, in one file');

  // THE BOARD IS A FIFTH NAMING SITE, and it goes through the same function.
  // lib/draft/ourBoard.js builds the ranked room's rows straight from
  // sim_player_pool, so without this a defense would reach a room carrying the
  // provider's DEF field - the one thing dstName.js exists to prevent.
  const board = stripComments(src('lib/draft/ourBoard.js'));
  assert.match(board, /import \{ displayName \} from '\.\.\/fantasy\/dstName\.js';/);
  assert.match(board, /name: displayName\(r, clubs\),/);
  assert.ok(!board.includes("'D/ST'"), 'and it has no private D/ST grammar either');
...
test('the DEF quick line names its number; both rooms render line 2 as whole-fact tokens', () => {
  ...
  const tracker = stripComments(src('components/sim/TrackerRoom.js'));
  // RE-PINNED (lineTwo, 2 Sep 2026). This pinned each room's own
  // `{quick && quick.map((q) => <span className="q" key={q}>· {q}</span>)}`.
  // Line 2's order now comes from lib/fantasy/lineTwo.js (the MY TEAM read
  // rides ahead of the stats under seatSort); the guarantee is the same - one
  // token per stat, "· " prefixed, never joined - asserted where it now lives.
  const tokens = lineTwoTokens({ pos: 'DST', team: 'HOU', quick: q }).filter((t) => t.kind === 'quick');
  assert.deepEqual(tokens.map((t) => t.text), ['42 SACKS', '12 INT'], 'one token per stat, text intact');
  assert.match(room, /return <span className="q" key=\{t\.text\}>· \{t\.text\}<\/span>;/);
  assert.match(tracker, /return <span className="trk-quick" key=\{t\.text\}>· \{t\.text\}<\/span>;/);
