 ...
 */
/**
 * Which singleton positions this roster has CAPPED - startable slots all
 * filled. The K/DST-cap law's shared reader: the cap is the roster's
 * startable count (a 2-K console roster caps at 2), never a hardcoded 1.
 * @param {object} rosterSlots the room's shape
 * @param {Array}  myPicks     the seat's picks ({ position } each)
 */
export function cappedPositions(rosterSlots = {}, myPicks = []) {
  const norm = (p) => ({ PK: 'K', DEF: 'DST' }[p] ?? p);
  const counts = {};
  for (const p of myPicks) {
    const k = norm(p?.position);
    counts[k] = (counts[k] ?? 0) + 1;
  }
  const capped = new Set();
  for (const pos of SINGLETON_POSITIONS) {
    if ((counts[pos] ?? 0) >= (rosterSlots?.[pos] ?? 0) && (rosterSlots?.[pos] ?? 0) > 0) capped.add(pos);
  }
  return capped;
}

export function bestAvailableAtMyPick(available, myNextOverall, limit = 3, { capped = new Set() } = {}) {
  if (myNextOverall == null) return [];
  const norm = (p) => ({ PK: 'K', DEF: 'DST' }[p] ?? p);
  return (available ?? [])
    .slice()
    // THE CAP LAW: never show a DST to a roster that holds one. A capped
    // position's next copy has zero marginal value - excluded, not sunk.
    .filter((p) => !capped.has(norm(p.position)))
