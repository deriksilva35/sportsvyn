...
export async function getResults(draftId, userId) {
  ...
  const grade = engine.gradeRoster(userRecs);
  const ppv = (r) => r.adpAtPick - r.overallPick; // engine convention: negative = value
  const skill = userRecs.filter((r) => SKILL.has(r.slotPos) && !r.synthetic);
  const bestValue = skill.length ? skill.reduce((a, b) => (ppv(b) < ppv(a) ? b : a)) : null;
  const biggestReach = skill.length ? skill.reduce((a, b) => (ppv(b) > ppv(a) ? b : a)) : null;
  return {
    draft, config, userTeamIndex,
    userPicks: userRecs.map(pickDTO),
    rosterValueTotal: grade.rosterValueTotal,
    positionalBalance: grade.positionalBalance,
    bestValue: bestValue ? pickDTO(bestValue) : null,
    biggestReach: biggestReach ? pickDTO(biggestReach) : null,
    pivot: grade.pivot ? { ...pickDTO(grade.pivot), needWeight: grade.pivot.needWeight } : null,
    byeStackWarnings: grade.byeStackWarnings,
  };
}
