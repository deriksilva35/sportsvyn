...
import { withholdFrom, isWithheld, chooseSnapshot } from '../draft/pool.js';
...
// THE 32 CLUBS, for naming a defense (lib/fantasy/dstName.js). Loaded once
// per process: the table is static and every pool load, room load and undo
// would otherwise pay a query for 32 rows that never change. A failed load is
...
export async function startDraftFor(userId, presetId, pickPosition, opts = {}) {
  ...
  // pool_source, not source: an imported league drafts against its own pool.
...
async function resolveCustomPool(cfg, { ranked = false } = {}) {
  ...
  // A RANKED ROOM REACHES BACK FOR A SNAPSHOT THAT FITS (ruled). FFC's feed
  // thins in season - ppr/12 went 194 -> 116 -> 78 rows in three days - and the
  // newest snapshot stopped seating a 96-pick room, which closed the game for
  // a reason that had nothing to do with the game. Today's snapshot still wins
  // whenever it can; see chooseSnapshot for why it never reaches past one that
  // fits. A PRACTICE MOCK IS UNCHANGED: its config is the reader's own choice
  // and "bigger than the current ADP pool" is the honest answer there.
  if (ranked) {
    const chosen = chooseSnapshot(
      await snapshotSizes(mapped.pair.scoringFormat, mapped.pair.teamsCount),
      realDemand,
    );
    if (!chosen) return { err: { ok: false, reason: 'pool_too_small' } };
    const pool = await getPoolAt(mapped.pair.scoringFormat, mapped.pair.teamsCount, chosen.snapshotDate);
    if (!pool.length) return { err: { ok: false, reason: 'no_pool' } };
    // The count came from the same table in the same breath, so a pool that
    // fitted a moment ago and does not now means the snapshot changed under
    // us - refuse rather than seat a room that cannot finish.
    if (realDemand > pool.length) return { err: { ok: false, reason: 'pool_too_small' } };
    return { mapped, snapshotDate: chosen.snapshotDate, pool };
  }
...
async function loadPlayable(userId, draftId, { requireUserTurn = true, now = new Date() } = {}) {
  ...
  const pool = await getPoolAt(draft.pool_scoring_format, draft.pool_teams_count, draft.pool_snapshot_date, draft.pool_source ?? 'ffc');
