...
async function finalizeStart(config, pickPosition, opts, poolCtx) {
  ...
  return { ok: true, draftId, status: 'in_progress', pickPosition: pos, overallPick: state.overallPick, onTheClock: 'user', aiPicksMade: made.length - kept, keepersSeeded: kept };
}
...
async function resolveCustomPool(cfg, { ranked = false } = {}) {
  ...
  if (ranked) {
    ...
    return { mapped, snapshotDate: board.computedAt, pool: board.rows, source: OUR_SOURCE };
  }
...
export async function startCustomDraftFor(userId, configInput, pickPosition, opts = {}) {
  ...
  const { mapped, snapshotDate, pool, source: poolSource } = r;
...
async function loadPlayable(userId, draftId, { requireUserTurn = true, now = new Date() } = {}) {
  const draft = (await sql`SELECT * FROM drafts WHERE id = ${draftId} AND user_id = ${userId} LIMIT 1`)[0];
...
export async function makePickFor(userId, draftId, ffcPlayerId, { now = new Date() } = {}) {
  ...
  const { state, userTeamIndex, ranked, withheld } = l;
...
export async function timerAutoPickFor(userId, draftId, { now = new Date() } = {}) {
  ...
  const l = await loadPlayable(userId, draftId, { now });
  if (l.err) return l.err;
  const { state, userTeamIndex, ranked } = l;
  if (ranked && new Date(ranked.locksAt).getTime() <= new Date(now).getTime()) {
    return { ok: false, reason: 'week_locked', locksAt: ranked.locksAt };
  }
