...
/**
 * A reader's imported leagues. SEPARATE FROM getPresets, deliberately.
 *
 ...
 * everybody. This query is scoped by user_id and by source, which is the only
 * safe way to put a personal config on a shared page.
 */
export async function getMyLeagues(userId) {
  if (userId == null) return [];
  const leagues = await sql`SELECT id, name, teams_count, scoring_format, roster_slots, pick_timer_seconds,
                    external_league_id, teams, draft_date, pool_source,
                    (SELECT count(*)::int FROM draft_config_keepers k WHERE k.config_id = c.id) AS keeper_count
               FROM draft_configs c
              WHERE user_id = ${userId} AND source = 'fantrax'
 ...
 *
 * Ownership is the whole gate: the config must belong to the caller. The free
 * count still applies - a league draft is a practice draft against real
 * keepers, not a ranked entry.
 */
export async function startLeagueDraftFor(userId, configId, opts = {}) {
  const config = (await sql`
    SELECT id, user_id, name, teams_count, scoring_format, roster_slots, pick_timer_seconds,
           is_preset, source, pool_source, external_league_id, teams
      FROM draft_configs WHERE id = ${configId} AND user_id = ${userId} LIMIT 1`)[0];
  if (!config || config.source !== 'fantrax') return { ok: false, reason: 'league_not_found' };
  const mine = (config.teams ?? []).find((t) => t.isMine === true);
  if (!mine?.slot) return { ok: false, reason: 'no_seat' };
  // THE SEAT IS A FRANCHISE. Default = the reader's own team (isMine); any
  // other seat plays THAT team this run - its column, its keepers, its roster -
  // and the reader's real team is a bot at its own column, keepers and all.
  let seat = mine.slot;
  if (opts.seat != null) {
  ...
  return finalizeStart(config, seat, { ...opts, franchise: true }, {
    snapshotDate, pool,
    poolScoringFormat: config.scoring_format, poolTeamsCount: config.teams_count,
  });
}
