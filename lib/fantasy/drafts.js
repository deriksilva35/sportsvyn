 ...
 *
 * THE CELL IS DERIVED FROM THE RUN'S SEAT (084). A keeper row names its owner
 * and its round; `seat` is where the reader sits in this draft (pick_position),
 * runSeats() swaps the reader into it, and each keeper's overall follows from
 * (round, its owner's seat) through the snake. A default start is the identity
 * seating, so every draft made before 084 derives exactly the cells it was
 * drawn with. An owner the teams list cannot seat is a hard fail: the league's
 * membership and its keepers disagree.
 */
async function loadKeepers(config, seat) {
  ...
  const { keeperPicks, runSeats } = await import('../fantrax/keeperSeed.js');
  const { seatOf } = runSeats(config.teams, seat);
...
async function finalizeStart(config, pickPosition, opts, poolCtx) {
  ...
  // user_seat (084) is the audit of a CHOSEN seat: the seat the person tapped
  // for this run, null when they took the default. pick_position stays the
  // seat every reader derives from; the two agree whenever user_seat is set.
  const userSeat = opts.chosenSeat === true ? pos : null;
  ...
  const keepers = await loadKeepers(config, pos);
 ...
 * fantraxTeamId matched getLeagues.teamId); opts.seat, 1..teams_count, moves
 * the reader there for THIS RUN ONLY - the team imported at that seat takes
 * the reader's (runSeats), the keepers' cells re-derive, and the config is not
 * touched. A config with no isMine entry is still refused rather than
...
export async function startLeagueDraftFor(userId, configId, opts = {}) {
  ...
  return finalizeStart(config, seat, { ...opts, chosenSeat }, {
    snapshotDate, pool,
    poolScoringFormat: config.scoring_format, poolTeamsCount: config.teams_count,
  });
}
...
async function loadPlayable(userId, draftId, { requireUserTurn = true } = {}) {
  ...
  const state = rebuildState(config, pool, draft.pick_position, pickRows, await loadKeepers(config, draft.pick_position));
...
export async function getDraftForRoom(draftId, userId) {
  ...
  // THE READER'S MINORS, from the league's teams jsonb - the devy shelf Fantrax
  // keeps off the draft. Read by OWNER (isMine), not by seat: since 084 the
  // reader may sit anywhere this run, and the team imported at that slot is
  // somebody else - reading minors by pick_position would show their devy.
  const minors = (config.teams ?? []).find((t) => t.isMine === true)?.minors ?? [];
  ...
  const keepers = await loadKeepers(config, draft.pick_position);
...
export async function getResults(draftId, userId) {
  ...
  const config = await loadConfig(draft.config_id);
  const pool = await getPoolAt(draft.pool_scoring_format, draft.pool_teams_count, draft.pool_snapshot_date, draft.pool_source ?? 'ffc');
  const state = rebuildState(config, pool, draft.pick_position, picks, await loadKeepers(config, draft.pick_position), { needWeight: true });
  const userTeamIndex = draft.pick_position - 1;
