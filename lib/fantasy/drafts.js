 ...
 *
 * A SEAT CONFLICT IS A HARD FAIL: the seat Fantrax names and the seat the snake
 * implies must agree, or the provider's order changed under us.
 */
async function loadKeepers(config) {
  const rows = await sql`
    SELECT team_slot, round, pick_in_round, fantrax_player_id, player_name, position, adp, team
      FROM draft_config_keepers WHERE config_id = ${config.id}
     ORDER BY round, pick_in_round`;
  if (!rows.length) return null;
  const { keeperPicks } = await import('../fantrax/keeperSeed.js');
  const { recs, conflicts } = keeperPicks(rows, config.teams_count);
  if (conflicts.length) {
    throw new Error(`keeper seat conflict on ${conflicts.length} pick(s): ${JSON.stringify(conflicts[0])}`);
  }
 ...
 *
 * THE SEAT IS NOT CHOSEN. In a preset the reader picks a slot; in a league the
 * slot was assigned by the commissioner and recorded at import as the teams
 * entry marked isMine (the one whose fantraxTeamId matched getLeagues.teamId).
 * A config with no such entry is refused rather than defaulted to slot 1 -
 * drafting from somebody else's seat is not that league's draft.
 *
...
export async function startLeagueDraftFor(userId, configId, opts = {}) {
  ...
  return finalizeStart(config, mine.slot, opts, {
    snapshotDate, pool,
    poolScoringFormat: config.scoring_format, poolTeamsCount: config.teams_count,
  });
}
...
export async function getDraftForRoom(draftId, userId) {
  ...
  // THE SEAT'S MINORS, from the league's teams jsonb - the devy shelf Fantrax
  // keeps off the draft. Read by seat, not by isMine: the draft's pick_position
  // IS the seat, and the two agree by construction (startLeagueDraftFor).
  const minors = (config.teams ?? []).find((t) => t.slot === draft.pick_position)?.minors ?? [];
