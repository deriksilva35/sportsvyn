...
// ===========================================================================
// THE DATABASE FIXTURE IS REGISTERED BEFORE ANY TEST - AND THAT IS THE FIX
// ===========================================================================
// It used to sit two hundred lines down, below eight already-registered
// tests, in a file that does top-level `await import`. node:test therefore
// registered these hooks too late to bracket the tests, and the observed
// order was:
//
//     before's wipe   ->   after's wipe   ->   before's INSERT
//
// The sentinel was created AFTER the teardown had already run, so it
// survived, and `after`'s "the sentinel user must be gone" saw 1. That is
// the whole of the intermittent chunk-B failure that has been blamed on
// parallelism for two relays: it is not parallelism, it is hook ordering
// inside one file, and it reproduces alone, three runs out of three.
//
// The per-run marker below is still right and still necessary - two runs of
// this file on one database must not share a cleanup pattern - but it was
...
// THE MARKER IS PER RUN, NOT PER FILE (test-iso relay). This hook used to
// wipe 'collegesurface-%@example.invalid' and then assert the count was zero -
// so a second run of this file inserting between the DELETE and the SELECT
// made the first one fail. It did, intermittently, in chunk B for two relays.
const MARK = fixtureMark('collegesurface');
async function wipe() { await sql`DELETE FROM users WHERE email LIKE ${MARK.like}`; }
let USER = null;

before(async () => {
  await sweepStale(sql, MARK);   // only rows an hour old, never a live sibling's
  await wipe();
  USER = (await sql`INSERT INTO users (name, email)
    VALUES ('CollegeSurface Sentinel', ${MARK.email('sentinel')})
    RETURNING id`)[0].id;
});
after(async () => {
  await wipe();
  const left = await sql`SELECT count(*)::int n FROM users WHERE email LIKE ${MARK.like}`;
...
test('the toggle sits with the class row, not in the position row, and carries the sort with it', () => {
  ...
  // And the list is filtered on the league axis. THE ARGUMENT IS poolAll NOW
  // (v2): kicked players ride the same filters and the same sort as everyone
  // else so the room can grey them rather than drop them, and the league axis
  // must keep working over that combined list.
  assert.match(room, /filterPlayers\(poolAll, \{ position: filter, team, search, cls, league: college \? 'ncaaf' : 'nfl' \}\)/);
...
test('the served pick path refuses a college player when the bench is full, in the same grammar as any illegal pick', () => {
  ...
  // THE LAW IS THE GATE AND ITS ANSWER, not their adjacency. The rolling-pool
  // relay put a 'player_kicked_off' branch between the two - a different
  // refusal for a different reason - and the thing this pins is that LEGALITY
  // is still decided by canRoster with humanPick, and still answers
  // 'illegal_pick'.
  assert.match(makePick, /canRoster\(state, state\.teams\[userTeamIndex\], player, round, null, \{ humanPick: true \}\)/);
  assert.match(makePick, /reason: 'illegal_pick'/);
  assert.ok(makePick.indexOf('canRoster(') < makePick.indexOf("reason: 'illegal_pick'"),
    'the gate still precedes its refusal');
...
test('college PPG is COALESCEd per category, season-stamped, league-scoped, and never guesses', () => {
  ...
  assert.match(act, /getPlayerSeasonSummaries\(ids, scoringFormat, season\),\s*\n\s*getCollegeSeasonSummaries\(ids, scoringFormat\),/);
});
