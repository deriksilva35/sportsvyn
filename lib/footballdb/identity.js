...
// any year. Inferred from which category tab(s) a row's fields came from,
// checked in this order because it is the order a player can only belong to
// one of: a passer is a QB even if he also has carries; a rusher with no
// passing is an RB even if he also caught a few passes.
//
...
// identically under PPR). Every receiving-only row defaults to WR here. This
// is a real imprecision, not a silent one: the ingest report counts how many
...
export function inferPosition(row) {
  if (row.passAtt != null || row.passYds != null || row.passTd != null) return 'QB';
  if (row.rushAtt != null || row.rushYds != null) return 'RB';
  if (row.rec != null || row.recYds != null) return 'WR'; // TE indistinguishable from this source
  if (row.fgm != null || row.fga != null || row.xp != null) return 'PK';
  return null; // Defense-tab-only row: no slot in the QB/RB/WR/TE/FLEX/PK/DEF mechanic
}
