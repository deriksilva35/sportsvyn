...
test('the createUser hook does NOT await the send', () => {
  ...
  assert.match(code, /createUser\(\{\s*user\s*\}\)\s*\{\s*fireWelcomeEmail\(user\);\s*\}/,
    'createUser must fire-and-forget; an await here re-couples signup to the mailer');
  assert.ok(!/await\s+fireWelcomeEmail/.test(code), 'the send must never be awaited in the auth path');
});
