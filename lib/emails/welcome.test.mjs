...
test('the createUser hook does NOT await the send', () => {
  // This is the whole promise: a Resend outage cannot cost us an account.
  // `await fireWelcomeEmail(...)` would quietly re-couple them.
  const code = stripComments(src('auth.js'));
  assert.match(code, /events:\s*\{/, 'auth config must register events');
  // Asserted as the INVARIANT, not as a layout. An earlier version pinned the
  // hook's exact one-line body and broke the moment createUser legitimately
  // grew a second statement (the first_seen_context stamp) - a test that fails
  // on formatting teaches people to edit tests instead of reading them.
  assert.match(code, /createUser\(\{\s*user\s*\}\)/, 'the hook must exist and take the new user');
  assert.match(code, /fireWelcomeEmail\(user\)/, 'and must start the send');
  assert.ok(!/await\s+fireWelcomeEmail/.test(code),
    'the send must never be awaited - that would re-couple signup to the mailer');
});
...
test('the OTP trigger is fire-and-forget, like the adapter hook', () => {
  const code = stripComments(src('lib/auth/emailOtp.js'));
  assert.ok(!/await\s+fireWelcomeEmail/.test(code),
    'awaiting it would couple a sign-in to a mail vendor');
});
