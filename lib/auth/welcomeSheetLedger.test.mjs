...
test('WHICH control dismissed it is recorded, not inferred', () => {
  ...
  assert.deepEqual(DISMISS_CONTROLS, ['primary', 'tracker', 'backdrop', 'escape']);
  // 'purchase' is retired with the buy button it named. Keeping a token for a
  // control that cannot fire would describe the sheet as it was, not as it is,
  // and no row ever carried it - the ledger shipped into zero traffic.
  assert.ok(!DISMISS_CONTROLS.includes('purchase'));
...
test('ALL FOUR exit paths report which one they were', () => {
  ...
  // The tracker link routes as well as dismissing, so it goes through a named
  // handler rather than an inline arrow.
  assert.match(sheet, /className="wsheet-secondary" onClick=\{toTracker\}/);
  assert.match(sheet, /function toTracker\(\) \{\s*\n\s*dismiss\('tracker'\);/);
});

test('the tracker link DISMISSES BEFORE it scrolls, and the scroll is guarded', () => {
  // A missing anchor must not turn a dismissal into a dead tap - the sheet
  // closing is the part the user asked for.
  const fn = sheet.slice(sheet.indexOf('function toTracker()'), sheet.indexOf('if (welcomed) return null;'));
  assert.ok(fn.indexOf("dismiss('tracker')") < fn.indexOf('scrollIntoView'), 'dismiss first');
  assert.match(fn, /getElementById\(TRACKER_ANCHOR\)\?\.scrollIntoView/);
  assert.match(fn, /catch \{/, 'and the scroll cannot throw');
  // The anchor has to exist on the thing it points at.
  const trk = stripComments(src('components/sim/TrackerStart.js'));
  assert.match(trk, /id="tracker-start"/);
  assert.match(sheet, /export const TRACKER_ANCHOR = 'tracker-start'/);
});

test('the tracker setup fields carry inline hints', () => {
  // Every field is asked BEFORE the user has seen what the Tracker does, and the
  // most common reason not to start a setup form is not knowing whether you can
  // change your mind.
  const trk = stripComments(src('components/sim/TrackerStart.js'));
  assert.match(trk, /Name the teams in your league \(editable on draft night\)/);
  assert.match(trk, /How many seats, how it scores, and where you pick - all editable on draft night/);
  assert.ok(!/[—–]/.test(trk.match(/trk-hint">[^<]+/g)?.join(' ') ?? ''), 'hyphens only');
});
