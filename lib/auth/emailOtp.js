...
export async function redeemEmailCode(sql, {
  ...
  // How this account arrived, for users.first_seen_context. Passed IN rather
  // than resolved here: this module is unit-tested with no request context, and
  // reading cookies from inside it would make that impossible.
  firstSeenContext = null,
}) {
  ...
  //
  // THE WELCOME EMAIL FIRES FROM THE INSERT BRANCH BELOW, not from Auth.js's
  // events.createUser. That event is raised by the ADAPTER's createUser, and
  // this flow deliberately never calls it: the OTP path owns redemption itself
  // (auth.js forces useVerificationToken to null for exactly that reason) and
  // writes the user row directly. Hooking the adapter event alone meant the
  // first production magic-link signup got no mail and left no trace - the
  // event simply never ran on this path.
  //
  // This branch is the honest trigger: reaching it means the SELECT missed and
  // the INSERT succeeded, so a user was created here, once. The `else` branch
  // is an existing account signing in again and must never re-send.
  //
  // POST-SEASON REFACTOR CANDIDATE: routing this flow through
  // adapter.createUser would collapse the two triggers into one, at the cost of
  // changing a working auth path - deliberately not done during launch week.
  let user = (await sql`SELECT id FROM users WHERE email = ${identifier} LIMIT 1`)[0];
  if (!user) {
    // first_seen_context goes in the INSERT rather than a follow-up UPDATE: this
    // is the moment of creation, so the provenance is written atomically with
    // the row it describes and there is no window where the account exists
    // unlabelled. NULL when the caller could not determine it - honest, and the
    // same thing every pre-058 row says.
    user = (await sql`
      INSERT INTO users (email, "emailVerified", first_seen_context)
      VALUES (${identifier}, ${now.toISOString()}, ${firstSeenContext})
      RETURNING id`)[0];
    // AWAITED, same as the adapter hook. fireWelcomeEmail registers the send
    // with after(), so this returns as soon as the work is scheduled and the
    // sign-in still does not wait on a mail vendor - but the work now survives
    // the response. The previous shape was fire-and-forget, which on serverless
    // means the promise is discarded when the invocation is frozen: no mail, no
    // ledger row, no error. That cost user 19 their welcome email on the Apple
    // route on 2026-08-09, and this site had exactly the same hole.
    //
    // Safe to await: sendWelcomeEmail catches every path and returns a reason
    // string, so it cannot throw into the sign-in.
    await fireWelcomeEmail({ id: user.id, email: identifier });
  } else {
