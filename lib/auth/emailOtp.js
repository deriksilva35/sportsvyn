...
export async function redeemEmailCode(sql, {
  ...
  if (!user) {
    ...
    user = (await sql`
      ...
      RETURNING id`)[0];
    // Fire-and-forget, same contract as the adapter hook: a mail vendor must
    // never be able to fail a sign-in. Not awaited, cannot throw.
    fireWelcomeEmail({ id: user.id, email: identifier });
  } else {
