...
/** Record one attempt in the pollers' ledger. Never throws. */
async function record({ ok, userId, reason }) {
  try {
    await sql`
      ...
      VALUES ('welcome-email', 'send', now(), now(), ${ok},
              ...
              ${ok ? null : String(reason ?? 'unknown').slice(0, 500)})`;
  } catch {
    // The ledger failing must not fail the send path either.
    console.error('[welcome-email] could not record outcome', { userId, ok, reason });
  }
}
...
export async function sendWelcomeEmail(user) {
  ...
  try {
    if (!welcomeEmailEnabled()) return 'disabled';
    const to = user?.email;
    ...
    if (!to) { await record({ ok: false, userId, reason: 'no-email' }); return 'no-email'; }
    ...
    if (res?.error) { await record({ ok: false, userId, reason: res.error.message ?? 'resend-error' }); return 'failed'; }
    await record({ ok: true, userId });
    return 'sent';
  } catch (e) {
    await record({ ok: false, userId, reason: e?.message });
    return 'failed';
  }
}
