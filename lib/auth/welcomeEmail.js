 ...
 */
/**
 * The RFC 8058 unsubscribe headers for a signed unsubscribe URL.
 *
 * Shared with scripts/broadcast.mjs so the transactional welcome and any
 * one-off broadcast present the same control to the same mail clients. A
 * mailto fallback is included because some clients still prefer it and a
 * header listing only an https target is treated as partial support.
 */
export function unsubscribeHeaders(url) {
  return {
    'List-Unsubscribe': `<${url}>, <mailto:unsubscribe@sportsvyn.com>`,
    'List-Unsubscribe-Post': 'List-Unsubscribe=One-Click',
  };
}

/**
 * A CLICK THROUGH OUR OWN DOMAIN. Every href in a broadcast body is rewritten
 * to /api/email/click?c=<campaign>&u=<userId>&t=<sig>&to=<url>; the route
 * verifies the signature, ledgers the click to sync_runs (source
 * 'email-click') and 302s to `to`. Signed exactly like the unsubscribe link,
 * over campaign + user + destination, so a link cannot be re-pointed or
 * re-attributed. The launch email's clicks are lost - this is for the next
 * send onward.
 */
export function clickToken({ campaign, userId, to }) {
  const secret = process.env.NEXTAUTH_SECRET || process.env.AUTH_SECRET || 'dev-only-unsub-secret';
  return createHmacSync('sha256', secret).update(`click:${campaign}:${userId}:${to}`).digest('hex').slice(0, 32);
}
export function clickUrlFor({ campaign, userId, to, base = baseUrl() }) {
  const q = new URLSearchParams({ c: campaign, u: String(userId), t: clickToken({ campaign, userId, to }), to });
  return `${base}/api/email/click?${q.toString()}`;
}
...
export async function sendWelcomeEmail(user) {
  ...
  try {
    ...
    const res = await resend.emails.send({
      ...
      html: mail.html,
      text: mail.text,
      // ONE-CLICK UNSUBSCRIBE, AS A HEADER. The link in the body was the only
      // way out, which means the only readers who take it are the ones who
      // scroll. Gmail and Apple Mail surface a native Unsubscribe control when
      // these two headers are present, and both now effectively require them
      // for bulk mail - a sender without them gets filed as spam rather than
      // argued with.
      //
      // List-Unsubscribe-Post is what makes it ONE CLICK rather than a
      // navigation: the client POSTs directly and never opens the page. Our
      // route is a GET, and RFC 8058 requires the POST target to work, so it
      // is listed alongside the mailto rather than instead of it. See the
      // broadcast script, which sets the same pair.
      headers: unsubscribeHeaders(await unsubscribeUrlFor(userId)),
    });
    if (res?.error) {
