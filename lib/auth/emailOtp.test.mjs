...
test('both paths consume one token: code use kills the magic link', async () => {
  ...
  assert.equal(res.ok, true);
  // the verification_token the magic link would redeem is gone
...
test('both paths consume one token: link use kills the code', async () => {
  const id = ident('linkfirst');
  const tokenHash = await seed({ identifier: id, rawToken: 'rawtok-lf', code: '555555' });
  // simulate the magic link being used: Auth.js deletes the verification_token row
