...
export const apiSportsFootball = {
  ...
  games: ({ league, season, date, team } = {}) => {
    // NULL AND UNDEFINED BOTH MEAN "not supplied". An `!== undefined` guard
    // alone lets an explicit null through as the literal string "null", which
    // the provider rejects with "The Date field must contain a valid date" -
    // and the importer passes date: null for a whole-season pull, so that was
    // every season import.
    const q = new URLSearchParams();
    const set = (k, v) => { if (v !== undefined && v !== null && v !== '') q.set(k, String(v)); };
    set('league', league);
    set('season', season);
    set('date', date);
    set('team', team);
  ...
  // ---- per-game detail ------------------------------------------------------
  // Probed 2026-08-11 against a finished preseason game and a 2024 REG control.
  // events and player statistics serve BOTH phases; team statistics returns
  // zero rows for preseason, which is a provider coverage gap rather than a
  // plan limit, so callers must treat an empty team box as normal in August.

  /** Scoring plays only - TD and FG. No drives, no non-scoring snaps. */
  events: (id) => get(`/games/events?id=${id}`),

  /** Per-player lines, grouped. Groups appear only when something happened. */
  playerStats: (id) => get(`/games/statistics/players?id=${id}`),

  /** Team totals. REG only - empty on preseason. */
  teamStats: (id) => get(`/games/statistics/teams?id=${id}`),
};
