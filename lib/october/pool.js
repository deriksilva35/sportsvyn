 ...
 */
export function poolRows({ roster = [], seasonStats = [], matchId, teamAbbr, probableId = null, probableName = null }) {
 ...
 */
export function startersOnly(rows = [], { lineup = null, awayAbbr = null, homeAbbr = null } = {}) {
  ...
  for (const r of rows) {
    const side = sideOf(r);
    if (side && r.kind === 'arm' && r.probable) hasProbable[side] = true;
  }

  const out = [];
  const SIDE_ORDER = { away: 0, home: 1 };
  for (const r of rows) {
    const side = sideOf(r);
    if (r.kind === 'arm') {
      if (r.probable) { out.push({ ...r, order: null, starting: true, probablePending: false }); continue; }
  ...
  out.sort((a, b) => {
    ...
    // ONE CLUB'S CARD AT A TIME, AWAY FIRST, THEN THE ORDER. Sorting on the
    // order alone interleaved the two lineups - 1st, 1st, 2nd, 2nd - which reads
    // as a list nobody batted in. A posted panel is two batting orders, one
    // under the other, in the order the clubs hit.
    const as = SIDE_ORDER[a.side] ?? 2;
    const bs = SIDE_ORDER[b.side] ?? 2;
    if (as !== bs && (a.order != null || b.order != null)) return as - bs;
    // THE BATTING ORDER IS THE ORDER. A bat with a slot outranks one without,
    // because a posted card is a better fact than a season average.
...
export async function octoberPool(contest, { rebuild = false } = {}) {
  ...
  const pool = { builtAt: new Date().toISOString(), byGame, incomplete: failed > 0 };
