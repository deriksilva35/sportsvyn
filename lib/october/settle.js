...
import { SLOTS, DNF, dayState, slotsFor } from './rules.js';
import { slotPoints, batLine, armLine, round1 } from './scoring.js';
 ...
 */
export function scoreCard(lineup = {}, statBy = new Map(), matchBy = new Map(), board = null) {
  // THE DAY'S OWN SLOTS. A short slate has a shorter card - rules.js
  // cardSizeFor() says why - and scoring five slots on a two-game day would
  // report three permanent empties as the reader's fault.
  const use = board ? slotsFor(board) : SLOTS;
  const slots = use.map((slot) => {
  ...
  return {
    ...
    complete: filled === use.length && settled === use.length,
  };
}
...
export async function settleOctoberDay(contest, { now = new Date() } = {}) {
  ...
  for (const e of entries) {
    ...
    const card = scoreCard(e.lineup ?? {}, statBy, matchBy, board);
