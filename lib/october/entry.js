...
import { SLOTS, refuseReason, cardProgress, dayState, nextLock, maxPerGame, effectiveKickoff, DNF } from './rules.js';
import { scoreCard, boxFor } from './settle.js';
import { usedPlayers, octoberPool, startersOnly, notStarting } from './pool.js';
...
export async function saveOctoberPick(userId, contestId, slot, player, { now = new Date() } = {}) {
  ...
  const used = await usedPlayers(userId, contest.season_year, { excludeContestId: contestId, preview: isPreview(contest) });
  ...
  const reason = refuseReason(lineup, slot, player, {
    board: contest.board ?? [], used: new Set(used.keys()), now, statusBy, kickoffBy,
  });
...
export async function octoberView(userId, contest, { now = new Date(), withPool = true } = {}) {
  ...
  const [{ statBy, matchBy }, used, pool] = await Promise.all([
    boxFor(contest),
    usedPlayers(userId, contest.season_year, { excludeContestId: contest.id, preview: isPreview(contest) }),
