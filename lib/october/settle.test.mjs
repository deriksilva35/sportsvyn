...
test('THE BURN survives a day boundary, and MAX TWO survives a swap', () => {
  const now = new Date('2026-09-29T16:00:00Z');
  // Burn: spent on an earlier day, refused today, and told which day.
  const used = new Set(['7']);
  assert.equal(refuseReason({}, 'bat1', { playerId: 7, matchId: 1, kind: 'bat' },
    { board: BOARD, used, now }), 'used');
  // Max two: the arm counts as a player from its game.
  const two = { arm: pick(10, 2), bat1: pick(1, 2) };
  assert.equal(refuseReason(two, 'bat2', { playerId: 5, matchId: 2, kind: 'bat' },
    { board: BOARD, now }), 'max_two_from_game');
