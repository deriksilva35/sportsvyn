 ...
 */
export async function ensureOctoberDay({ season, day, rows, now = new Date() }) {
...
/** The day a reader is looking at: today's card, else the next one with games. */
export async function currentOctoberDay({ now = new Date() } = {}) {
  ...
  const [c] = await sql`
    SELECT id, season_year, puzzle_date, board, meta, opens_at, locks_at, settled, settled_at
     ...
     ORDER BY settled ASC, puzzle_date DESC
     LIMIT 1`;
  return c ?? null;
}
