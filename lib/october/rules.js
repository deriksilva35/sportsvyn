// lib/october/rules.js - the four rules, in one place, PURE.
//
...
//
// THERE IS NO BURN. A fifth rule used to say "a player used once is gone for the
// rest of the postseason", and it is gone: October is a fresh five every day.
// What that rule cost was the best players - a reader who spent Judge and Ohtani
// in the wild cards played the World Series out of the bottom of the pool, and
// the card got worse to read every day it survived. THE RUN KEEPS ITS BURN, where
// it belongs: that game is nine picks across one bracket and spending a player is
// the whole shape of it. October and the Run are different games about the same
// month, and this is the line between them.
 ...
 * @param opts     { board, now, statusBy, kickoffBy }
 *
 * NO `used` HERE. October has no burn - see the header. The Run's own
 * refuseReason (lib/run/rules.js) still takes one and still enforces it.
 */
export function refuseReason(lineup, slot, player, {
  board = [], now = new Date(), statusBy = null, kickoffBy = null,
} = {}) {
  ...
  // NO PLAYER TWICE ON ONE CARD. This is the only uniqueness rule left, and it
  // is a rule about the SHAPE of a card rather than about the tournament: five
  // slots holding four men is four players, not five.
...
/**
 * THE MATCH-UP, AS A READER WRITES IT: "MIN @ SF", and "MIN @ SF · G2" when it is
 * the second game of a doubleheader.
 *
 * IT EXISTS BECAUSE THE CARD WAS PRINTING A SLUG. The next-lock label read
 * `next.slug.toUpperCase()`, so the header said
 *
 *     MLB-2026-09-23-MIN-SF-G2 · 12:45 PM PT
 *
 * which is a database key on a screen. The abbreviations are already on the board
 * entry; only the game number is not, and it lives in the slug's own -g<N>
 * suffix, which is how a doubleheader is keyed. Reading one segment off the slug
 * is not the same as printing the slug.
 */
export function gameLabel(g) {
  const away = g?.away?.abbr ?? null;
  const home = g?.home?.abbr ?? null;
  const side = away && home ? `${away} @ ${home}` : null;
  const dh = /-g(\d+)$/.exec(String(g?.slug ?? ''));
  // A DOUBLEHEADER'S FIRST GAME IS NOT LABELLED "G1" unless the slug says so -
  // the provider keys one as -g1 and the other -g2, and when it keys neither
  // there is only one game and nothing to disambiguate.
  return [side, dh ? `G${dh[1]}` : null].filter(Boolean).join(' · ') || null;
}
