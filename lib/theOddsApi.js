...
export const PROP_MARKETS = {
  ...
  cfb: ['player_rush_yds', 'player_receptions', 'player_reception_yds',
    'player_anytime_td', 'player_1st_td'],
  epl: ['player_goal_scorer_anytime', 'player_first_goal_scorer', 'player_last_goal_scorer',
    'player_shots_on_target', 'player_shots', 'player_assists'],
};

/**
 * PROBED, NOT GUESSED (28 Aug 2026, one event per sport, 20 credits total).
 * These keys were asked for and the vendor does NOT price them. Named here so
 * their absence is a recorded finding rather than a gap someone re-discovers:
 *
 *   NFL   pass_attempts, pass_completions, pass_interceptions, rush_attempts,
 *         kicking_points, sacks, tackles_assists
 *   CFB   the same, PLUS pass_tds and pass_yds - college quarterbacks are not
 *         priced for passing at all by these books
 *
 * DELIBERATELY SKIPPED though the vendor DOES price them: EPL's
 * player_to_receive_card and player_to_receive_red_card. They are real markets
 * and they map onto nothing we ingest - the EPL player-stats table records
 * cards, but a disciplinary market is a different kind of claim from a
 * performance one, and adding it would put "will this player be booked" on a
 * board whose stance is explain-don't-pick. If that changes it is a ruling,
 * not a code change.
 */
export const PROBED_UNPRICED = Object.freeze({
  nfl: ['player_pass_attempts', 'player_pass_completions', 'player_pass_interceptions',
    'player_rush_attempts', 'player_kicking_points', 'player_sacks', 'player_tackles_assists'],
  cfb: ['player_pass_tds', 'player_pass_yds', 'player_pass_attempts', 'player_pass_completions',
    'player_pass_interceptions', 'player_rush_attempts', 'player_kicking_points'],
  epl: [],
});
export const PROBED_SKIPPED = Object.freeze({
  epl: ['player_to_receive_card', 'player_to_receive_red_card'],
});
...
export const ANYTIME_MARKETS = new Set([
  'player_anytime_td', 'player_1st_td',
