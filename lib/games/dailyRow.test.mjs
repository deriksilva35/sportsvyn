...
test('Done: a graded run, as a share of the ceiling', () => {
  ...
  assert.equal(r.line2, 'played · 81.3% of best'); assert.deepEqual(r.pill, { label: 'Done', tone: 'jade' }); assert.equal(r.tile, 'done');
});
test('a run that fell short never reads "100% of best" on the lobby row', () => {
  // 0.99966 of the ceiling is eight-points-short territory: the old whole
  // number wrote it as 100.
  const r = dailyRowFor({ board, run: { startedAt: 'x', completedAt: 'y', pct: 0.99966 }, uid: 7 });
  assert.equal(r.line2, 'played · 99.9% of best');
  const perfect = dailyRowFor({ board, run: { startedAt: 'x', completedAt: 'y', pct: 1 }, uid: 7 });
  assert.equal(perfect.line2, 'played · 100% of best', 'and a real perfect board still says so');
});
