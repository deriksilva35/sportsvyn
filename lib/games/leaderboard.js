...
import { SEASON_TABLE_MIN_FIELD } from './lobby.js';
import { numbersOf } from '../util/scoresOf.js';

/** Top scorers for one settled Weekly or Draft contest. */
export async function scoreLeaderboard(contestId, uid = null, { limit = 5, game = null } = {}) {
  ...
  const ranked = rows.map((r, i) => ({
    ...
    // `game` decides which method line rides the row - this reader serves both
    // the Weekly and the Draft grade pages, and the two personas describe
    // themselves differently on each.
    ...houseMark({ isHouse: r.is_house === true, handle: r.handle }, game),
  }));
...
export async function pickemBoardLeaderboard(contestId, uid = null, { limit = 5 } = {}) {
  ...
  const ranked = rows.map((r, i) => ({
    rank: i + 1, userId: r.user_id, name: displayName({ id: r.user_id, handle: r.handle }),
    score: Number(r.score),
    ...houseMark({ isHouse: r.is_house === true, handle: r.handle }, 'pickem'),
  }));
  const top = ranked.slice(0, limit);
  const mine = uid == null ? null : (ranked.find((r) => r.userId === uid) ?? null);
  return {
...
export async function draftFieldLeaderboard(contestId, uid = null, { limit = 5 } = {}) {
  const rows = await sql`
    SELECT e.user_id, e.score, u.handle, u.is_house, d.pick_position AS seat
  ...
  const ranked = rows.map((r, i) => ({
    ...
    ...houseMark({ isHouse: r.is_house === true, handle: r.handle }, 'draft'),
  }));
  const top = ranked.slice(0, limit);
  const mine = uid == null ? null : (ranked.find((r) => r.userId === uid) ?? null);
  return {
...
/**
 * "1, 2, 4-6, 8-12" - a sorted seat list collapsed to ranges (relay 2b-fix
 * item 6). Twelve seats with two drafters means ten "nobody drafted it"
 * rows, which is a table of absence; one line naming the gap says the same
 * thing and lets the rows that carry a result be the table.
 *
 * A RUN OF TWO STAYS A LIST ("4, 5", never "4-5") - the hyphen form is
 * shorter only from three up, and "4-5" reads as a range the reader has to
 * expand for no saving.
 */
export function seatRangeLabel(seats) {
  // numbersOf FIRST, THEN the Set: coercing first turned a null seat into a 0
  // and put a "seat 0" in the range nobody drafted from.
  const sorted = [...new Set(numbersOf(seats))].sort((a, b) => a - b);
  ...
  runs.push([start, prev]);
  return runs
    .map(([a, b]) => (b - a >= 2 ? `${a}-${b}` : Array.from({ length: b - a + 1 }, (_, i) => a + i).join(', ')))
    .join(', ');
}
...
export async function draftSeatSeasonTable(teamsCount) {
  // SAME FIELD FLOOR AS THE SEASON TABLE (relay 2b-fix-2 item 5) - a seat's
  // season average is built from the same pct, so a one-entrant week would
  // hand whichever seat that player happened to take a free 100%.
  const rows = await sql`
