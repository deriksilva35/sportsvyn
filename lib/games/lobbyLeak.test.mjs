 ...
 * HISTORY READS THE SEASON BOARD (daily_boards) since the Daily got a
 * yesterday; an open board is one whose closes_at is still ahead. */
async function openDayAnswer() {
  const r = await sql`
    SELECT id, to_char(edition_date,'YYYY-MM-DD') d, season_year, ceiling
      FROM daily_boards WHERE now() < closes_at ORDER BY edition_date LIMIT 1`;
  return r[0] ?? null;
}
...
test('LEAK: a SIGNED-OUT fetch carries no open-board answer', async () => {
  ...
  if (open) {
    ...
    assert.equal(sealed.perfect, undefined, 'nor a ceiling');
    assert.equal(sealed.top, undefined, 'nor anybody\'s score');
    assert.equal(sealed.href, undefined, 'nor a link to results that do not exist yet');
    assert.equal(wire.includes(String(open.ceiling)), false, 'the open board\'s ceiling is nowhere on the wire');
  }
...
test('LEAK: a SIGNED-IN fetch carries the reader\'s own state and nobody else\'s open-day result', async () => {
  const u = await sql`SELECT user_id FROM daily_board_runs WHERE picks IS NOT NULL ORDER BY id DESC LIMIT 1`;
...
test('LEAK: no OTHER player\'s open-day score appears for any viewer', async () => {
  ...
  const rivals = await sql`
    SELECT r.user_id, r.score FROM daily_board_runs r
     WHERE r.board_id = ${open.id} AND r.picks IS NOT NULL AND r.score IS NOT NULL`;
