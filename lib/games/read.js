...
async function todaysBoardsRows({ uid, dailyHome, pickem, weeklyHome, draftHome, live, playingToday, now }) {
  ...
  const [nextWeekly, nextDraft, nextPickem] = await Promise.all([
    weeklyContest ? null : nextContest({ now }).catch(() => null),
    draftContest ? null : nextDraftContest({ now }).catch(() => null),
    pickem ? null : sql`
      SELECT opens_at FROM contests WHERE game_type = 'pickem' AND opens_at > ${now.toISOString()}
       ORDER BY opens_at ASC LIMIT 1`.then((r) => r[0] ?? null).catch(() => null),
  ]);
  ...
  // ---- Pick'em ----------------------------------------------------------------
  const pk = (() => {
    if (!pickem) return { line1: 'no board yet', line2: null, pill: preOpenPill(nextPickem?.opens_at), tile: '' };
