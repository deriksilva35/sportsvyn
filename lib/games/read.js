...
import { cardState, seasonStrip, boardSection, GAME_ORDER, GAME_META, HERO_TAGLINE, HERO_CTA, GAME_GLYPHS } from './lobby.js';
import { currentContest, getWeeklyHome } from '../weekly/entries.js';
import { weeklyBoardTable } from '../weekly/live.js';
import { getDraftHome } from '../draft/entry.js';
import { DRAFT_CONFIG, DRAFT_ROUNDS } from '../draft/contest.js';
 ...
 */
async function todaysBoardsRows({ uid, dailyHome, pickem, weeklyHome, draftHome, live, playingToday, now }) {
  ...
  // ---- Pick'em ----------------------------------------------------------------
  const pk = (() => {
    if (!pickem) return { line1: 'no board yet', line2: null, pill: { label: 'Locked', tone: 'muted' }, tile: '' };
    const line1 = `Board ${pickem.boardNumber} · ${String(pickem.sport ?? '').toUpperCase()} Week ${pickem.week} · ${pickem.total} games`;
    if (uid == null) return { line1, line2: `${pickem.total} games`, locksAt: pickem.firstKickoff, locksPre: 'first lock ', pill: null, tile: '' };
    return {
  ...
  // ---- The Weekly ----------------------------------------------------------------
  const wk = (() => {
    if (!weeklyContest) return { line1: 'opens with NFL Week 1', line2: null, pill: { label: 'Locked', tone: 'muted' }, tile: '' };
    const week = weeklyContest.week;
    const line1 = `Week ${week} · six from ${poolSize != null ? poolSize.toLocaleString('en-US') : '-'} · PPR`;
    if (uid == null) return { line1, locksAt: weeklyContest.locks_at, locksPre: 'locks ', pill: null, tile: '' };
  ...
  const dr = (() => {
    ...
    if (!draftContest) return { line1: 'opens with NFL Week 1', line2: null, pill: { label: 'Locked', tone: 'muted' }, tile: '' };
    const week = draftContest.week;
    const line1 = `Week ${week} · ${teamsCount} seats · ${DRAFT_ROUNDS} rounds · ${clockSeconds}s`;
    if (uid == null) return { line1, locksAt: draftContest.locks_at, locksPre: 'rooms lock ', pill: null, tile: '' };
  ...
  return [
    ...
    { key: 'pickem', glyph: GAME_GLYPHS.pickem, name: GAME_META.pickem.name, href: GAME_META.pickem.href, ...pk },
...
export async function gamesLobby(userId = null, { now = new Date() } = {}) {
  ...
  const [live, dailyHome, yesterday, table, streak, me, pickem, playingToday, bestStreak, boardsPlayed, ofBest, nflWeek, weeklyHome, draftHome] = await Promise.all([
    liveContests({ now }),
  ...
  // THE HERO (relay 2a item 3) - signed-out readers get none (the hero is
  // about THIS viewer's own unmet lock, and a stranger has none to name).
  let hero = null;
  if (uid != null) {
    ...
    const pickemEntered = Boolean(pickem?.entered);
    const allFourEntered = dailyHome != null && weeklyHome != null && draftHome != null && pickem != null
    ...
    if (allFourEntered) {
      ...
      hero = appointmentHero({ live, weeklyHome, draftHome, pickem, now });
    }
  }

  // TODAY'S BOARDS - four rows, relay 2a item 4.
  const boardRows = await todaysBoardsRows({
    uid, dailyHome, pickem, weeklyHome, draftHome, live, playingToday, now,
  }).catch(() => []);
  ...
  if (uid != null) {
    ...
    const pkRow = !pkMine
      ? { label: 'Pick’em', middle: `needs ${PICKEM_TABLE_MIN_BOARDS} boards · correct % across every board`, rank: null }
      : pkMine.rank == null
        ? { label: 'Pick’em', middle: `${pkMine.pct}% · ${pkMine.boardsPlayed} of ${PICKEM_TABLE_MIN_BOARDS} needed`, rank: null }
        : { label: 'Pick’em', middle: `${pkMine.pct}% · ${pkMine.correct} of ${pkMine.played} correct`, rank: pkMine.rank };
