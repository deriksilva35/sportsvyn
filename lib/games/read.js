  ...
  if (uid != null) {
    ...
    if (!hero) {
      ...
      if (weeklyHome?.state === 'settled' && weeklyHome.played && weeklyHome.pct != null) {
        graded.push({ label: `Week ${weeklyHome.week} · Weekly`, pct: weeklyHome.pct });
      }
      if (draftHome?.state === 'settled' && draftHome.played && draftHome.pct != null) {
        graded.push({ label: `Week ${draftHome.week} · Draft`, pct: draftHome.pct });
      }
      for (const p of pickemSports) {
        if (p.settled && p.record) {
          graded.push({
            label: `Board ${p.boardNumber} · Pick'em`,
            pct: Math.round((p.record.correct / p.record.played) * 1000) / 10,
          });
        }
      }
      if (graded.length) {
        const avgPct = Math.round((graded.reduce((a, g) => a + g.pct, 0) / graded.length) * 10) / 10;
        hero = {
          key: 'settled-recap',
