...
import { dailyRowFor } from './dailyRow.js';
...
export async function pickemTable(uid = null, { limit = 10, sport = null } = {}) {
  ...
  const entries = await sql`
    SELECT e.contest_id, e.user_id, e.lineup, u.handle, u.is_house
  ...
  for (const e of entries) {
    ...
    if (!byUser.has(e.user_id)) byUser.set(e.user_id, { handle: e.handle, isHouse: e.is_house, correct: 0, played: 0, boardsPlayed: 0 });
  ...
  const rows = [...byUser.entries()]
    .map(([userId, u]) => ({
      ...
      ...houseMark({ isHouse: u.isHouse === true, handle: u.handle }, 'pickem'),
    }))
 ...
 */
// Exported for lib/you/reads.js (YOU TAB v1): the You tab needs the caller's
// own Weekly and Draft rows, which is the same table the lobby's leaderboard
// pane draws.
export async function gameSeasonTable(gameType, uid = null, { limit = 10 } = {}) {
  ...
  const entries = await sql`
    SELECT ce.user_id, ce.contest_id, u.handle, u.is_house, (ce.meta->>'pct')::numeric AS pct
  ...
  for (const e of entries) {
    if (!byUser.has(e.user_id)) byUser.set(e.user_id, { handle: e.handle, isHouse: e.is_house, sum: 0, weeksPlayed: 0 });
    const u = byUser.get(e.user_id);
  ...
  const rows = [...byUser.entries()]
    .map(([userId, u]) => ({
      ...
      // gameSeasonTable is called with 'weekly' and 'draft'; the method line
      // is the one for the game whose table this is.
      ...houseMark({ isHouse: u.isHouse === true, handle: u.handle }, gameType),
    }))
 ...
 */
async function todaysBoardsRows({ uid, dailyHome, dailyV2 = null, yesterday = null, pickemNfl, pickemCfb, weeklyHome, draftHome, live, playingToday, now }) {
  ...
  // ---- Daily ----------------------------------------------------------------
  // FROM THE v2 BOARD AND RUN (FRESH-USER FIXES, D9) - see lib/games/dailyRow.js.
  const daily = dailyRowFor({
    board: dailyV2?.board ?? null, run: dailyV2?.run ?? null, uid,
    playingToday: dailyV2?.playingToday ?? 0, edition: dailyHome?.edition ?? null,
  });
...
export async function gamesLobby(userId = null, { now = new Date(), pickemSeasonSport = null } = {}) {
  ...
  const [live, dailyHome, yesterday, table, streak, me, pickemNfl, pickemCfb, playingToday, bestStreak, boardsPlayed, ofBest, nflWeek, weeklyHome, draftHome, dailyV2] = await Promise.all([
    liveContests({ now }),
  ...
  const boardRows = await todaysBoardsRows({ yesterday,
    uid, dailyHome, dailyV2, pickemNfl, pickemCfb, weeklyHome, draftHome, live, playingToday, now,
  }).catch(() => []);
...
async function yesterdayV2(uid) {
  ...
  const grade = run?.picks != null ? regradeStoredRun(board, run.picks).grade ?? null : null;
...
async function historyV2(uid) {
  ...
  return boards.map((b) => {
    ...
    const grade = b.closed && run?.picks != null ? regradeStoredRun(b, run.picks).grade ?? null : null;
    const you = uid == null ? undefined : youCellV2(run, grade);
