...
export function nowCard(v = {}, { now = new Date() } = {}) {
  ...
  //
  // 'signed-out' IS AN OPEN BOARD, and leaving it out put "Nothing open" on
  // the marquee card of the page a first-time visitor lands on - while the
  // Daily was open and closing at 9. lobbyV2 hands a stranger state
  // 'signed-out' for both the Daily and the Weekly (there is no run to have
  // a state ABOUT), and the fall-through took that all the way to the
  // nothing branch. Caught by serving the branch signed out, not by a
  // fixture: every fixture I wrote had a reader in it.
  //
  // THE CARD STILL DOES NOT LIE TO THEM. A stranger has no streak line and
  // no season, so the line is the board's shape alone, and the screen swaps
  // the CTA to Sign in (components/games/LobbyV3.js) because the href is
  // gated there, in one place, for the card and all four rows at once.
  if (daily && (daily.state === 'play' || daily.state === 'in-progress' || daily.state === 'signed-out')) {
    return {
