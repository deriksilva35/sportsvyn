...
export function pickemRow({ nfl = null, cfb = null, uid = null } = {}) {
  ...
  // `tiles` is the SPLIT (LOBBY PICK'EM SPLIT relay). The aggregate above is
  // kept because the lobby intro still counts the open games off `pill` and
  // `firstLock`, and because a single live board renders in the old
  // full-width shape, which needs the old `lines` and `href`.
  return {
    key: 'pickem', title: "Pick'em", glyph: '✓', href: `/pickem/${pickemHrefSport(candidates)}`,
    lines, pill, firstLock, tiles: pickemTiles({ nfl, cfb, uid }),
  };
}

/**
 * ONE TILE PER BOARD, each stating what is true of ITS board alone.
 *
 * The single row said "NFL ... / CFB ..." on two lines under one pill, so a
 * finished NFL board and an open CFB board shared a single state - the pill
 * could only be right about one of them. Each tile now carries its own count,
 * its own lock and its own pill.
 *
 * A board that does not exist gets NO TILE (relay item 2). Two tiles sit side
 * by side; one renders full width in the old shape; none omits the section.
 */
export function pickemTiles({ nfl = null, cfb = null, uid = null } = {}) {
  const tiles = [];
  for (const [label, p] of [['NFL', nfl], ['CFB', cfb]]) {
    if (!p) continue;
    const sport = p.sport ?? label.toLowerCase();
    ...
    const picked = Number(p.picked ?? pickedOpen);
    const at = p.nextKickoff ?? null;
    const lines = []; let pill = null;
    if (p.settled) {
      lines.push({ text: `Settled${p.record ? ` · ${p.record.correct} of ${p.record.played}` : ''}`, at: null });
      pill = { label: 'Locked', tone: 'muted' };
    } else if (pickable === 0) {
      lines.push({ text: 'All games kicked · grading', at: null });
      pill = { label: 'Locked', tone: 'muted' };
    } else {
      const kicked = total - pickable;
      if (uid == null) {
        lines.push({ text: `${total} game${total === 1 ? '' : 's'}`, at: null });
      } else {
        lines.push({ text: `${picked} of ${total}${kicked > 0 ? ` · ${pickable} still open` : ''}`, at: null });
        pill = pickable - pickedOpen > 0 ? { label: 'Pick', tone: 'volt' } : { label: 'Done', tone: 'jade' };
      }
      lines.push({ text: `${kicked > 0 ? 'next' : 'first'} lock `, at });
    }
    // THE DISC IS THE GAME'S MARK, not the board's name - the name is the
    // label right under it, and a 24px disc reading "NFL" beside a heading
    // reading "NFL" says one thing twice.
    tiles.push({ key: `pickem-${sport}`, sport, label, glyph: '✓', href: `/pickem/${sport}`, lines, pill });
  }
  return tiles;
}
