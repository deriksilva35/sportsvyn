...
import { dailyCard, weeklyCard, pickemRow, pickemTiles, pickemHrefSport, draftRow, introLine, tonightTitle, numberWord, timeLeft, DASH } from './lobbyV2Shape.js';
...
test("Pick'em: the line states the whole board, the row goes where the picks are, the pill says PICK only when something is open", () => {
  // 24-game CFB board, fifteen already kicked, nothing picked; 16-game NFL
  // board, all sixteen picked, two kicked.
  ...
  assert.deepEqual(r.lines.map((l) => l.text), [
    'NFL 16 of 16 · 14 still open · next lock ',
    'CFB 0 of 24 · 9 still open · next lock ',
  ], 'the whole board, not a shrinking denominator');
  assert.equal(r.lines[1].at, cfb.nextKickoff);
  // NFL complete, CFB open -> the row opens CFB
  assert.equal(r.href, '/pickem/cfb');
  assert.deepEqual(r.pill, { label: 'Pick', tone: 'volt' }, 'something is open');
  assert.equal(r.firstLock, cfb.nextKickoff, 'the soonest of the two');

  // an UNTOUCHED board, nothing kicked yet: "first lock", no "still open"
  const fresh = { sport: 'cfb', total: 24, pickable: 24, pickedOpen: 0, picked: 0, nextKickoff: '2026-09-12T16:00:00Z' };
  assert.equal(pickemRow({ nfl: null, cfb: fresh, uid: 1 }).lines[1].text, 'CFB 0 of 24 · first lock ');
  // and a partly picked untouched board keeps the true numerator
  assert.equal(pickemRow({ nfl: null, cfb: { ...fresh, pickedOpen: 5, picked: 5 }, uid: 1 }).lines[1].text, 'CFB 5 of 24 · first lock ');

  // BOTH COMPLETE -> DONE, and the row opens the soonest-locking board
  const bothDone = pickemRow({ nfl, cfb: { ...cfb, pickedOpen: 9, picked: 24 }, uid: 1 });
  assert.deepEqual(bothDone.pill, { label: 'Done', tone: 'jade' });
  assert.equal(bothDone.href, '/pickem/cfb', 'CFB locks first');
  assert.equal(pickemRow({ nfl: { ...nfl, nextKickoff: '2026-09-12T20:00:00Z' }, cfb: { ...cfb, pickedOpen: 9, picked: 24 }, uid: 1 }).href, '/pickem/nfl');

  // BOTH OPEN -> the soonest next kickoff wins
  assert.equal(pickemRow({ nfl: { ...nfl, pickedOpen: 0, picked: 0 }, cfb, uid: 1 }).href, '/pickem/cfb');
  assert.equal(pickemRow({ nfl: { ...nfl, pickedOpen: 0, picked: 0, nextKickoff: '2026-09-12T18:00:00Z' }, cfb, uid: 1 }).href, '/pickem/nfl');

  // ONE BOARD ONLY -> that board, even when it is finished
  assert.equal(pickemRow({ nfl: null, cfb, uid: 1 }).href, '/pickem/cfb');
  assert.equal(pickemRow({ nfl: { ...nfl, pickedOpen: 14 }, cfb: null, uid: 1 }).href, '/pickem/nfl');

  // every row kicked -> grading, and the pill locks
  const done = pickemRow({ nfl: { ...nfl, pickable: 0, pickedOpen: 0 }, cfb: null, uid: 1 });
  assert.equal(done.lines[0].text, 'NFL all games kicked · grading');
  assert.deepEqual(done.pill, { label: 'Locked', tone: 'muted' });
  assert.equal(pickemRow({ nfl: null, cfb: null, uid: 1 }).lines[0].text, 'NFL no board yet');

  // signed out: the board's size and its next lock, no pill, same routing
  const out = pickemRow({ nfl, cfb, uid: null });
  assert.deepEqual(out.lines.map((l) => l.text), ['NFL 16 games · next lock ', 'CFB 24 games · next lock ']);
  assert.equal(out.pill, null); assert.equal(out.href, '/pickem/cfb');
});

test('pickemHrefSport: needs-picks first, then the soonest lock', () => {
  const A = { sport: 'nfl', open: 0, at: '2026-09-13T17:00:00Z' };
  const B = { sport: 'cfb', open: 9, at: '2026-09-12T23:00:00Z' };
  assert.equal(pickemHrefSport([A, B]), 'cfb', 'the one that needs picks');
  assert.equal(pickemHrefSport([{ ...A, open: 3 }, B]), 'cfb', 'both need picks -> soonest');
  assert.equal(pickemHrefSport([{ ...A, open: 3, at: '2026-09-12T18:00:00Z' }, B]), 'nfl');
  assert.equal(pickemHrefSport([A, { ...B, open: 0 }]), 'cfb', 'neither needs picks -> soonest lock');
  assert.equal(pickemHrefSport([B]), 'cfb'); assert.equal(pickemHrefSport([]), 'nfl', 'a safe default, never a blank href');
  assert.equal(pickemHrefSport([{ sport: 'nfl', open: 0, at: null }, { sport: 'cfb', open: 0, at: '2026-09-12T23:00:00Z' }]), 'cfb', 'a board with nothing left to lock sorts last');
});
...
// ---------------------------------------------------------------------------
// THE PICK'EM SPLIT (LOBBY PICK'EM SPLIT relay). One tile per board, each one
// stating what is true of ITS board alone.
// ---------------------------------------------------------------------------
const CFB = { sport: 'cfb', total: 24, pickable: 9, pickedOpen: 0, picked: 0, nextKickoff: '2026-09-12T23:00:00Z' };
const NFL = { sport: 'nfl', total: 16, pickable: 14, pickedOpen: 14, picked: 16, nextKickoff: '2026-09-13T17:00:00Z' };

test('pickemTiles: two boards, two tiles, each with its own href, counts and lock', () => {
  const t = pickemTiles({ nfl: NFL, cfb: CFB, uid: 1 });
  assert.equal(t.length, 2);
  assert.deepEqual(t.map((x) => x.label), ['NFL', 'CFB'], 'NFL leads, CFB follows');
  assert.deepEqual(t.map((x) => x.href), ['/pickem/nfl', '/pickem/cfb']);
  assert.deepEqual(t[0].lines.map((l) => l.text), ['16 of 16 · 14 still open', 'next lock ']);
  assert.deepEqual(t[1].lines.map((l) => l.text), ['0 of 24 · 9 still open', 'next lock ']);
  assert.equal(t[0].lines[1].at, NFL.nextKickoff, 'each tile carries its OWN lock');
  assert.equal(t[1].lines[1].at, CFB.nextKickoff);
});

test('pickemTiles: the pill is per board - NFL done and CFB open is Done and Pick, not one pill', () => {
  const t = pickemTiles({ nfl: NFL, cfb: CFB, uid: 1 });
  assert.deepEqual(t.map((x) => x.pill.label), ['Done', 'Pick'], 'the two boards disagree, and both are stated');
  assert.equal(t[0].pill.tone, 'jade'); assert.equal(t[1].pill.tone, 'volt');
  // and the single aggregate row could only ever say one of them
  assert.equal(pickemRow({ nfl: NFL, cfb: CFB, uid: 1 }).pill.label, 'Pick', 'which is the bug this split fixes');
});

test('pickemTiles: a board with nothing left to pick reads Locked, settled reads Settled', () => {
  const kicked = pickemTiles({ nfl: { ...NFL, pickable: 0, pickedOpen: 0 }, cfb: null, uid: 1 });
  assert.equal(kicked[0].pill.label, 'Locked');
  assert.deepEqual(kicked[0].lines.map((l) => l.text), ['All games kicked · grading']);
  const done = pickemTiles({ nfl: null, cfb: { ...CFB, settled: true, record: { correct: 17, played: 24 } }, uid: 1 });
  assert.equal(done[0].pill.label, 'Locked');
  assert.deepEqual(done[0].lines.map((l) => l.text), ['Settled · 17 of 24']);
});

test('pickemTiles: only boards that exist get a tile, and none means none', () => {
  assert.deepEqual(pickemTiles({ nfl: NFL, cfb: null, uid: 1 }).map((x) => x.sport), ['nfl']);
  assert.deepEqual(pickemTiles({ nfl: null, cfb: CFB, uid: 1 }).map((x) => x.sport), ['cfb']);
  assert.deepEqual(pickemTiles({ nfl: null, cfb: null, uid: 1 }), [], 'no board, no tiles, no section');
});

test('pickemTiles: signed out states the size of the board and offers no pill', () => {
  const t = pickemTiles({ nfl: NFL, cfb: CFB, uid: null });
  assert.deepEqual(t.map((x) => x.pill), [null, null], 'a stranger has no state to report');
  assert.deepEqual(t[0].lines.map((l) => l.text), ['16 games', 'next lock ']);
  assert.deepEqual(t[1].lines.map((l) => l.text), ['24 games', 'next lock ']);
});

test('pickemTiles: a board with nothing kicked yet says first lock, not next lock', () => {
  const fresh = { sport: 'cfb', total: 24, pickable: 24, pickedOpen: 0, picked: 0, nextKickoff: '2026-09-12T23:00:00Z' };
  assert.equal(pickemTiles({ nfl: null, cfb: fresh, uid: 1 })[0].lines[1].text, 'first lock ');
});

test("pickemRow still carries the aggregate, because the intro counts off it", () => {
  const r = pickemRow({ nfl: NFL, cfb: CFB, uid: 1 });
  assert.equal(r.pill.label, 'Pick'); assert.equal(r.firstLock, CFB.nextKickoff);
  assert.equal(r.tiles.length, 2, 'and it carries the split alongside');
});
