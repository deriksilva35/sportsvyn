...
export function dailyRowFor({ board = null, run = null, uid = null, playingToday = 0, edition = null } = {}) {
  ...
  if (run?.completedAt) {
    // THE ONE FORMATTER, off the stored ratio (score/ceiling), so a board that
    // fell eight points short never reads "100% of best" on this row either.
    const pct = pctOfCeiling(run.pct, 1);
