...
const GRADED_HREF = { weekly: '/weekly', pickem: '/pickem/nfl', draft: '/draft' };
...
async function gradedWeekV3(uid) {
  ...
  const ranked = await sql`
    SELECT c.game_type, c.sport, c.perfect, e.user_id, e.score, e.lineup, e.meta,
  ...
  for (const type of ['weekly', 'pickem', 'draft']) {
    ...
    if (type === 'pickem') {
      ...
      rows.push({ key: type, mark: GRADED_MARK[type], name: GRADED_NAME[type],
        ...
        value: `${correct}-${played - correct}`, href: GRADED_HREF[type] });
      continue;
    }
    const r = got[0];
...
/** "4:10 PM" in ET, the one zone every lock time on this site is stated in. */
...
export function octoberRowV3(contest, now = new Date()) {
  ...
  return {
    ...
    rightLabel: preview ? null : (lock ? ET_TIME.format(lock) : null),
...
export function runRowV3(contest, now = new Date()) {
  ...
  return {
    ...
    line: open && first
      ? `Locks ${ET_TIME.format(first)} · ${ET_DOW.format(first)}`
      : `${label} · locked`,
    right: preview ? 'PREVIEW' : null,
    rightLabel: preview ? null : (open && first ? ET_DOW.format(first) : null),
    tone: open ? 'live' : 'done',
  };
}
...
function resultsShape(days, graded) {
  ...
  return {
    ...
    gradedWeek: [
      ...(graded.rows ?? []),
      ...(days.length ? [{
        key: 'daily', mark: 'D', name: 'The Daily', href: '/daily',
