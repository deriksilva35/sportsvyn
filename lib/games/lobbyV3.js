...
export async function mlbRowsV3(uid, now = new Date()) {
  ...
  const next = run
    ? await (async () => (await import('../run/entry.js')).nextRunLock(run, { now }))().catch(() => undefined)
    : undefined;
  return [octoberRowV3(oct, now), runRowV3(run, now, next)];
}
...
/**
 * @param next  the round's next CLUB lock (lib/run/rules.js nextClubLock), or
 *   null when every club has started. The lock is per club, so the round's
 *   own first pitch says nothing about whether it is still open - a row that
 *   read meta.firstPitch called a round "locked" while most of its clubs were
 *   still pickable. Undefined (not read) falls back to that first pitch.
 */
export function runRowV3(contest, now = new Date(), next = undefined) {
  const base = { key: 'run', mark: 'R', name: 'The Run · nine a round', href: '/run' };
  if (!contest) return { ...base, line: 'Opens with the bracket', tone: 'muted' };
  const preview = contest.meta?.preview === true;
  const firstPitch = contest.meta?.firstPitch ? new Date(contest.meta.firstPitch) : null;
  const first = next === undefined
    ? (firstPitch && firstPitch.getTime() > new Date(now).getTime() ? firstPitch : null)
    : (next?.kickoffAt ? new Date(next.kickoffAt) : null);
  const open = first != null;
  const label = contest.meta?.label ?? 'Round';
  return {
    ...base,
    // A ROUND WITH A CLUB STILL AHEAD NAMES THE NEXT LOCK; one whose every
    // club has started names the round it is.
    line: open
      ? `Next lock ${PT_TIME.format(first)} PT · ${PT_DOW.format(first)}`
      : `${label} · locked`,
    right: preview ? 'PREVIEW' : null,
