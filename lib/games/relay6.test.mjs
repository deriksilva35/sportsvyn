...
test('the caption copy is untouched, bar the percentage it now formats', () => {
  // The item said to leave the caption alone, and its SHAPE is unchanged:
  // edition/practice, year, then points, percentage and clock. What changed is
  // where the percentage comes from - pctOfCeiling, the one formatter, which
  // prints one decimal and OMITS itself entirely when there is no ceiling.
  // The old `{grade.pct}%` is what made a 2363.1-of-2371.1 board read "100%".
  assert.match(BOARD, /\{ranked \? edition : 'Practice'\} · \{year\}/);
  assert.match(BOARD, /\{grade\.mine\.toLocaleString\(\)\} pts\{pctLabel \? ` · \$\{pctLabel\}` : ''\} · \{clockLabel\}/);
  assert.doesNotMatch(BOARD, /\{grade\.pct\}%/, 'no surface prints the whole-number pct any more');
});

// ------------------------------------------------- 3: /games for a stranger

// GAMES v3: the pane is components/games/LobbyV3.js now. The stranger block
// and both explainer entrances moved with it deliberately - the mock draws
// neither, and relay 6's decision outranks a layout mock on a surface the
// launch email links to.
const GAMES = src('app/games/page.js') + '\n' + src('components/games/LobbyV3.js');
...
test('the stranger lines are verbatim and sit under the hero', () => {
  ...
  // GAMES v3: Read the game and the pane links are both gone from this
  // screen, so the block's neighbours are new - it sits after the week's
  // foot line and before the always-rendered explainer ghost.
  const foot = GAMES.indexOf('className="gv-foot">{v.foot}');
  const lines = GAMES.indexOf('lob-stranger');
  const more = GAMES.indexOf('className="gv-more"');
  assert.ok(foot > -1 && foot < lines, "under the week's foot line");
  assert.ok(lines < more, 'and above the explainer ghost');
});
...
test('they are SIGNED OUT only, and the existing ghost link is untouched', () => {
  ...
  // The always-rendered link stays for everyone - this is a second entrance,
  // not a replacement. On v3 it is the .gv-more ghost.'
...
test('both explainer links point where the email points', () => {
  ...
  assert.equal((GAMES.match(/href="\/games\/how-it-works"/g) ?? []).length, 2,
    'two entrances to the one explainer: the stranger block, and the foot ghost');
});
