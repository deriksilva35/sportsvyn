...
test('the stranger lines are verbatim and sit under the hero', () => {
  ...
  // GAMES TAB v2: the hero and Today's boards are gone; the stranger block
  // sits after Read the game and before the pane links, signed out only.
  const read = GAMES.indexOf('data-section="read"');
  const lines = GAMES.indexOf('lob-stranger');
  const more = GAMES.indexOf('className="lv-more"');
  assert.ok(read > -1 && read < lines, 'under Read the game');
  assert.ok(lines < more, 'and above the pane links');
});
