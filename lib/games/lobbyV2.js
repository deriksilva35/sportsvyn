...
async function latestGraded(uid) {
  ...
  return {
    ...
    rank: mine ? { rank: mine.rank, of: rows.length } : null,
  };
}
...
async function tonight(uid) {
  ...
  const [spreads, picks] = await Promise.all([
  ...
  return rows.map((r) => {
    ...
    return {
      ...
      home: { name: r.home_short ?? r.home_name, abbr: r.home_abbr ?? (r.home_short ?? r.home_name), colors: teamColors(r.home_c1, r.home_c2), score: r.home_score },
      away: { name: r.away_short ?? r.away_name, abbr: r.away_abbr ?? (r.away_short ?? r.away_name), colors: teamColors(r.away_c1, r.away_c2), score: r.away_score },
...
/** The latest published Daily Card (any non-preview article). */
async function latestRead() {
  const [a] = await sql`
    SELECT slug, title, subtitle, coalesce(length(body), 0) AS body_len
      FROM articles WHERE status = 'published' AND type <> 'preview'
...
export async function lobbyV2(userId = null, { now = new Date() } = {}) {
  ...
  const [v2, v1, graded, best, streak, wHome, pkNfl, pkCfb, dHome, dNext, games, read, week, me] = await Promise.all([
  ...
  const intro = introLine({ daily, weekly, pickem, draft });
  return {
