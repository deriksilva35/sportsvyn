...
export function draftRowV3({ state = 'none', onTheClock = false, pick = null, roundsToGo = null,
  ...
  bestSix = null, toPlay = null, rosterSize = null } = {}) {
  ...
  if (bestSix != null) {
    return {
      ...base,
      line: join('roster in', `best six ${n1(bestSix)}`, toPlay != null ? `${toPlay} to play` : null),
      right: String(n1(bestSix)), rightLabel: 'best six', tone: 'live',
    };
  }
  // A ROOM THAT IS DONE AND A WEEK THAT HAS NOT STARTED IS NOT "NOTHING".
  // The reader drafted, the entry is stored, the contest is locked and no
  // game has scored yet - so there is no best six to print and there IS
  // something to say. This state was collapsed into the blank row below and
  // served user 1 a dash on the night his own room finished.
  if (state === 'entered') {
    return {
      ...base,
      line: join('roster in', rosterSize != null ? `${rosterSize} players` : null,
        toPlay != null ? `${toPlay} to play` : null),
      right: null, rightLabel: null, tone: null,
    };
  }
