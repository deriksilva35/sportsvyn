...
/** Which deadline a surface should name right now: the first kickoff while
 *  it is still ahead, the window close (locks_at) after it. */
export function lockPhase({ firstKickoff = null, locksAt = null, now = new Date() } = {}) {
  const first = new Date(firstKickoff ?? NaN).getTime();
  const beforeFirst = Number.isFinite(first) && new Date(now).getTime() < first;
  return { beforeFirst, at: beforeFirst ? firstKickoff : locksAt, label: beforeFirst ? 'first kickoff ' : 'locks ' };
}
