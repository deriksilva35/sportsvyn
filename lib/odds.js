...
export function consensusOdds(books) {
  ...
  // ONE MEDIAN IN THIS FILE, the exported one below. This function used to carry
  // its own identical copy and coerce each side with Number() on the way in; both
  // halves of that are gone. The raw value goes in and median() decides absence.
  const home = median(books.map((b) => b.home));
  const draw = median(books.map((b) => b.draw));
  const away = median(books.map((b) => b.away));
...
export function consensusOdds2Way(books) {
  if (!Array.isArray(books) || books.length === 0) return null;
  // RAW, NOT Number()-ed: the coercion here was the bug - a book with a null
  // price on one side became a 0 and a 0 is finite. median() drops absences.
  const a = median(books.map((x) => x.a));
  const b = median(books.map((x) => x.b));
...
// devigField(decimals): proportional de-vig across a FULL outright field (32-way
// NFL / ~130-way CFB title market). raw_i = 1/decimal_i; pct_i = raw_i / Σraw * 100.
// Returns pcts (same order, null for a bad price) summing to ~100, or null if
// nothing is priceable. Same proportional method as the 2-way path — a large field
// just means a large overround that normalizes out across all outcomes.
export function devigField(decimals) {
  if (!Array.isArray(decimals) || decimals.length === 0) return null;
  const raws = decimals.map((d) => decimalToImplied(Number(d)));
  const sum = raws.reduce((a, r) => a + (r ?? 0), 0);
  if (!(sum > 0)) return null;
  return raws.map((r) => (r == null ? null : (r / sum) * 100));
}
...
// THE ABSENCE FILTER IS numbersOf's (lib/util/scoresOf.js). The old body filtered
// raw values on Number.isFinite, which is correct on its own - null is not finite -
// but every CALLER coerced first (`Number(x.a)`), and Number(null) is 0. A price
// of 0 is finite, so a book with no price on a side entered the median as decimal
// odds of ZERO. numbersOf decides absence BEFORE coercing, so the callers can now
// hand it raw values and a missing price simply is not there.
//
// UNROUNDED, deliberately: scoresOf.js's medianOf rounds to the house one decimal,
// which is right for a score and wrong for a price - 2.82 would become 2.8 and the
// de-vig downstream would be off.
export function median(values) {
  const s = numbersOf(values).sort((a, b) => a - b);
