...
export async function getPlayerBySlug(slug) {
  const rows = await sql`
    SELECT
      ...
      p.metadata,
      -- Bio columns (NULL until the API-Sports /players backfill runs;
      -- the bio grid renders only the fields that come back populated).
      p.birthdate,
      p.nationality,
      p.height_cm,
      p.club_name,
      ...
      -- Gridiron columns (migration 076) + the provider ids. external_ids is
      -- what bridges a rostered NFL player to the 181,182 stat rows, which key
      -- to a different identity table; see lib/gridiron/playerStats.js.
      p.external_ids,
      p.position_group,
      p.weight_kg,
      p.college,
...
export async function getTeamSquad(teamId) {
  ...
  const rows = await sql`
    SELECT slug, full_name, known_as, position, current_team_jersey_number,
           photo_url_source,
           -- Gridiron columns (migration 076). NULL on the 1,248 soccer rows,
           -- which is what lets one reader serve both sports: the soccer render
           -- ignores them and the gridiron render keys off position_group.
           position_group, height_cm, weight_kg, college, experience_years
      FROM players
