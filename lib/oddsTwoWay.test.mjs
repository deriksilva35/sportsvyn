...
// --------------------------------------------------------------- absences
// Number(null) IS 0 AND 0 IS FINITE, and a decimal odd of 0 is not a price. The
// old path coerced each side with Number() on the way into median(), so a book
// carrying the outcome with no price on it entered the consensus at the very
// bottom. See lib/util/scoresOf.js.

test('a book with no price on a side is ABSENT, not a 0.00 price', () => {
  // The trap, stated - so a reader can see why the assertions below matter.
  assert.equal(Number(null), 0);
  assert.equal(Number.isFinite(Number(null)), true);

  assert.equal(median([1.9, null, 2.1]), 2.0, 'null is dropped, not sorted to the front');
  assert.equal(median([null, '']), null);
  assert.equal(median(['1.9', '2.1']), 2.0, 'a numeric string is still a price');

  const books = [
    { a: 1.90, b: 2.00 },
    { a: null, b: 1.95 },   // this book has no price on side a
    { a: 2.00, b: 1.90 },
  ];
  // Side a is the median of TWO prices (1.95), not of three with a 0 in them
  // (which would be 1.90 - and on a two-book market, 0.95).
  assert.deepEqual(consensusOdds2Way(books), { a: 1.95, b: 1.95 });
  // And the shape that used to run here, for the contrast:
  assert.equal(median(books.map((x) => Number(x.a))), 1.9, 'the old answer');
});
