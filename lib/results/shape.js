...
/**
 * THE SCORES OF A LIST OF ENTRIES, and it exists because of ONE trap:
 * Number(null) IS 0 AND 0 IS FINITE. A DNF entry's score is NULL, so a plain
 * `.map(Number).filter(Number.isFinite)` puts it in the field as a ZERO - which
 * pulled a three-entry median from 120.7 to 118.8 the first time this ran, and
 * would have drawn a bar at the bottom of every distribution for every entry
 * that did not play.
 *
 * A DNF IS COUNTED SEPARATELY AND BELONGS IN ITS OWN BAR (see distribution's
 * `dnf`), which is exactly the distinction this filter keeps alive.
 */
...
export function distribution(scores = [], { mine = null, ceiling = null, dnf = 0 } = {}) {
  const vals = (scores ?? []).map(Number).filter(Number.isFinite).sort((a, b) => a - b);
...
/** The median of an already-sorted list of numbers. */
export function medianOf(sorted = []) {
  if (!sorted.length) return null;
  const mid = Math.floor(sorted.length / 2);
  return r1(sorted.length % 2 ? sorted[mid] : (sorted[mid - 1] + sorted[mid]) / 2);
}
