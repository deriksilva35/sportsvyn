...
import { header, distribution, axisFor, lostIt, lastName, scoresOf, noEntryLine } from './shape.js';
...
export async function dailyResults(boardId, userId = null) {
  ...
  // THE PERFECT BOARD WITHOUT A READER. daily_boards.best_roster is the edition's
  // own frozen ceiling - eight cards, each already carrying its slot, points and
  // meta - so a reader with no run still gets the thing they missed. The grader's
  // rows are preferred when there are any, because those carry its verdicts.
  const ceilingRows = rows.length
    ? rows.map((r) => slotRow(r.best, { hit: r.hit }))
    : (board.best_roster ?? []).map((p) => slotRow(p, { hit: false }));
  ...
  return {
    ...
    ceilingWord: 'perfect',
    // A BOARD, NOT A WEEK. The Daily's contest is one edition, so "this week"
    // would be wrong by six days. graded is null when there is no submitted run
    // for this reader - which is also true of an attempt started and abandoned,
    // and that is the honest reading: the board has no result from them.
    played: graded?.ok === true,
    noEntryLine: noEntryLine('board'),
    header: header({
    ...
    slotCount: graded?.ok ? graded.grade.slotCount : ceilingRows.length,
