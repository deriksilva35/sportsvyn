...
before(async () => {
  ...
  // FIVE USERS, AND THE FIFTH NEVER PLAYS ANYTHING. users[4] exists only to be
  // absent: it is the reader the no-entry tests read every contest as, and it has
  // to be a real user id because "signed out" (userId null) and "signed in and
  // did not play" reach the readers by different routes and both must land on the
  // same header.
  for (let i = 0; i < 5; i += 1) {
...
test('THE DAILY reads a real DEV board, and the two columns pair', async () => {
  ...
  assert.equal(v.played, true, 'a reader with a submitted run PLAYED');
...
test('NO ENTRY IS NOT A ZERO: the four readers say so, and still hand over the field', async () => {
  // users[4] has no entry in any of these contests, and the Daily board has no
  // run for it either.
  const absent = users[4].id;
  const cases = [
    ['weekly', await weeklyResults(weeklyId, absent), "You didn't play this week"],
    ['draft', await draftResults(draftId, absent), "You didn't play this draft"],
    ['pickem', await pickemResults(pickemId, absent), "You didn't play this week"],
    ['daily', await dailyResults(DEV_DAILY_BOARD, absent), "You didn't play this board"],
  ];
  for (const [game, v, line] of cases) {
    assert.equal(v.played, false, `${game}: played must be false`);
    assert.equal(v.noEntryLine, line, `${game}: the line names the game's own period`);
    // NOT A ZERO AND NOT A RANK. Every one of these would be a claim about a
    // reader who was not there.
    assert.equal(v.header.score, null, `${game}: no score`);
    assert.equal(v.header.rank, null, `${game}: no rank`);
    assert.equal(v.header.rankLabel, null, `${game}: no ordinal`);
    assert.equal(v.header.topPct, null, `${game}: no "top n%"`);
    assert.equal(v.header.pctOfCeiling, null, `${game}: no percent of the ceiling`);
    // THE CEILING AND THE FIELD SURVIVE - they are the reason this screen is
    // still worth serving to somebody who did not play.
    assert.ok(Number.isFinite(v.header.ceiling), `${game}: the ceiling is still a number`);
    assert.ok(v.field.played >= 1, `${game}: the field still has its entries`);
    assert.ok(v.field.distribution.bars.length >= 12, `${game}: the distribution is still drawn`);
    assert.ok(v.field.rows.length >= 1, `${game}: the board still has rows`);
    // AND NOBODY IS MARKED "you" IN A FIELD THIS READER IS NOT IN.
    assert.equal(v.field.rows.some((r) => r.you), false, `${game}: no row claims to be you`);
    // NO READER MARK ON THE SCALE EITHER: four labels, not five, and no bucket
    // lit as theirs. The fifth label is the reader's own and axisFor used to fall
    // back to the high value there, printing the ceiling twice.
    assert.equal(v.field.axis.length, 4, `${game}: four axis labels`);
    assert.equal(v.field.axis.some((a) => String(a).startsWith('you')), false, `${game}: no "you ·" label`);
    assert.equal(v.field.distribution.bars.some((b) => b.me), false, `${game}: no bucket is theirs`);
    assert.equal(v.field.distribution.myBucket, null, `${game}: and no myBucket`);
  }
  // THE CEILING LINEUP IS STILL THERE for the two games that print one.
  const [, weekly] = cases[0];
  assert.ok(weekly.ceiling.length >= 1, 'the optimal lineup is still rendered');
  assert.equal(weekly.mine.length, 0, 'and there is no lineup of dashes beside it');
  assert.deepEqual(weekly.lostIt, [], 'nothing was lost by somebody who did not play');
  assert.equal(weekly.toCeiling, null, 'and there is no distance to the ceiling');
  // PICK'EM: no record, no hit rate - "0-0" and "0%" were the old answers.
  const [, , pickem] = [null, null, cases[2][1]];
  assert.equal(pickem.record, null, "no 0-0 record");
  assert.equal(pickem.hitRate, null, 'no 0% hit rate');
});

test('SIGNED OUT LANDS ON THE SAME HEADER as signed in and absent', async () => {
  // Two different routes into the readers - userId null skips the entry query
  // altogether, a real id runs it and finds nothing - and one answer.
  const out = await weeklyResults(weeklyId, null);
  const absent = await weeklyResults(weeklyId, users[4].id);
  assert.equal(out.played, false);
  assert.equal(out.noEntryLine, absent.noEntryLine);
  assert.deepEqual(out.header, absent.header);
  assert.equal(out.field.played, absent.field.played);
});

test('AND A REAL ZERO IS NOT AN ABSENCE: an entry that scored nothing still played', async () => {
  // The distinction this whole state exists for, pinned from the other side. A
  // sentinel entry with score 0 is a reader who played badly, and the header must
  // print their zero and their rank rather than "You didn't play".
  const [u] = await sql`
    INSERT INTO users (email, handle) VALUES (${`${NS}-zero@example.invalid`}, ${`${NS}-hz`}) RETURNING id`;
  try {
    await sql`
      INSERT INTO contest_entries (contest_id, user_id, lineup, score, base_score, meta, locked_at, created_at, updated_at)
      VALUES (${weeklyId}, ${u.id}, '{}'::jsonb, 0, 0, ${JSON.stringify({ dnf: false, pct: 0 })}::jsonb,
              now(), now(), now())`;
    const v = await weeklyResults(weeklyId, u.id);
    assert.equal(v.played, true, 'a zero is a score, not an absence');
    assert.equal(v.header.score, 0);
    assert.equal(v.header.pctOfCeiling, 0, '0% of the ceiling is TRUE of somebody who scored 0');
  } finally {
    await sql`DELETE FROM contest_entries WHERE contest_id = ${weeklyId} AND user_id = ${u.id}`;
    await sql`DELETE FROM users WHERE id = ${u.id}`;
    const [{ n }] = await sql`SELECT count(*)::int AS n FROM users WHERE id = ${u.id}`;
    assert.equal(n, 0, 'the zero-score sentinel is removed');
  }
});
