...
export async function weeklyResults(contestId, userId = null) {
  ...
  // THE CEILING WITHOUT A READER. pairRows needs two sides, so with no entry
  // `rows` is empty and the optimal lineup - which is a fact about the SLATE, not
  // about anybody - would vanish with it. It is the reason this screen is still
  // worth serving to somebody who did not play, so it is read straight off the
  // contest's stored perfect lineup instead.
  const ceilingRows = rows.length
    ? rows.map((r) => slotRow(r.best, { hit: r.verdict === 'hit' }))
    : (view.perfectPicks ?? []).map((p) => slotRow(p, { hit: false }));
  ...
  return {
    ...
    // NO ENTRY IS NOT A ZERO SCORE. view.you is null when this reader has no
    // entry in the contest - signed out, or simply did not play - and the header
    // then prints noEntryLine instead of a rank, a score and a 0% bar. The field
    // and the ceiling are still theirs to read; what is absent is them.
    played: view.you != null,
    ...
    slotCount: rows.length || ceilingRows.length,
