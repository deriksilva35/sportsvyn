...
/**
 * LIVE-ELSE-NEXT PER FOLLOWED TEAM, any league.
 *
 ...
 *
 * IT USED TO SAY `WHERE m.kickoff_at >= now`, WHICH IS A BUG (TODAY TAB v2,
 * Q2). A game that kicked off an hour ago and is still being played was
 * filtered out, so a follower watching their team play was shown that team's
 * NEXT game instead of the one on television. The condition is now the same
 * live-else-next the lobby's Tonight strip uses: a live game, or a scheduled
 * one still ahead, with live sorted first WITHIN each team by the ORDER BY.
 *
 * The name kept "Next" because the common answer still is the next game; the
 * live case is the exception it was wrong to drop.
 */
export async function myFollowedTeamNext(userId, { now = new Date(), limit = 6 } = {}) {
  ...
  const rows = await sql.query(
    `SELECT DISTINCT ON (f.team_id)
       ...
       LEFT JOIN teams a ON a.id = m.away_team_id
      WHERE f.user_id = $1
        AND (m.status = 'live' OR (m.status = 'scheduled' AND m.kickoff_at >= $2))
      ORDER BY f.team_id, (m.status = 'live') DESC, m.kickoff_at ASC`,
    [Number(userId), new Date(now).toISOString()],
  );
  // A LIVE GAME LEADS THE LIST, then the soonest kickoff. Sorting on kickoff
  // alone would bury a game in progress behind one that has not started.
  return rows
