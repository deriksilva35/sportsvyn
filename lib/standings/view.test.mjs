...
test('spreadLabel rounds to the nearest half point - a median is not a line', () => {
  // WHY. odds_markets stores a MEDIAN across books (consensus_method), and a
  // median over an even number of books lands on a quarter point. On 8 Sep
  // 2026 /pickem/cfb served "Miami −54.3" (median of four books, -54.25),
  // "Indiana −55.3" (-55.25) and "Houston −50.8" (-50.75). No sportsbook has
  // ever posted a tenth of a point; the number read as a typo. The half point
  // is the unit the market trades in, so it is the unit shown.
  assert.equal(spreadLabel({ spreadHome: -54.25, homeAbbr: 'MIA', awayAbbr: 'FAMU' }), 'MIA −54.5');
  assert.equal(spreadLabel({ spreadHome: -55.25, homeAbbr: 'IND', awayAbbr: 'HOW' }), 'IND −55.5');
  assert.equal(spreadLabel({ spreadHome: -50.75, homeAbbr: 'HOU', awayAbbr: 'SOU' }), 'HOU −51');
  // BOTH SPORTS - the NFL's lines are already half points and must come
  // through completely unchanged, integers still printing without a decimal.
  assert.equal(spreadLabel({ spreadHome: -3.5, homeAbbr: 'SEA', awayAbbr: 'NE' }), 'SEA −3.5');
  assert.equal(spreadLabel({ spreadHome: -3, homeAbbr: 'SEA', awayAbbr: 'NE' }), 'SEA −3');
  assert.equal(spreadLabel({ spreadHome: 9.5, homeAbbr: 'CHI', awayAbbr: 'GB' }), 'GB −9.5');
  // The rounding rides on the magnitude, so the away favourite rounds too.
  assert.equal(spreadLabel({ spreadHome: 6.25, homeAbbr: 'A', awayAbbr: 'B' }), 'B −6.5');
  // A quarter below the half rounds DOWN to it, not up to the whole point.
  assert.equal(spreadLabel({ spreadHome: -6.24, homeAbbr: 'A', awayAbbr: 'B' }), 'A −6');
  assert.equal(spreadLabel({ spreadHome: -6.26, homeAbbr: 'A', awayAbbr: 'B' }), 'A −6.5');
  // Never a tenth, on any input.
  for (const v of [-54.25, -55.25, -50.75, -6.24, -6.26, -0.3, -12.4, -7.7]) {
    const out = spreadLabel({ spreadHome: v, homeAbbr: 'A', awayAbbr: 'B' });
    if (out) assert.doesNotMatch(out, /\.[1234679]/, `${v} rendered a non-half decimal: ${out}`);
  }
});
