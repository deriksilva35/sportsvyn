...
export function spreadLabel({ spreadHome, homeAbbr, awayAbbr }) {
  ...
  // ROUNDED TO THE NEAREST HALF POINT. The stored number is a MEDIAN across
  // books (odds_markets.consensus_method), and a median over an even count
  // lands on a quarter: Miami -54.25 across four books rendered as "−54.3",
  // a number no sportsbook has ever posted and which reads as a typo. The
  // half point is the unit the market actually trades in, so that is the unit
  // shown; the raw value is untouched in the database and in scoring.
  const mag = Math.round(Math.abs(v) * 2) / 2;
