...
test('A LINE ITEM TAKES ITS LEAGUE FROM THE MATCH, not the odds row', () => {
  // odds_markets.league_id is null on every current game-market row, so taking
  // it from there gave all 77 line items a null league and the wire's league
  // join dropped every one of them - written, and rendered nowhere. Caught on
  // the served page, not by any test, which is why there is one now.
  const q = strip(src('lib/wire/lines.js'));
  const select = q.slice(q.indexOf('SELECT o.match_id'), q.indexOf('FROM odds_markets'));
  assert.match(select, /m\.league_id/, 'the match knows its league');
  assert.doesNotMatch(select, /o\.league_id/, 'the odds row does not');
});

test('THE WIRE SORTS ON THE SAME CLOCK IT GROUPS BY', () => {
  // Day headers group on published_at. Ordering by seen_at made them jump and
  // repeat down one page - "Aug 25, Aug 29, Aug 30, Jul 31" - because a
  // backfill saw three weeks of club items in the same second.
  const r = strip(src('lib/wire/read.js'));
  assert.match(r, /ORDER BY COALESCE\(n\.published_at, n\.seen_at\) DESC/);
  const page = strip(src('components/wire/WirePage.js'));
  assert.match(page, /dayKey\(it\.published_at \?\? it\.seen_at\)/,
    'the grouping key must be the sort key');
});

test('THE REGISTRY PROMPT IS COERCED - postgres numeric is a string', () => {
  // temperature is numeric(3,2), so the driver hands over "0.40" and the API
  // rejects a string outright. Every take in the first live tick failed 400 on
  // it, and the sentinel had passed because the fallback's 0.4 is a JS number -
  // the registry path was the one thing the sentinel could not exercise.
  const code = strip(src('lib/wire/take.js'));
  const fn = code.slice(code.indexOf('export async function takePrompt'),
    code.indexOf('export async function generateTake'));
  assert.match(fn, /Number\(r\.temperature\)/);
  assert.match(fn, /Number\(r\.max_tokens\)/);
  assert.doesNotMatch(fn, /return r;/, 'the row must not be handed on unconverted');
  // and the error carries the provider's message rather than 80 truncated chars
  assert.match(code, /"message"\\s\*:\\s\*"\(\[\^"\]\+\)"/);
});

test('"UPDATED N AGO" READS THE SORT CLOCK, not the sighting clock', () => {
  // It read seen_at while the list sorted on published_at, so a wire whose
  // newest item was minutes old announced "updated 9 h ago" - the backfill's
  // sighting time. The same two-clock mistake as the day headers.
  const r = strip(src('lib/wire/read.js'));
  const fn = r.slice(r.indexOf('export async function wireTeaser'), r.indexOf('export function updatedLabel'));
  assert.match(fn, /top\.published_at \?\? top\.seen_at/);
  assert.doesNotMatch(fn, /items\[0\]\?\.seen_at/);
  // and the over-read is sized to the rejection rate rather than guessed
  assert.match(r, /LIMIT \$\{limit \* 12\}/);
});

test('THE TAKE RIDER IS BOUNDED BY THE CLOCK, not just by a count', () => {
  // Eight model calls at a few seconds each ran the whole tick past its limit:
  // the 15:52 run was killed with finished_at null, so the ledger could not
  // even record why it died. A count alone is not a budget when each item
  // costs an unpredictable network round trip.
  const route = strip(src('app/api/cron/wire/route.js'));
  assert.match(route, /TAKE_BUDGET_MS/);
  assert.match(route, /Date\.now\(\) > takeDeadline/);
  assert.match(route, /takes\.skipped/, 'a skipped item is reported, not silently dropped');
  // and the per-tick count is small enough that the budget is a backstop
  assert.match(route, /takeCandidates\(\{ limit: 4 \}\)/);
});

test('A TAKE IS ONLY ATTEMPTED ON AN ITEM THAT WILL RENDER', () => {
  // Three of the first nine takes landed on club items the allowlist drops - a
  // phone-in radio show among them. A model call spent on a page that does not
  // exist.
  const code = strip(src('lib/wire/take.js'));
  const fn = code.slice(code.indexOf('export async function takeCandidates'));
  // THE RENDER PATH'S OWN PREDICATE, imported. A second copy would drift from
  // the first the moment either is tuned, and then the generator and the page
  // would disagree about what the wire is.
  assert.match(fn, /const \{ renderable \} = await import\('\.\/read\.js'\)/);
  assert.match(fn, /renderable\(rows\)\.slice\(0, limit\)/);
  assert.doesNotMatch(fn, /CLUB_ALLOW|clubAllowed\(/, 'no second copy of the rule');
  // and it over-reads, or the filter would return a short list
  assert.match(fn, /LIMIT \$\{limit \* 8\}/);
});

test('LANE 1 AND INJURIES ARE ALWAYS ELIGIBLE - only club is filtered', () => {
  // renderable() is the single predicate, and it only inspects club rows.
  const rows = [
    { lane: 'line', headline: 'anything at all' },
    { lane: 'final', headline: 'anything at all' },
    { lane: 'milestone', headline: 'anything at all' },
    { lane: 'poll', headline: 'anything at all' },
    { lane: 'contest', headline: 'anything at all' },
    { lane: 'record', headline: 'anything at all' },
    { lane: 'injury', headline: 'Brian Branch, S, DET · PUP-P' },
    { lane: 'club', headline: 'Big Blue Kickoff Live | Call (201) 939-4513' },
    { lane: 'club', headline: 'Giants sign LB Bobby Okereke to an extension' },
  ];
  const kept = renderable(rows);
  assert.equal(kept.length, 8, 'every lane-1 row and the injury survive');
  assert.equal(kept.some((r) => r.headline.includes('Kickoff Live')), false,
    'the radio show is the one that goes');
  assert.equal(kept.some((r) => r.headline.includes('Okereke')), true);
});
