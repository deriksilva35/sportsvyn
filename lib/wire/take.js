...
export async function takePrompt() {
  try {
    ...
    if (r?.system_prompt) {
      // POSTGRES NUMERIC COMES BACK AS A STRING. temperature is numeric(3,2),
      // so the driver hands over "0.40" and the API rejects a string outright -
      // every take in the first live tick failed 400 on it, while the sentinel
      // passed because the fallback's 0.4 is a JS number. Coerced here, at the
      // boundary, rather than trusted anywhere downstream.
      return {
        system_prompt: r.system_prompt,
        model: r.model ?? MODEL,
        max_tokens: r.max_tokens == null ? MAX_TOKENS : Number(r.max_tokens),
        temperature: r.temperature == null ? 0.4 : Number(r.temperature),
      };
    }
  } catch { /* fall through */ }
...
export async function generateTake(item, env, { prompt } = {}) {
  ...
  try {
    ...
    // The whole message, trimmed of its JSON envelope: a reason truncated at
    // "temperatu" cost a tick to diagnose.
    const raw = String(e?.message ?? e);
    const m = /"message"\s*:\s*"([^"]+)"/.exec(raw);
    return { ok: false, reason: `api_error:${(m?.[1] ?? raw).slice(0, 120)}` };
  }
...
/**
 * Items that could take one: a resolved team, no take yet, recent - AND, for
 * club items, one that will actually render.
 *
 * A TAKE ON AN UNRENDERABLE HEADLINE IS A MODEL CALL SPENT ON A PAGE THAT DOES
 * NOT EXIST. Three of the first nine takes landed on items the render
 * allowlist drops: a phone-in radio show, a morning column, a roster gallery.
 * The Giants +2.75 line was good prose nobody would ever see.
 *
 * IT IMPORTS THE RENDER PATH'S OWN PREDICATE rather than restating it. A second
 * copy of the allowlist would drift from the first the moment either is tuned,
 * and then the generator and the page would disagree about what the wire is -
 * which is the failure that produced those three takes in the first place.
 *
 * LANE 1 IS ALWAYS ELIGIBLE. line, final, milestone, poll, record and contest
 * are our own writing and always render; only club passes through the filter.
 * Injuries are always eligible too - they are a provider's report, not a club's
 * marketing, and the allowlist was never about them.
 */
export async function takeCandidates({ limit = 8 } = {}) {
  const { renderable } = await import('./read.js');
  // OVER-READ THEN FILTER, the same shape leagueWire uses: roughly four in five
  // club items are dropped, so asking for exactly `limit` would return a short
  // list padded with nothing.
  const rows = await sql`
    SELECT id, headline, lane, team_ids, league_id
     ...
     LIMIT ${limit * 8}`;
  return renderable(rows).slice(0, limit);
}
