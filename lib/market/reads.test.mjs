...
test('TABS: both axes survive every link', () => {
  ...
  // filter already applied. One control silently resetting the other is the
  // bug the /scores toolbar had when its filters lived in component state.
  assert.match(PAGE, /function marketHref\(filter, tab\)/);
  assert.match(PAGE, /href=\{marketHref\(filter, k\)\}/, 'the tab row carries the filter');
  assert.match(PAGE, /href=\{marketHref\(k, tab\)\}/, 'the chips carry the tab');
  // The default tab is omitted so /market stays /market.
  assert.match(PAGE, /if \(tab && tab !== DEFAULT_TAB\) q\.push\(`tab=\$\{tab\}`\)/);
});
...
test('TABS: each board renders under exactly one tab', () => {
  assert.match(PAGE, /\{tab === 'lines' \? leagues\.map/);
  assert.match(PAGE, /\{tab === 'props' && shownProps\.length \?/);
  assert.match(PAGE, /\{tab === 'futures' \?/);
});

test('the props band honours the league chip, like every other tab', () => {
  // The shipped band ignored ?f= because it only rendered on the unfiltered
  // view. Under tabs a reader can hold CFB and switch to PROPS, and an active
  // CFB chip above three leagues of rows is a control that lies.
  assert.match(PAGE, /const shownProps = MARKET_LEAGUES\.includes\(filter\)\s*\n\s*\? props\.filter\(\(c\) => c\.leagueSlug === filter\)\s*\n\s*: props;/);
});
