...
test('EVERY SLATE IN reads.js ADMITS LIVE AND NEVER A FINAL', () => {
  // The game page's props panel was empty through the exact hours it was for,
  // because this reader kept the scheduled-only filter propsBoard had already
  // dropped. Three queries carry it: pricedSlate, and both branches of
  // propsSlate (one by match id, one by league).
  const clauses = READS.match(/\(m\.status = 'live'\s*OR \(m\.status = 'scheduled' AND m\.kickoff_at > now\(\)\)\)/g) ?? [];
  assert.equal(clauses.length, 3, 'all three slates carry the identical predicate');

  // A FINAL IS HISTORY - the result belongs to the game page, not to a market
  // read. Nothing here may admit one, and it is excluded by naming what is IN.
  assert.equal(/m\.status = 'final'/.test(READS), false);
  // And the old filter is GONE, not merely added to: a leftover would quietly
  // re-empty whichever surface still ran through it.
  assert.equal(/m\.status = 'scheduled' AND m\.kickoff_at > now\(\)\n/.test(READS), false);
  assert.equal(/AND m\.status = 'scheduled'\s*\n\s*AND m\.kickoff_at > now\(\)/.test(READS), false);

  // The status must reach the card, or no surface can tell a live game apart.
  assert.match(READS, /m\.status AS match_status/);
  assert.match(READS, /matchStatus: r\.match_status/);
});

test('THE GAME PAGE MOUNTS THE PROPS PANEL ON A LIVE GAME, and the lines strip stays frozen', () => {
  // Widening the reader was not enough: both game pages gated the fetch AND the
  // mount on isPreGame, so a live game rendered no panel however many rows the
  // slate returned. showsProps() is separate from isPreGame precisely so that
  // widening one never widens the other - the pre-kickoff consensus strip is a
  // pre-game read by rule and must keep freezing at kickoff.
  const FMT = strip(src('lib/gridiron/oddsFormat.js'));
  assert.match(FMT, /export function showsProps/);
  assert.match(FMT, /status === 'scheduled' \|\| status === 'live'/);
  assert.equal(/'final'/.test(FMT.slice(FMT.indexOf('showsProps'))), false, 'a final is never a props surface');

  for (const f of ['app/cfb/game/[slug]/page.js', 'app/nfl/game/[slug]/page.js']) {
    const PG = strip(src(f));
    assert.match(PG, /const propsCard = showsProps\(game\.status\)/, `${f} fetches props on a live game`);
    assert.match(PG, /showsProps\(game\.status\) && propsCard \? <PropsPanel/, `${f} mounts the panel`);
    // The odds strip is untouched and still freezes at kickoff.
    assert.match(PG, /isPreGame\(game\.status\) && odds \? <OddsStrip/, `${f} keeps the strip pre-game`);
  }
});

test('A FINAL NEVER REACHES THE LINES TAB, and the card carries the status to prove it', () => {
  // pricedSlate admits live and scheduled. The exclusion is by naming what is
  // IN, so a status nobody has thought about yet stays out by default rather
  // than leaking in - and a final is history, whose home is the game page.
  const slate = READS.slice(READS.indexOf('export async function pricedSlate'));
  const body = slate.slice(0, slate.indexOf('export async function', 10));
  assert.match(body, /\(m\.status = 'live'\s*OR \(m\.status = 'scheduled' AND m\.kickoff_at > now\(\)\)\)/);
  assert.equal(/'final'/.test(body), false, 'no final clause, and none needed');
  // The status must reach the card, or no Lines surface can tell a live game
  // apart from one that has not kicked off.
  assert.match(body, /m\.status AS match_status/);
  assert.match(READS, /matchStatus: r\.match_status/);
});

test('THE LINES TAB STAMPS A LIVE GAME PRE-KICK - cards and table alike', () => {
  const PAGE_SRC = strip(src('app/market/page.js'));
  assert.match(PAGE_SRC, /const live = card\.matchStatus === 'live'/, 'the card reads its status');
  assert.match(PAGE_SRC, /<i className="live">LIVE<\/i>/, 'and wears the red mark');
  assert.match(PAGE_SRC, /Prices are pre-kick/, 'in the same word as the props surfaces');

  // The table view is the same tab one click away; it must not print a kickoff
  // time a live game is already past.
  const TABLE = strip(src('components/market/LineTable.js'));
  assert.match(TABLE, /r\.matchStatus === 'live'/);
  assert.match(TABLE, /pre-kick/);
  // And the row builder has to carry it, or the table can never see it.
  assert.match(strip(src('lib/market/lineTables.js')), /matchStatus: card\.matchStatus/);
});

test('THE PROPS PANEL STAMPS A LIVE GAME PRE-KICK, in the same word as the index', () => {
  const PANEL = strip(src('components/gridiron/PropsPanel.js'));
  assert.match(PANEL, /card\.matchStatus === 'live'/, 'the panel reads the status it is now sent');
  assert.match(PANEL, /pre-kick/, 'and says the price stopped at kickoff');
  assert.match(PANEL, /className="live"/, 'wearing the same red mark');
  // The pre-game wording survives for a game that has not kicked off.
  assert.match(PANEL, /pre-kickoff consensus/);
});
...
test('TWO DEFAULTS, DELIBERATELY OPPOSITE', () => {
  // On PROPS the INDEX is the default (Player Props Part B; it was the table);
  // on LINES and FUTURES the CARDS are. The rule this pins has not changed -
  // each tab's UNMARKED url renders that tab's own lead screen, and every
  // marked one still names a view that exists.
  assert.match(PAGE, /tab === 'props'\s*\n\s*\? \(sp\.view === 'charts' \? 'charts' : sp\.view === 'table' \? 'table' : 'index'\)\s*\n\s*: \(sp\.view === 'table' \? 'table' : 'cards'\)/);
  // BOTH SHIPPED VIEWS KEPT THEIR URLS. This is the half that makes the move
  // safe: ?view=table and ?view=charts still resolve to what they always drew.
  assert.match(PAGE, /sp\.view === 'charts' \? 'charts'/);
  assert.match(PAGE, /sp\.view === 'table' \? 'table'/);
});
