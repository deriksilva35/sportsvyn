...
import { resolveProps } from './propLinking.js';
 ...
 *
 * LIVE AND SCHEDULED, NEVER A FINAL. A price on a game that is over is a
 * fossil and the result belongs to the game page; a price on a game being
 * PLAYED is still the last thing the books said, and a reader watching one
 * wants it - stamped "pre-kick", never presented as a running line, because
 * the consensus stops updating at kickoff. Every surface drawing these cards
 * carries m.status through for exactly that reason.
 */
export async function pricedSlate({ leagues = MARKET_LEAGUES, limitPerLeague = 24 } = {}) {
  const rows = await sql`
    SELECT l.slug AS league_slug, m.id AS match_id, m.slug AS match_slug,
           m.kickoff_at, m.status AS match_status, m.week, m.season_phase,
  ...
  for (const r of rows) {
    ...
    if (!card) {
      card = {
        ...
        // THE SLATE ADMITS LIVE GAMES, so the card must be able to say so: its
        // numbers froze at kickoff and a live card states that rather than
...
export async function futuresBoards({ topN = 5 } = {}) {
  const rows = await sql`
    SELECT l.slug AS league_slug, om.selection_label, om.american_odds,
           om.implied_probability, om.movement_24h_prob
      FROM odds_markets om
  ...
  // `all` carries the WHOLE FIELD for the futures table; `top` is the card's
  // five. Same query, two slices - the table is where the other 130 live, and
  // re-reading for it would risk two answers to one price.
  return [...byLeague.entries()].map(([slug, all]) => ({
    leagueSlug: slug, priced: all.length, top: all.slice(0, topN), all,
  }));
}
...
export async function propsSlate({ leagues = MARKET_LEAGUES, matchIds = null } = {}) {
  const rows = matchIds
    ? await sql`
        SELECT l.slug AS league_slug, m.id AS match_id, m.kickoff_at,
               m.status AS match_status, m.home_team_id, m.away_team_id,
  ...
  for (const r of rows) {
    if (!byMatch.has(r.match_id)) {
      byMatch.set(r.match_id, {
        ...
        leagueSlug: r.league_slug,
        kickoffAt: r.kickoff_at,
        // THE SLATE ADMITS LIVE GAMES, so the surface drawing this card must be
        // able to tell one apart - the consensus stops updating at kickoff and
        // a live card's numbers are a pre-kick stamp, not a running price.
        matchStatus: r.match_status,
        home: { name: r.home_name, abbreviation: r.home_abbr },
        away: { name: r.away_name, abbreviation: r.away_abbr },
        // Carried for the linker, which resolves a vendor label against the
        // two rosters of the event it was priced on.
  ...
  // THE ROWS LEARN WHO THEY ARE ABOUT (Player Props Part B), so a panel row can
  // open the player's card. It is the SAME resolver the board uses - one
  // roster load per event, then a pure pass - rather than a second opinion
  // about which "J. Smith" a vendor label means. A row that does not resolve
  // keeps every number it had and simply is not a link: the gap is ours.
  const cards = [...byMatch.values()];
  const flat = cards.flatMap((c) => c.rows.map((r) => ({
    match_id: c.matchId, selection_label: r.label,
    home_team_id: c.homeTeamId ?? null, away_team_id: c.awayTeamId ?? null,
  })));
  if (flat.length) {
    try {
      const links = await resolveProps(sql, flat);
      const ids = [...new Set([...links.values()].map((v) => v.playerId))];
      const slugs = new Map();
      if (ids.length) {
        for (const p of await sql`SELECT id, slug FROM players WHERE id = ANY(${ids})`) {
          slugs.set(p.id, p.slug);
        }
      }
      for (const c of cards) {
        for (const r of c.rows) {
          const link = links.get(`${c.matchId}|${r.label}`) ?? null;
          r.playerSlug = link ? slugs.get(link.playerId) ?? null : null;
        }
      }
    } catch {
      // A failed link leaves every row exactly as it was - unlinked, priced,
      // and readable. The panel is a courtesy on a game page.
      for (const c of cards) for (const r of c.rows) r.playerSlug = null;
    }
  }
  return cards;
}
