...
export async function propsSlate({ leagues = MARKET_LEAGUES, matchIds = null } = {}) {
  ...
  for (const r of rows) {
    ...
    // AS-OFFERED covers two cases now: anytime markets, and any O/U the vendor
    // prices one side only (player_shots_on_target today). Both arrive with a
    // NULL implied_probability because neither was de-vigged, so the ROW's own
    // data decides - not a list of market keys that would drift.
    if (meta?.anytime || r.implied_probability == null) card.hasAnytime = true;
    card.rows.push({
      ...
      impliedPct: meta?.anytime ? null
        : (r.implied_probability == null ? null : Number(r.implied_probability)),
      moveProb: r.movement_24h_prob == null ? null : Number(r.movement_24h_prob),
    });
  }
