...
/**
 * THE LINES DROPDOWN'S OPTIONS — ITS OWN DENOMINATOR.
 *
 * Props has 33 priced games; lines has its own set, and they are not the same
 * list. Offering a reader on LINES a game that only has props would be a
 * control that filters to nothing, so this is derived from the SAME cards the
 * lines view is about to render rather than from the props census.
 *
 * NO NEW QUERY: pricedSlate is already in the page's hand. A dropdown that
 * re-read the database could offer a game the board below it does not show.
 *
 * BOARD GAMES FIRST, then league, then kickoff - the same editorial rule the
 * CFB band and the props dropdown use.
 */
export function linesGames(byLeague, { boardIds, leagues } = {}) {
  ...
  for (const slug of leagues ?? []) {
    for (const card of byLeague.get(slug) ?? []) {
      const onBoard = boardIds?.has(card.matchId) ?? false;
      const away = card.away?.abbreviation || card.away?.name || '?';
      const home = card.home?.abbreviation || card.home?.name || '?';
      out.push({
        matchId: card.matchId,
        leagueSlug: card.leagueSlug,
        onBoard,
        kickoffAt: card.kickoffAt,
        label: `${slug.toUpperCase()} · ${away} at ${home}`
          + (card.kickoffAt ? ` · ${when.format(new Date(card.kickoffAt))}` : '')
          + (onBoard ? ' · Board' : ''),
      });
    }
  }
  return out.sort((x, y) => (y.onBoard ? 1 : 0) - (x.onBoard ? 1 : 0)
  ...
  for (const slug of leagues) {
    for (const card of byLeague.get(slug) ?? []) {
      ...
      const base = {
        ...
        leagueSlug: card.leagueSlug,
        kickoffAt: card.kickoffAt,
        // Carried so the table can say a game is on, exactly as the card does.
