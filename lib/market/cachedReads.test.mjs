...
writeFileSync(PROPS, "export async function propsBoard(s) { return { rows: [], total: 0, echo: s }; } export async function propsGames() { return []; }\n");
...
test('every wrapper survives the JSON round trip with its reader\'s shape', async () => {
  ...
  assert.deepEqual((await C.cachedPropsBoard({ league: 'nfl' })).echo, { league: 'nfl' }, 'the filters reach the reader');
});
