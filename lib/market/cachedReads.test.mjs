...
writeFileSync(PROPS, [
  "export const calls = []; let fails = 0; export function failNext(n) { fails = n; }",
  "export async function propsBoardRows(league) { calls.push(league); if (fails > 0) { fails -= 1; throw new Error('permit'); } return [{ matchId: 1, league }, { matchId: 2, league }]; }",
...
test('THE PROPS ROWS ARE HELD IN MEMORY: one read per league per minute, shared by concurrent requests, never in the data cache', async () => {
  const { calls } = await import(pathToFileURL(PROPS).href);
  C._resetPropsMemo(); calls.length = 0;
  const t0 = 1_000_000;
  // many filter combinations, arriving together, on one league: ONE read
  await Promise.all([
    C.cachedPropsBoard({ league: 'cfb', game: 1, team: 'ALA', q: 'x', minHitPct: 60 }, { now: t0 }),
    C.cachedPropsBoard({ league: 'cfb', pos: 'WR', sort: 'hit' }, { now: t0 }),
    C.cachedPropsBoard({ league: 'cfb', game: 2 }, { now: t0 + 1000 }),
  ]);
  assert.deepEqual(calls, ['cfb']);
  await C.cachedPropsBoard({ league: 'nfl' }, { now: t0 + 2000 });
  assert.deepEqual(calls, ['cfb', 'nfl'], 'a league is its own entry');
  await C.cachedPropsBoard({ league: 'cfb' }, { now: t0 + C.PROPS_MEMO_MS - 1 });
  assert.equal(calls.length, 2, 'still fresh');
  await C.cachedPropsBoard({ league: 'cfb' }, { now: t0 + C.PROPS_MEMO_MS });
  assert.deepEqual(calls, ['cfb', 'nfl', 'cfb'], 'read again once a minute has passed');
  // 3.95 MB is over the data cache's 2 MB item limit: the rows must not go there
  assert.ok(!seen.some((x) => String(x.keys[0]).startsWith('market:propsBoardRows')), 'no data-cache entry for the rows');
});

test('A FAILED READ IS FORGOTTEN AT ONCE, not served for a minute', async () => {
  const stub = await import(pathToFileURL(PROPS).href);
  C._resetPropsMemo(); stub.calls.length = 0;
  stub.failNext(1);
  await assert.rejects(C.cachedPropsBoard({ league: 'mlb' }, { now: 5_000_000 }));
  await new Promise((r) => setImmediate(r));
  const b = await C.cachedPropsBoard({ league: 'mlb' }, { now: 5_000_001 });
  assert.equal(b.rows.length, 2);
  assert.deepEqual(stub.calls, ['mlb', 'mlb']);
});
