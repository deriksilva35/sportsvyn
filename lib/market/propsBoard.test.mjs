...
test('THE SLATE ADMITS LIVE GAMES AND NEVER A FINAL - in both queries', () => {
  // The board and the game selector must agree, or the dropdown offers a game
  // the board cannot show (or hides one it can). The predicate is written out
  // in both, so it is asserted in both.
  const BOARD = strip(src('lib/market/propsBoard.js'));
  const clauses = BOARD.match(/\(m\.status = 'live'\s*OR \(m\.status = 'scheduled' AND m\.kickoff_at > now\(\)\)\)/g) ?? [];
  assert.equal(clauses.length, 2, 'the board query and propsGames() carry the same rule');

  // A FINAL IS HISTORY. No surface of the slate may admit one - if this ever
  // grows a third status, it is a deliberate decision and this line must move.
  assert.equal(/m\.status = 'final'/.test(BOARD), false);
  assert.equal(/m\.status <> 'final'/.test(BOARD), false, 'excluded by naming what is IN, not what is out');
  // And the old scheduled-only filter is gone, not merely added to.
  assert.equal(/AND m\.status = 'scheduled' AND m\.kickoff_at > now\(\)/.test(BOARD), false);
});

test('THE EXISTING BAND STAMPS A LIVE ROW TOO - the filter change reaches it', () => {
  // Widening the slate put live games into /market's props band as well, so
  // the band must not print a kickoff that has already passed, nor label a
  // frozen consensus as the current market.
  const BOARD = strip(src('components/market/PropsBoard.js'));
  assert.match(BOARD, /matchStatus === 'live'/, 'the band reads the status it is now sent');
  assert.match(BOARD, /pre-kick/, 'and says the number stopped at kickoff');
  assert.match(BOARD, /className="live"/, 'a live row wears the mark instead of a kickoff time');
});
...
test('ZERO CLIENT COMPONENTS on the props surface', () => {
  ...
  for (const f of ['components/market/PropsTable.js', 'components/market/PropsFilters.js',
    'components/market/PropsBoard.js', 'components/market/GameFilter.js']) {
...
test('the game dropdown is a GET form, not a handler', () => {
  // THE CLAIM IS UNCHANGED; ONLY ITS ADDRESS MOVED. The LINES tab now offers
  // the same control, so the markup lives in one shared component rather than
  // in two copies that could drift. Still a form, still no handler.
  const F = strip(src('components/market/GameFilter.js'));
  assert.match(F, /<form action="\/market" method="get" className="pb-gameform">/);
  assert.match(F, /<select name="game"/);
  assert.match(strip(src('components/market/PropsFilters.js')), /<GameFilter tab="props"/);
});
