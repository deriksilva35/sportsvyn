...
// team / pos / mkt / hit are the props INDEX's filters (Player Props Part B).
// They ride the same whitelist as everything else, so an index a reader has
// narrowed is a URL they can send.
...
/**
 * The unmarked view for a tab. Props leads with the INDEX since Player Props
 * Part B; ?view=table and ?view=charts are unchanged and still reachable.
 */
export function defaultView(tab) {
  return tab === 'props' ? 'index' : 'cards';
}

/** The unmarked sort for a tab+view. */
export function defaultSort(tab, view) {
  // The index answers "what is on next"; the table and the charts answer
  // "what moved". Each view's unmarked sort is its own question.
  if (tab === 'props') return view === 'index' ? 'kickoff' : 'move';
