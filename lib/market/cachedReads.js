...
import { propsBoard, propsGames } from './propsBoard.js';
...
// The props board takes the reader's FILTERS (from the URL, not the reader);
// unstable_cache keys on the arguments, so each filter combination is its own
// entry for a minute.
const propsBoardCached = unstable_cache(async (state) => propsBoard(state), ['market:propsBoard:v1'], opts);
...
export async function cachedPropsBoard(state) { return propsBoardCached(state); }
