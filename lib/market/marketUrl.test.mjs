...
test('canonical URLs carry no litter', () => {
  ...
  // THE PROPS DEFAULT MOVED TO THE INDEX (Player Props Part B), so ?view=table
  // is now MEANINGFUL on props and must survive - it names the view that used
  // to be unmarked. The unmarked props URL is the index.
  assert.equal(marketHref({ tab: 'props', view: 'index' }, {}), '/market?tab=props');
  assert.equal(marketHref({ tab: 'props', view: 'table' }, {}), '/market?tab=props&view=table');
...
test('THE DEFAULT VIEW DEPENDS ON THE TAB, so omission must too', () => {
  assert.equal(defaultView('props'), 'index');
  assert.equal(defaultView('lines'), 'cards');
  assert.equal(defaultView('futures'), 'cards');
  // ?view=index is litter on props; ?view=table and ?view=charts are not, and
  // BOTH still resolve - the two views that shipped kept their URLs, only the
  // unmarked one moved.
  assert.ok(!/view=/.test(marketHref({ tab: 'props' }, { view: 'index' })));
  assert.match(marketHref({ tab: 'props' }, { view: 'table' }), /view=table/);
  assert.match(marketHref({ tab: 'props' }, { view: 'charts' }), /view=charts/);
