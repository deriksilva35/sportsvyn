...
export async function tick(sql, nowUtc, { notify = notifyEvent } = {}) {
  ...
  for (const r of liveDue) {
    out.live.push({ edition: r.edition_date, result: await notify(`daily-live:${r.edition_date}`) });
  }
  ...
  for (const r of revealedDue) {
    out.revealed.push({ edition: r.edition_date, result: await notify(`daily-revealed:${r.edition_date}`) });
  }
