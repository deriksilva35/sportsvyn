...
test('the corpus has exactly 33 eligible seasons today', async () => {
  // 31 (1980-1999, 2015-2025) + 2000 + 2001 (footballdb 2000/2001 ingest).
  const rows = await sql`SELECT DISTINCT season_year FROM nfl_player_season_totals`;
  assert.equal(rows.length, 33, 'a change here means the corpus grew or shrank - update the count deliberately, never silently');
});
