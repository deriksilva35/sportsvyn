...
//
// EIGHT SLOTS: QB/RB/RB/WR/WR/TE/FLEX/K (the Daily v2.0 ruling,
// docs/design/mocks/daily-v2.html). One FLEX became a required TE.
//
// THE TE SLOT IS AFFORDABLE NOW AND WAS NOT BEFORE, and the difference is the
// corpus, not the taste. The original eight dropped the TE slot because
// footballdb cannot tell a TE from a WR - its tab-presence heuristic defaults
// every receiving-only row to WR - so 1980-1998 carries almost no TE-tagged
// row at all (1 of 28 teams across 1980-1991) and a dedicated TE slot made
// fillability a GATE: 43% of random 1982 draws could complete one, and 1991
// could not complete a single one out of 200.
// RANKED IS 2002-2025 NOW (see RANKED_MIN_SEASON), where the position is
// resolved per season from nflverse rather than inferred from a box score.
// Measured on PROD before this changed: 300 draws in each of the 23 seasons
// in range, 6,900 draws, ZERO needing a redraw, with a mean of 7.8 of twelve
// cards carrying a tight end.
//
// THE PRE-2002 SEASONS ARE NOT LOST - they are out of the RANKED set only.
// The corpus stays 1980-2025 whole, for free play, the archive, and whatever
// era mode comes later.
export const SLOTS = Object.freeze(['QB', 'RB', 'RB', 'WR', 'WR', 'TE', 'FLEX', 'K']);

// THE SHAPE EVERY BOARD CREATED BEFORE migrations/103 WAS PLAYED UNDER. A
// frozen edition keeps its own shape forever: readers take board.slots and
// fall back to this only for a row written before the column existed. It is
// NOT a default for new boards - ensureBoardForDate writes SLOTS explicitly.
export const LEGACY_SLOTS = Object.freeze(['QB', 'RB', 'RB', 'WR', 'WR', 'FLEX', 'FLEX', 'K']);

/**
 * The slot shape to read a stored board with. ALWAYS the board's own, when it
 * has one - never today's constant.
 *
 * Eleven of the seventeen runs stored on PROD would have failed to regrade if
 * the reader kept importing the constant ("Steve Largent (WR) is not eligible
 * for TE"), and two of the nine boards would have become unsolvable. This
 * function is the whole fix, and every reader calls it.
 */
export function slotsOf(board) {
  const s = board?.slots;
  return Array.isArray(s) && s.length ? s : LEGACY_SLOTS;
}

// THE EARLIEST SEASON A RANKED EDITION MAY DRAW. 2002 is where the modern
// corpus starts (BDL game stats begin there; 2001 and earlier return empty),
// and it is the floor below which a per-season position does not exist. The
// CORPUS is not filtered by this - only the ranked draw is.
export const RANKED_MIN_SEASON = 2002;
...
/** A closed edition's results - the receipt, the best roster, the day's
 * leaderboard. ONE constant: the midnight push, the lobby's Yesterday line,
 * the Latest Answer pane and History all build the href here. */
export const dailyResultsPath = (date) => `${DAILY_V2_PATH}/${date}`;
