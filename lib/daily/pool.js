...
// ---------------------------------------------------------------------------
/**
 * CAREER PPR PER GAME - one number, in the game's own currency.
 *
 * The board is scored in PPR and the answer is a PPR total, so a resume that
 * reads "11.2 PPG" is denominated in the same unit the player is about to be
 * judged in. The per-position volume rates it replaced - pass yds/gm, scrim
 * yds/gm, rec yds/gm - were honest but forced a translation on every row: a
 * quarterback's 264 and a receiver's 82 are not comparable numbers, and picking
 * between them is exactly what the round asks you to do.
 *
 * RATES, NEVER TOTALS, AND THE REASON IS THE CORPUS. Our box scores begin in
 * 2015. Any player already playing before that has the front of his career
 * missing, so a TOTAL taken from these tables is simply wrong - Tom Brady shows
 * 126 games where he played about 335. A RATE survives the truncation intact:
 * he really did average what he averaged over the 126 games we hold. A rate is
 * honest where a total lies.
 *
 ...
 *
 * WEEK-INVARIANT BY CONSTRUCTION: career-to-today across the whole corpus,
 * identical on every board, so it cannot narrow which week is on the screen. It
 * is not as-of the puzzle week - that would vary with the draw and leak the
 * era. One consequence worth knowing: the live season is in the index, so the
 * number drifts as games land. Boards are frozen at generation, so each board
 * keeps the figure it was built with and two boards a month apart can disagree
 * by a tenth. That is correct behaviour, not drift to be fixed.
 *
 * NO POSITION BRANCH ANY MORE. PPG is defined the same way for all four, which
 * also means a player whose listed position moved between seasons can no longer
 * get an incoherent row.
 */
export function careerStat(pos, career) {
  const g = Number(career?.g ?? 0);
  if (!Number.isFinite(g) || g <= 0) return null;
  // 'daily' NOT 'ppr' - see the note beside RECEPTION_PTS.daily in scoring.js.
  const pts = fantasyPoints(career, 'daily');
  if (!Number.isFinite(pts)) return null;
  return `${(Math.round((pts / g) * 10) / 10).toFixed(1)} PPG · ${g} g`;
}
