...
test('SET-MATCH BEFORE DISPLAY: the same player at a different (but legal) slot is a MATCH', () => {
  ...
  const slots = ['RB', 'FLEX'];
  let play = initBoardPlay(teams, slots);
  play = commitPick(play, teams[0], teams[0].card[0], 1); // Zeke -> FLEX (index 1)
  play = commitPick(play, teams[1], teams[1].card[0], 0); // Barkley -> RB (index 0)
  ...
  // moved fires on a LABEL change now (ruling, item 6) - whether it fires
  // here depends on the solver's own RB-vs-FLEX tie-break for Zeke, which
  // this fixture does not pin down (both are his own eligible slots) - a
  // DEDICATED, solver-queried-first fixture below pins the label-change
  // case down deterministically, the same technique the RB1-vs-RB2 test
  // above already uses for its own tie-break.
  assert.ok(zekeRow.moved === null || zekeRow.moved === 'RB' || zekeRow.moved === 'FLEX');
});

// -----------------------------------------------------------------------
// THE NOTE, PINNED DOWN (ruling, item 6): fires on a LABEL change - FLEX vs
// RB, FLEX vs WR - never on an index change within the same label (that's
// the RB1-vs-RB2 test above, which stays silent). This fixture calls
// solveBoard() itself first, then DELIBERATELY places Zeke at whichever
// slot the solver did NOT choose for him - guaranteeing a genuine label
// mismatch regardless of which way the solver's own tie-break fell.
// -----------------------------------------------------------------------
test('a genuine label change (FLEX vs RB) earns the moved note', () => {
  const teams = [
    team('T1', [{ position: 'RB', name: 'Zeke', points: 20, meta: '' }]),
    team('T2', [{ position: 'RB', name: 'Barkley', points: 3, meta: '' }]),
  ];
  ...
  const zekeSlot = optimum.bySlot.find((b) => b.player.name === 'Zeke').slot;
  const otherSlot = zekeSlot === 'RB' ? 'FLEX' : 'RB';
  const zekeSolverIdx = slots.indexOf(zekeSlot);
  const otherIdx = slots.indexOf(otherSlot);

  let play = initBoardPlay(teams, slots);
  play = commitPick(play, teams[0], teams[0].card[0], otherIdx); // Zeke -> the slot the solver did NOT pick for him
  play = commitPick(play, teams[1], teams[1].card[0], zekeSolverIdx); // Barkley -> the slot the solver DID pick for Zeke

  const grade = gradeBoard(play, teams, slots);
  assert.equal(grade.pct, 100);
  assert.equal(grade.matchedCount, 2);

  const zekeRow = grade.rows[otherIdx];
  assert.equal(zekeRow.hit, true);
  assert.equal(zekeRow.you.name, 'Zeke');
  assert.equal(zekeRow.moved, otherSlot, 'the solver had him at a different label - the note fires');
});
...
// -----------------------------------------------------------------------
test('a same-LABEL swap (RB1 vs RB2) resolves to a MATCH, NO note, and a legal, ceiling-equal permutation', () => {
  const teams = [
  ...
  // NO NOTE (ruling, item 6): the pristine best roster's label for Etienne
  // was already 'RB' (just a different RB instance) - a label change is
  // what earns the note, not an index change, and RB-to-RB is not one.
  assert.equal(row.moved, null, 'RB1-to-RB2 shares one label - silent, even though the permutation did move him');
