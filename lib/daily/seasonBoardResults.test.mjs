...
test('no ceiling - zero or absent alike - loses the percentage and keeps the line', () => {
  const { grade } = regradeStoredRun(board, FX.picks, SLOTS);
  for (const perfect of [0, null]) {
    const line = yesterdayLine({ date: '2026-09-08', grade: { ...grade, perfect }, score: '1840.2' });
    assert.equal(line.text, 'Yesterday · 1,840.2 · 4 of 8', `perfect=${perfect}: never "0%", never a dash`);
  }
});
