...
test('a board with no ceiling loses the percentage and keeps the rest of the line', () => {
  const { grade } = regradeStoredRun(board, FX.picks, SLOTS);
  const line = yesterdayLine({ date: '2026-09-08', grade: { ...grade, perfect: 0 }, score: '1840.2' });
  assert.equal(line.text, 'Yesterday · 1,840.2 · 4 of 8', 'no percentage at all - never "0%"');
});
