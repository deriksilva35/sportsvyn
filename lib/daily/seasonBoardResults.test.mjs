...
test("Derik's yesterday line: 'Yesterday · 1,840.2 · 90.2% · 4 of 8', from a REGRADE, not the stored matched=0", () => {
  // ONE DECIMAL NOW (lib/daily/format.js). 1840.2 of a 2039.4 ceiling is
  // 90.23%, which the old whole-number rounding wrote as "90%" - the same
  // rounding that wrote a 99.66% board as "100%". The regrade half of this
  // test is unchanged and is still the point: matched comes from re-reading
  // the picks, not from the stored matched=0.
  const { grade } = regradeStoredRun(board, FX.picks, SLOTS);
  const line = yesterdayLine({ date: '2026-09-08', grade, score: '1840.2' });
  assert.deepEqual(line, { text: 'Yesterday · 1,840.2 · 90.2% · 4 of 8', href: '/daily/board/2026-09-08' });
});

test('a zero ceiling reads "0.0%" on the line, per the relay, and never divides', () => {
  // A board whose ceiling is zero has no attainable points, which cannot
  // happen for a minted edition; this pins the arithmetic rather than the
  // product. The relay's rule is "0.0%", not an omission.
  const { grade } = regradeStoredRun(board, FX.picks, SLOTS);
  const line = yesterdayLine({ date: '2026-09-08', grade: { ...grade, perfect: 0 }, score: '1840.2' });
  assert.equal(line.text, 'Yesterday · 1,840.2 · 0.0% · 4 of 8');
  assert.doesNotMatch(line.text, /NaN|Infinity/);
});

test('a ceiling we do not have drops the percentage and keeps the rest of the line', () => {
  const { grade } = regradeStoredRun(board, FX.picks, SLOTS);
  const line = yesterdayLine({ date: '2026-09-08', grade: { ...grade, perfect: null }, score: '1840.2' });
  assert.equal(line.text, 'Yesterday · 1,840.2 · 4 of 8', 'no percentage at all - never "0%", never a dash');
});
...
test("Derik's Sep 9 row is a DNF and stays a DNF - the you cell says so", () => {
  ...
  const { grade } = regradeStoredRun(board, FX.picks, SLOTS);
  // THE CELL CARRIES THE FORMATTED STRING NOW, one decimal, off the stored
  // ratio - the same "90.2%" the Yesterday line shows for the same run.
  assert.deepEqual(youCellV2({ picks: FX.picks, score: '1840.2', pct: '0.9023' }, grade), { played: true, score: 1840.2, pct: '90.2%', matched: 4, slotCount: 8 });
  assert.equal(youCellV2({ picks: FX.picks, score: '2371.1', pct: '0.99966' }, grade).pct, '99.9%',
    'and a run that fell short never reads 100% in the lobby either');
});
