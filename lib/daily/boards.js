...
import { seasonStandings, dayLeaderboard, topWithSelf, seasonKeyFor, annotateMovement } from './standings.js';
import { displayName } from './handles.js';

/** The viewer's best REVEALED score - the Daily page's stat row. Same law as
 * every read here: an open day's score never leaves the database. */
export async function bestDay(userId) {
  if (userId == null) return null;
  const r = await sql`
    SELECT max(e.score) AS best FROM puzzle_entries e
    JOIN puzzle_days p ON p.puzzle_date = e.puzzle_date
    WHERE p.revealed AND e.user_id = ${Number(userId)} AND e.score IS NOT NULL`;
  return r[0]?.best == null ? null : Number(r[0].best);
}
...
export async function overall(userId = null, n = 10, seasonKey = null, { memberIds = null } = {}) {
  ...
  const table = seasonStandings(inSeason.map(mapRow), daysPlayable);

  // MOVEMENT vs the previous revealed day: the same rows, one more PURE pass
  // with the latest day excluded - no snapshot table, no second query. On a
  // one-day season every move is null and the arrows render as '-'.
  const prevRows = inSeason.filter((r) => r.d !== through);
  annotateMovement(table, prevRows.length ? seasonStandings(prevRows.map(mapRow), daysPlayable - 1) : []);
