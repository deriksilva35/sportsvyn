...
test('Jack Trudeau 1995 CAR -> QB (33 pass attempts, zero touches - the floor does not apply with nothing to compete against)', () => {
  // real DEV row: pass_att=33, zero yards/TD recorded (0 passing points),
  // rush_att=0, rec=0 - the exact case the amendment fixes: the points-based
  // gate could never call this QB (0 points), and the OLD 100-attempt floor
  // refused it outright. With no touches at all, the one real signal on the
  // row - 33 real pass attempts - is what decides it.
  const row = { passAtt: 33, passYds: 0, passTd: 0, passInt: 0, rushAtt: 0, rec: 0 };
  assert.equal(inferPosition(row), 'QB');
});

test('Harvey Williams 1995 -> RB, unchanged by the no-touches amendment (he has touches)', () => {
  const row = {
    passAtt: 1, passYds: 13, passTd: 1,
    rushAtt: 255, rushYds: 1114, rushTd: 9,
    rec: 54, recYds: 375, recTd: 0,
...
test('90 pass attempts with big rushing -> RB, unchanged by the no-touches amendment (he has touches)', () => {
  const row = {
    passAtt: 90, passYds: 600, passTd: 4, passInt: 2,
    rushAtt: 120, rushYds: 800, rushTd: 8,
  };
...
test('a punter with 1 pass attempt and no touches -> QB (harmless - a punter never reaches a board card anyway)', () => {
  const row = { passAtt: 1, passYds: -2, passTd: 0, passInt: 0, rushAtt: 0, rec: 0 };
  assert.equal(inferPosition(row), 'QB');
});
