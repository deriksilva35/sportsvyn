...
/**
 * May the clock start? PURE - the one decision that gates it, testable
 * without a DOM or a component. THE CLOCK CANNOT START SIGNED OUT (ruling,
 * 5b): a signed-out visitor on the edition path sees the rules card with a
 * sign-in surface in place of Start, so this should be unreachable from a
 * real tap - it exists as the belt under that suspender, the same
 * discipline pickOutcome's own "unreachable from a real tap" comment
 * already follows for a NO SLOT row.
 */
export function startClock(userId) {
  if (userId == null) return { ok: false, reason: 'signin' };
  return { ok: true };
}
...
/**
 * Empty `slotIndex` and give its team back. Returns a NEW state.
 *
 * THIS REVERSES A DELIBERATE OLD RULE, and the v2.0 mock is what reversed it:
 * "keep swapping - tap any filled slot to clear it". The previous board was
 * commit-on-open - a team you looked at was a team you had spent - and the
 * whole modal flow existed to enforce it. The ranked board is a three-minute
 * puzzle with a lock button at the end, so a misplaced pick is now a mistake
 * to fix rather than a fate.
 *
 * THE TEAM COMES BACK ONLY IF NOTHING ELSE HOLDS IT. `used` is a set of team
 * keys and one team can legally appear in only one slot, so the rebuild is
 * from what remains rather than a decrement - a decrement is how a team gets
 * released twice.
 */
export function clearSlot(state, slotIndex) {
  const roster = state.roster.map((r, i) => (i === slotIndex ? { ...r, pick: null } : r));
  const used = new Set(roster.filter((r) => r.pick).map((r) => r.pick.teamKey));
  return { ...state, roster, used };
}
