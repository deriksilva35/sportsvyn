...
import { eligibleForSlot, shapeBestRoster, slotsOf, DAILY_ROUND_SECONDS, DAILY_ROUND_GRACE_SECONDS } from './boardShape.js';
...
export function buildRosterFromPicks(board, picks, slots) {
  if (!Array.isArray(picks) || picks.length !== slots.length) {
    // NULL SAYS "null", NOT "object". typeof null is 'object' in JavaScript, so
    // a STARTED-BUT-UNSUBMITTED run - picks IS NULL, which is exactly what
    // startRun writes and what a DNF stays as - used to report "expected 8
    // picks, got object". That sentence reads like a malformed payload, and it
    // cost two reports their accuracy: the rows were healthy DNFs and were read
    // as corrupt data. Nothing else about this path changes; the product never
    // reaches here for a null anyway, every reader guarding on picks first.
    const got = picks === null ? 'null' : (Array.isArray(picks) ? picks.length : typeof picks);
    return { ok: false, reason: `expected ${slots.length} picks, got ${got}` };
  }
 ...
 */
export function regradeStoredRun(board, picks, slots = null) {
  // THE BOARD'S OWN SHAPE, NOT TODAY'S CONSTANT (migrations/103). A caller may
  // still pass one explicitly - the tests do - but the default is the shape
  // this edition was created, solved and graded under, which is what keeps a
  // receipt from before the ranked TE slot regrading exactly as it always did.
  const shape = slots ?? slotsOf(board);
  const built = buildRosterFromPicks(board, picks, shape);
  ...
  // either.
  const play = { slots: shape.slice(), teams: board.board, roster: built.roster, used: [...built.used] };
  const grade = gradeFromOptimum(play, board.board, optimum, shape);
 ...
 */
export async function submitRun(sql, { boardId, userId, picks, elapsedS, slots = null, now = null }) {
  const boards = await sql`SELECT * FROM daily_boards WHERE id = ${boardId}`;
  const board = boards[0];
  if (!board) return { ok: false, reason: 'no such board', status: 404 };
  // GRADED UNDER THE BOARD'S OWN SHAPE. A submit is checked against the slots
  // the reader was actually shown, which are the board's, not the constant the
  // server happens to hold today.
  const shape = slots ?? slotsOf(board);
  ...
  const regraded = regradeStoredRun(board, picks, shape);
