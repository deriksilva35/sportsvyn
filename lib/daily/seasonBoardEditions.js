...
import { SLOTS, RANKED_MIN_SEASON } from './boardShape.js';
...
import { metaFor, shapeBestRoster } from './boardShape.js';

// metaFor lives in boardShape.js now (pure, reachable from the grade module);
// re-exported so app/daily/board/page.js's import keeps working unchanged.
export { metaFor };
...
/**
 * Which epoch actually governs "is an edition live" right now - PURE given
 * the env object (never reads process.env itself, so this stays testable
 * without mutating real environment state). ON PROD, DAILY_V2_EPOCH IS THE
 * ONLY TRUTH: DAILY_V2_EPOCH_OVERRIDE is read ONLY when VERCEL_ENV is not
 * 'production' (a Preview-deployment rehearsal knob, set on the Vercel
 * environment itself via the CLI - never in a committed file), and is
 * ignored outright on Production even if someone sets it there by mistake.
 * Callers pass the result to isEditionLive() as its `epoch` argument -
 * this function decides WHICH epoch, isEditionLive decides live-or-not.
 */
export function effectiveEpoch(env = process.env) {
  if (env.VERCEL_ENV === 'production') return DAILY_V2_EPOCH;
  return env.DAILY_V2_EPOCH_OVERRIDE || DAILY_V2_EPOCH;
}


/**
 * BOTH HALVES OF AN EDITION ROW, FROM ONE FUNCTION. Pure. The INSERT below
 * takes its board and best_roster values from here and nowhere else, so the
 * two cannot be shaped differently again - and editionHalves.test.mjs asserts
 * that every player entry on either side carries the same keys.
 */
export function buildEditionHalves(teams, optimum) {
  return { board: shapeTeams(teams), best_roster: shapeBestRoster(optimum.bySlot) };
}
...
function shapeTeams(teams) {
  return teams.map((t) => ({
    key: t.key,
    // DISPLAY CODE, NOT THE RAW team_key (relay: position-inference fix,
    // item 5). A footballdb row whose team never resolved to a current
    // franchise abbreviation stores its RAW name in team_key (migrations/
    // 089's own ruling - never rewritten) - this was rendering full text
    // ("Oakland Raiders") on the chip and card because shapeTeams() never
    // called displayTeamCode() at all, not because the map lacked an entry.
...
// SEASON WEIGHTING (ruling): 2015+ (BDL, full modern stat coverage) draws
// 3x as often as anything older - name recognition carries the game, and
// the pre-1999/2015+ corpus is otherwise drawn uniformly regardless of how
// familiar a season actually is to a reader. Weight lives here, as a plain
// function of the season number alone, not a second list to keep in sync
// with the corpus.
function seasonWeight(season) {
  return season >= 2015 ? 3 : 1;
}
 ...
 * given its inputs. WEIGHTED (seasonWeight) - a season drawn once per
 * weight point, one seeded pick from the expanded pool, still exactly one
 * rng() call, still fully deterministic per editionDate.
 */
export function pickEligibleSeason(seasonsPresent, recentSeasons, editionDate, windowDays = RECENCY_WINDOW_DAYS) {
  ...
  const weighted = eligible.flatMap((s) => Array(seasonWeight(s)).fill(s));
...
export async function ensureBoardForDate(sql, editionDate) {
  ...
  // THE RANKED SET IS 2002-2025, NOT THE WHOLE CORPUS (v2.0 ruling b). The
  // corpus stays 1980-2025 and is untouched by this clause - free play, the
  // archive and any later era mode still have every season. What the floor
  // buys is the TE slot: below 2002 a tight end is inferred from a box score
  // rather than read per season, and a required TE slot turns that scarcity
  // into a fillability gate (1991: 0 of 200 draws completable).
  const seasonsPresent = (await sql`
    SELECT DISTINCT season_year FROM nfl_player_season_totals
     WHERE season_year >= ${RANKED_MIN_SEASON}
     ORDER BY 1
  `).map((r) => r.season_year);
  ...
  // ONE SHAPE FOR BOTH HALVES. best_roster used to be optimum.bySlot stored
  // raw, which is how the first-ever graded run read undefined for every
  // best-roster name. See shapeBestRoster in boardShape.js.
  const halves = buildEditionHalves(draw.teams, optimum);
  // THE SHAPE IS STORED WITH THE BOARD (migrations/103). A board is frozen in
  // every other respect - its twelve cards, its ceiling, its best roster - and
  // until now its SLOTS were not: they were a constant every reader imported
  // fresh, so changing the constant re-read every edition ever played. It is
  // written explicitly here, never defaulted, so a shape change is a change to
  // NEW boards only.
  const inserted = await sql`
    INSERT INTO daily_boards (edition_date, season_year, seed, board, ceiling, best_roster, slots, opens_at, closes_at, live_notify_at)
    VALUES (${editionDate}, ${season}, ${seed}, ${JSON.stringify(halves.board)}::jsonb,
            ${optimum.total}, ${JSON.stringify(halves.best_roster)}::jsonb,
            ${JSON.stringify(SLOTS)}::jsonb, ${opensAt}, ${closesAt}, ${liveNotifyAt})
