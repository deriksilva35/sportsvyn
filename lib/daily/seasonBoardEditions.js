...
export async function ensureBoardForDate(sql, editionDate) {
  ...
  const nextDate = new Date(`${editionDate}T00:00:00Z`);
  nextDate.setUTCDate(nextDate.getUTCDate() + 1);
  const closesAt = await easternLocalToUtc(`${nextDate.toISOString().slice(0, 10)} 00:00:00`);
  ...
  const inserted = await sql`
    ...
    VALUES (${editionDate}, ${season}, ${seed}, ${JSON.stringify(shapeTeams(draw.teams))}::jsonb,
            ${optimum.total}, ${JSON.stringify(optimum.bySlot)}::jsonb, ${opensAt}, ${closesAt})
