...
test('THE TWO SHAPES DISAGREE ABOUT A TE-LESS BOARD, and that is the whole point', () => {
  // THIS TEST USED TO ASSERT THE OPPOSITE, under the ruling that removed the
  // TE slot: "TE-less teams still complete a board". That ruling was superseded
  // by v2.0, which put the TE slot back for the 2002-2025 ranked set, so the
  // same board now has two honest answers depending on which shape it is
  // solved under - and a board carries its own shape (migrations/103).
  const teams = Array.from({ length: 8 }, (_, i) => team(`T${i}`, [
    { position: 'QB', points: 10 + i }, { position: 'RB', points: 8 + i },
    { position: 'WR', points: 6 + i }, { position: 'PK', points: 3 },
  ]));
  // The shape these teams were drawn for: two FLEX slots, no TE required.
  assert.equal(solveBoard(teams, LEGACY_SLOTS).ok, true,
    'no team here has any TE row, and under the legacy eight the board still completes');
  // The ranked shape asks for a tight end, and there is not one anywhere.
  const ranked = solveBoard(teams, SLOTS);
  assert.equal(ranked.ok, false);
  assert.match(ranked.reason, /feasible/);
});
