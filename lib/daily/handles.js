...
/**
 * A CLAIM OVER AN EXISTING HANDLE IS REFUSED, and this is the whole decision,
 * pure, so it is testable without a session or a database. Returns the
 * refusal to send back, or null when the claim may proceed.
 */
export function refuseClaimOverExisting(me) {
  if (!me || me.handle == null) return null;
  return {
    ok: false,
    reason: 'has_handle',
    handle: me.handle,
    message: `You already have a handle, @${me.handle}. It was not changed.`,
  };
}
...
export const RESERVED = new Set([
  ...
  // THE HOUSE PERSONAS, for the reason this list already exists. These five
  // are real accounts that play the games and appear on leaderboards labelled
  // as the house (lib/house/personas.js), so a player able to take the handle
  // could sit beside them looking exactly like one of them. Both spellings of
  // each, because the underscore is the only thing between them.
  // The_Book is reserved before it ships, which is the point of reserving.
  'the_chalk', 'thechalk', 'the_fade', 'thefade', 'the_book', 'thebook',
  'the_gut', 'thegut', 'the_homer', 'thehomer',
]);
