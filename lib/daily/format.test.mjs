...
test('ZERO AND ABSENT ARE ONE ANSWER (ruled): null, and the caller omits', () => {
  // THE TWO THE RULING NAMES.
  assert.equal(pctOfCeiling(5, 0), null);
  assert.equal(pctOfCeiling(5, null), null);
  // and every other way to have no ceiling
  for (const bad of [undefined, '0', -5, NaN, Infinity, 'x']) {
    assert.equal(pctOfCeiling(100, bad), null, String(bad));
  }
  // NEVER A DASH, NEVER "0%": the callers print nothing at all, which the
  // render tests on the results screen and the Yesterday line assert.
});
...
test('THE (pct, 1) PATH: a stored ratio against a ceiling of 1', () => {
  // daily_board_runs.pct already holds score/ceiling, so youCellV2, the lobby
  // row and the leaderboards pass a ceiling of 1.
  assert.equal(pctOfCeiling(1, 1), '100%');
  assert.equal(pctOfCeiling(0.9996, 1), '99.9%', 'a ratio that rounds to 100.0 is still held short');
  assert.equal(pctOfCeiling(0.9966260385475096, 1), '99.7%', "Derik's stored ratio");
  assert.equal(pctOfCeiling(null, 1), null, 'a DNF row has no ratio, and the caller omits');
});
