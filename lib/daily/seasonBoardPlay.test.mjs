...
import {
  ...
  filledCount, teamsLeft, pickOutcome, commitPick, startClock,
...
test('startClock refuses when userId is null, and the reason is signin', () => {
  const r = startClock(null);
  assert.equal(r.ok, false);
  assert.equal(r.reason, 'signin');
});

test('startClock allows a real userId', () => {
  const r = startClock(482);
