...
export function inferPosition(row) {
  ...
  // QB is only a CANDIDATE at 100+ pass attempts, and only WINS if passing
  // is still the largest of the three scoring components - unchanged from
  // before the amendment, since this gate is about identifying a real
  // starting quarterback, not about touches.
  const qbEligible = n(r.passAtt) >= 100 && pass > rush && pass > rec;
  ...
  const recCt = n(r.rec);
  if (rushAtt === 0 && recCt === 0) return null;
