...
test('handles render on the board, unclaimed accounts render as Player <hex>', async () => {
  ...
  for (const r of rows) {
    ...
    // A HANDLE OR 'Anonymous' - nothing else, and NO HEX SUFFIX (relay
    // 2b-fix item 3 retired the old "Player e740" pseudonym).
    assert.ok(
      r.name.startsWith('@') || r.name === 'Anonymous',
      `unexpected display name: ${r.name}`,
    );
    assert.equal(/^Player #?\d+$/.test(r.name), false, 'never a raw user id');
    assert.equal(/[0-9a-f]{4}/.test(r.name), false, 'no hex id on any surface');
  }
});
