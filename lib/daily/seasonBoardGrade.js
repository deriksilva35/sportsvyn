 ...
 * PURE, mutates nothing passed in. Returns { best, movedFrom } - `best` is
 * the permuted array (same 8 players, same total - a relabeling, not a
 * re-solve), `movedFrom[i]` is the ORIGINAL name that sat at index i before
 * any swap, so the caller can tell which rows actually moved.
 *
...
function permuteBest(mineArr, best, slots) {
  ...
  const movedFrom = out.map((b) => b.name);
...
export function gradeFromOptimum(play, teams, optimum, slots) {
  ...
  const { best, movedFrom } = permuteBest(mineArr, optArr, slots);

  const rows = mineArr.map((you, i) => {
    const b = best[i];
    const hit = b.name === you.name;
    const ahead = !hit && you.points > b.points;
    const moved = hit && movedFrom[i] !== you.name ? you.slot : null;
