...
// THIS PAIRING IS UNCHANGED BY THE DISPLAY ORDER BELOW - only where a row
// APPEARS moved, not which two players it compares.
//
// ROWS DISPLAY IN POSITION ORDER (ruling): QB, RB, RB, WR, WR, FLEX, FLEX,
// K - the same order as the roster row on the board, matched and swap rows
// interleaved by the SLOT INDEX your own pick occupies. Grouping by outcome
// was doing no work the verdict pill was not already doing; slotIndex (0-7,
// play.roster's own index, not the position NAME - two slots share 'RB' and
// two share 'FLEX', so the name alone cannot order them) is what makes the
// sort exact.
...
export function gradeBoard(play, teams, slots) {
  ...
  const mineArr = play.roster.map((r, slotIndex) => ({
    ...
    meta: r.pick.player.meta, slot: r.pos, slotIndex,
  }));
  ...
  })).sort((a, b) => (a.you?.slotIndex ?? Infinity) - (b.you?.slotIndex ?? Infinity));
