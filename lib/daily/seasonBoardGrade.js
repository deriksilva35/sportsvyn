...
export function gradeFromOptimum(play, teams, optimum, slots) {
  // AN EMPTY SLOT SCORES ZERO AND MATCHES NOTHING. The clock can submit a
  // partial roster; its holes are real rows here (name null), so the grade
  // still has eight rows and the best roster still pairs against all eight.
  const mineArr = play.roster.map((r, slotIndex) => (r.pick ? {
  ...
  } : { name: null, abbr: null, points: 0, meta: '', slot: r.pos, slotIndex, position: null }));
  ...
  const rows = mineArr.map((you, i) => {
    const b = best[i];
    const hit = you.name != null && b.name === you.name;
