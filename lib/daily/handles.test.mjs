...
test('ANON IS NOT THE USER ID, AND NOT A HEX SUFFIX EITHER (relay 2b-fix item 3)', () => {
  const n = anonName(37, 'secret');
  assert.equal(n, 'Anonymous');
  assert.equal(/37/.test(n), false, 'the id must not survive into the label');
  assert.equal(/[0-9a-f]{4}/.test(n), false, 'no hex id on any surface');
});

test('anon derives NOTHING from the id - two unclaimed accounts read alike', () => {
  // The deliberate cost of item 3's ruling, pinned so it reads as a choice
  // rather than a regression: the old HMAC suffix made an unclaimed rival
  // recognisable week to week, and that is exactly the id-shaped string the
  // ruling forbids. Claiming a handle is the fix, not a second pseudonym.
  assert.equal(anonName(37, 'k'), anonName(38, 'k'));
  assert.equal(anonName(37, 'keyA'), anonName(37, 'keyB'));
});

test('displayName prefers the handle and falls back cleanly', () => {
  assert.equal(displayName({ id: 9, handle: 'pylon' }), '@pylon');
  assert.equal(displayName({ id: 9, handle: null }), 'Anonymous');
