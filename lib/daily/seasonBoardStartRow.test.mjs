...
// PER-RUN MARKER (test-iso relay). These two sentinels used FIXED emails and
// FIXED handles - and users.handle carries its own unique index
// (idx_users_handle_lower, migration 066) - so two runs of this file did not
// race, they collided outright on the insert.
const MARK = fixtureMark('startrow');
...
before(async () => {
  ...
  await sql`DELETE FROM users WHERE email = ${MARK.email('one')}`;
  userId = (await sql`
    INSERT INTO users (email, handle) VALUES (${MARK.email('one')}, ${MARK.handle('one')})
    RETURNING id`)[0].id;
  // AND THE BOARD GETS THE SAME TREATMENT, for the same reason one line up.
  // EDITION is fixed (the reads under test look the board up by it) and
  // daily_boards.edition_date is unique, so a run that died before its teardown
  // left a row that failed the NEXT run's insert - and failed it in `before`,
  // which turns one orphan into nine red tests that all read as a code defect.
  // Runs first, then the board: daily_board_runs carries the FK.
  const stale = await sql`SELECT id FROM daily_boards WHERE edition_date = ${EDITION}`;
  ...
  boardId = (await sql`
    ...
    VALUES (${EDITION}, 2097, 'startrow-seed', ${JSON.stringify(BOARD)}::jsonb, 100,
            '[]'::jsonb, ${JSON.stringify(SLOTS)}::jsonb, ${OPENS}::timestamptz, ${CLOSES}::timestamptz)
    RETURNING id`)[0].id;
});
...
test('submit without a start is refused as never started, not silently inserted', async () => {
  ...
  await sql`DELETE FROM users WHERE email = ${MARK.email('two')}`;
  const other = (await sql`
    INSERT INTO users (email, handle) VALUES (${MARK.email('two')}, ${MARK.handle('two')})
    RETURNING id`)[0].id;
  try {
    const r = await submitRun(sql, {
...
// ---------------------------------------------------------------------------
// THREE MINUTES, ENFORCED ON THE SERVER (daily-clock). The started row from
// the first test above was stamped at OPENS; these submit against it.
// ---------------------------------------------------------------------------
test('1c. a submit at 3:06 after start is refused: time expired, and the row stays a DNF', async () => {
  const at = new Date(new Date(OPENS).getTime() + 186_000).toISOString();
  const r = await submitRun(sql, { boardId, userId, picks: [], elapsedS: 0, slots: SLOTS, now: at });
  assert.equal(r.ok, false);
  assert.equal(r.reason, 'time expired');
  assert.equal(r.status, 409);
  const [row] = await sql`SELECT picks FROM daily_board_runs WHERE board_id = ${boardId} AND user_id = ${userId}`;
  assert.equal(row.picks, null, 'picks stay NULL - a DNF, absent from every leaderboard');
});

test('1c. inside the round plus grace the time gate passes and shape is checked next', async () => {
  const at = new Date(new Date(OPENS).getTime() + 184_000).toISOString();
  const r = await submitRun(sql, { boardId, userId, picks: [], elapsedS: 0, slots: SLOTS, now: at });
  assert.equal(r.ok, false);
  assert.notEqual(r.reason, 'time expired', 'the gate let it through');
  assert.match(r.reason, /expected 8 picks/, 'it reached the grader');
});
