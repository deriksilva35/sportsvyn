...
//
// THE CLOCK IS THE GAME, and the clock is SERVER LAW. The 3:00 in the browser
...
// the tap and the row. Rejecting a lineup the player finished at 2:58 because
...
// not a seventh slot of thinking time. It is applied to the DEADLINE, never to
// the score.
//
// THE CLOCK WENT 2:00 -> 3:00 after the second playtest. The interface fixes
// (auto-advance, the denser pool) gave back time the UI had been eating, and
// with that time returned the round was still tight rather than comfortable -
// so the extra minute is a ruling about the GAME, not compensation for the
// interface. GRACE DID NOT MOVE: it covers a network, not a decision.

export const CLOCK_MS = 180_000;   // 3:00
...
/**
 * Where focus goes after a pick fills `from`: the next slot with nothing in it,
 * wrapping past the end, skipping everything already filled.
 *
 * PURE, and it lives here rather than in the component because it is a rule of
 * the round - "you are never left staring at a slot you have already filled" -
 * and the playtest showed the interface was eating clock without it.
 *
 * Returns `from` when the board is complete: with six of six filled there is
 * nowhere honest to move, and jumping somewhere arbitrary would be worse than
 * standing still.
 */
export function nextOpenSlot(from, lineup, slots = SLOTS) {
  const i = slots.indexOf(from);
  // Rotate so the search starts AFTER the current slot and wraps back to it.
  const order = i < 0 ? slots : [...slots.slice(i + 1), ...slots.slice(0, i + 1)];
  return order.find((s) => lineup?.[s] == null) ?? from;
}
