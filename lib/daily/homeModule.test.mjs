...
const {
  dailyHomeView, yesterdayView, editionNo, editionLabel, ENTRIES_FLOOR, DAILY_EPOCH,
...
test('THE MODULE HAS NO REVEALED STATE, because it could never reach one', () => {
  // This module asks about TODAY, and today closes at midnight ET - the same
  // instant the date rolls to tomorrow. getDay(today) can only return 'closed'
  // after the date it was asked about stopped being today. The revealed branch
  // that shipped here was unreachable by construction, and on 17 Aug edition
  // 001's answer appeared nowhere on the homepage as a result.
  assert.equal(dailyHomeView({ date: '2026-08-16', dayState: 'closed', day: DAY, entry: ENTRY }), null);
  assert.equal(dailyHomeView({ date: '2026-08-16', dayState: 'closed', day: DAY, entry: null }), null);
});
...
test('LEAK: NO state of this module may carry the answer, closed included', () => {
  ...
  // And a closed day is not this module at all any more - it is the strip.
  assert.equal(dailyHomeView({ date: '2026-08-16', dayState: 'closed', day: DAY, entry: wrongGuess }), null);
});


// ---------------------------------------------------------------------------
// YESTERDAY'S ANSWER - the strip
// ---------------------------------------------------------------------------

const YDAY = {
  puzzle_date: '2026-08-16', season_year: 2017, week: 12,
  revealed: true, perfect: { total: 169.5 },
};
const WINNER = { name: '@pylon', score: 115.2 };

test('YESTERDAY carries the answer, because a revealed day is public', () => {
  const v = yesterdayView({ day: YDAY, entry: null, winner: WINNER });
  assert.equal(v.season, 2017);
  assert.equal(v.week, 12);
  assert.equal(v.edition, '001');
  assert.equal(v.href, '/daily/2026-08-16');
});

test('YESTERDAY: a player who played gets their own result', () => {
  const v = yesterdayView({
    day: YDAY, winner: WINNER,
    entry: { locked_at: 'x', score: 115.2, guess_season: 2017, guess_week: 4, bonus_pct: 0.05 },
  });
  assert.equal(v.played, true);
  assert.equal(v.score, 115.2);
  assert.equal(v.tier, 'STARTER');
  assert.equal(v.pct, 68, '115.2 of 169.5 is 68%');
  assert.equal(v.seasonRight, true);
  assert.equal(v.weekRight, false);
});

test('YESTERDAY: someone who did not play gets the perfect as the yardstick', () => {
  const v = yesterdayView({ day: YDAY, entry: null, winner: WINNER });
  assert.equal(v.played, false);
  assert.equal(v.perfect, 169.5);
  assert.equal(v.score, undefined, 'no score to invent');
});

test('YESTERDAY: a DNF is not played - a zero would be a lie', () => {
  const v = yesterdayView({ day: YDAY, entry: { locked_at: null, score: null }, winner: WINNER });
  assert.equal(v.played, false);
});

test('YESTERDAY renders nothing before any day has revealed', () => {
  assert.equal(yesterdayView({ day: null }), null);
  assert.equal(yesterdayView({ day: { ...YDAY, revealed: false } }), null);
  assert.equal(yesterdayView(), null);
});

test('YESTERDAY tolerates a missing winner - a day can have no entries', () => {
  const v = yesterdayView({ day: YDAY, entry: null, winner: null });
  assert.equal(v.winner, null);
  assert.equal(v.perfect, 169.5, 'and still has an answer to show');
});
