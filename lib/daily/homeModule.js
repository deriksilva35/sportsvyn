...
//
// TWO STATES, DERIVED, NEVER PASSED IN:
...
//
// THERE IS NO 'revealed' STATE, AND THERE NEVER COULD HAVE BEEN. This module
// asks about TODAY, and today's row closes at midnight ET - the same instant
// todayEt() rolls to tomorrow. So getDay(today) can return 'closed' only after
// the date it was asked about has stopped being today. The revealed branch that
// shipped here was unreachable by construction: on 17 Aug, edition 002 showed
// its play state and edition 001's outcome appeared nowhere on the homepage.
// Yesterday's answer is a SEPARATE strip - yesterdayView() below - because it
// is a different question about a different day.
//
// NEITHER SHAPE CARRIES SEASON OR WEEK. Not as null - ABSENT - so a component
// cannot print them by accident and a test can assert on the serialized view.
// Only yesterdayView(), which describes a day that has already revealed, is
// allowed to carry the answer.
...
export function dailyHomeView({ date, dayState, day = null, entry = null, scores = [] } = {}) {
  ...
  // A closed day is not this module's business - see the header. Returning
  // null rather than rendering something is the honest answer: by the time a
  // day is closed, it is yesterday, and yesterday has its own strip.
  if (dayState === 'closed') return null;
...
/**
 * YESTERDAY'S ANSWER - the compact strip beneath today's board.
 *
 * ALWAYS PRESENT ONCE ANY DAY HAS REVEALED, because it is the recruiting
 * poster: it is the only place a passer-by sees that this game has a real
 * answer, real scores and other people playing it. It is deliberately NOT the
 * box score - three rows, muted, one link out. The reveal page is the stats
 * home and already carries all sixty-four rows.
 *
 * NO LEAK SURFACE. A revealed day is public by definition - it is the same
 * data /daily/[date] serves to anyone with the URL, and the share card puts on
 * a picture. That is why this strip may carry the season and week that the
 * today-module may not.
 *
 * @param {object} a
 * @param {object} a.day    the revealed puzzle_days row (board dropped)
 * @param {object} a.entry  the viewer's entry for THAT day, or null
 * @param {object} a.winner { name, score } from the day's board, or null
 */
export function yesterdayView({ day = null, entry = null, winner = null } = {}) {
  if (!day || !day.revealed) return null;
  const date = typeof day.puzzle_date === 'string'
    ? day.puzzle_date.slice(0, 10)
    : new Date(day.puzzle_date).toISOString().slice(0, 10);
  const perfect = Number(day.perfect?.total ?? 0) || null;

  const base = {
    date,
    edition: editionLabel(editionNo(date)),
    season: day.season_year ?? null,
    week: day.week ?? null,
    perfect,
    winner: winner?.name ? { name: winner.name, score: Number(winner.score) } : null,
    href: `/daily/${date}`,
  };

  // A player who never locked gets the perfect score as the yardstick instead
  // of their own - a DNF has no result to report and a zero would be a lie.
  if (!entry?.locked_at) return { ...base, played: false };
