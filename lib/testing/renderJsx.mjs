...
export async function renderJsxExport(filePath, exportName, props = {}) {
  ...
  // AUTOMATIC RUNTIME, NOT CLASSIC: it works whether or not the source file
  // itself imports React (Next's own convention is not to), and resolves
  // correctly here because - unlike a data: URL import, which cannot
  // resolve a bare "react/jsx-runtime" specifier at all (confirmed
  // empirically before settling on this file) - the compiled code below is
  // written to a real file under the project root, where node_modules
  // resolution works exactly as it would for any other file here.
