...
// Display-oriented helper for the homepage Sportsvyn-Now block. Returns
// every match currently in status='live' with team display fields joined
// in, ordered by kickoff_at. Returns [] when nothing is live — the
// caller renders the "Next Up" variant from the schedule reader instead
// of a fake live score.
export async function getCurrentLiveMatches() {
  return await sql`
    SELECT
      m.id, m.slug, m.kickoff_at, m.status,
      m.home_score, m.away_score,
      ht.name AS home_name, ht.abbreviation AS home_abbr, ht.flag_svg_path AS home_flag_svg,
      at.name AS away_name, at.abbreviation AS away_abbr, at.flag_svg_path AS away_flag_svg
    FROM matches m
