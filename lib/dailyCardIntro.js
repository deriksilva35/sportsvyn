...
export async function assembleEnvelope({ ptDay }) {
  const [wcFixtures, live, top5] = await Promise.all([
  ...
  // WC-only. Friendlies no longer enter the AI writer's envelope; an
  // empty slate (off-day) is the correct/honest input.
  const slate = wcFixtures;
