...
export async function readFixturesByPtDay({ leagueSlug, ptStart, ptEnd }) {
  const rows = await sql`
    SELECT
      ...
      m.home_penalties,
      m.away_penalties,
      m.metadata->'slot_home'->>'label' AS slot_home_label,
      m.metadata->'slot_away'->>'label' AS slot_away_label,
  ...
  return rows.map((r) => ({
    ...
    home: {
      ...
      label: r.slot_home_label,
      resolved: r.home_id != null,
    },
    away: {
      id: r.away_id,
      name: r.away_name,
      ...
      label: r.slot_away_label,
      resolved: r.away_id != null,
    },
  }));
}
