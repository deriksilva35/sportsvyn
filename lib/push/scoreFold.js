...
// THE FOLD IS A FOOTBALL RULE AND NOTHING ELSE. A touchdown and its try are one
// play scored twice; no other sport this product carries has that shape. A
// baseball inning can move the board by six, seven or eight runs and every one
// of those is a whole event that happened at once - held for ninety seconds and
// then announced as a touchdown, which is what this file did before it took a
// sport. Traced on 22 Sep: a 6-run inning HELD and timed out labelled
// "touchdown", and a 6-run inning followed by a 1-run single FOLDED THEM INTO
// ONE, seven runs announced once as a touchdown.
//
// SO THE SPORT GATES THE WHOLE MECHANISM, not the delta. Guessing from the
// delta ("nobody scores exactly 6 in baseball") is the same class of mistake:
// it is a coincidence about numbers standing in for a fact about a sport.
const FOLDS = new Set(['football']);

export function sportFolds(sport) {
  return FOLDS.has(String(sport ?? 'football'));
}
 ...
 */
export function onScore(pending, { delta, state, play = null, now = Date.now(), sport = 'football' } = {}) {
  const d = Number(delta);

  // A SPORT THAT DOES NOT FOLD EMITS IMMEDIATELY, ALWAYS. No hold is opened, an
  // inherited hold is flushed rather than stranded, and no delta is ever
  // labelled from the football table.
  if (!sportFolds(sport)) {
    const emit = [];
    if (pending) {
      emit.push({
        state: pending.state, kind: null, scorer: pending.play?.scorer ?? null,
        credit: pending.play?.credit ?? null, folded: false, timedOut: true,
      });
    }
    emit.push({
      state, kind: play?.kind ?? null, scorer: play?.scorer ?? null,
      credit: play?.credit ?? null, folded: false, timedOut: false,
    });
    return { pending: null, emit };
  }
