...
test('scoreKindLabel names every real delta', () => {
  assert.equal(scoreKindLabel(6), 'Touchdown');
  assert.equal(scoreKindLabel(7), 'Touchdown');
  assert.equal(scoreKindLabel(8), 'Touchdown');
...
test('a delta of 2 is a safety unless this team\'s last score was itself a touchdown', () => {
  assert.equal(scoreKindLabel(2), 'Safety');
  assert.equal(scoreKindLabel(2, { priorWasTouchdown: false }), 'Safety');
  assert.equal(scoreKindLabel(2, { priorWasTouchdown: true }), 'Two-point');
});

test('anything else gets no prefix - no name beats a guess', () => {
  for (const d of [0, 4, 5, 9, 10, -3, -7, null, undefined]) {
    assert.equal(scoreKindLabel(d), null, `delta ${d}`);
  }
});

test('the prefix rides the title only for a real "score" event with a real scoreline', () => {
  const withKind = pushPayload('score', { ...GAME, homeScore: 14, awayScore: 10, scoreKind: 'Touchdown' });
  assert.equal(withKind.title, 'Touchdown · NE 10, SEA 14');
  // Never on a different event, even if a caller mistakenly passed one.
  const wrongEvent = pushPayload('quarter', { ...GAME, homeScore: 14, awayScore: 10, period: 2, scoreKind: 'Touchdown' });
  assert.equal(wrongEvent.title, 'NE 10, SEA 14');
  // Never invented when the scores themselves are missing.
  const noScore = pushPayload('score', { ...GAME, homeScore: null, awayScore: null, scoreKind: 'Touchdown' });
