...
test('the prefix rides the title only for a real "score" event with a real scoreline', () => {
  // SCORE FIRST, EVENT SECOND (ruling R3). The kind used to LEAD the title;
  // the numbers are what survives a glance, so they do now and the word that
  // says why they moved follows them.
  const withKind = pushPayload('score', { ...GAME, homeScore: 14, awayScore: 10, scoreKind: 'SEA touchdown' });
  assert.equal(withKind.title, 'NE 10, SEA 14 · SEA touchdown');
...
test('a missing score never becomes a zero', () => {
  ...
  // Kickoff moved into the title, but a match with no scoreline has no title
  // to carry it, so it falls back to the body rather than vanishing.
...
test('ONE DEVICE TABLE, ONE REVOCATION PATH', () => {
  ...
  // THE STATEMENT MOVED to lib/push/tokenHealth.js in migration 106, because
  // the revoke now carries a strike count and three hand-written copies of
  // that arithmetic is how the paths drift. One revocation path is still the
  // rule - there is now literally one.
  assert.match(d, /await recordSend\(sql, d\.token, res\)/);
  assert.doesNotMatch(d, /DELETE FROM device_tokens/);
  const h = strip(src('lib/push/tokenHealth.js'));
  assert.match(h, /UPDATE device_tokens\s+SET strikes = strikes \+ 1, last_rejected_at = now\(\), revoked_at = now\(\)/);
...
test('R3: THE FIRST LINE SAYS WHAT THE ROW WILL DO', () => {
  ...
  // "OFF" IS NOT SILENCE WHEN A FLOOR IS UNDER IT (OFF IS RESET relay). With
  // red zone on, turning a game off returns it to the league row, so the line
  // says where the game goes rather than promising a silence it will not
  // deliver - the same rule that retired "Silenced by".
  assert.equal(summaryLine({ ...DEFAULTS, master: false }, { leagueFloor: true }), 'Back to red zone');
  assert.equal(summaryLine(OFF, { leagueFloor: true }), 'Back to red zone');
  assert.equal(summaryLine(null, { leagueFloor: true }), 'Back to red zone');
  // Every OTHER line is untouched by the floor: it only ever renames "Off".
  for (const p of [DEFAULTS, { ...OFF, master: true, final: true }, { ...OFF, master: true }]) {
    assert.equal(summaryLine(p, { leagueFloor: true }), summaryLine(p));
  }

  // In the JSX it is the first line of the signed-in body, from the read prefs
  // and the floor - both, now, and the floor comes from the same fetch.
  const t = strip(src('components/alerts/AlertBell.js'));
  const body = t.slice(t.indexOf('al-signin'), t.indexOf('al-row al-row--master'));
  assert.match(body, /<p className="al-summary" aria-live="polite">\{prefs \? summaryLine\(p, \{ leagueFloor \}\) : '\\u00a0'\}<\/p>/);
  assert.match(t, /setLeagueFloor\(Boolean\(j\.leagueFloor\)\)/, 'and the sheet reads it from the route');
});
