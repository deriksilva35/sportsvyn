...
import { PUSH_COPY, copyFor, renderCopy } from './copy.js';
...
test('renderCopy leaves an unfilled {key} literal rather than blanking it', () => {
  const c = renderCopy('weekly-reminder:217', {});
  assert.equal(c.body, 'One hour to lock. {n_set} of 6 set.');
});
...
test('draft-settled substitutes room_rank and the season rank separately', () => {
  const c = renderCopy('draft-settled:3', { week: 1, room_rank: '2nd', rank: 40, field: 214 });
  assert.equal(c.body, 'Week 1 is graded. 2nd in your room, 40 of 214.');
});
