...
const MARK = fixtureMark('strikes');   // per run (test-iso relay)
async function wipe() { await sql`DELETE FROM users WHERE email LIKE ${MARK.like}`; }
let USER;
// THE TOKEN IS THE PRIMARY KEY of device_tokens, so a constant is not a race
// but a straight collision between two runs. 64 hex-ish chars, run-scoped.
const TOK = (n) => `STRIKE${MARK.run.replace(/[^a-z0-9]/gi, '').toUpperCase()}${String(n).padStart(4, '0')}`
...
before(async () => {
  ...
  USER = (await sql`INSERT INTO users (name, email)
    VALUES ('StrikeTest', ${MARK.email('user')}) RETURNING id`)[0].id;
});
after(wipe);
