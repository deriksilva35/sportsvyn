...
const OPEN_ANNOUNCE_HOURS = 24;

/**
 * Which contests of one game_type are due their OPEN push right now:
 * opened, and still inside the announce window. CREATION IS IRRELEVANT
 * (ruling) - this asks the state, not "did this tick just make the row".
  ...
  return sql`
    SELECT id, locks_at FROM contests
     WHERE game_type = ${gameType}
       ...
       AND opens_at > ${new Date(now.getTime() - OPEN_ANNOUNCE_HOURS * 3_600_000).toISOString()}`;
}
