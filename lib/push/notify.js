 ...
 * just reported ok.
 *
 * room_rank is GONE (relay 1b): a ranked room's other 11 seats are AI-filled
 * draft_picks, not scored contest_entries, so there was never a real number
 * to put there - room_rank was flagged as an unresolved gap in the prior
 * relay's report and renderCopy() shipped the literal "{room_rank}" text.
 * The copy now says the user's own score against the whole season's real
 * entrants (rank/field, same shape as the Weekly's) instead.
 */
export async function notifyDraftSettled(contestIds) {
  ...
  for (const id of contestIds ?? []) {
    ...
    const recipients = entries.map((e, i) => ({
      userId: e.user_id,
      params: { week: c.week, pts: Number(e.score), rank: i + 1, field },
    }));
