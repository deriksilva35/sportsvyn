...
import { apnsConfig, sendToToken, alertPayload, gateReport } from './apns.js';
import { recordSend } from './tokenHealth.js';
import { copyFor, renderCopy, RenderCopyError } from './copy.js';
import { DAILY_V2_EPOCH } from '../daily/seasonBoardEditions.js';
import { maybeAlert } from '../pollers/alerts.js';
import { ordinal } from '../standings/view.js';
...
export async function notifyPersonalized(eventId, recipients) {
  ...
  let sent = 0; let gone = 0; let failed = 0; let noDevice = 0; let skipped = 0;
  for (const { userId, params } of recipients) {
    let copy;
    try {
      copy = renderCopy(eventId, params);
    } catch (err) {
      // NO BRACE EVER LEAVES renderCopy() (ruling): a missing param throws
      // instead of shipping "{room_rank}" to a lock screen. Skip just THIS
      // recipient's send - the others still go out - and alert once so the
      // gap gets fixed instead of quietly shipping broken copy forever.
      skipped += 1;
      const detail = err instanceof RenderCopyError ? { eventId: err.eventId, missing: err.missing } : null;
      await maybeAlert(sql, {
        source: SOURCE,
        subject: `[push] ${eventId} missing param for user ${userId}`,
        body: err.message,
        detail,
      }).catch(() => {});
      continue;
    ...
    for (const token of tokens) {
      ...
      // THE SAME LEDGER AND THE SAME STRIKE COUNT AS EVERY OTHER SEND LOOP.
      // This one is the personalised path - weekly/draft settled and their
      // reminders - and it is the loop most likely to be forgotten, because
      // its copy is per recipient and its shape looks different. It is not
      // different: one row per device, one recordSend, no private revoke.
      await sql`
        INSERT INTO push_sends (device_token, match_id, event_key, title, body, ok, status_code, error)
        VALUES (${token}, NULL, ${eventId}, ${copy.title ?? null}, ${copy.body ?? null},
                ${Boolean(r.ok)}, ${r.status ?? null}, ${r.reason ?? null})
        ON CONFLICT (device_token, event_key) DO NOTHING`.catch(() => {});
      await recordSend(sql, token, r);
      if (r.ok) { sent += 1; continue; }
      if (r.gone) { gone += 1; continue; }
      failed += 1;
    }
  }
  ...
  await sql`
    ...
    eventId, outcome: 'sent', sent, gone, failed, noDevice, skipped, recipients: recipients.length,
    host: cfg.host, env: cfg.sandbox ? 'sandbox' : 'production', topic: cfg.topic,
    gate: { armed: gate.armed, pemLines: gate.APNS_KEY.pemLines },
  })}::jsonb
     WHERE id = ${rowId}`;
  return { sent, gone, failed, noDevice, skipped };
}
...
export async function notifyEvent(eventId, { excludeMasterOff = false, params = null } = {}) {
  ...
  let copy;
  try {
    copy = params ? renderCopy(eventId, params) : copyFor(eventId);
  } catch (err) {
    // Thrown BEFORE the claim below, so an unfilled param never spends the
    // send-once claim - the next tick still gets a real shot once the
    // missing param is fixed, instead of the event being marked sent forever
    // over broken copy nobody actually received.
    const detail = err instanceof RenderCopyError ? { eventId: err.eventId, missing: err.missing } : null;
    await maybeAlert(sql, { source: SOURCE, subject: `[push] ${eventId} missing param`, body: err.message, detail }).catch(() => {});
    return { skipped: 'render error' };
  }
  ...
  for (const token of tokens) {
    ...
    // ONE ROW PER DEVICE, THE SAME SHAPE A GAME PUSH WRITES (migration 106).
    // This loop used to keep totals only, which is why three days of one
    // device receiving nothing read as "sent 49, failed 0" and could not be
    // investigated at all. match_id is NULL - a cron event has no game - and
    // push_sends_once is UNIQUE on (device_token, event_key), so the send-once
    // claim above is mirrored per device here for free.
    //
    // FAIL-SOFT, ALWAYS. A ledger that could throw would turn a bookkeeping
    // problem into a missed notification, which is the wrong way round.
    await sql`
      INSERT INTO push_sends (device_token, match_id, event_key, title, body, ok, status_code, error)
      VALUES (${token}, NULL, ${eventId}, ${copy.title ?? null}, ${copy.body ?? null},
              ${Boolean(r.ok)}, ${r.status ?? null}, ${r.reason ?? null})
    ...
    // THE STRIKE COUNT, through the same function the game path calls. A
    // rejected token is revoked AND counted, so register can refuse to revive
    // one that keeps dying; a success resets it to zero.
    await recordSend(sql, token, r);
  ...
  // THE HOST IS ON THE ROW, NOT IN AN ENV VAR NOBODY CAN READ AFTERWARDS.
  // Two runtimes send these - Vercel's crons and the droplet's poller - and
  // "which APNs did this go to" took an hour to answer the one night it
  // mattered. gateReport carries booleans and lengths only; no secret.
  const gate = gateReport();
  await sql`
    UPDATE sync_runs SET finished_at = now(), ok = true,
           summary = ${JSON.stringify({
    eventId, outcome: 'sent', sent, gone, failed, devices: tokens.length,
    host: cfg.host, env: cfg.sandbox ? 'sandbox' : 'production', topic: cfg.topic,
    gate: { armed: gate.armed, pemLines: gate.APNS_KEY.pemLines },
  })}::jsonb
     WHERE id = ${rowId}`;
 ...
 *
 * TAKES THE CREATOR'S OWN SUMMARY (relay 2c item 5), not a bare id - sport,
 * board number, game count and the lock time all come straight from
 * ensurePickemBoard()'s return, computed once at creation, rather than a
 * second read of the row this function would otherwise need to re-derive
 * board_number from. {sport} arrives already upper-cased ('NFL'/'CFB') -
 * a fixed short code reads naturally in caps the way a name would not.
 */
export async function notifyPickemOpen({ id: boardId, sport, boardNumber, games, locksAt }) {
  return notifyEvent(`pickem-open:${boardId}`, {
    params: { sport: String(sport).toUpperCase(), n: boardNumber, games, lock_local: etStamp(locksAt) },
  });
}
...
export async function notifyPickemReminder() {
  // ROLLING LOCK (R4): the reminder keys on the board's FIRST kickoff, not
  // locks_at (now the last kickoff). Same 2h band, same send-once key.
  const rows = await sql`
    SELECT c.id FROM contests c
     WHERE c.game_type = 'pickem' AND NOT c.settled
       AND c.opens_at <= now()
       AND (SELECT min((g->>'kickoff_at')::timestamptz) FROM jsonb_array_elements(c.board) g) > now()
       AND (SELECT min((g->>'kickoff_at')::timestamptz) FROM jsonb_array_elements(c.board) g) <= now() + interval '2 hours'`;
  const out = [];
...
/**
 * Which contests of one game_type are due their OPEN push right now:
 * opened, and not yet locked. CREATION IS IRRELEVANT (ruling) - this asks
 * the state, not "did this tick just make the row". The right edge of the
 * window is the CONTEST'S OWN locks_at, not a fixed hours constant - a
 * fixed cutoff would silently stop announcing a contest that is still
 * genuinely open (the Weekly/Draft's own window runs well past a day), and
 * locks_at is already the moment the contest becomes useless to announce.
 * The send-once claim itself lives entirely inside notifyEvent(), exactly
 * as pickem-open's own; this function only answers the time-window half,
 * which is why it is exported and tested on its own - a contest whose
 * opens_at has not arrived yet must never be selected, no matter how early
 * ensureWeek() happened to create the row.
 */
export async function dueOpenContests(gameType, { now = new Date() } = {}) {
  return sql`
    SELECT id, locks_at FROM contests
     WHERE game_type = ${gameType}
       AND opens_at <= ${now.toISOString()}
       AND locks_at > ${now.toISOString()}`;
}

/**
 * The Weekly OPEN - called by the weekly-board route on EVERY run, not only
 * the one that creates (ruling: "creation is irrelevant"). A daily cron
 * inside a 24h window guarantees exactly one tick ever finds a contest both
 * inside the window and unclaimed, so this is safe to call unconditionally;
 * notifyEvent's own send-once claim is what actually decides.
 */
export async function notifyWeeklyOpen() {
  const rows = await dueOpenContests('weekly');
  const out = [];
  for (const c of rows) out.push(await notifyEvent(`weekly-open:${c.id}`, { params: { lock_local: etStamp(c.locks_at) } }));
  return out;
}
...
export async function notifyDraftOpen() {
  const rows = await dueOpenContests('draft');
  const out = [];
  for (const c of rows) out.push(await notifyEvent(`draft-open:${c.id}`, { params: { lock_local: etStamp(c.locks_at) } }));
  return out;
}
...
export async function notifyWeeklyReminder() {
  // ROLLING LOCK (R4): the hour-out reminder keys on the week's FIRST
  // kickoff (matches, REG, the contest's season/week), not locks_at.
  const rows = await sql`
    SELECT c.id FROM contests c
     WHERE c.game_type = 'weekly' AND NOT c.settled AND c.opens_at <= now()
       AND (SELECT min(m.kickoff_at) FROM matches m JOIN leagues l ON l.id = m.league_id WHERE l.slug = c.sport AND m.season_year = c.season_year AND m.week = c.week AND m.season_phase = 'REG') > now() + interval '45 minutes'
       AND (SELECT min(m.kickoff_at) FROM matches m JOIN leagues l ON l.id = m.league_id WHERE l.slug = c.sport AND m.season_year = c.season_year AND m.week = c.week AND m.season_phase = 'REG') <= now() + interval '60 minutes'`;
  const out = [];
  for (const c of rows) {
...
/**
 * THE LAST GAMES KICK (R4): one reminder on the final day, in the same
 * [45, 60]-minute band before locks_at - which is now the week's last kickoff.
 * Personalised with the count still open so the copy can say "N of 6 set".
 */
export async function notifyWeeklyLastKick() {
  const rows = await sql`
  ...
  for (const c of rows) {
    ...
    out.push(await notifyPersonalized(`weekly-lastkick:${c.id}`, recipients));
  }
  return out;
}
...
export async function notifyWeeklySettled(contestIds) {
  ...
  for (const id of contestIds ?? []) {
    ...
    if (!c) continue;
    // pct COMES FROM meta, NOT RECOMPUTED: lib/weekly/settle.js already wrote
    // it, frozen against the exact ceiling stored in the same settle - a
    // second independent computation here could only ever match by
    // coincidence, or drift if either formula ever changed alone.
    const entries = await sql`
      SELECT user_id, score, meta FROM contest_entries
       WHERE contest_id = ${id} AND score IS NOT NULL
       ORDER BY score DESC, user_id ASC`;
    const field = entries.length;
    const recipients = entries.map((e, i) => ({
      userId: e.user_id,
      params: { week: c.week, pts: Number(e.score), pct: e.meta?.pct ?? null, rank: i + 1, field },
    }));
 ...
 * just reported ok.
 *
 * room_rank IS BACK (relay 2b item 1): it was dropped in relay 1b because a
 * ranked room's other 11 seats were AI-filled draft_picks, not scored
 * contest_entries, so there was no real number to put there. Settlement now
 * scores every seat the same way it scores the user's own (lib/draft/room.js)
 * and stores the result on entry.meta.room - read here, not recomputed, so
 * the copy can never drift from what settle actually wrote.
 */
export async function notifyDraftSettled(contestIds) {
  ...
  for (const id of contestIds ?? []) {
    ...
    const recipients = entries.map((e, i) => ({
      userId: e.user_id,
      params: {
        week: c.week, pts: Number(e.score), rank: i + 1, field,
        room_rank: e.meta?.room?.rank != null
          ? `${ordinal(e.meta.room.rank)} of ${e.meta.room.of}`
          : '-',
      },
    }));
