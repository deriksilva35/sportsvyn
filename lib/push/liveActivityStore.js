...
/**
 * IS THIS READER'S OWN ACTIVITY RUNNING FOR THIS GAME?
 *
 * The switch in the Alerts sheet has to come back ON when the page is
 * reopened, and the only durable record of a running Activity is this table -
 * the app posts it here at start (registerActivity) and stamps ended_at at
 * end. Nothing on the web side keeps that state, and nothing should: the
 * Activity outlives the page that started it.
 *
 * SCOPED TO THE READER, unlike liveActivitiesFor()'s per-match fan-out read.
 * A NULL user_id row belongs to nobody in particular (the column is nullable
 * so a deleted account does not cascade a running Activity away), and a
 * switch must not come back on because a stranger's Activity is running.
 */
export async function viewerActivityFor(sql, { matchId, userId }) {
  if (userId == null) return false;
  const rows = await sql`
    SELECT 1 FROM live_activities
     WHERE match_id = ${Number(matchId)} AND user_id = ${Number(userId)}
       AND ended_at IS NULL AND revoked_at IS NULL
     LIMIT 1`;
...
/**
 * WHICH TRANSITIONS END A CARD.
 *
 *   final -> 'end'      anything else -> null or 'update'
 *
 * THE 'update' RETURN IS VESTIGIAL AND THE CALLER NO LONGER READS IT (LIVE
 * ACTIVITY - THE LIVE LINE relay). It encoded the Part C ruling - score or
 * quarter only, roughly 15-20 pushes a game - which was right for a card
 * carrying six fields and wrong for one carrying nine. A card that shows
 * possession, the down and the last play goes stale on a play that scores
 * nothing and ends no quarter, which is most plays. The poller decides an
 * update by comparing the CARD'S OWN CONTENT across ticks
 * (services/live-poller/poll.mjs), which is the only definition that cannot
 * drift from what the card says.
 *
 * THE FINAL RULING SURVIVES INTACT, and it is why this function survives: a
 * poll that sees the last score and the whistle together sends a single 'end'
 * carrying the final score, not an update chased by an end. That is a
 * statement about the transition list, which is what this function reads.
 */
...
export async function pushLiveActivities(sql, {
  ...
  // sentIds CARRIES WHO WAS ACTUALLY PUSHED, not how many. The count alone
  // cannot answer "how many pushes has THIS card taken in the last hour", which
  // is the question the cadence ledger exists to answer and the one nothing in
  // this system could answer before: live_activities stores updated_at and no
  // history at all.
  const out = { matchId: matchId ?? null, event, activities: 0, sent: 0, failed: 0, revoked: 0, skipped: 0, sentIds: [] };
  ...
  for (const r of rows) {
    ...
    if (res?.ok) {
      out.sent += 1;
      out.sentIds.push(r.activity_id);
