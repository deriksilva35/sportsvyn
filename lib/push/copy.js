 ...
 * carry tokens. A key with no matching param is left as the literal
 * "{key}" text rather than silently blanked, so a caller that forgot a
 * param sees it immediately in the rendered string instead of a quiet gap.
...
export function renderCopy(eventId, params = {}) {
  ...
  return { ...copy, title: fill(copy.title), body: fill(copy.body) };
}
