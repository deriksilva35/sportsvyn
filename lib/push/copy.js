...
import { DAILY_V2_PATH, dailyResultsPath } from '../daily/boardShape.js';
...
export const PUSH_COPY = {
  ...
  'daily-revealed': {
    ...
    // YESTERDAY'S RESULTS, NEVER TODAY'S BOARD. The event id carries the
    // edition ('daily-revealed:2026-09-08'); the tap lands on that edition's
    // results route. It used to be DAILY_V2_PATH, which at 00:01 ET is the
    // NEW board's rules card - a push about a settled board that opened
    // tomorrow's clock instead.
    urlFor: (edition) => dailyResultsPath(edition),
    url: DAILY_V2_PATH,
  },
  ...
  // TWO SPORTS, ONE TEMPLATE (relay 2c item 5). {sport} arrives already
  // upper-cased ('NFL'/'CFB') from notifyPickemOpen's own call - a fixed
  // two-or-three-letter sport code reads naturally in caps the way a name
  // would not, so the substitution does the casing rather than the template.
  'pickem-open': {
    title: "Pick'em {sport} is open",
    body: "Pick'em {sport} Board {n} is open · {games} games · first lock {lock_local}",
  ...
  'weekly-reminder': {
    title: 'First kickoff in an hour',
    body: 'First kickoff in an hour. {n_set} of 6 set - each slot locks at its player\'s kickoff.',
    url: '/weekly',
  },
  'weekly-lastkick': {
    title: 'Last games kick in an hour',
    body: 'Last games kick in an hour. {n_set} of 6 set - the week closes at kickoff.',
    url: '/weekly',
  },
  'weekly-settled': {
  ...
  'draft-settled': {
    ...
    body: 'Week {week} is graded. {room_rank} in your room, {rank} of {field}.',
    url: '/draft',
  },
  // OCTOBER. Two sentences because a DNF is not a score: "you got 0" is true
  // and useless, and the reader needs to know it was an empty slot rather
  // than a bad day, because the fix is different.
  'october-settled': {
    title: 'Your five are in',
    body: '{day} is graded. {pts} pts, {rank} of {field}. Best card {best}.',
    url: '/october',
  },
  // THE RUN. Open is one sentence for everyone; the nudge and the result are
  // per reader. A DNF gets its own template for the same reason October's
  // does - "you scored 0" is true and useless.
  'run-open': {
    title: 'The Run · {round} is open',
    body: 'Set your nine - two arms, seven bats, from the clubs still alive. Each club locks at its first pitch.',
    url: '/run',
  },
  'run-reminder': {
    title: 'One day to the first lock',
    body: '{n_set} of {of} set. Each club locks at its first pitch of the round.',
    url: '/run',
  },
  'run-settled': {
    title: 'Your {round} round is in',
    body: '{pts} pts, {rank} of {field}. Best nine {best}.',
    url: '/run/board',
  },
  'run-dnf': {
    title: 'The Run · DNF',
    body: 'No slot on your nine was filled, so {round} does not count. Best nine {best}.',
    url: '/run/board',
  },
  'october-dnf': {
    title: 'October · DNF',
    body: 'A slot was empty at first pitch on {day}, so it does not count. Best card {best}.',
    url: '/october',
  },
};
...
export function copyFor(eventId) {
  const [prefix, ...rest] = String(eventId ?? '').split(':');
  const entry = PUSH_COPY[prefix];
  if (!entry) return null;
  // An entry with urlFor derives its url from the event id's suffix (the
  // edition date); the static url stays as the fallback for a bare prefix.
  // IDENTITY FOR EVERY ENTRY WITHOUT urlFor - callers and tests compare the
  // returned template to PUSH_COPY[prefix] itself. Only a keyed entry with a
  // suffix yields a derived copy, and that copy drops urlFor.
  const suffix = rest.join(':');
  if (!entry.urlFor || !suffix) return entry;
  const { urlFor, ...copy } = entry;
  return { ...copy, url: urlFor(suffix) };
}
