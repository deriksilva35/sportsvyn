...
// OFF IS RESET: a red-zone subscriber who SILENCES one game. master=false on
// the match row, every other switch left as it was.
const uLeagueMuted = await mkUser('league-muted');
await mkDevice(uLeagueMuted, 'league-muted');
await rz(uLeagueMuted);
await sql`
  INSERT INTO alert_prefs (user_id, scope, scope_id, master, kickoff, score, quarter, close, final_only)
  VALUES (${uLeagueMuted}, 'match', ${matchId}, false, true, true, true, true, true)`;

// AND THE READER WITH NOTHING UNDERNEATH: a silenced match row, no follow, no
// league row. Off must still mean off for them.
const uMutedAlone = await mkUser('muted-alone');
await mkDevice(uMutedAlone, 'muted-alone');
await sql`
  INSERT INTO alert_prefs (user_id, scope, scope_id, master, kickoff, score, quarter, close, final_only)
  VALUES (${uMutedAlone}, 'match', ${matchId}, false, true, true, true, true, true)`;

// A FOLLOWER WHO MUTES A GAME. Follows the home team, no team row, no league
// row, match row master=false. Their mute must be a mute.
const uFollowerMuted = await mkUser('follower-muted');
await mkDevice(uFollowerMuted, 'follower-muted');
await sql`INSERT INTO user_team_follows (user_id, team_id) VALUES (${uFollowerMuted}, ${homeId})`;
await sql`
  INSERT INTO alert_prefs (user_id, scope, scope_id, master, kickoff, score, quarter, close, final_only)
  VALUES (${uFollowerMuted}, 'match', ${matchId}, false, true, true, true, true, true)`;
...
// ---------------------------------------------------------------------------
// "OFF" IS RESET, NOT EXCLUSION
// ---------------------------------------------------------------------------

test('A RED-ZONE SUBSCRIBER WHO MUTES A GAME STILL GETS ITS SCORES AND FINAL', async () => {
  // THE DEFECT THIS CLOSES, from 20 Sep: with red zone on, a reader tapped ten
  // games off in two minutes - meaning "stop the per-game noise" - and every
  // silenced row then suppressed red zone for that game as well. He lost the
  // whole 1pm slate and the poller read audience=2 eligible=0 all afternoon.
  const rows = await audienceFor(sql, match);
  const m = rows.find((r) => r.userId === uLeagueMuted);
  assert.ok(m, 'a muted match row must not remove them from the audience');
  assert.equal(m.prefs.source, 'league', 'the silenced row is absent for precedence');
  assert.equal(wants(m.prefs, 'score'), true, 'the game returns to the league floor');
  assert.equal(wants(m.prefs, 'final'), true, 'and so does its final');
});

test('OFF STILL MEANS OFF WITH NOTHING UNDERNEATH', async () => {
  // The other half of the rule, and the one that would be a disaster to get
  // wrong: no team row, no follow, no league row. A silenced match row is the
  // only claim there is, so it applies - turning a game off must never turn it
  // back on to DEFAULTS.
  const rows = await audienceFor(sql, match);
  const m = rows.find((r) => r.userId === uMutedAlone);
  assert.ok(m, 'they are still in the audience - the row exists');
  assert.equal(m.prefs.source, 'match');
  assert.equal(m.prefs.master, false);
  for (const e of ['kickoff', 'score', 'quarter', 'close', 'final']) {
    assert.equal(wants(m.prefs, e), false, `${e} must stay silent`);
  }
});

test('ONLY A ROW\'S OWN SWITCHES OVERRIDE THE FLOOR', () => {
  const lp = { master: true, kickoff: false, score: true, quarter: false, close: false, final: true };
  // master=false is a reset: the floor applies.
  const reset = resolvePrefs({
    matchPref: { master: false, kickoff: true, score: true, quarter: true, close: true, final: true },
    leaguePref: lp,
  });
  assert.equal(reset.source, 'league');
  assert.equal(wants(reset, 'score'), true);

  // score=false with master ON is a statement about THIS GAME, and it stands.
  const muted = resolvePrefs({
    matchPref: { master: true, kickoff: false, score: false, quarter: false, close: false, final: true },
    leaguePref: lp,
  });
  assert.equal(muted.source, 'match', 'a live row still wins whole');
  assert.equal(wants(muted, 'score'), false, 'no scores for this game, red zone or not');
  assert.equal(wants(muted, 'final'), true, 'and its own final switch still applies');
});

test('THE FALL-THROUGH ORDER: a SAVED ROW only - team, then league, and never a follow', () => {
  const lp = { master: true, kickoff: false, score: true, quarter: false, close: false, final: true };
  const off = { master: false, kickoff: true, score: true, quarter: true, close: true, final: true };
  const tp = { master: true, kickoff: true, score: false, quarter: false, close: false, final: false };

  // A team row is the next claim down from a silenced match row.
  const toTeam = resolvePrefs({ matchPref: off, teamPref: tp, leaguePref: lp });
  assert.equal(toTeam.source, 'team');
  assert.equal(wants(toTeam, 'score'), false, "the team row's own switches apply");

  // Then the league floor.
  const toLeague = resolvePrefs({ matchPref: off, leaguePref: lp });
  assert.equal(toLeague.source, 'league');
  assert.equal(wants(toLeague, 'kickoff'), false, "the floor's own switches, not DEFAULTS");

  // AND NEVER TO A FOLLOW'S DEFAULTS. A follower who mutes one of their team's
  // games means mute; falling through to DEFAULTS would make OFF louder than
  // what it replaced, and leave them no way to silence a followed game at all.
  // With no saved row underneath, the silenced row stands.
  const followerMute = resolvePrefs({ matchPref: off });
  assert.equal(followerMute.source, 'match');
  assert.equal(followerMute.master, false);
  for (const e of ['kickoff', 'score', 'quarter', 'close', 'final']) {
    assert.equal(wants(followerMute, e), false, `${e} must stay silent`);
  }
});


test('A FOLLOWER WHO MUTES A GAME GETS NOTHING - no team row, no league row', () => {
  // THE NARROWING, END TO END. This reader follows the home team, so they are
  // in the audience by the follow arm; they saved no team row and have no
  // league floor. Their match row says master=false, and nothing may fall
  // through to DEFAULTS underneath it - a mute that gets louder is not a mute.
  return audienceFor(sql, match).then((rows) => {
    const m = rows.find((r) => r.userId === uFollowerMuted);
    assert.ok(m, 'the follow still admits them to the audience');
    // 'both': the follow arm AND the match arm each admit them - the match arm
    // does not check master, because being in the audience and wanting the
    // event are different questions and wants() answers the second.
    assert.equal(m.via, 'both');
    assert.equal(m.prefs.source, 'match', 'the silenced row stands - nothing is below it');
    assert.equal(m.prefs.master, false);
    for (const e of ['kickoff', 'score', 'quarter', 'close', 'final']) {
      assert.equal(wants(m.prefs, e), false, `${e} must not be sent`);
    }
  });
});
