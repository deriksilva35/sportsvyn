...
test('THE FALL-THROUGH ORDER: team, then follow-default, then league', () => {
  const lp = { master: true, kickoff: false, score: true, quarter: false, close: false, final: true };
  ...
  // With no team row, a FOLLOW is still a claim, and it means DEFAULTS.
  const toFollow = resolvePrefs({ matchPref: off, followed: true });
  assert.equal(toFollow.source, 'default');
  assert.equal(wants(toFollow, 'kickoff'), true);

  // With neither, the league floor.
