...
import {
  parseRegister, parseEnd, registerActivity, endActivity, viewerActivityFor,
  liveActivitiesFor, revokeActivity, pushLiveActivities, activityEventFor,
...
after(async () => {
  ...
  // THIS RUN'S ROWS, NOT THE WHOLE PREFIX - the same correction the rider's
  // teardown already carries (test-iso relay), applied here because this file
  // still had the old shape and it bit: a run killed by the droplet's memory
  // reaper left two sentinel-la-<stamp> users behind, and every LATER run of
  // this file then failed its own teardown on rows it had never created.
  // Deleting by NS and asserting on NS makes the check about the run making
  // it.
  //
  ...
  // sibling's row.
  await sql`DELETE FROM live_activities WHERE activity_id LIKE 'sentinel-la-%'
              AND created_at < now() - interval '1 hour'`;
  await sql`DELETE FROM users WHERE email LIKE 'sentinel-la-%'
              AND created_at < now() - interval '1 hour'`;
  const [left] = await sql`
    SELECT (SELECT count(*)::int FROM live_activities WHERE activity_id LIKE ${`${NS}%`}) AS acts,
           (SELECT count(*)::int FROM users WHERE email LIKE ${`${NS}%`}) AS users`;
  assert.equal(left.acts, 0, 'this run\'s activities left behind');
  assert.equal(left.users, 0, 'this run\'s users left behind');
});
...
// ---------------------------------------------------------------------------
// THE CADENCE (Part C ruling)
// ---------------------------------------------------------------------------

test('a score or a quarter moves the card; a poll on its own does not', () => {
  assert.equal(activityEventFor([{ event: 'score' }]), 'update');
  assert.equal(activityEventFor([{ event: 'quarter' }]), 'update');
  assert.equal(activityEventFor([]), null);
  assert.equal(activityEventFor(), null);
});

test('KICKOFF AND CLOSE DO NOT MOVE IT', () => {
  // A kickoff is 0-0 with no clock worth showing; 'close' is a judgement about
  // a game, not a change to any of the six. Either would spend a push on a
  // card that looks identical afterwards.
  assert.equal(activityEventFor([{ event: 'kickoff' }]), null);
  assert.equal(activityEventFor([{ event: 'close' }]), null);
  assert.equal(activityEventFor([{ event: 'kickoff' }, { event: 'close' }]), null);
});

test('THE FINAL IS ONE OF THE STREAM, not on top of it', () => {
  // A poll that sees the last score and the whistle together sends a single
  // end carrying the final score - not an update chased by an end.
  assert.equal(activityEventFor([{ event: 'score' }, { event: 'final' }]), 'end');
  assert.equal(activityEventFor([{ event: 'final' }]), 'end');
  assert.equal(activityEventFor([{ event: 'quarter' }, { event: 'score' }, { event: 'final' }]), 'end');
});

test('two card-worthy transitions in one poll are still one push', () => {
  assert.equal(activityEventFor([{ event: 'score' }, { event: 'quarter' }]), 'update');
});
...
test('one update goes to every live Activity on the match, once each', async () => {
  ...
  // ALL TEN CROSS THE WIRE. This is the payload the phone actually decodes,
  // so it is pinned in full.
...
test('a match with no Activities is zero work and no error', async () => {
  ...
  // sentIds JOINED THE SHAPE (LA-CADENCE). The counts alone cannot say WHICH
  // cards were pushed, which is what the hourly ledger needs; an empty match
  // returns an empty list, not a missing key, so a caller can always iterate it.
  assert.deepEqual(out, {
    matchId: -1, event: 'update', activities: 0, sent: 0, failed: 0, revoked: 0, skipped: 0, sentIds: [],
  });
});


// ---------------------------------------------------------------------------
// THE SWITCH'S OWN READ (LIVE ACTIVITY DOOR relay)
// ---------------------------------------------------------------------------

test('viewerActivityFor: the reader\'s own running Activity, and nobody else\'s', async () => {
  // THIS BLOCK OWNS THE MATCH FOR THE LENGTH OF IT. The tests above leave
  // Activities running on the same fixture match on purpose - that is what
  // they are testing - so an assertion about "nothing running" has to start
  // from a state it created, not from whatever the file happened to leave.
  await sql`DELETE FROM live_activities WHERE match_id = ${matchId}`;
  assert.equal(await viewerActivityFor(sql, { matchId, userId }), false, 'nothing running yet');

  await registerActivity(sql, { activityId: A1, pushToken: TOKEN, userId, matchId, startedAt: new Date() });
  assert.equal(await viewerActivityFor(sql, { matchId, userId }), true);

  // SCOPED TO THE READER. A switch that came back on because a stranger's
  // Activity was running would offer to end an Activity this reader never
  // started - and liveActivitiesFor()'s per-match read would do exactly that.
  assert.equal(await viewerActivityFor(sql, { matchId, userId: otherUserId }), false);
  assert.equal(await viewerActivityFor(sql, { matchId: -1, userId }), false, 'another game');

  // ENDED IS OFF. The app stamps ended_at when the Activity ends, so a
  // reopened page must not show a switch for a card that is gone.
  await endActivity(sql, { activityId: A1, userId });
  assert.equal(await viewerActivityFor(sql, { matchId, userId }), false);
});

test('viewerActivityFor: a revoked Activity is off, and a signed-out reader is off', async () => {
  await sql`DELETE FROM live_activities WHERE match_id = ${matchId}`;
  await registerActivity(sql, { activityId: A2, pushToken: TOKEN2, userId, matchId, startedAt: new Date() });
  assert.equal(await viewerActivityFor(sql, { matchId, userId }), true);
  await revokeActivity(sql, A2);
  assert.equal(await viewerActivityFor(sql, { matchId, userId }), false, '070s revocation path counts too');
  // No reader, no query at all - the route never reaches the table for a
  // signed-out visitor.
  assert.equal(await viewerActivityFor(sql, { matchId, userId: null }), false);
});
