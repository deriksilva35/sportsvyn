...
export function pushPayload(event, {
  ...
  leagueSlug, slug, scoreKind = null, scorer = null,
  ...
  //
  // THE SCORER IS NAMED WHERE THE PLAY GAVE ONE (ruling R3, item b). A safety
  // names nobody by design - the text is about the team that conceded it, and
  // the two points go the other way - so the comma clause is simply absent
  // rather than filled with a guess.
  let suffix = null;
  if (event === 'kickoff') suffix = 'Kickoff';
  else if (event === 'score' && scoreKind) suffix = scorer ? `${scoreKind}, ${scorer}` : scoreKind;
