...
test('copyFor keys on the prefix and refuses the unknown', () => {
  ...
  // daily-revealed DERIVES ITS URL FROM THE EDITION (the Daily has a
  // yesterday): the template is the source, the returned copy is the
  // template with url pointed at that edition's results, and urlFor never
  // leaks into the payload.
  {
    const c = copyFor('daily-revealed:2026-08-19');
    assert.equal(c.title, PUSH_COPY['daily-revealed'].title);
    assert.equal(c.body, PUSH_COPY['daily-revealed'].body);
    assert.equal(c.url, '/daily/board/2026-08-19');
...
test("the three Pick 'em pushes land on the board, and their callers exist", () => {
  ...
  assert.match(board, /await notifyPickemOpen\(res\.summary\)\.catch/,
    'creation announces, once per sport, on the fire that created');
  ...
  // ROLLING LOCK (R4): the band is measured from the board's FIRST kickoff,
  // not locks_at (which is now the last kickoff).
  assert.match(notify, /min\(\(g->>'kickoff_at'\)::timestamptz\) FROM jsonb_array_elements\(c\.board\) g\) > now\(\)/);
  assert.match(notify, /min\(\(g->>'kickoff_at'\)::timestamptz\) FROM jsonb_array_elements\(c\.board\) g\) <= now\(\) \+ interval '2 hours'/);
  assert.doesNotMatch(notify.slice(notify.indexOf('notifyPickemReminder'), notify.indexOf('notifyPickemSettled')), /locks_at > now\(\)/, 'the reminder no longer reads locks_at');
...
test('the ledger claim is written before any token is read or sent', () => {
  ...
  // The token fan-out itself was extracted into liveDeviceTokens() (relay
  // 5b item 3, so v2's tick can filter and inspect it independently) -
  // this looks for the CALL SITE inside notifyEvent, not the query text,
  // which now lives in that function's own body instead.
  const fanout = body.indexOf('liveDeviceTokens(');
...
test('a dead token is revoked, not deleted', () => {
  // THE RULE IS UNCHANGED; ITS HOME MOVED. The revoke used to be written out
  // in each send loop - dispatch.js once, notify.js twice - and migration 106
  // added a strike count that all three have to maintain identically. Three
  // copies of that arithmetic is how they drift, so the statement now lives in
  // lib/push/tokenHealth.js recordSend() and every loop calls it. Revoked and
  // never deleted is still the decision: a dead endpoint is not a withdrawn
  // consent, and revoked_at is the only evidence of which it was.
  const h = stripComments(src('lib/push/tokenHealth.js'));
  assert.match(h, /SET strikes = strikes \+ 1, last_rejected_at = now\(\), revoked_at = now\(\)/);
  assert.doesNotMatch(h, /DELETE FROM device_tokens/);
  const t = stripComments(src('lib/push/notify.js'));
  assert.match(t, /await recordSend\(sql, token, r\)/, 'and notify.js routes through it');
