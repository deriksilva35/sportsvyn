...
test('EXACTLY THE SIX FIELDS, and a seventh does not get through', () => {
  const s = contentState({
    awayAbbr: 'IND', awayScore: 7, homeAbbr: 'KC', homeScore: 14,
    period: 'Q2', clock: '1:39',
    possession: 'KC', redZone: true, // not in the contract
  });
  assert.deepEqual(Object.keys(s).sort(), ['awayAbbr', 'awayScore', 'clock', 'homeAbbr', 'homeScore', 'period']);
  assert.deepEqual(s, { awayAbbr: 'IND', awayScore: 7, homeAbbr: 'KC', homeScore: 14, period: 'Q2', clock: '1:39' });
});
