...
import { contentState, stateFromMatch, liveLine, afterSnap, gameUrlFor, SITE_ORIGIN } from './liveActivityState.js';
...
test('stateFromMatch of nothing THROWS under test - a missing league is never quietly football', () => {
  // 24 Sep: the poller built every MLB card update with no leagueSlug, and
  // the card said "Q3". Under node --test a caller that forgets the league
  // fails its own suite.
  assert.throws(() => stateFromMatch(undefined), /no leagueSlug/);
  assert.throws(() => stateFromMatch({ ...GAME, leagueSlug: undefined }), /no leagueSlug/);
});

test('...and in production it WARNS and still builds the ten', () => {
  ...
  try {
    ...
    process.env.NODE_TEST_CONTEXT = ctx;
    console.warn = warn;
  }
  assert.equal(said.length, 1);
  assert.match(said[0], /no leagueSlug/);
});
...
// A PLAIN PLAY WITH ITS NUMBERS. The card derives the state AFTER it
// (afterSnap), so every fixture carries the yards it gained: 3rd & 7 at the
// IND 34, eight yards gained, is 1st & 10 at the IND 26.
const play = (o = {}) => ({
  driveNumber: 1, playNumber: 1, period: 2, clock: '1:39',
  down: 3, distance: 7, yardsToGoal: 34, yardsGained: 8, offenseTeamId: 48056,
  playType: 'pass-reception', text: 'Mahomes pass complete to Kelce for 8 yards', ...o,
});
...
test('the three fields, from the plays the game page reads', () => {
  ...
  // THE STATE AFTER THE PLAY: 3rd & 7 at the IND 34, eight gained - a first
  // down at the IND 26. The defence's half, so the spot wears their letters.
  assert.equal(line.situation, '1st & 10 · IND 26');
...
test('A STOPPAGE IS NOT A SNAP, which is the whole reason there are two readers', () => {
  ...
  const line = liveLine({
    plays: [
      play({ playNumber: 1, playType: 'rush', down: 2, distance: 6, yardsToGoal: 41, yardsGained: 5, text: 'Hunt run for 5' }),
      play({ playNumber: 2, playType: 'timeout', down: 4, distance: 1, yardsToGoal: 0, yardsGained: 0, text: 'Timeout KC' }),
    ],
    homeTeamId: 48056, teamAbbr: ABBR,
  });
  assert.equal(line.situation, '3rd & 1 · IND 36', 'after the last REAL play, not the timeout\'s numbers');
...
test('goal to go says G, and midfield says 50', () => {
  // A first down inside the 10 is goal to go: 3rd & 5 at the 14, six gained.
  const g = liveLine({ plays: [play({ down: 3, distance: 5, yardsToGoal: 14, yardsGained: 6 })], homeTeamId: 48056, teamAbbr: ABBR });
  assert.equal(g.situation, '1st & G · IND 8');
  // Short of it, still goal to go: 1st & G at the 6, two gained.
  const g2 = liveLine({ plays: [play({ playType: 'rush', down: 1, distance: 6, yardsToGoal: 6, yardsGained: 2 })], homeTeamId: 48056, teamAbbr: ABBR });
  assert.equal(g2.situation, '2nd & G · IND 4');
  const mid = liveLine({ plays: [play({ down: 2, distance: 10, yardsToGoal: 60, yardsGained: 10 })], homeTeamId: 48056, teamAbbr: ABBR });
  assert.equal(mid.situation, '1st & 10 · 50');
});

test('the AWAY side on offence wears its own letters, and the spot wears the home side\'s', () => {
  const line = liveLine({
    plays: [play({ offenseTeamId: 48074, yardsToGoal: 22, yardsGained: 3 })],
    homeTeamId: 48056, teamAbbr: ABBR,
  });
  assert.equal(line.possession, 'IND');
  assert.equal(line.situation, '4th & 4 · KC 19');
});
...
test('THE LINE RIDES stateFromMatch, so one card is built one way', () => {
  ...
  assert.deepEqual(s, {
    awayAbbr: 'IND', awayScore: 7, homeAbbr: 'KC', homeScore: 14, period: 'Q2', clock: '1:39',
    possession: 'KC', situation: '1st & 10 · IND 26',
...
// ---------------------------------------------------------------------------
// THE STATE AFTER THE LAST PLAY (LIVE CARD LINE FIXES relay)
// ---------------------------------------------------------------------------
//
// ATL at GB, 24 Sep. The card printed "1st & 14 · ATL 4" under the run that
// had just gone to the ATL 13 - the play row's own SNAP, the state before it.
// These are the recon's rows, verbatim in their numbers (PROD, match 21571,
// drive 17), with GB home.

const ATL = 48082; const GB = 48071;
const GBABBR = new Map([[GB, 'GB'], [ATL, 'ATL']]);
const ROW1 = { driveNumber: 17, playNumber: 1, period: 3, clock: '0:55', down: 1, distance: 10, yardsToGoal: 92,
  yardsGained: -4, offenseTeamId: ATL, playType: 'penalty', scoring: false,
  text: 'Bi.Robinson left guard to ATL 17 for 9 yards (E.Williams).PENALTY on ATL, Offensive Holding, 10 yards' };
const ROW2 = { driveNumber: 17, playNumber: 2, period: 3, clock: '0:28', down: 1, distance: 14, yardsToGoal: 96,
  yardsGained: 9, offenseTeamId: ATL, playType: 'rush', scoring: false,
  text: 'Bi.Robinson left end to ATL 13 for 9 yards (E.Cooper; E.Williams).' };
const END_Q3 = { driveNumber: null, playNumber: 135, period: 3, clock: '0:00', down: 2, distance: 5, yardsToGoal: 87,
  yardsGained: 0, offenseTeamId: ATL, playType: 'end-period', scoring: false, text: 'END QUARTER 3' };
const gb = (plays) => liveLine({ plays, homeTeamId: GB, teamAbbr: GBABBR });

test('AFTER ROW 2 THE CARD READS "2nd & 5 · ATL 13"', () => {
  const line = gb([ROW1, ROW2]);
  assert.equal(line.situation, '2nd & 5 · ATL 13');
  assert.equal(line.lastPlay, ROW2.text);
  assert.equal(line.possession, 'ATL');
  // THE QUARTER BREAK INSIDE A HALF CARRIES THE BALL OVER. The Q3 end row had
  // landed by the 02:37:18 tick; Q4 opened at exactly 2nd & 5 at the ATL 13.
  assert.equal(gb([ROW1, ROW2, END_Q3]).situation, '2nd & 5 · ATL 13');
});

test('AFTER THE PENALTY ROW (row 1) THE SITUATION IS EMPTY - empty beats stale', () => {
  const line = gb([ROW1]);
  assert.equal(line.situation, '');
  assert.equal(line.lastPlay, ROW1.text, 'the words still say what happened');
});

test('afterSnap: plain plays only, from the numbers, never the text', () => {
  const p = (o) => ({ down: 2, distance: 7, yardsToGoal: 40, yardsGained: 3, playType: 'rush', scoring: false, ...o });
  assert.deepEqual(afterSnap(p()), { down: 3, distance: 4, yardsToGoal: 37 });
  assert.deepEqual(afterSnap(p({ playType: 'pass-incompletion', yardsGained: 0 })), { down: 3, distance: 7, yardsToGoal: 40 });
  assert.deepEqual(afterSnap(p({ playType: 'sack', yardsGained: -8 })), { down: 3, distance: 15, yardsToGoal: 48 });
  assert.deepEqual(afterSnap(p({ playType: 'pass-reception', yardsGained: 12 })), { down: 1, distance: 10, yardsToGoal: 28 });
  // CFB files the same four types in words.
  assert.deepEqual(afterSnap(p({ playType: 'Pass Reception', yardsGained: 12 })), { down: 1, distance: 10, yardsToGoal: 28 });
  assert.deepEqual(afterSnap(p({ playType: 'Rush' })), { down: 3, distance: 4, yardsToGoal: 37 });
  // Nothing else is derived.
  for (const t of ['penalty', 'punt', 'kickoff', 'field-goal-good', 'pass-interception-return',
    'fumble-recovery-opponent', 'rushing-touchdown', 'timeout', 'Penalty', 'Punt']) {
    assert.equal(afterSnap(p({ playType: t })), null, t);
  }
  assert.equal(afterSnap(p({ scoring: true })), null, 'a score');
  assert.equal(afterSnap(p({ down: 4, yardsGained: 2 })), null, 'short on 4th is the ball going the other way');
  assert.deepEqual(afterSnap(p({ down: 4, distance: 2, yardsGained: 2 })), { down: 1, distance: 10, yardsToGoal: 38 });
  assert.equal(afterSnap(p({ yardsToGoal: 5, yardsGained: 5, distance: 5 })), null, 'across the goal line');
  assert.equal(afterSnap(p({ yardsToGoal: 97, yardsGained: -4 })), null, 'behind its own - a safety');
  assert.equal(afterSnap(p({ yardsGained: null })), null, 'no stored yards, no derivation');
  assert.equal(afterSnap(p({ down: null })), null);
  // THE TEXT IS NEVER READ: a play whose words say "touchdown" but whose
  // numbers say a 3-yard run is a 3-yard run.
  assert.deepEqual(afterSnap(p({ text: 'TOUCHDOWN' })), { down: 3, distance: 4, yardsToGoal: 37 });
});

test('THE END OF A HALF ENDS THE SITUATION; a quarter break inside one does not', () => {
  const run = { ...ROW2, period: 2, clock: '0:04' };
  const half = { ...END_Q3, period: 2, playType: 'end-of-half' };
  assert.equal(gb([run, half]).situation, '', 'the second half starts with a kickoff');
  assert.equal(gb([run, { ...half, playType: 'end-period' }]).situation, '', 'an end-period row closing Q2 too');
  assert.equal(gb([{ ...ROW2, period: 4 }, { ...END_Q3, period: 4, playType: 'end-of-regulation' }]).situation, '');
  assert.equal(gb([{ ...ROW2, period: 1 }, { ...END_Q3, period: 1 }]).situation, '2nd & 5 · ATL 13');
});
