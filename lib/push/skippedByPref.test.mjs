...
// counted, journaled outcome, not silence (SF at LAR, 10 Sep: one final-only
...
// one iOS device whose match pref is master on, score OFF, the final on
const AUDIENCE_ROW = {
  token: 'tok-1', user_id: 1, platform: 'ios', endpoint: null, p256dh: null, auth: null, via_follow: false, via_match: true,
  t_master: null, m_master: true, m_kickoff: true, m_score: false, m_quarter: true, m_close: true, m_final_only: true,
};
...
test('audience 1 with score off on a score: eligible 0, sent 0, skippedByPref 1, the summary line emitted', async () => {
  drainPushCounts();
  ...
  // the same row with score ON is eligible and sent - the counter stays put. final_only
  // stays true: it is the Final trigger now (R2), not a suppressor, so it silences nothing
  const sql2 = fakeSql([{ ...AUDIENCE_ROW, m_score: true }]); const lines2 = []; const sent2 = [];
...
test('the poller hands its logger to dispatch, and the heartbeat carries skippedByPref', () => {
  ...
  // The dispatch call moved into sendOne() when the score fold landed - one
  // call site for every event the rider sends, including a folded touchdown
  // and a timed-out hold - and the logger still rides it.
  assert.match(poll, /const sendOne = async \(event, state\) => \{\s*const r = await dispatch\(sql, \{ match, event, state, log \}\);/);
  assert.match(poll, /dispatch\(sql, \{\s*match: p\.match,[\s\S]*?log,\s*\}\)/, 'the timeout sweep logs too');
  // THE LOGGER, NOT THE ARGUMENT LIST. This pinned the exact neighbouring
  // arguments and broke when MLB added `enrich` between them - a true
  // assertion failing for a false reason. What has to hold is that index
  // hands pollOnce its own logger rather than letting it default to a no-op,
  // which is what made a whole class of push failures silent.
  assert.match(index, /pollOnce\(sql, \{[\s\S]*?\blog,[\s\S]*?\}\)/, 'index passes its logger');
  assert.match(index, /normalise: lg\.normalise/);
  // AND THE PER-LEAGUE ENRICHMENT RIDES TOO. MLB's outs and count come from a
  // second call that only its registry entry knows how to make; dropping this
  // argument would leave every baseball card without a count and nothing would
  // fail.
  assert.match(index, /enrich: lg\.enrich \?\? null/);
