...
/**
 * THE PASSER, on a play that has one.
 *
 * A passing touchdown names two men and the push should say both - "P.Mahomes
 * to R.Rice" is the sentence a reader wants, and the receiver alone loses who
 * threw it. The passer is the name immediately before the word that makes it a
 * pass: "pass" in the NFL grammar, "pass complete" in the CFB one.
 *
 * NULL ON EVERYTHING ELSE. A run, a return and a recovery have one man in
 * them, and inventing a second would be worse than naming one.
 */
export function passerOf(text) {
  const t = clean(text);
  const at = t.search(/\bpass(?:\s+complete)?\b/i);
  if (at < 0) return null;
  const before = t.slice(0, at);
  const names = [...before.matchAll(new RegExp(NAME, 'g'))].map((m) => m[1]);
...
// A PLAY THE DEFENCE TOOK THE BALL ON. Any of these words means the man who
// scored took it FROM the passer rather than from him, so the passer is not
// part of the credit.
const TURNOVER = /\bintercepted\b|\binterception\b|\bFUMBLES?\b|\bfumbled\b|RECOVERED by/i;

/** The yardage on a field goal, as a number. */
function fgYards(text) {
  const m = /(\d+)\s+yard field goal/i.exec(String(text ?? ''))
    ?? /field goal attempt from (\d+)/i.exec(String(text ?? ''));
  return m ? Number(m[1]) : null;
}

function kickerOf(text) {
  const m = FG.exec(String(text ?? '')) ?? FG_CFB.exec(String(text ?? ''));
  return m ? m[1] : null;
}

/**
 * THE CREDIT LINE - who did it, in the words the body will print.
 *
 *   passing touchdown   "P.Mahomes to R.Rice"
 *   rushing touchdown   "D.Henry"
 *   return / recovery   "D.Knight"        (one man reached the end zone)
 *   field goal          "H.Butker 42 yards"
 *   safety              null              (its text names the team that
 *                                          CONCEDED it; the points go the
 *                                          other way, and naming anybody here
 *                                          would name the wrong side)
 *
 * NULL IS A REAL ANSWER and every caller renders it as nothing rather than as
 * a gap. A push with no credit line is a push that did not know, which is the
 * truth on a play we could not read.
 */
export function creditOf(kind, text) {
  if (kind === 'field goal') {
    const who = kickerOf(text);
    if (!who) return null;
    const yds = fgYards(text);
    return yds == null ? who : `${who} ${yds} yards`;
  }
  if (kind !== 'touchdown') return null;
  const scorer = scorerOf(text);
  if (!scorer) return null;

  // A PASS THE DEFENCE CAUGHT IS NOT A PASS TO THE SCORER. "C.Gonzales pass
  // intercepted by N.Cull ... N.Cull return 76 yards ... TOUCHDOWN" has the
  // word `pass` in it and a name before that word, and crediting it as
  // "C.Gonzales to N.Cull" reads as a touchdown THROWN to Cull - the exact
  // opposite of what happened to Gonzales. The same is true of a fumble a
  // defender returned. One man reached the end zone on those plays and he is
  // the only one the credit may name.
  if (TURNOVER.test(text)) return scorer;

  const passer = passerOf(text);
  // The passer must not also be the scorer - a quarterback who ran it in is
  // one man, and "P.Mahomes to P.Mahomes" is how that would otherwise read.
  return passer && passer !== scorer ? `${passer} to ${scorer}` : scorer;
}

/**
 * Read one `plays` row.
 * @param {object} play  {play_type, text, home_score, away_score, period, clock}
 * @returns {null|{kind, scorer, credit, tryResolved, tryKind, tryPoints, homeScore, awayScore}}
...
export function parseScoringPlay(play) {
  ...
  if (TD_WORD.test(text)) {
    ...
    return {
      ...
      credit: creditOf('touchdown', text),
  ...
  if (SAFETY_WORD.test(text) || type === 'safety') {
    return { kind: 'safety', scorer: null, credit: null, tryResolved: false, tryKind: null, tryPoints: null, ...scores };
  }

  if (/field goal/i.test(text) && !/no good|missed|blocked/i.test(text)) {
    const m = FG.exec(text) ?? FG_CFB.exec(text);
    return {
      kind: 'field goal', scorer: m ? m[1] : null, credit: creditOf('field goal', text),
      tryResolved: false, tryKind: null, tryPoints: null, ...scores,
    };
  }
