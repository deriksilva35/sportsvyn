...
// Exactly these, no more, until both sides agree a seventh. They are built in
// liveActivityState.js and re-exported here, so the push path and the bridge
// path cannot drift into two answers.
//
...
// THE SIX LIVE IN liveActivityState.js, not here. The bridge path needs them
// in a client bundle that cannot carry node:http2, and two builders would be
// two answers to what the card says. Re-exported so every existing importer
// of contentState keeps working.
import { contentState } from './liveActivityState.js';

export { contentState, stateFromMatch, gameUrlFor } from './liveActivityState.js';
