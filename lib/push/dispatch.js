...
export async function audienceFor(sql, match) {
  const rows = await sql`
    SELECT DISTINCT ON (d.token)
      ...
      FROM (
        -- FOLLOWERS OF EITHER SIDE. team_id travels along so the team-scoped
        -- pref below can be looked up per the specific team followed.
        SELECT f.user_id, f.team_id FROM user_team_follows f
         WHERE f.team_id IN (${match.home_team_id}, ${match.away_team_id})
        UNION
        -- A SAVED MATCH ROW IS SUFFICIENT ON ITS OWN. team_id is NULL here
        -- on purpose - this entry point carries no team-scoped pref, only
        -- the match-scoped one joined below.
        SELECT ap.user_id, NULL::integer AS team_id
          FROM alert_prefs ap
...
export async function dispatch(sql, { match, event, state = {}, senders = null, log = () => {} }) {
  ...
  for (const d of audience) {
    ...
    const claimed = await sql`
      INSERT INTO push_sends (device_token, match_id, event_key, title, body)
