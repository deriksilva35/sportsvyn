...
export function wants(prefs, event) {
  ...
  // THE FINAL SENDS ON MASTER ALONE (ruling). It used to ride the `quarter`
  // toggle, on the theory that "end of each quarter and the final" was one
  // idea to a reader. It is not: the final is the ONE moment everybody who
  // subscribed to a game wants, and quarter ends are a detail some people
  // want on top. Bundling them meant the default pref set - which has
  // quarter off - silently withheld the result of the game from anybody who
  // reached the audience through a team follow, and 20729 on 5 Sep got
  // sixteen score alerts and no final.
  //
  // BEFORE final_only, deliberately. final_only means "only the final", and
  // this returning true first is what makes that read correctly rather than
  // depending on the branch below.
  if (event === 'final') return true;

  // final_only IS A SUPPRESSOR: everything that is not the final is off.
  if (p.final_only) return false;

  switch (event) {
    case 'kickoff': return Boolean(p.kickoff);
    case 'score': return Boolean(p.score);
    // QUARTER ENDS ONLY, now that the final has its own rule.
