...
export function resolvePrefs({
  teamPref = null, matchPref = null, leaguePref = null, followed = false, scope = null,
} = {}) {
  ...
  // about the game.
  //
  // AND IT FALLS THROUGH ONLY WHEN THERE IS SOMETHING TO FALL TO. With no
  // team row, no follow and no league row, a silenced match row still means
  // off - otherwise turning a game off would turn it back on to DEFAULTS,
  // which is the opposite of what the tap said. `followed` names the
  // follow-default claim, which is a claim without being a row.
  const silenced = Boolean(matchPref) && !matchPref.master;
  const hasLower = Boolean(teamPref) || Boolean(leaguePref) || Boolean(followed);
