...
import { BASEBALL, sportOf } from '../live/vocabulary.js';
...
export function resolvePrefs({
  teamPref = null, matchPref = null, leaguePref = null, scope = null,
} = {}) {
  // MOST SPECIFIC WINS: match, then team, then follow-default, then league.
  // Each is a WHOLE row, not a field-by-field merge: a reader who turns score
  // off for one game must not have it turned back on by something they set in
  // March.
  //
  // "OFF" IS A RESET, NOT AN EXCLUSION. A match row with master=false is
  // treated as ABSENT here, and the next claim down applies. This is the case
  // that cost a reader the whole 1pm slate on 20 Sep: with red zone on, he
  // tapped ten games off in two minutes - meaning "stop the per-game noise" -
  // and each silenced row then suppressed red zone for that game too. Muting
  // one game is not a statement about the league floor under it. ONLY A ROW'S
  // OWN SWITCHES override that floor: score=false on a match row still means
  // no scores for that game, red zone or not, because that is a statement
  // about the game.
  //
  // AND IT FALLS THROUGH ONLY TO A SAVED ROW - a team row, or the league floor.
  // NOT TO A FOLLOW'S DEFAULTS, deliberately: a follower who mutes one of
  // their team's games means mute, and falling through to DEFAULTS would make
  // OFF louder than what it replaced and leave them no way to silence a
  // followed game at all. A follow is a claim on the GAME (it is why they are
  // in the audience) but it is not a claim about what this row should say.
  //
  // So with no team row and no league row, a silenced match row still means
  // off - which is also what stops "turn a game off" from turning it back ON
  // to DEFAULTS, the opposite of what the tap said.
  const silenced = Boolean(matchPref) && !matchPref.master;
  const hasLower = Boolean(teamPref) || Boolean(leaguePref);
  const effectiveMatch = silenced && hasLower ? null : matchPref;

  if (effectiveMatch) return { ...DEFAULTS, ...strip(effectiveMatch), source: 'match' };
  if (teamPref) return { ...DEFAULTS, ...strip(teamPref), source: 'team' };
  if (leaguePref) return { ...DEFAULTS, ...strip(leaguePref), source: 'league' };
...
/**
 * BASEBALL'S CLOSE GAME IS A DIFFERENT SENTENCE: within one run, eighth inning
 * or later. There is no clock to be under five minutes of, and eight points is
 * a blowout in a sport where six runs is a rout - a football rule applied to
 * baseball would have fired on every game in the fourth and never fired at the
 * moment it exists for.
 *
 * ONE RUN, NOT TWO, because a one-run game in the eighth is the definition of
 * the thing a reader wants to be told about; and EIGHTH OR LATER rather than
 * ninth, because the eighth is when a bullpen decides it.
 */
export const CLOSE_MLB_MAX_DIFF = 1;
export const CLOSE_MLB_MIN_INNING = 8;
...
export function isCloseGame({ period, clock, homeScore, awayScore, sport = null } = {}) {
  const h = Number(homeScore), a = Number(awayScore);
  if (!Number.isFinite(h) || !Number.isFinite(a)) return false;

  // THE SPORT DECIDES THE SENTENCE, and it is named by LEAGUE SLUG because that
  // is what every caller holds. An absent sport is football, which is what this
  // function meant before baseball existed - so every existing caller keeps its
  // answer to the letter.
  if (sportOf(sport) === BASEBALL) {
    const inning = Number(period);
    if (!Number.isFinite(inning) || inning < CLOSE_MLB_MIN_INNING) return false;
    return Math.abs(h - a) <= CLOSE_MLB_MAX_DIFF;
  }
