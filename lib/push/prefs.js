 ...
 * follows."
 */
export function resolvePrefs({ teamPref = null, matchPref = null, leaguePref = null, scope = null } = {}) {
  // MOST SPECIFIC WINS: match, then team, then league (NFL RED ZONE ruling).
  // A league row is the FLOOR, never an override - a reader who turned one
  // game off has turned that game off, red zone or not, and a reader who set
  // their team's alerts did so knowing more than "every NFL game" does. Each
  // is a WHOLE row, not a field-by-field merge, for the reason above: a
  // reader who turns score off for one game must not have it turned back on
  // by something they set in March.
