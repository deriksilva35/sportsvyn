...
import { pushWarn } from '../push/warn.js';
...
/**
 * A MISSING ABBREVIATION IS NO LONGER FATAL (defect 1). It used to return
 * null here, which killed the Wire event AND - via pushPayload's identical
 * guard - every push for that match, silently. 105 of 243 CFB teams have
 * no abbreviation, so this was not an edge case. The caller may pass
 * `away`/`home` team objects to enable the short_name/derived fallbacks;
 * a bare string abbr still works exactly as before.
 */
export function scoreHeadline({
  awayAbbr, awayScore, homeAbbr, homeScore, liveState, away = null, home = null, matchId = null,
}) {
  ...
  if (a.source !== 'abbreviation' || h.source !== 'abbreviation') {
    pushWarn('abbrFallback', { matchId, detail: `away=${a.source} home=${h.source}` });
  }
  // Only a team with NO name at all still refuses - there is genuinely
  // nothing to print.
  if (!a.value || !h.value) {
    pushWarn('headlineNull', { matchId, detail: 'no abbreviation, short_name or name' });
    return null;
  }
  if (points(awayScore) == null || points(homeScore) == null) {
    pushWarn('headlineNull', { matchId, detail: 'score null' });
    return null;
  }
  const line = `${a.value} ${points(awayScore)}, ${h.value} ${points(homeScore)}`;
...
export function toScoreRow(m, liveState) {
  const headline = scoreHeadline({
    ...
    // The fallback chain needs more than the abbr column - see teamAbbr.js.
