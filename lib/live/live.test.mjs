...
import { cadence, sleepUntilNext, kickoffDelta, afterPoll, LIVE_SEC, IDLE_SEC } from './cadence.js';
...
test('an idle loop sleeps toward the pre-kick edge, never past it, never longer than the idle interval', () => {
  // The slate is read from our own table, which other writers change, so an
  // idle tick never sleeps longer than IDLE_SEC on one reading; and it never
  // sleeps PAST the pre-kick edge either - SF-LAR, 10 Sep: the 00:25:00 tick
  // read the slate a few hundred ms before the edge, slept 300 s and opened
  // the window five minutes late. Idle sleep = min(IDLE_SEC, max(1 s,
  // nextKickoff - now - PRE_KICK_MIN)).
  const far = cadence([{ status: 'scheduled', kickoffAt: '2026-09-14T02:00:00Z' }], NOW);
  assert.equal(sleepUntilNext(far, NOW), IDLE_SEC, 'a kickoff hours away: the idle interval, not a nine-hour sleep');
  // kickoff at T+40m -> edge at T+30m -> capped at 300 s
  assert.equal(sleepUntilNext(cadence([{ status: 'scheduled', kickoffAt: '2026-09-13T18:40:00Z' }], NOW), NOW), 300);
  // kickoff at T+12m -> edge at T+2m -> sleeps exactly 120 s, and the tick that wakes at the edge is pre-kick
  const t12 = cadence([{ status: 'scheduled', kickoffAt: '2026-09-13T18:12:00Z' }], NOW);
  assert.equal(t12.state, 'idle'); assert.equal(sleepUntilNext(t12, NOW), 120, 'sleeps 2m');
  const atEdge = new Date(NOW.getTime() + 120_000);
  assert.equal(cadence([{ status: 'scheduled', kickoffAt: '2026-09-13T18:12:00Z' }], atEdge).state, 'pre-kick', 'opens at exactly k-10m00s');
  // a few hundred ms short of the edge (the 10 Sep case): sleeps the remainder, not 300
  assert.equal(sleepUntilNext(cadence([{ status: 'scheduled', kickoffAt: '2026-09-13T18:10:00.700Z' }], NOW), NOW), 1, 'the last second, then the edge');
  // two kickoffs -> the earliest wins
  const two = cadence([{ status: 'scheduled', kickoffAt: '2026-09-13T19:00:00Z' }, { status: 'scheduled', kickoffAt: '2026-09-13T18:15:00Z' }], NOW);
  assert.equal(two.nextKickoffAt, '2026-09-13T18:15:00.000Z'); assert.equal(sleepUntilNext(two, NOW), 300, 'edge at T+5m -> 300 (= IDLE_SEC exactly)');
  const two2 = cadence([{ status: 'scheduled', kickoffAt: '2026-09-13T19:00:00Z' }, { status: 'scheduled', kickoffAt: '2026-09-13T18:13:00Z' }], NOW);
  assert.equal(sleepUntilNext(two2, NOW), 180, 'earliest kickoff at T+13m -> 3m');
  // no slate ahead -> the idle interval; a live loop never defers
  assert.equal(sleepUntilNext(cadence([], NOW), NOW), IDLE_SEC);
  assert.equal(sleepUntilNext(cadence([{ status: 'live' }], NOW), NOW), 30, 'a live loop never defers');
  assert.equal(kickoffDelta('2026-09-13T18:12:00Z', NOW), 'k-12m00s'); assert.equal(kickoffDelta('2026-09-13T17:56:20Z', NOW), 'k+3m40s'); assert.equal(kickoffDelta(null, NOW), 'k-?');
});
...
test('AFTER A POLL FLIPS A GAME LIVE, THE NEXT SLEEP IS THE LIVE ONE (CHC @ BOS, 25 Sep: live 21:38:53Z, next write 21:43:56Z)', () => {
  const now = new Date('2026-09-25T21:38:50Z');
  const opts = { preKickMin: 240, preKickSec: 300 };
  // the slate read BEFORE the poll: one game scheduled, first pitch 21:30
  const before = cadence([{ status: 'scheduled', kickoffAt: '2026-09-25T21:30:00Z' }], now, opts);
  assert.equal(before.state, 'pre-kick'); assert.equal(before.sleepSec, 300);
  const after = afterPoll(before, { wentLive: 1 }, opts);
  assert.equal(after.state, 'live'); assert.equal(sleepUntilNext(after, now), LIVE_SEC);
  // nothing flipped: unchanged
  assert.equal(afterPoll(before, { wentLive: 0 }, opts), before);
  // already live, or capped: unchanged - the cap outranks
  const live = { state: 'live', sleepSec: LIVE_SEC, liveCount: 2 };
  assert.equal(afterPoll(live, { wentLive: 1 }), live);
  const capped = applyCap(before, 10 ** 9, 'cfb');
  assert.equal(capped.capped, true);
  assert.equal(afterPoll(capped, { wentLive: 1 }, opts), capped);
  // a league's own live interval is honoured
  assert.equal(afterPoll(before, { wentLive: 1 }, { liveSec: 15 }).sleepSec, 15);
});

test('the poller counts the rows a poll turned live and hands the count to afterPoll before it sleeps', () => {
  const REPO = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..', '..');
  const poll = readFileSync(path.join(REPO, 'services/live-poller/poll.mjs'), 'utf8');
  assert.match(poll, /if \(after\.status === 'live' && m\.status !== 'live'\) out\.wentLive \+= 1;/);
  const idx = readFileSync(path.join(REPO, 'services/live-poller/index.mjs'), 'utf8');
  const a = idx.indexOf('decision = afterPoll(decision, { wentLive: r.wentLive ?? 0 }');
  assert.ok(a > 0, 'afterPoll is applied to the poll result');
  assert.ok(a < idx.indexOf('await sleep(sleepUntilNext(decision, now) * 1000);'), 'and before the loop sleeps');
});
