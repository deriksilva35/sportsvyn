 ...
 *
 * PRE-KICK HAS ITS OWN SLEEP NOW, AND ITS OWN LENGTH, per league. Baseball
 * needs a pre-kick window measured in HOURS, not minutes, because the thing it
 * is waiting for is not the first pitch - it is the posted batting order, which
 * goes up two or three hours earlier and is the entire pool of bats two games
 * offer. Four hours of 30-second polling would be 480 polls to read one card, so
 * `preKickSec` lets that window be slow (5 min) while the live window stays
 * fast. Both default to the old behaviour, so the football leagues are
 * bit-for-bit unchanged: ten minutes at 30 seconds.
 */
...
export function cadence(games, now = new Date(), opts = {}) {
  ...
  const preKickSec = opts.preKickSec ?? liveSec;
  ...
  return {
    state,
    // A LIVE GAME OUTRANKS A PRE-KICK ONE. The states are not exclusive and the
    // fastest applicable one wins, which is why this asks about `live` rather
    // than trusting the label: a slate with one game in progress and another an
    // hour out must poll at the live rate.
    sleepSec: state === 'idle' ? idleSec : state === 'pre-kick' ? preKickSec : liveSec,
    liveCount: live,
...
/**
 * PURE. The decision to SLEEP on, after a poll. The cadence is read from the
 * slate BEFORE the poll, so the poll that flips a game to live was made in the
 * pre-kick state and the loop then slept the pre-kick interval - five minutes
 * for MLB - with a game already in progress (CHC @ BOS, 25 Sep: live at
 * 21:38:53Z, next write 21:43:56Z). A poll that turned a row live makes the
 * next sleep the live one. A capped decision is left alone: the cap outranks.
 */
export function afterPoll(decision, { wentLive = 0 } = {}, opts = {}) {
  if (!decision || decision.capped || !(wentLive > 0) || decision.state === 'live') return decision;
  return { ...decision, state: 'live', sleepSec: opts.liveSec ?? LIVE_SEC, liveCount: Math.max(decision.liveCount ?? 0, wentLive) };
}
