...
// Editorial overrides for players API-Sports mistags. Keyed by api_sports id.
// Takes precedence over normalizePosition at every write path so the
// override survives every future squad sync. Add an entry only for a
// confirmed mismatch between API-Sports classification and the player's
// actual role; raw_position is still preserved in metadata.
export const POSITION_OVERRIDES = {
  // Vinicius Junior (Brazil) -- API classifies 'Midfielder'; he plays as a winger.
  '762': 'ATT',
};

export function resolvePosition(apiPlayerId, rawPosition) {
  return POSITION_OVERRIDES[String(apiPlayerId)] ?? normalizePosition(rawPosition);
}
...
export async function importSquadForTeam({ teamId, apiSportsTeamId }) {
  ...
  for (const p of players) {
    ...
    const position = resolvePosition(apiPlayerId, p.position);
