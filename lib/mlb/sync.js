...
export async function upsertMlbTeams(sql, leagueId, rows) {
  ...
  for (const t of teams) {
    const r = await sql`
      ...
      -- (league_id, slug), NOT (slug). teams is unique on the PAIR - checked
      -- against PROD rather than assumed - and slugs genuinely repeat across
      -- leagues: 'senegal' exists three times, once per soccer competition.
      -- ON CONFLICT (slug) names no unique index and throws on the first row.
