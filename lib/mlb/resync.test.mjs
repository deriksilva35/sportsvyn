...
// July 2082 - outside any real schedule - and carries a sentinel BDL id; the
// teardown removes them and asserts it did.
//
// 2082, AND NO OTHER TEST'S YEAR. These fixtures were first filed as season
// 2099, and lib/weekly/weeklyDb.test.mjs clears `season_year IN (2097..2099)`
// across every league: run beside it (a parallel suite, or another machine on
// the same DEV) it deleted these rows mid-test, and four tests failed on rows
// that were not there.
//
...
// AND PER-PROCESS DAYS. A slug is the day and the clubs, so two runs of this
// file at once (two worktrees, or another machine on the same DEV) asked for
// the same slugs, and one of them got -g2. Each process files its games in its
// own twenty days of 2082 - a year no other test uses (see the header).
const DAYMS = 86_400_000;
const OFF = (process.pid % 150) * 20;
const D = (n) => new Date(Date.UTC(2082, 0, 1) + (OFF + n - 1) * DAYMS).toISOString().slice(0, 10);
const id = (n) => BASE + n;
let league; let T;                                           // T.a..T.f: { bdl, abbr }

const row = (n, date, a, h, extra = {}) => ({
  id: id(n), date, season: 2082, season_type: 'regular', postseason: false,
...
after(async () => {
  // OWN ROWS ONLY: every fixture carries one of this process's sentinel BDL
  // ids. A sweep of a shared namespace (slug LIKE 'mlb-2099-%') was what let a
  // second concurrent run of this file delete the first one's rows mid-test.
  await sql`DELETE FROM matches WHERE league_id = ${league}
             AND (external_ids->>'bdl_game_id')::bigint BETWEEN ${BASE} AND ${BASE + 99}`;
  const [left] = await sql`SELECT count(*)::int n FROM matches WHERE league_id = ${league}
             AND (external_ids->>'bdl_game_id')::bigint BETWEEN ${BASE} AND ${BASE + 99}`;
...
test('BOTH ORDERINGS: a new doubleheader is filed the same whichever order BDL lists it', async () => {
  const early = row(1, `${D(1)}T17:05:00Z`, T.a, T.b); const late = row(2, `${D(1)}T23:05:00Z`, T.a, T.b);
  const early2 = row(3, `${D(2)}T17:05:00Z`, T.a, T.b); const late2 = row(4, `${D(2)}T23:05:00Z`, T.a, T.b);
  await writeMlbMatches(sql, league, [late, early]);
  await writeMlbMatches(sql, league, [early2, late2]);
  const base1 = `mlb-${D(1)}-${lo(T.a.abbr)}-${lo(T.b.abbr)}`; const base2 = `mlb-${D(2)}-${lo(T.a.abbr)}-${lo(T.b.abbr)}`;
...
test('THE MOVED GAME 1: the filed row keeps its slug, the moved one takes -g2 and its new time - both orders', async () => {
  for (const [k, [away, home]] of [[10, [T.c, T.d]], [20, [T.e, T.f]]]) {
    ...
    const g2 = row(k + 1, `${D(3)}T23:05:00Z`, away, home);
    const g1 = row(k + 2, `${D(4)}T23:15:00Z`, away, home);
    await writeMlbMatches(sql, league, [g2, g1]);
    const base = `mlb-${D(3)}-${lo(away.abbr)}-${lo(home.abbr)}`;
    assert.equal((await byBdl(k + 2)).slug, `mlb-${D(4)}-${lo(away.abbr)}-${lo(home.abbr)}`);
    // BDL moves game 1 into the 3rd as a doubleheader
    const moved = { ...g1, date: `${D(3)}T20:05:00Z` };
    feed = k === 10 ? [moved, g2] : [g2, moved];
    const r = await resync(D(3), D(4));
    ...
    assert.equal(new Date(m.kickoff_at).toISOString(), `${D(3)}T20:05:00.000Z`);
    const ch = r.changes.find((c) => c.bdl === String(id(k + 2)));
    assert.equal(ch.slugFrom, `mlb-${D(4)}-${lo(away.abbr)}-${lo(home.abbr)}`);
    assert.equal(ch.kickoffFrom, `${D(4)}T23:15:00.000Z`);
  }
});

test('A NEW MAKEUP ID is inserted beside the filed game, with its result', async () => {
  const filed = row(30, `${D(5)}T22:35:00Z`, T.b, T.a);
  await writeMlbMatches(sql, league, [filed]);
  feed = [filed, finalOf(row(31, `${D(5)}T17:35:00Z`, T.b, T.a), 3, 5)];
  const r = await resync(D(5), D(5));
  const base = `mlb-${D(5)}-${lo(T.b.abbr)}-${lo(T.a.abbr)}`;
...
test('THE LIVE GUARD: a game BDL calls in progress gets its new time, and keeps OUR state', async () => {
  const g = row(40, `${D(6)}T23:10:00Z`, T.c, T.a);
...
test('GONE OR MOVED OUT: 404 cancels an unplayed row and never a final; a game moved out of the window is followed by id', async () => {
  const sched = row(50, `${D(7)}T19:05:00Z`, T.d, T.e);
  const fin = finalOf(row(51, `${D(7)}T17:05:00Z`, T.e, T.f), 1, 2);
  const out = row(52, `${D(7)}T23:05:00Z`, T.f, T.a);
  await writeMlbMatches(sql, league, [sched, fin, out]);
  feed = [];
  byIdOnly = new Map([[String(id(52)), { ...out, date: `${D(20)}T23:05:00Z` }]]);
  const r = await resync(D(7), D(7));
  ...
  assert.equal(new Date(m.kickoff_at).toISOString(), `${D(20)}T23:05:00.000Z`);
  assert.equal(m.slug, `mlb-${D(20)}-${lo(T.f.abbr)}-${lo(T.a.abbr)}`, 'moved to another day: filed under it');
...
test('POSTSEASON: an if-necessary game takes its series\' round; a round is never unset by the re-sync', async () => {
  ...
  const g1 = post(60, `${D(8)}T23:05:00Z`);
  await writeMlbMatches(sql, league, [g1], new Map([[String(id(60)), 'division']]));
  const g5 = post(61, `${D(9)}T23:05:00Z`);             // BDL adds game 5
  feed = [g1, g5];
  const r = await resync(D(8), D(9));
...
test('firstPitchOfDay is the American day\'s earliest, not the next game', () => {
  const now = new Date('2082-07-01T15:00:00Z');                 // 11am in New York
  const ks = ['2082-07-01T17:05:00Z', '2082-07-01T23:05:00Z', '2082-07-02T01:40:00Z', '2082-07-02T17:05:00Z'];
  assert.equal(firstPitchOfDay(ks, now).toISOString(), '2082-07-01T17:05:00.000Z');
  // after the first pitch the day's first pitch is still the same one - no second pre-game run
  assert.equal(firstPitchOfDay(ks, new Date('2082-07-01T20:00:00Z')).toISOString(), '2082-07-01T17:05:00.000Z');
});
