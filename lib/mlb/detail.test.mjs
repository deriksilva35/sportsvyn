...
test('NOTHING TO WRITE ISSUES NO STATEMENT', async () => {
  ...
  assert.equal(await writeMlbDetail(sql, 1, { line_score: {} }), false, 'a row the UPDATE did not touch is not a write');
  assert.equal(called, 1);
  const wrote = () => Promise.resolve([{ id: 1 }]);
  assert.equal(await writeMlbDetail(wrote, 1, { line_score: {} }), true);
});

test('THE DETAIL WRITE IS SKIPPED WHEN EVERY KEY ALREADY HOLDS THAT VALUE', () => {
  // The poller calls it for every candidate on every poll since 25 Sep; an
  // unconditional UPDATE there was a write per game per 30 s for nothing.
  const src = readFileSync(new URL('./detail.js', import.meta.url), 'utf8');
  assert.match(src, /IS DISTINCT FROM d\.value/);
  assert.match(src, /RETURNING id`;\s*return r\.length > 0;/);
});
