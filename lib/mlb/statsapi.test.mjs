...
test('A DOUBLEHEADER IS TWO GAMES UNDER ONE KEY, and the clock separates them', async () => {
  const { pickByKickoff } = await import('./statsapi.js');
  // The probe that forced this: 2026-09-22 returns TWO TB @ NYY games on one
  // officialDate with different starters. A Map(key -> value) would have kept
  // one and put its pitcher and its runners on both cards.
  const pair = [
    { gamePk: '823543', gameNumber: 1, gameDate: '2026-09-22T17:05:00Z' },
    { gamePk: '823494', gameNumber: 2, gameDate: '2026-09-22T22:35:00Z' },
  ];
  assert.equal(pickByKickoff(pair, '2026-09-22T17:05:00Z').gamePk, '823543');
  assert.equal(pickByKickoff(pair, '2026-09-22T22:40:00Z').gamePk, '823494',
    'five minutes of provider disagreement is nothing next to five hours');

  // ONE CANDIDATE NEEDS NO CLOCK AT ALL - most keys have exactly one game, and
  // asking the time of it would only invent a way to fail.
  assert.equal(pickByKickoff([pair[0]], null).gamePk, '823543');
  assert.equal(pickByKickoff([pair[0]], 'not a date').gamePk, '823543');

  // A PAIR WE CANNOT SEPARATE BY AN HOUR IS REFUSED. Guessing there puts one
  // game's runners on the other game's diamond.
  const tight = [
    { gamePk: 'a', gameDate: '2026-09-22T17:05:00Z' },
    { gamePk: 'b', gameDate: '2026-09-22T17:35:00Z' },
  ];
  assert.equal(pickByKickoff(tight, '2026-09-22T17:10:00Z'), null);
  // And so is a nearest candidate that is nowhere near.
  assert.equal(pickByKickoff(pair, '2026-09-23T17:05:00Z'), null);
  assert.equal(pickByKickoff([], '2026-09-22T17:05:00Z'), null);
  assert.equal(pickByKickoff(null, '2026-09-22T17:05:00Z'), null);
  // Several candidates and no kickoff of our own: refused, not coin-flipped.
  assert.equal(pickByKickoff(pair, null), null);
});

test('THREE CLUBS ARE SPELLED DIFFERENTLY BY THE TWO PROVIDERS, and it was measured', async () => {
  const { STATSAPI_ABBR, ourAbbr } = await import('./statsapi.js');
  // Read off the 2026-09-22 schedule's 30 abbreviations against ours on PROD.
  // Unaliased, this cost exactly three of sixteen games their probables and
  // looked like three games with no starter announced - a real state, so an
  // invisible failure.
  assert.deepEqual(STATSAPI_ABBR, { AZ: 'ARI', CWS: 'CHW', ATH: 'OAK' });
  assert.equal(Object.keys(STATSAPI_ABBR).length, 3,
    'a fourth disagreement must be measured, not absorbed - add it on purpose');
  assert.equal(ourAbbr('AZ'), 'ARI');
  assert.equal(ourAbbr('CWS'), 'CHW');
  assert.equal(ourAbbr('ATH'), 'OAK');
  // EVERY OTHER CLUB AGREES, so the map is an exception list and not a
  // normaliser: anything unlisted passes through exactly as sent.
  for (const a of ['NYY', 'TB', 'LAD', 'SF', 'KC', 'WSH']) assert.equal(ourAbbr(a), a);
  assert.equal(ourAbbr('nyy'), 'NYY');
  assert.equal(ourAbbr(null), null);
  assert.equal(ourAbbr(''), null);
});
