...
export async function writeMlbDetail(sql, matchId, detail) {
  ...
  // NOTHING WRITTEN WHEN NOTHING CHANGED. The poller calls this for every
  // candidate on every poll; each key is replaced whole, so a row whose
  // metadata already holds every key at the same value is left alone.
  const r = await sql`
    UPDATE matches
       SET metadata = COALESCE(metadata, '{}'::jsonb) || ${JSON.stringify(detail)}::jsonb,
           updated_at = now()
     WHERE id = ${matchId}
       AND EXISTS (SELECT 1 FROM jsonb_each(${JSON.stringify(detail)}::jsonb) d
                    WHERE (COALESCE(matches.metadata, '{}'::jsonb) -> d.key) IS DISTINCT FROM d.value)
    RETURNING id`;
  return r.length > 0;
}
