...
// --- THE PITCH (migration 113) ---------------------------------------------

test('a play row carries its pitch, and a non-pitch row carries none', () => {
  // The real shape, off /mlb/v1/plays?game_id=5060116.
  const pitch = shapeMlbPlay({
    order: 395819728, inning: 8, inning_type: 'Bottom', type: 'Pitch',
    text: 'Ball', home_score: 5, away_score: 3, scoring_play: false,
    outs: 1, balls: 1, strikes: 0, batter_id: 500, pitcher_id: 600,
    pitch_type: 'Four-Seam Fastball', pitch_velocity: 94,
  });
  assert.equal(pitch.pitchType, 'Four-Seam Fastball');
  assert.equal(pitch.pitchVelocity, 94, 'a NUMBER - "100" would sort before "94" as text');
  // AN INNING MARKER IS A PLAY AND NOT A PITCH, and says so with nulls rather
  // than with an empty string that would draw an empty pitch row.
  const marker = shapeMlbPlay({
    order: 395819727, inning: 1, inning_type: 'Top', type: 'Start Inning',
    text: 'Top of the 1st inning', home_score: 0, away_score: 0, scoring_play: false,
    outs: 0, balls: 0, strikes: 0,
  });
  assert.equal(marker.pitchType, null);
  assert.equal(marker.pitchVelocity, null);
  // 0 IS NOT A SPEED AND null IS NOT 0 - the count's 0s survive as 0s, which is
  // the same rule migration 111 made for outs.
  assert.equal(marker.outs, 0);
  assert.equal(marker.balls, 0);
  const unmeasured = shapeMlbPlay({ order: 1, pitch_type: 'Slider', pitch_velocity: null });
  assert.equal(unmeasured.pitchType, 'Slider');
  assert.equal(unmeasured.pitchVelocity, null, 'a pitch nobody clocked is not a 0mph pitch');
});

test('the writer sends both pitch columns, on insert and on conflict', () => {
  const src = readFileSync(new URL('./playsImport.js', import.meta.url), 'utf8');
  const stmt = src.slice(src.indexOf('INSERT INTO plays'), src.indexOf('return { written }'));
  for (const col of ['pitch_type', 'pitch_velocity']) {
    assert.ok(stmt.includes(col), `${col} is not written at all`);
    assert.match(stmt, new RegExp(`${col} = EXCLUDED\\.${col}`),
      `${col} is inserted but never UPDATED - a pitch corrected by the provider would keep the first reading forever`);
  }
});
