...
/**
 * PURE. The live feed's boxscore -> both clubs' POSTED BATTING ORDERS, or null.
 *
 * battingOrder IS AN EMPTY ARRAY BEFORE THE CARD IS POSTED, not an absent key -
 * measured on 2026-09-23's WSH @ DET while it was still "Scheduled": both sides
 * came back `battingOrder: []` with 28 players already listed under `players`.
 * So an empty array is the "not posted yet" signal and a 9-long array is the
 * card; a reader that checked for the KEY would have seen a lineup on every
 * game in the schedule.
 *
 * THE ORDER IS THE ARRAY'S OWN ORDER. Each player also carries a battingOrder
 * STRING ("100" for the leadoff spot, "400" for the cleanup spot, "401" for a
 * substitute who took it), and dividing that by 100 is a second way to get the
 * same number that disagrees the moment somebody is replaced. The index is the
 * slot.
 *
 * @returns { away, home } where each side is an array of
 *   { id, name, position, order } or NULL when that side has not posted.
 *   Null for the whole object when there is no boxscore to read at all - which
 *   is "we do not know", and is not the same as "not posted".
 */
export function lineupsFrom(feed) {
  const bs = feed?.liveData?.boxscore ?? null;
  if (!bs || typeof bs !== 'object' || !bs.teams) return null;
  const sideOf = (s) => {
    const t = bs.teams?.[s] ?? null;
    const order = Array.isArray(t?.battingOrder) ? t.battingOrder : null;
    if (!order || !order.length) return null;
    return order.map((pid, i) => {
      const p = t?.players?.[`ID${pid}`] ?? null;
      return {
        id: String(pid),
  ...
  const away = sideOf('away'); const home = sideOf('home');
  if (!away && !home) return null;
  return { away, home };
}

/**
 * PURE. The feed's own probable starters -> the shape the board already holds.
 *
 * THEY ARE ON THE SAME DOCUMENT AS THE BATTING ORDER. gameData.probablePitchers
 * carries { away, home } with an id and a fullName, and it carries ONE SIDE
 * ONLY when only one club has announced - measured on 2026-09-23's WSH @ DET,
 * which came back with `home` and no `away` while still Scheduled. So this
 * returns a partial object rather than nothing: half an answer is the normal
 * state for three hours, and dropping it would keep an announced starter
 * unannounced until his opponent announced too.
 */
export function probablesFromFeed(feed) {
  const pp = feed?.gameData?.probablePitchers ?? null;
  if (!pp || typeof pp !== 'object') return null;
  const one = (side) => {
    const p = pp?.[side] ?? null;
...
/**
 * ONE FETCH, THREE READINGS. The live state, the batting orders AND the
 * probable starters are on the SAME feed document, so a live game's lineup and
 * a pre-kick game's starter cost nothing on top of the diamond the poller
 * already pulls - and a second fetch for either would double this provider's
 * call count on every poll.
 */
export async function fetchGameFeed(gamePk, { env = process.env } = {}) {
  if (!statsApiEnabled(env) || gamePk == null) return { live: null, lineups: null, probables: null, calls: 0 };
  const feed = await getJson(`/v1.1/game/${encodeURIComponent(gamePk)}/feed/live`);
  if (!feed) return { live: null, lineups: null, probables: null, calls: 0 };
  return { live: liveFrom(feed), lineups: lineupsFrom(feed), probables: probablesFromFeed(feed), calls: 1 };
}

/** The posted batting orders alone, for the pre-kick pass. NULL when off. */
export async function lineupsFor(gamePk, { env = process.env } = {}) {
  const { lineups } = await fetchGameFeed(gamePk, { env });
  return lineups;
}
...
export async function fetchLiveState(gamePk, { env = process.env } = {}) {
  const { live } = await fetchGameFeed(gamePk, { env });
  return live;
}
