...
/**
 * ONE FETCH, BOTH READINGS. The live state and the batting orders are on the
 * SAME feed document, so a live game's lineup costs nothing on top of the
 * diamond the poller already pulls - and a second fetch for it would double
 * this provider's call count on every live poll.
 */
export async function fetchGameFeed(gamePk, { env = process.env } = {}) {
  if (!statsApiEnabled(env) || gamePk == null) return { live: null, lineups: null, calls: 0 };
  const feed = await getJson(`/v1.1/game/${encodeURIComponent(gamePk)}/feed/live`);
  if (!feed) return { live: null, lineups: null, calls: 0 };
  return { live: liveFrom(feed), lineups: lineupsFrom(feed), calls: 1 };
}
