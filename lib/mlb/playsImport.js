...
export function shapeMlbPlay(row) {
  ...
  return {
    ...
    // THE PITCH ITSELF (migration 113). Unnormalised on purpose - the tab
    // prints the provider's own name and nothing compares it - and the velocity
    // stays a NUMBER, because "94" and "94.3" are the same fact measured to
    // different precision and a string would sort 100 before 94.
    pitchType: txt(row.pitch_type),
    pitchVelocity: num(row.pitch_velocity),
  };
}
  ...
  for (const raw of rows ?? []) {
    ...
    await sql`
      ...
      VALUES (${matchId}, ${p.providerPlayId}, ${p.playNumber}, ${p.period}, ${p.text},
              ...
              ${p.batterId}, ${p.pitcherId},
              ${p.pitchType}, ${p.pitchVelocity}, now(), now())
      ON CONFLICT (match_id, provider_play_id) DO UPDATE SET
        ...
        pitch_type = EXCLUDED.pitch_type, pitch_velocity = EXCLUDED.pitch_velocity,
        updated_at = now()`;
    written += 1;
  }
  return { written };
