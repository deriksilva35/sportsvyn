// lib/mlb/fantasyPoints.js - what a line is worth. PURE.
//
// ONE SCORER, TWO GAMES. October (five a day) and The Run (nine a round) are
// different games with the same currency, and the relay's own rule is that
// they share this file. Two copies of a points table is two answers to "how
// did I score 94.5", and the first time they drifted - one relay editing the
// stolen-base value, the other not - a reader comparing their October total
// to their Run total would be comparing two different sports.
//
// IT LIVES IN lib/mlb/, NOT IN EITHER GAME'S FOLDER, for the same reason: a
// scorer inside lib/october/ that lib/run/ imports is a dependency pointing
// the wrong way, and the next game would have to pick one of them to depend
// on. It belongs to the sport.
//
// THE TABLE IS PRINTED ON BOTH CARDS (docs/design/mocks/october-v0_2.html's
// live footer, docs/design/mocks/the-run-v0_1.html's rules line) because a
// scoring rule a player cannot see is a rule they cannot play to.
//
