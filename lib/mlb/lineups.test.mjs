...
test('post: the bats are the card, and nobody else', () => {
  const rows = [
  ...
  // The arm first (it is the probable), then the two bats BY PPG - Misner's 12
  // ahead of Diaz's 8, even though Diaz bats first. The batting order never
  // sorts; it rides in the sub-line.
  assert.deepEqual(names, ['The Starter', 'Kameron Misner', 'Yandy Diaz']);
...
test("The Run's panel: bats are the posted card, arms are starters only", () => {
  const rows = [
  ...
  // BATS BY PPG, each carrying his order for "bats Nth". NO RP IS OFFERED -
  // the reliever is gone, and the benched bat is off the card.
  assert.deepEqual(cut.rows.map((r) => r.name),
    ['A Starter', 'Kameron Misner', 'Yandy Diaz']);
  // EVERY SP SURVIVES, the one deliberate difference from October: a Run arm
  // scores every start his club gives him in a SERIES, so the game-2 and
  // game-3 starters are worth as much as game 1's.
  assert.equal(cut.rows.filter((r) => r.kind === 'arm').length, 1);
  assert.equal(cut.rows.find((r) => r.name === 'Yandy Diaz').order, 1);
});
...
test('post: bats rank by PPG ACROSS both clubs - the batting order never sorts', () => {
  const home = NINE.map((p, i) => ({ id: 900 + i, name: `H Bat ${i + 1}`, pos: 'RF' }));
  const rows = [
    // The away club bats 1-2-3 and is the WORSE club by PPG. A panel grouped by
    // club, or ordered by lineup slot, buries the best bat in the game below
    // three worse ones; this list is something a reader picks from.
    ...NINE.slice(0, 3).map((p, i) => ({ playerId: `a${i}`, name: p.name, kind: 'bat', team: 'TB', ppg: 5 - i })),
    ...home.slice(0, 3).map((p, i) => ({ playerId: `h${i}`, name: p.name, kind: 'bat', team: 'NYY', ppg: 9 - i })),
  ];
  const cut = startersOnly(rows, { lineup: lineupsFrom(feed(NINE, home)), awayAbbr: 'TB', homeAbbr: 'NYY' });
  assert.deepEqual(cut.rows.map((r) => `${r.team}${r.order}:${r.ppg}`),
    ['NYY1:9', 'NYY2:8', 'NYY3:7', 'TB1:5', 'TB2:4', 'TB3:3']);
  // THE ORDER IS STILL ON EVERY ROW - it is a fact about the player that the
  // sub-line prints, not a way to rank him.
  assert.deepEqual(cut.rows.map((r) => r.order), [1, 2, 3, 1, 2, 3]);
});

test('post: an arm outranks every bat, and the probable outranks every arm', () => {
  const rows = [
    { playerId: 'b', name: 'Yandy Díaz', kind: 'bat', team: 'TB', ppg: 99 },
    { playerId: 'a2', name: 'Second Arm', kind: 'arm', team: 'TB', ppg: 1, position: 'SP', probable: false },
    { playerId: 'a1', name: 'The Starter', kind: 'arm', team: 'TB', ppg: 2, position: 'SP', probable: true },
  ];
  const cut = startersOnly(rows, { lineup: lineupsFrom(feed(NINE, [])), awayAbbr: 'TB', homeAbbr: 'NYY' });
  assert.deepEqual(cut.rows.map((r) => r.name), ['The Starter', 'Yandy Díaz']);
});
...
test('the poller writes the probables on the pre-kick pass, beside the lineups', async () => {
  ...
  // FROM BDL NOW (lib/mlb/probables.js), not off the statsapi feed or schedule.
  assert.match(src, /const p = await mlbProbables\(m, row, \{ now \}\);/);
  assert.doesNotMatch(src, /probables = feed\.probables/);
});
