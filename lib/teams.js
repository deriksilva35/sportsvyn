...
export async function getTeamBySlug(slug) {
  ...
  const rows = await sql`
    SELECT
      ...
      -- The league was JOINed but never selected, so the page had no way to
      -- know an NFL team from a World Cup one - which is why the breadcrumb,
      -- the anchor rail and the schedule heading were all hardcoded to soccer.
    ...
    ORDER BY
      ...
      -- NAMED LEAGUE PRECEDENCE, replacing a bare WC-or-not boolean whose
      -- remainder fell through to id ASC - i.e. to whichever row was imported
      -- first. That tiebreak was live and wrong for exactly one real team:
      -- the slug "georgia" is BOTH the country (international-friendlies, id
      -- 1198) and
      -- the Georgia Bulldogs (cfb, id 48122). The lower id won, so /team/georgia
      -- served a soccer page with seven anchor pills and no roster, and the
      -- Bulldogs page - one of 243 CFB teams now carrying a full roster - was
      -- unreachable while sitting in the sitemap.
      --
      -- Every league we actually publish is named here; unnamed ones share 9
      -- and keep falling through to id ASC, which is why this changes exactly
      -- one of the 52 duplicated slugs. Verified against PROD before shipping,
      -- not reasoned about: the other 51 resolve to the same row as before.
      CASE lg.slug
        WHEN 'fifa-wc-2026' THEN 0
        WHEN 'nfl'          THEN 1
        WHEN 'cfb'          THEN 2
        WHEN 'epl'          THEN 3
        ELSE 9
...
export async function getTeamMatches(teamId) {
  const rows = await sql`
    SELECT
      ...
      -- season_year so the schedule heading can say "2026 Season" rather than
      -- a bare "Season" - the column was never selected here.
      m.season_year,
