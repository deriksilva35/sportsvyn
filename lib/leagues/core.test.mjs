...
test('the lobby card lists leagues or pitches, and routes /leagues', () => {
  // GAMES TAB v2: the module became one link in the pane's foot
  // (components/games/LobbyV2.js), still signed-in only, still /leagues.
  const t = stripComments(src('app/games/page.js')) + stripComments(src('components/games/LobbyV2.js'));
...
test('the season chip is the BEST tier, colored from config', () => {
  ...
  // RE-RATIFIED (legibility phase 2): the page's hand-written jade
  // conditional died with the shared SeasonBoard - the chip renders there,
  // colored by tierClass (config), pinned in seasonBoard.test.mjs.
  const page = stripComments(src('app/leagues/[id]/page.js'));
  assert.match(page, /<SeasonBoard table=\{season\} userId=\{uid\} \/>/);
});
