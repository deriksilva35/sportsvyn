...
test('the league card reads the Daily through the scoped readers only', () => {
  const t = stripComments(src('app/leagues/page.js'));
  assert.match(t, /dayBoard\(revealedDate, uid, 5, \{ memberIds: members \}\)/);
  assert.match(t, /overall\(uid, 5, null, \{ memberIds: members \}\)/);
