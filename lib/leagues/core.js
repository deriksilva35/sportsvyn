...
import { CODE_ALPHABET, CODE_LENGTH, REFUSALS } from './code.js';
...
export async function joinLeague(userId, rawCode) {
  ...
  if (code.length !== CODE_LENGTH) return { ok: false, reason: REFUSALS.not_a_code };
  const [lg] = await sql`SELECT id, name FROM player_leagues WHERE join_code = ${code}`;
  if (!lg) return { ok: false, reason: REFUSALS.no_league };
  await sql`
 ...
 * this preview - and the join button beside it - are reachable by walking
 * ids, which the six-char code gate was not. Name + count is what leaks;
 * boards and identities stay sealed. If id-walking ever matters, the fix is
 * slugging the route with the join code, not tightening this read.
 */
...
/** Join by league id - the /leagues/[id] preview's button (frame 3). */
export async function joinLeagueById(userId, leagueId) {
  const [lg] = await sql`SELECT id, name FROM player_leagues WHERE id = ${leagueId}`;
