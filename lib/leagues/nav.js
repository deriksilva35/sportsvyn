...
//
// CALENDAR ORDER, RATIFIED: Daily (live now) → Pick'em (first lock Aug 29,
// per the first-kickoff law) → Weekly (Sep 10) → Draft (Sep 10) → Season
// (first settle). The ghost dates render in mono on the pills; they live HERE
// so the pills and any copy that cites them cannot drift apart. Pick'em's is
// the pre-board fallback only - once a board exists the league page overrides
// it from the contest's snapshotted locks_at (lib/pickem/read).

export const LEAGUE_TABS = [
  { key: 'daily', label: 'Daily', ghost: false, date: null },
  { key: 'pickem', label: "Pick'em", ghost: true, date: 'Aug 29' },
