...
/**
 * THE ONE AP READER EVERY SURFACE USES (TOP 25 relay, R1/R4).
 *
 * Resolves the season and the week itself - latestPollSeason then
 * currentApWeek - so no caller ever hardcodes a week. When the latest poll is
 * older than the games on screen (the week-2 slate reading week 1's poll
 * until the Monday cron runs), the badge is still the poll AS PUBLISHED,
 * which is what every scoreboard does, and nothing in the UI says otherwise.
 *
 * TIES KEEP THEIR NUMBER (R2): two teams at 14 both read "#14". The rank is
 * the poll's, not a position in a list.
 *
 * AP ONLY (R3). coaches_rankings stays unread.
 *
 * @returns {{ ranks: Map<number, number>, season: number|null, week: number|null }}
 */
export async function currentApRanks() {
  const season = await latestPollSeason(AP_POLL).catch(() => null);
  const week = season == null ? null : await currentApWeek(season).catch(() => null);
  const ranks = season == null || week == null ? new Map() : await apRankMap({ season, week }).catch(() => new Map());
  return { ranks, season, week };
}
