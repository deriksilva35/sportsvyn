...
test('CFB has no game log yet, and does not fake one from season totals', () => {
  const page = src('app/player/[slug]/page.js');
  assert.match(page, /canHaveStats && !isCfb \? gameLog\(/);
  // The pill follows the section: no Game Log pill without a log.
