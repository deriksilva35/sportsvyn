 ...
 */
export async function cfbGameLog(playerId, columns, { limit = 4, season = null } = {}) {
  if (playerId == null || !columns?.length) return [];
  const keys = columns.map((c) => c.key);
  // Season-scoped reads take the whole season; `limit` serves the four-row
  // preview. A truncated season would make the chart claim a stretch it does
  // not cover.
  const rows = await sql.query(
    `SELECT g.season, g.week, g.opponent, g.result, ${keys.map((k) => `g.${k}`).join(', ')}
       FROM cfb_player_game_stats g
      WHERE g.player_id = $1
        AND ($3::int IS NULL OR g.season = $3::int)
      ORDER BY g.season DESC, g.week DESC
      LIMIT $2`,
    [Number(playerId), season == null ? Number(limit) : 40, season == null ? null : Number(season)],
  );
  return rows.map((r) => {
