...
test('the scoreboard and CFB game page render the one badge component', () => {
  ...
  // The game-page badge moved with the row it sits in: both game pages now
  // render components/gridiron/GameTeamRow.js (TEAM FOLLOWING relay, R3). The
  // CFB page still sources the rank; the NFL page passes none, and the badge
  // renders null on a null rank, which is what let one row serve both.
  assert.match(strip(src('components/gridiron/GameTeamRow.js')), /<RankBadge rank=\{rank\} size="big" \/>/);
