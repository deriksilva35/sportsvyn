...
test('all three insertion points render the one badge component', () => {
  ...
  assert.match(strip(src('components/pickem/PickemBoard.js')),
...
test('THE PICKEM VIEW CHANGE - the wire now carries ranks, which it did not', () => {
  ...
  assert.match(strip(src('lib/pickem/entry.js')), /gameRows\(\{ board: contest\.board, liveById, picks, now, apRanks \}\)/);
});
...
test('CONTRAST: a dark board must declare its own ink surface', () => {
  // THE SHIPPED DEFECT. The hub shell is data-surface="paper", which sets
  // color: var(--ink) - #0A0A0A. PollBoard painted an --ink-2 (#141414)
  // background without flipping the surface, so team names rendered at a
  // measured 1.07:1 contrast and were invisible, while the points column
  // survived because .pb-v sets its colour explicitly (5.31:1). With the
  // surface declared it is 16.87:1.
  const pb = src('components/gridiron/PollBoard.js');
  assert.match(pb, /<section className="pb gi-instrument" aria-label=\{poll\.name\} data-surface="ink">/);
  assert.match(pb, /className="pb-empty" data-surface="ink"/);
  // Second lock: the team name states its colour rather than inheriting, so a
  // future wrapper that loses the attribute cannot re-hide it.
  assert.match(src('components/gridiron/pollboard.css'), /\.pb-t \{[^}]*color: var\(--paper/s);

  // And the boards that were already right must stay right.
  for (const f of ['components/gridiron/EditorialBoard.js', 'components/gridiron/PlayoffPicture.js']) {
    assert.match(src(f), /data-surface="ink"/, `${f} must keep declaring its surface`);
  }
});
