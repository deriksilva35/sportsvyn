...
test('the passing line is C/ATT - the college column, not two NFL columns', () => {
  ...
  assert.deepEqual(t.rows[0], { name: 'Jaden Craig', cells: ['20/32', '175', '0', '0'] });
});

test('a carry is CAR, and the rushing table keeps provider order', () => {
  const t = cfbTablesFor(ROWS, 'TCU').find((x) => x.group === 'rushing');
  assert.deepEqual(t.headings, ['CAR', 'YDS', 'TD', 'LONG']);
  assert.ok(t.rows.some((r) => r.name === 'Jeremy Payne' && r.cells.join() === '25,142,0,48'));
});
...
test('the page READS our table - no provider call at render time', () => {
  ...
  assert.match(PAGE, /import \{ cfbBoxScore \} from '@\/lib\/cfb\/boxScore'/);
...
test('TAB ONLY WHEN DATA - the NFL rule, unchanged', () => {
  const PAGE = src('app/cfb/game/[slug]/page.js');
  assert.match(PAGE, /const panels = boxTeams\.length \? \[\{ key: 'players', label: 'PLAYER LINES' \}\] : \[\]/);
  assert.match(PAGE, /\{panels\.length \? \(/, 'no rail at all when there is nothing to show');
});
