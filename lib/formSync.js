...
async function upsertTeamInLeague(leagueId, apiTeam) {
  ...
  // The /fixtures team payload doesn't include `code` (only id/name/logo/winner),
  // so apiTeam.code is undefined. Resolve abbreviation by joining to ANY existing
  // team row sharing the same api_sports id (the WC import populates team.code as
  // abbreviation, so most national teams already have it set in another league).
  let abbreviation = apiTeam.code ?? null;
  if (!abbreviation) {
    const fromExisting = await sql`
      SELECT abbreviation FROM teams
      WHERE external_ids->>'api_sports' = ${apiSportsId}
        AND abbreviation IS NOT NULL
      LIMIT 1
    `;
    abbreviation = fromExisting[0]?.abbreviation ?? null;
  }

  const flagPath = abbreviation ? flagcdnUrl(abbreviation) : null;
  ...
  const inserted = await sql`
    ...
    VALUES (
      ${leagueId}, ${slug}, ${apiTeam.name}, ${apiTeam.name}, ${abbreviation}, ${flagPath},
    ...
    ON CONFLICT (league_id, slug) DO UPDATE SET
      ...
      flag_svg_path = COALESCE(EXCLUDED.flag_svg_path, teams.flag_svg_path),
