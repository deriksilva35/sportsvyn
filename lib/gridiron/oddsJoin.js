...
import { toUtc, COMPETITIVE_PHASES } from './ingest.js';
...
export const CFB_TEAM_OVERRIDES = {
  ...
  'appalachian state mountaineers': 'App State', // our stored name is "App State"
...
export function resolveTeamId(sport, oddsName, teamsByNorm, teamNormsDesc) {
  ...
  // NFL IS EXACT-ONLY and stays that way: BDL and the book agree on those 32
  // names, and a prefix rule there would let "New York" match whichever of two
  // teams sorted first.
  //
  // EPL JOINS CFB ON THE PREFIX RULE, and the reason is the same shape: the
  // book prints the full registered club name and we store the common one.
  // Ten of twenty EPL events failed to join on 27 Aug for exactly this -
  // "Newcastle" vs "Newcastle United", "Brighton" vs "Brighton and Hove
  // Albion", "Tottenham" vs "Tottenham Hotspur", "Coventry" vs "Coventry
  // City". Not one was a time-window miss, so widening the window would have
  // fixed nothing.
...
export async function joinEventsToMatches(sql, { leagueSlug, sport, events }) {
  ...
  const matchRows = await sql`
    ...
    WHERE league_id = ${leagueId}
      -- A NULL PHASE IS NOT AN UNCOMPETITIVE ONE. Equality against a set
      -- evaluates to NULL for a NULL column, so this predicate excluded every row
      -- whose league does not carry a phase at all - which is all 370 EPL
      -- matches, and is why the first EPL tick matched 0 of 20 events while
      -- reporting ok. The phase filter exists to keep PRESEASON out of the
      -- gridiron join; a league with no phases has no preseason to exclude.
      AND (season_phase IS NULL OR season_phase = ANY(${COMPETITIVE_PHASES}::text[]))
