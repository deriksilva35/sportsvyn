...
export async function joinEventsToMatches(sql, { leagueSlug, sport, events }) {
  ...
  for (const m of matchRows) {
    ...
    byPair.set(`${m.home_team_id}|${m.away_team_id}`, m);
  }
  ...
  for (const e of evs) {
    ...
    // Path 2 — first contact: resolve teams + confirm the pair within the window.
    ...
    const cand = homeId && awayId ? byPair.get(`${homeId}|${awayId}`) : null;
    if (cand) {
      const commenceIso = await toUtc(e.commence_time, null, 'oddsapi');
      const kickMs = new Date(cand.kickoff_at).getTime();
      const commMs = commenceIso ? new Date(commenceIso).getTime() : NaN;
      if (Number.isFinite(commMs) && Math.abs(kickMs - commMs) <= JOIN_WINDOW_MS) {
        ...
        await sql`
          ...
          WHERE id = ${cand.id}`;
        captured += 1;
        matched.push({ event: e, matchId: cand.id });
        continue;
      }
    }
