...
export async function toUtc(dateTimeStr, dateTimeUtcField, provider) {
  switch (provider) {
    ...
    case 'cfbd':
      ...
      // "2025-09-05T00:20:00.000Z"). Offset-safe direct parse.
...
const STATUS_MAP = {
  'bdl:nfl': {
    final: 'final',
    'final/ot': 'final',
    // Not-yet-played games carry the KICKOFF DATETIME as their status string
    // (e.g. "9/9 - 8:20 PM EDT"), one distinct value per game — handled by the
    // datetime pattern in mapStatus() below, not a table key.
