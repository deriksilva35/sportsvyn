...
export function makeRunSummary() {
  return {
    ...
    unmapped: {},              // { '<provider status token>': n } - see noteUnmapped
    timeResolvedFromFallback: 0, // times sourced from a fallback payload
  };
}

// How many DISTINCT unmapped tokens a summary row may name. A pathological
// feed shipping garbage-per-game must not bloat a ledger row; past the cap
// the count still lands, under unmappedOverflow.
export const UNMAPPED_TOKEN_CAP = 5;

/**
 * Name the unmapped token IN THE LEDGER. Saturday 22 Aug logged
 * unknownStatus=63 across the ten-game slate and the morning read could not
 * say which provider word went unmapped - runtime logs were gone. The token
 * is a PROVIDER VOCABULARY WORD (a status string like 'Q1' or 'Delayed'),
 * never a score, never an id: that is the leak-safety contract, and the
 * callers below only ever pass the status field itself.
 */
export function noteUnmapped(runSummary, token) {
  if (!runSummary) return;
  runSummary.unknownStatus += 1;
  if (runSummary.unmapped == null) runSummary.unmapped = {};
  const t = String(token ?? '(empty)').slice(0, 40);
  if (t in runSummary.unmapped) { runSummary.unmapped[t] += 1; return; }
  if (Object.keys(runSummary.unmapped).length >= UNMAPPED_TOKEN_CAP) {
    runSummary.unmappedOverflow = (runSummary.unmappedOverflow ?? 0) + 1;
    return;
  }
  runSummary.unmapped[t] = 1;
}
...
const STATUS_MAP = {
  'bdl:nfl': {
    final: 'final',
    'final/ot': 'final',
    // MULTI-OVERTIME FINALS, from an EXHAUSTIVE CENSUS, not a sample. Swept every
    // /nfl/v1/games row for every season 2002-2025 (6,502 games) and tallied the
    // distinct status strings rather than guessing from the one token a backfill
    // happened to trip over. 'Final/2OT' appeared exactly twice in the whole
    // 24-season span: the 2003 NFC divisional Panthers 29-Rams 23 (Jan 10 2004,
    // week "2" of that postseason) and the 2012 AFC divisional Ravens 38-Broncos
    // 35 "Mile High Miracle" (Jan 12 2013) - the game that first surfaced this
    // gap. No 'Final/3OT' or higher token exists anywhere in the census; if a
    // future season ever produces one, mapStatus's fail-loud path below still
    // catches it rather than this key silently matching on a word fragment.
    'final/2ot': 'final',
    ...
    tbd: 'scheduled',
    // NEVER PLAYED, from the same census. 'Postponed' appears twice - the 2014
    // Jets@Bills lake-effect-snowstorm game (week 12, moved to Detroit and played
    // days later under what BDL records as a separate game) and the 2017
    // Bucs@Dolphins Hurricane Irma game (week 1, rescheduled to week 11). Both
    // original slots carry no score in the census and stay 'Postponed' rather
    // than resolving to 'final' - the game that was actually played has its own
    // game_id and ingests normally under that one. 'Canceled' (BDL's spelling,
    // one L, not our CHECK's 'cancelled') appears once: the 2022 Bills@Bengals
    // game after Damar Hamlin's collapse, which was never resumed or replayed -
    // the only one-L 'Canceled' game in the whole 24-season census, not a family
    // of them.
    postponed: 'postponed',
    canceled: 'cancelled',
    ...
    // The historical census above cannot confirm this one: it only ever sees a
    // season's TERMINAL status, never an in-progress tick from a game that has
    // since finished. mapStatus's unknown-token path still fails loud for it
    // (log.error + runSummary.unknownStatus++ + skip the status write, leaving
    // the prior status intact), and maybeAlert emails on unknownStatus > 0. The
    // first live tick self-reports the real token and it gets added here from
    // evidence, the same discipline this file has always used. Adding a
    // speculative key would defeat that alert — a wrong guess maps silently
    // instead of shouting.
  },
  ...
  'apisports:nfl': {
    ...
    'halftime': 'live',
    'in play': 'live',
    // Observed on the live feed 13 Aug, at quarter breaks - six sweeps carried
    // it, each incrementing unknownStatus and skipping the status write. A game
    // between periods is IN PLAY, not finished: mapping it anywhere else would
    // have taken a live game off the board for the length of a commercial.
    'end of period': 'live',
    pst: 'postponed',
    postponed: 'postponed',
    canc: 'cancelled',
...
export function mapStatus(provider, sport, rawStatus, runSummary) {
  ...
  if (provider === 'cfbd' && sport === 'cfb') {
    ...
    noteUnmapped(runSummary, '(cfbd shape)');
    return null;
  }
  ...
  if (provider === 'apisports' && rawStatus && typeof rawStatus === 'object') {
    ...
    if (token == null) {
      ...
      noteUnmapped(runSummary, '(no short/long)');
      return null;
    }
  ...
  if (!table) {
    ...
    noteUnmapped(runSummary, `(no table ${provider}:${sport})`);
    return null;
  }
  ...
  // The normalized token, so the ledger word matches the table key a fix
  // would add.
  noteUnmapped(runSummary, norm);
  return null;
}
...
/**
 * The phases that count as REAL COMPETITION, named once for every reader that
 * has to make that distinction.
 *
 * PRE is stored and displayed - /scores lists preseason games, badged - but it
 * is excluded anywhere a preseason result would be mistaken for a competitive
 * one: team records, player stat lines, the Today lede's countdown, and the
 * betting market. An exhibition result is a result about nothing.
 *
 * This is the list; lib/gridiron/topicEnvelope.js and the odds readers import
 * it rather than each keeping their own copy, because three copies of a rule
 * are three chances to update two of them.
 */
export const COMPETITIVE_PHASES = ['REG', 'POST'];
...
export function apiSportsPhaseAndWeek(stage, week, runSummary) {
  ...
  if (phase == null) {
    ...
    // unknownStatus is an OVERLOADED counter (status tokens, stage/week
    // labels, quarter words all land in it) - the ledger prefix is what
    // tells the vocabularies apart after the fact.
    noteUnmapped(runSummary, `(stage) ${s}`);
  ...
  if (phase === 'POST') {
    ...
    if (n == null) {
      ...
      noteUnmapped(runSummary, `(post week) ${wl}`);
  ...
  noteUnmapped(runSummary, `(week label) ${wl}`);
...
// ---------------------------------------------------------------------------
// (f) Game-detail shapes: scoring plays and player stat lines
// ---------------------------------------------------------------------------
// API-Sports serves both as PROSE. /games/events gives the quarter as a word
// ("Second"), and /games/statistics/players gives every value as a string,
// including compounds ("15/19", "1-2") and plain numbers ("188"). Both are
// normalised HERE rather than at the page, for the same reason datetimes are:
// one boundary, one place to be wrong.

const QUARTER_WORDS = {
  first: 1, second: 2, third: 3, fourth: 4,
  // Overtime periods continue the numbering rather than getting a magic value,
  // so line_scores[4] and quarter 5 refer to the same column.
  overtime: 5, 'over time': 5, ot: 5,
  'second overtime': 6, 'third overtime': 7,
};

/**
 * "Second" -> 2. Returns null on anything unrecognised rather than guessing:
 * a wrong quarter puts a touchdown in the wrong period, which is worse than an
 * event the page groups under "unknown".
 */
export function quarterIndex(word, runSummary) {
  const w = String(word ?? '').trim().toLowerCase();
  if (!w) return null;
  const hit = QUARTER_WORDS[w];
  if (hit) return hit;
  // "Q3" / "3" / "3rd" also parse, since a provider that changes its mind about
  // spelling should not silently lose a quarter of the game.
  const m = w.match(/^q?(\d+)/);
  if (m) return Number(m[1]);
  log.error('quarterIndex: unrecognised quarter', { word });
  noteUnmapped(runSummary, `(quarter) ${w}`);
  return null;
}

/**
 * The provider's stat keys are DISPLAY LABELS - lowercase, spaced, occasionally
 * abbreviated past readability ("qb hts", "tfl", "ff"). Mapped to the field
 * names lib/fantasy/scoring.js already consumes, so fantasy points come out of
 * the same function the sim uses rather than a second implementation that
 * drifts.
 *
 * Only the keys the scorer needs are mapped. Everything else stays in the raw
 * `stats` object for display - the page shows what the provider sent.
 */
// The labels below are the provider's ACTUAL strings, read off a served payload
// (game 21464, both squads, every group) rather than guessed from the docs.
// Several are near-misses for the obvious name and would have parsed to nothing:
// receptions arrive as "total receptions", carries as "total rushes", a lost
// fumble as plain "lost", and the kicker's conversions as "extra point"
// singular. A silent miss here is the worst kind - the column still renders,
// just with the fantasy points quietly short.
const STAT_FIELDS = {
  // passing
  yards: { passing: 'passYds', rushing: 'rushYds', receiving: 'recYds' },
  'passing touch downs': { passing: 'passTd' },
  interceptions: { passing: 'int' },
  // rushing
  'total rushes': { rushing: 'rushAtt' },
  'rushing touch downs': { rushing: 'rushTd' },
  // receiving
  'total receptions': { receiving: 'rec' },
  'receiving touch downs': { receiving: 'recTd' },
  // loose ball. Only the LOST fumble is mapped: it is the one that scores, and
  // it scores against the player who lost it. "rec" and "rec td" in this group
  // are recoveries, which lib/fantasy/scoring.js prices as DST components - see
  // the note in gameDetail.js on why they are not credited to an individual.
  lost: { fumbles: 'fumblesLost' },
};

// Every entry above is GROUP-SCOPED, and that is load-bearing rather than tidy.
// "sacks" appears in BOTH the passing group (sacks the quarterback TOOK) and the
// defensive group (sacks a defender RECORDED). An unscoped map would have paid a
// quarterback a point for every time he was buried.

const num = (v) => {
  if (v == null) return null;
  const s = String(v).trim();
  if (s === '' || s === '-') return null;
  const f = Number.parseFloat(s.replace(/,/g, ''));
  return Number.isFinite(f) ? f : null;
};

/**
 * A "made/attempted" pair - "15/19", "2-2", "3/4" - as { made, att }. The
 * provider mixes both separators within one payload, so both are accepted.
 */
export function madeAtt(v) {
  const m = String(v ?? '').match(/^\s*(\d+)\s*[/-]\s*(\d+)\s*$/);
  return m ? { made: Number(m[1]), att: Number(m[2]) } : { made: null, att: null };
}

/**
 * Provider stat pairs -> { stats, parsed }.
 *
 * `stats` keeps the payload verbatim for display, `order` keeps the labels in
 * the sequence the provider sent them, and `parsed` is the typed subset
 * the fantasy scorer consumes. Nothing is invented: a key the provider did not
 * send is absent from `parsed`, not zero, so a scorer that treats absent as
 * zero is making that choice explicitly.
 */
export function parseStatLine(group, pairs) {
  const g = String(group ?? '').trim().toLowerCase().replace(/[\s-]+/g, '_');
  const stats = {};
  // The label order is captured SEPARATELY because the object cannot carry it:
  // this lands in a jsonb column, and Postgres re-sorts object keys on write.
  const order = [];
  for (const p of pairs ?? []) {
    if (p?.name == null) continue;
    const label = String(p.name);
    if (!(label in stats)) order.push(label);
    stats[label] = p.value ?? null;
  }
  const parsed = {};
  for (const [label, raw] of Object.entries(stats)) {
    const key = STAT_FIELDS[label]?.[g];
    if (key) {
      const v = num(raw);
      if (v != null) parsed[key] = v;
    }
  }
  // Compounds the label map cannot express.
  if (g === 'passing' && stats['comp att'] != null) {
    const { made, att } = madeAtt(stats['comp att']);
    if (made != null) { parsed.completions = made; parsed.attempts = att; }
  }
  if (g === 'kicking') {
    // "field goals" arrives as made/att; extra points likewise. The scorer takes
    // MAKES only - it has no per-kick distances, which is a stated limitation of
    // lib/fantasy/scoring.js rather than a gap here.
    const fg = madeAtt(stats['field goals']);
    if (fg.made != null) parsed.fgm = fg.made;
    const xp = madeAtt(stats['extra point']);   // singular, as served
    if (xp.made != null) parsed.xp = xp.made;
  }
  return { group: g, stats, order, parsed };
}
