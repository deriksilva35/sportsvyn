...
/**
 * THE HOUSE ZONE. Pacific, because that is what every reader-facing time on
 * this product already says - app/app/data.js fmtKickoffPt, and the deck it
 * feeds - and a card that printed Eastern would be the only surface disagreeing
 * with the rest about when a game is.
 *
 * IT IS A NAMED EXPORT, NOT A DEFAULT INSIDE THE FORMATTERS. The zone stays an
 * argument for the reason the header of this file gives; this constant just
 * means callers do not each spell the string, and a grep for it finds every
 * surface that opted into the house zone.
 */
export const HOUSE_TZ = 'America/Los_Angeles';

/**
 * "7:40 PM PT" - a bare clock time with the zone said out loud.
 *
 * THE SUFFIX IS NOT DECORATION. A time on a card with no zone on it is the
 * exact bug this file's header describes: a reader in Denver seeing 7:40 and
 * arriving an hour late, with nothing admitting which zone was meant. The
 * October card printed Eastern times with no suffix for its whole life.
 */
export function ptTime(iso, tz = HOUSE_TZ) {
  const p = kickoffParts(iso, tz);
  return p ? `${p.time} PT` : null;
}
...
/**
 * The single day a list falls on, or null when it falls on more than one.
 *
 * A LIST WITH NO DATED GAME AT ALL RETURNS NULL TOO, and that is not the same
 * answer as "many days" - both mean "do not name a day", which is all the
 * caller needs, and neither can honestly produce one.
 */
export function soleDayKey(games, tz) {
  let only = null;
  for (const g of games ?? []) {
    const k = dayKey(g?.kickoffAt, tz);
    if (!k) continue;
...
/**
 * How far `key` is from the day `now` falls on, in `tz`. 0 today, 1 tomorrow.
 *
 * COUNTED IN CALENDAR DAYS IN THE READER'S ZONE, not in elapsed hours. The two
 * disagree constantly and the reader only ever means the first: at 11:30 PM in
 * Los Angeles a game two hours away is TOMORROW by the clock and today by every
 * other measure, while the same instant read in New York genuinely is tomorrow.
 * Both keys are produced by dayKey in the zone we were given, so the difference
 * is a subtraction of two calendar dates and DST never enters it.
 */
export function dayOffset(key, now, tz) {
  const today = dayKey(now, tz);
  if (!key || !today) return null;
  // Midday UTC on each date: far enough from either boundary that no offset can
  // push the arithmetic onto a neighbouring day.
  const at = (k) => Date.UTC(+k.slice(0, 4), +k.slice(5, 7) - 1, +k.slice(8, 10), 12);
  return Math.round((at(key) - at(today)) / 86400000);
}
