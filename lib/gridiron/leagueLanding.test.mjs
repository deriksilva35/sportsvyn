...
test('THE TILE COUNT IS THE LEAGUE\'S GAMES - one for CFB, four for NFL', () => {
  ...
  // AND THE ORDER IS GAME_ORDER'S, FILTERED TO THE LEAGUE - not object-key
  // order, and not all of GAME_ORDER. This used to compare against the whole
  // of GAME_ORDER, which was the same list only while every game in it was an
  // NFL game; October (mlb) is the first that is not, and it is correctly
  // absent from an NFL strip while keeping its place in the lobby's order.
  assert.deepEqual(
    stripTiles({ leagueSlug: 'nfl', meta: GAME_META, order: GAME_ORDER, cards, signedIn: true }).map((t) => t.key),
    GAME_ORDER.filter((k) => GAME_META[k].leagues.includes('nfl')),
  );
  assert.ok(!stripGamesFor('nfl', GAME_META).includes('october'));
});
