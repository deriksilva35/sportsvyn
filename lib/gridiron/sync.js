// lib/gridiron/sync.js — league / team / game sync for the gridiron feeds
// (NFL via BallDontLie, CFB via CollegeFootballData). SCOPE: teams +
// team_season_membership + games ONLY. No player rows, no stats, no PBP — those
// await a schema-design pass.
//
// Conventions reused from the soccer sync (lib/syncFixture.js), NOT reinvented:
//   - leagues upsert ON CONFLICT (slug); teams upsert ON CONFLICT (league_id,
//     slug) so a name/city shared across leagues (NFL vs CFB) is two distinct,
//     league-scoped rows — never a shared row.
//   - external_ids stored as { provider_id: String(id) } jsonb.
//   - all datetimes go through ingest.toUtc(); statuses through ingest.mapStatus;
//     season phase through ingest.skipRule. No ad-hoc Date()/AT TIME ZONE here.
//
// Idempotency: games upsert by (league_id, external_ids->>'<provider>_game_id'),
// backed by the migration-045 partial unique indexes as the guard rail. Re-runs
// UPDATE in place, never duplicate.
//
// Per-quarter scores: matches has no period-score column and WC rows leave
// metadata = {} (no soccer shape to follow), so line scores land in metadata
// jsonb as { line_scores: { home: [...], away: [...] } }.
//
// CFB FBS/FCS policy: syncCfbTeams imports FBS teams only. A game with one FBS
// side and one FCS side IS ingested (it is a real game on the FBS team's
// schedule); the FCS opponent is created as a minimal stub team flagged
// metadata.gridiron_stub = true + metadata.classification, so schedules stay
// complete without importing all of FCS. FCS-vs-FCS games are skipped
// (out of launch scope).

import { sql } from '../db.js';
import { toUtc, mapStatus, skipRule, makeRunSummary } from './ingest.js';
import { mapLiveStatus, parseBdlProse } from '../live/vocabulary.js';
import { inspectKickoffRevision, describeRefusal } from './kickoffGuard.js';
import { syncCfbBroadcasts } from './broadcasts.js';
import { syncNflBroadcasts, espnScoreboardFetcher } from './nflBroadcasts.js';
import { syncCfbLiveScores, cfbdScoreboardFetcher } from './cfbScoreboard.js';
import { withAdvisoryLock } from '../pollers/lock.js';
import { LIVE_LOCK, YIELDED } from '../live/handshake.js';
import { syncCfbLiveLines } from '../cfb/bdlLive.js';
import { recordRun } from '../pollers/runRecorder.js';
import { maybeAlert } from '../pollers/alerts.js';

const BDL_BASE = 'https://api.balldontlie.io';
const CFBD_BASE = 'https://apinext.collegefootballdata.com';
const NFL_SLUG = 'nfl';
const CFB_SLUG = 'cfb';

function slugify(s) {
  return String(s ?? '')
    .normalize('NFD').replace(/[̀-ͯ]/g, '')
    .toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '');
}
const sleep = (ms) => new Promise((r) => setTimeout(r, ms));

// ---- provider fetch (keys from env; never hardcoded / logged) --------------
async function bdlGet(pathAndQuery) {
  const key = process.env.BDL_API_KEY;
  if (!key) throw new Error('BDL_API_KEY missing in env');
  for (let attempt = 0; attempt < 4; attempt++) {
    const res = await fetch(`${BDL_BASE}${pathAndQuery}`, { headers: { Authorization: key } });
    if (res.status === 429) { await sleep(15000); continue; } // free tier 5/min
    if (!res.ok) throw new Error(`BDL ${res.status} on ${pathAndQuery}: ${(await res.text()).slice(0, 200)}`);
    return res.json();
  }
  throw new Error(`BDL rate-limited (429) after retries on ${pathAndQuery}`);
}
async function cfbdGet(pathAndQuery) {
  const key = process.env.CFBD_API_KEY;
  if (!key) throw new Error('CFBD_API_KEY missing in env');
  const res = await fetch(`${CFBD_BASE}${pathAndQuery}`, { headers: { Authorization: `Bearer ${key}` } });
  if (!res.ok) throw new Error(`CFBD ${res.status} on ${pathAndQuery}: ${(await res.text()).slice(0, 200)}`);
  return res.json();
}

// ---- upserts (house conventions) -------------------------------------------
async function upsertLeague({ slug, name, short_name, sport, season_type, external_ids }) {
  const rows = await sql`
    INSERT INTO leagues (slug, name, short_name, sport, season_type, external_ids, data_provider_synced_at)
    VALUES (${slug}, ${name}, ${short_name}, ${sport}, ${season_type}, ${JSON.stringify(external_ids)}::jsonb, now())
    ON CONFLICT (slug) DO UPDATE SET
      name = EXCLUDED.name, short_name = EXCLUDED.short_name, sport = EXCLUDED.sport,
      season_type = EXCLUDED.season_type, external_ids = EXCLUDED.external_ids,
      data_provider_synced_at = now(), updated_at = now()
    RETURNING id, slug, sport, season_type`;
  return rows[0];
}

const hexOrNull = (v) => (/^#?[0-9a-f]{6}$/i.test(String(v ?? '').trim()) ? `#${String(v).trim().replace('#', '').toUpperCase()}` : null);

async function upsertTeam(leagueId, t) {
  const rows = await sql`
    INSERT INTO teams (
      league_id, slug, name, short_name, abbreviation, external_ids,
      current_conference, current_division, metadata, data_provider_synced_at,
      color_primary, color_secondary
    ) VALUES (
      ${leagueId}, ${t.slug}, ${t.name}, ${t.short_name ?? t.name}, ${t.abbreviation ?? null},
      ${JSON.stringify(t.externalIds)}::jsonb, ${t.current_conference ?? null}, ${t.current_division ?? null},
      ${JSON.stringify(t.metadata ?? {})}::jsonb, now(),
      ${t.color_primary ?? null}, ${t.color_secondary ?? null}
    )
    ON CONFLICT (league_id, slug) DO UPDATE SET
      name = EXCLUDED.name, short_name = EXCLUDED.short_name,
      abbreviation = COALESCE(EXCLUDED.abbreviation, teams.abbreviation),
      color_primary = COALESCE(EXCLUDED.color_primary, teams.color_primary),
      color_secondary = COALESCE(EXCLUDED.color_secondary, teams.color_secondary),
      external_ids = teams.external_ids || EXCLUDED.external_ids,
      current_conference = EXCLUDED.current_conference,
      current_division = EXCLUDED.current_division,
      metadata = teams.metadata || EXCLUDED.metadata,
      data_provider_synced_at = now(), updated_at = now()
    RETURNING id`;
  return rows[0].id;
}

async function upsertMembership(leagueId, teamId, seasonYear, { conference, division, conferenceSourceId }) {
  await sql`
    INSERT INTO team_season_membership (league_id, team_id, season_year, conference, division, conference_source_id)
    VALUES (${leagueId}, ${teamId}, ${seasonYear}, ${conference ?? null}, ${division ?? null}, ${conferenceSourceId ?? null})
    ON CONFLICT (league_id, team_id, season_year) DO UPDATE SET
      conference = EXCLUDED.conference, division = EXCLUDED.division,
      conference_source_id = EXCLUDED.conference_source_id, updated_at = now()`;
}

// Game upsert keyed on the provider game id (idempotent; migration-045 partial
// unique index is the DB guard rail). providerKey e.g. 'bdl_game_id'.
async function upsertGame(leagueId, providerKey, providerId, g, summary = null) {
  const ext = JSON.stringify({ [providerKey]: String(providerId) });
  // slug/kickoff/status come back too: the kickoff guard needs what we already
  // hold before deciding whether to accept what the provider now claims.
  const existing = (await sql`
    SELECT id, slug, kickoff_at, status FROM matches
     WHERE league_id = ${leagueId} AND external_ids->>${providerKey} = ${String(providerId)} LIMIT 1`)[0];
  if (existing) {
    // THE KICKOFF GUARD. A scheduled game whose kickoff moves by exactly a US
    // Eastern offset within a week of being played is a provider mislabel far
    // more often than a reschedule - see kickoffGuard.js for the 25 Aug case
    // that produced it. On refusal we KEEP what we have and record it; the
    // caller alerts. Nothing is dropped silently.
    const verdict = inspectKickoffRevision({
      current: existing.kickoff_at, incoming: g.kickoffAt, status: existing.status,
    });
    const kickoffAt = verdict.refuse ? existing.kickoff_at : g.kickoffAt;
    if (verdict.refuse && summary) {
      summary.kickoffRefused ??= [];
      summary.kickoffRefused.push(describeRefusal({
        slug: existing.slug, current: existing.kickoff_at,
        incoming: g.kickoffAt, deltaHours: verdict.deltaHours,
      }));
    }
    // A PROVIDER THAT SAYS NOTHING MUST NOT ERASE WHAT ANOTHER WRITER KNOWS.
    //
    // Both of this function's callers send the score their /games feed carries,
    // and BOTH feeds leave it NULL for the whole of a live game - CFBD until
    // `completed`, BDL until the box is final. This UPDATE used to write that
    // NULL straight over the running score, and the metadata assignment below
    // used to REPLACE the whole object, taking `drives` and `live_state` with
    // it. Measured on the 29 Aug opener: a live CFB game lost its score, its
    // clock and its drive envelopes for the full duration of the upsert loop -
    // 20.7 seconds observed, 14-52s across four ticks, every 5 minutes, all
    // slate long.
    //
    // SCORE: incoming non-null always wins, so a final still writes its points.
    // Incoming null PRESERVES - except when the row is (re)scheduled, where an
    // absent score is the truth and a stale one would be a lie about a game
    // that has not been played. Censused before choosing this: zero
    // postponed/cancelled rows and zero scheduled rows carrying a score exist
    // in NFL or CFB, so nothing today depends on null-over-value; the
    // scheduled arm is the guard for the reschedule that has not happened yet.
    //
    // METADATA: a TOP-LEVEL MERGE, the same form writeDriveEnvelopes already
    // uses (playsImport.js:105-113). `||` is one level deep, which is exactly
    // right here because every key involved - line_scores, drives, live_state -
    // is top level; a caller's line_scores still replaces the stored
    // line_scores, and the siblings it never mentions survive. Censused for the
    // opposite risk too: neither caller relies on replace-to-remove. Both send
    // only line_scores, and the apiSports-only keys (venue, venue_city,
    // apisports_week_label, detail) live on 49 PRE rows keyed by
    // apisports_game_id with ZERO bdl_game_id, so this statement never reaches
    // them.
    //
    // AND THE MERGE OWES ONE DEBT BACK. Preserving by default means live_state
    // would now OUTLIVE the game it described - the replace used to clear it as
    // a side effect, and syncCfbLiveScores only ever writes rows that are still
    // live, so nothing else would. Every consumer guards on status = 'live'
    // (Scoreboard.js:79, lineScore.js:73), so a stale chip is inert today - but
    // "inert today" is how a stale clock becomes visible later, and both
    // existing writers state the law outright (apiSportsImport.js:226,
    // syncFixture.js:265). So a non-live status nulls it explicitly. JSON null
    // rather than a key removal, because that is the shape those writers store
    // and `meta.live_state ?? null` is what the readers expect.
    await sql`
      UPDATE matches SET
        home_team_id = ${g.homeTeamId}, away_team_id = ${g.awayTeamId},
        kickoff_at = ${kickoffAt}, status = ${g.status},
        home_score = CASE WHEN ${g.status} = 'scheduled' THEN ${g.homeScore}::int
                          ELSE COALESCE(${g.homeScore}::int, matches.home_score) END,
        away_score = CASE WHEN ${g.status} = 'scheduled' THEN ${g.awayScore}::int
                          ELSE COALESCE(${g.awayScore}::int, matches.away_score) END,
        season_year = ${g.seasonYear}, season_phase = ${g.seasonPhase}, week = ${g.week},
        metadata = COALESCE(matches.metadata, '{}'::jsonb)
                   || ${JSON.stringify(g.metadata ?? {})}::jsonb
                   || CASE WHEN ${g.status} = 'live' THEN '{}'::jsonb
                           ELSE '{"live_state": null}'::jsonb END,
        external_ids = matches.external_ids || ${ext}::jsonb,
        data_provider_synced_at = now(), updated_at = now()
      WHERE id = ${existing.id}`;
    return { id: existing.id, inserted: false };
  }
  const rows = await sql`
    INSERT INTO matches (
      league_id, slug, home_team_id, away_team_id, kickoff_at, status,
      home_score, away_score, season_year, season_phase, week, metadata,
      external_ids, data_provider_synced_at
    ) VALUES (
      ${leagueId}, ${g.slug}, ${g.homeTeamId}, ${g.awayTeamId}, ${g.kickoffAt}, ${g.status},
      ${g.homeScore}, ${g.awayScore}, ${g.seasonYear}, ${g.seasonPhase}, ${g.week},
      ${JSON.stringify(g.metadata ?? {})}::jsonb, ${ext}::jsonb, now()
    ) RETURNING id`;
  return { id: rows[0].id, inserted: true };
}

async function teamMap(leagueId, providerKey) {
  const rows = await sql`
    SELECT id, external_ids->>${providerKey} AS pid FROM teams
     WHERE league_id = ${leagueId} AND jsonb_exists(external_ids, ${providerKey})`;
  return new Map(rows.map((r) => [r.pid, r.id]));
}

// ---------------------------------------------------------------------------
// (C.1) League bootstrap
// ---------------------------------------------------------------------------
export async function bootstrapLeagues() {
  const nfl = await upsertLeague({
    slug: NFL_SLUG, name: 'National Football League', short_name: 'NFL',
    sport: 'football', season_type: 'season-and-postseason', external_ids: { bdl_sport: 'nfl' },
  });
  const cfb = await upsertLeague({
    slug: CFB_SLUG, name: 'College Football', short_name: 'CFB',
    sport: 'football', season_type: 'season-and-postseason', external_ids: { cfbd_sport: 'cfb' },
  });
  return { nfl, cfb };
}

// ---------------------------------------------------------------------------
// (C.2) NFL teams  (BDL /nfl/v1/teams)
// ---------------------------------------------------------------------------
export async function syncNflTeams(leagueId, seasonYear = 2025) {
  const { data } = await bdlGet('/nfl/v1/teams');
  let teams = 0, memberships = 0;
  for (const t of data) {
    const teamId = await upsertTeam(leagueId, {
      slug: slugify(t.full_name), name: t.full_name, short_name: t.name, abbreviation: t.abbreviation,
      externalIds: { bdl_team_id: String(t.id) },
      current_conference: t.conference ?? null, current_division: t.division ?? null,
    });
    await upsertMembership(leagueId, teamId, seasonYear, { conference: t.conference ?? null, division: t.division ?? null });
    teams += 1; memberships += 1;
  }
  return { teams, memberships };
}

// ---------------------------------------------------------------------------
// (C.3) CFB teams  (CFBD /teams?year=)  — FBS only
// ---------------------------------------------------------------------------
export async function syncCfbTeams(leagueId, seasonYear = 2025) {
  const all = await cfbdGet(`/teams?year=${seasonYear}`);
  const fbs = all.filter((t) => t.classification === 'fbs');
  let teams = 0, memberships = 0;
  for (const t of fbs) {
    const teamId = await upsertTeam(leagueId, {
      slug: slugify(t.school), name: t.school, short_name: t.school, abbreviation: t.abbreviation ?? null,
      externalIds: { cfbd_team_id: String(t.id) },
      current_conference: t.conference ?? null, current_division: t.division ?? null,
      metadata: { classification: t.classification },
      // CFBD's /teams carries the school colors; keep them. Hex with the #.
      color_primary: hexOrNull(t.color), color_secondary: hexOrNull(t.alternateColor),
    });
    await upsertMembership(leagueId, teamId, seasonYear, { conference: t.conference ?? null, division: t.division ?? null });
    teams += 1; memberships += 1;
  }
  return { teams, memberships, filteredNonFbs: all.length - fbs.length };
}

// ---------------------------------------------------------------------------
// (D.1) NFL games  (BDL /nfl/v1/games, cursor pagination)
// ---------------------------------------------------------------------------
/**
 * BROADCASTS RIDE THE BASELINE HERE TOO, and for the same reason as CFB: a
 * network assignment is a schedule fact. The NFL's differs in WHEN it moves -
 * flex scheduling rewrites weeks 16-18 in-season rather than filling them in
 * gradually - which is a reason to keep asking, not a reason to ask every
 * five minutes.
 */
export async function syncNflGames(leagueId, seasonYear = 2025, { broadcasts = true } = {}) {
  const summary = makeRunSummary();
  const tmap = await teamMap(leagueId, 'bdl_team_id');
  let cursor = null, missingTeam = 0;
  do {
    const q = `/nfl/v1/games?seasons[]=${seasonYear}&per_page=100${cursor ? `&cursor=${cursor}` : ''}`;
    const { data, meta } = await bdlGet(q);
    for (const g of data) {
      const phase = g.postseason ? 'POST' : 'REG';
      const sr = skipRule(phase, summary);
      if (sr.skip) continue;
      // STATUS IS THE MACHINE FIELD. `status` on this feed is prose - the
      // kickoff before the game, the clock during it ("7:30 - 4th") - and
      // gating on it skipped every live row on opening night: the quarters
      // never wrote and the page showed dashes. status_state is the key.
      const unmappedState = [];
      const status = mapLiveStatus('bdl', g.status_state, unmappedState);
      if (status == null) {
        for (const t of unmappedState) summary.unmapped[t] = (summary.unmapped[t] ?? 0) + 1;
        summary.unknownStatus += 1;
        continue;
      }
      // THE CLOCK IS THE LIVE POLLER'S TO WRITE (upsertGamePreserve: this
      // caller sends line_scores and nothing that belongs to another writer).
      // services/live-poller parses the same prose with parseBdlProse into
      // metadata.live_state once a minute; here it is only COUNTED, so a
      // spelling the parser does not know shows up in this ledger row. The
      // quarters write regardless - score and line score never wait on a chip.
      if (status === 'live' && parseBdlProse(g.status) == null) {
        summary.unparsedProse ??= {};
        summary.unparsedProse[String(g.status)] = (summary.unparsedProse[String(g.status)] ?? 0) + 1;
      }
      const kickoffAt = await toUtc(g.date, null, 'bdl');
      if (kickoffAt == null) { summary.timeResolvedFromFallback += 1; continue; }
      const homeTeamId = tmap.get(String(g.home_team.id));
      const awayTeamId = tmap.get(String(g.visitor_team.id));
      if (homeTeamId == null || awayTeamId == null) { missingTeam += 1; continue; }
      const metadata = { line_scores: {
        home: [g.home_team_q1, g.home_team_q2, g.home_team_q3, g.home_team_q4, g.home_team_ot],
        away: [g.visitor_team_q1, g.visitor_team_q2, g.visitor_team_q3, g.visitor_team_q4, g.visitor_team_ot],
      } };
      const slug = `nfl-${seasonYear}-${phase.toLowerCase()}-w${g.week}-${slugify(g.visitor_team.abbreviation)}-${slugify(g.home_team.abbreviation)}`;
      await upsertGame(leagueId, 'bdl_game_id', g.id, {
        slug, homeTeamId, awayTeamId, kickoffAt, status,
        homeScore: g.home_team_score ?? null, awayScore: g.visitor_team_score ?? null,
        seasonYear, seasonPhase: phase, week: g.week, metadata,
      }, summary);
      summary.ingested += 1;
    }
    cursor = meta?.next_cursor ?? null;
  } while (cursor);
  // AFTER the games, never before - the join needs the matches this run wrote.
  let broadcastSummary = null;
  if (broadcasts) {
    broadcastSummary = await syncNflBroadcasts(leagueId, seasonYear, {
      fetchWeek: espnScoreboardFetcher(),
    });
  }
  return { ...summary, missingTeam, broadcasts: broadcastSummary };
}

// ---------------------------------------------------------------------------
// (D.2) CFB games  (CFBD /games?year=&seasonType=)  — FBS games; FCS stub policy
// ---------------------------------------------------------------------------
const CFB_PHASE = { regular: 'REG', postseason: 'POST' };

/**
 * THE OUTLET RIDES THE GAMES SYNC, but not on every tick.
 *
 * A broadcast assignment is a SCHEDULE fact - it is set on a Tuesday and does
 * not move while the ball is in the air. Re-reading it on a 5-minute live poll
 * would spend two provider calls every tick to re-learn something that changed
 * last week, so the media read is gated to the BASELINE run. The live poller
 * still scores the game; it just does not re-ask who is carrying it.
 *
 * THE LIVE SCORE RIDES EVERY TICK, and unlike the outlet it has to. CFBD
 * /games leaves homePoints/awayPoints NULL until `completed`, so the upsert
 * below writes NULL into home_score for the whole of a live game and a live
 * card renders the LIVE pill over a dash. /scoreboard carries the running
 * score, period and clock on the game id we already store; syncCfbLiveScores
 * reads it AFTER the upsert (which replaces metadata wholesale) so the
 * live_state it writes is the one that survives the tick. It costs ONE call,
 * and only when this league actually has a live game.
 */
export async function syncCfbGames(leagueId, seasonYear = 2025, {
  broadcasts = true, liveScores = true, liveLines = true,
} = {}) {
  const summary = makeRunSummary();
  const tmap = await teamMap(leagueId, 'cfbd_team_id');
  let fcsStubsCreated = 0, skippedNonFbsGame = 0;

  // Resolve or create a team from a game-side payload; FCS opponents become
  // flagged stubs so the FBS team's schedule stays complete.
  async function resolveSide(id, name, conference, classification) {
    const key = String(id);
    if (tmap.has(key)) return tmap.get(key);
    const teamId = await upsertTeam(leagueId, {
      slug: slugify(name), name, short_name: name, externalIds: { cfbd_team_id: key },
      current_conference: conference ?? null,
      metadata: { classification: classification ?? null, gridiron_stub: true },
    });
    await upsertMembership(leagueId, teamId, seasonYear, { conference: conference ?? null });
    tmap.set(key, teamId);
    if (classification !== 'fbs') fcsStubsCreated += 1;
    return teamId;
  }

  for (const st of ['regular', 'postseason']) {
    const games = await cfbdGet(`/games?year=${seasonYear}&seasonType=${st}`);
    for (const g of games) {
      const isFbs = g.homeClassification === 'fbs' || g.awayClassification === 'fbs';
      if (!isFbs) { skippedNonFbsGame += 1; continue; }
      const phase = CFB_PHASE[g.seasonType] ?? CFB_PHASE[st];
      const sr = skipRule(phase, summary);
      if (sr.skip) continue;
      const status = mapStatus('cfbd', 'cfb', { completed: g.completed, startDate: g.startDate, startTimeTBD: g.startTimeTBD }, summary);
      if (status == null) continue;
      const kickoffAt = await toUtc(g.startDate, null, 'cfbd');
      if (kickoffAt == null) { summary.timeResolvedFromFallback += 1; continue; }
      const homeTeamId = await resolveSide(g.homeId, g.homeTeam, g.homeConference, g.homeClassification);
      const awayTeamId = await resolveSide(g.awayId, g.awayTeam, g.awayConference, g.awayClassification);
      const metadata = { line_scores: { home: g.homeLineScores ?? null, away: g.awayLineScores ?? null } };
      const slug = `cfb-${seasonYear}-${phase.toLowerCase()}-w${g.week}-${slugify(g.awayTeam)}-${slugify(g.homeTeam)}`;
      await upsertGame(leagueId, 'cfbd_game_id', g.id, {
        slug, homeTeamId, awayTeamId, kickoffAt, status,
        homeScore: g.homePoints ?? null, awayScore: g.awayPoints ?? null,
        seasonYear, seasonPhase: phase, week: g.week, metadata,
      }, summary);
      summary.ingested += 1;
    }
  }
  // AFTER the games, never before: a media row can only be attached to a match
  // this run has already written, and the ids it joins on are the ones the loop
  // above just upserted.
  let broadcastSummary = null;
  if (broadcasts) {
    broadcastSummary = await syncCfbBroadcasts(leagueId, seasonYear, {
      fetchMedia: (year, st) => cfbdGet(`/games/media?year=${year}&seasonType=${st}`),
    });
  }
  // AFTER the upsert loop, always: upsertGame replaces metadata wholesale and
  // writes the provider's NULL points, so anything written before it would be
  // undone by it on this very tick.
  //
  // AND IT YIELDS TO THE DROPLET. The live poller holds a per-league advisory
  // lock for as long as it is inside a live window, polling the same four
  // columns every thirty seconds. Two writers on one row at different cadences
  // make a score that goes forwards on one tick and backwards on the next, and
  // the slower one wins half the races by arriving last. So this arm asks for
  // the lock and, if the droplet has it, does nothing.
  //
  // YIELDING IS NOT FAILING. The summary says so in words, because a tick
  // ledgered as failed every Saturday trains the alert to be ignored. Nothing
  // else on this tick yields - the schedule baseline, broadcasts and the live
  // box score all still run, because only this arm collides.
  let liveScoreSummary = null;
  if (liveScores) {
    const held = await withAdvisoryLock(LIVE_LOCK('cfb'), async () => syncCfbLiveScores(leagueId, {
      fetchScoreboard: cfbdScoreboardFetcher(),
    }), { connectionString: process.env.DATABASE_URL });
    liveScoreSummary = held.locked
      ? { decision: YIELDED, reason: 'live poller holds the lock' }
      : held.result;
  }
  // THE LIVE BOX SCORE RIDES THE SAME TICK, and must run LAST: it reads
  // matches.status to decide what to write, and the upsert loop above is what
  // sets that status. Board-scoped, so a tick with no live board game makes no
  // provider call at all. Its failure is contained - a live box score is an
  // overlay, and losing it must never fail the games sync that the scoreboard,
  // the strip and the settle all depend on.
  //
  // CONTAINED IS NOT THE SAME AS SILENT. A bare .catch() here once turned a
  // real throw into a string sitting inside this function's own return value
  // - never a failed sync_runs row, never an alert, invisible to everything
  // except someone reading this summary by hand. Four days of every game's
  // box score died behind exactly that shape. recordRun gives the overlay its
  // OWN ledger row (source 'cfb-live-lines') so a failure here marks ok=false
  // and reaches maybeAlert, without ever making syncCfbGames itself throw -
  // the containment this comment already promised stands; only the silence
  // is gone.
  let liveLineSummary = null;
  if (liveLines) {
    const liveLineRun = await recordRun(sql, {
      source: 'cfb-live-lines',
      kind: 'live-poll',
      run: () => syncCfbLiveLines(leagueId),
    });
    liveLineSummary = liveLineRun.ok ? liveLineRun.summary : { error: liveLineRun.error };
    if (!liveLineRun.ok) {
      await maybeAlert(sql, {
        source: 'cfb-live-lines',
        subject: '[pollers] cfb-live-lines FAILED',
        body: `source: cfb-live-lines\nleagueId: ${leagueId}\n\n${liveLineRun.error}`,
      });
    }
  }
  return { ...summary, fcsStubsCreated, skippedNonFbsGame,
    broadcasts: broadcastSummary, liveScores: liveScoreSummary,
    liveLines: liveLineSummary };
}
