...
test('the alert ROW carries the per-game detail, not just a subject line', async () => {
  // Without this, "which games, and by how much" three weeks later means
  // finding the sync run that happened to trigger the alert and reading ITS
  // summary - a join through a timestamp that a forensic read should not need.
  const { maybeAlert, ALERT_DETAIL_CAP } = await import('../pollers/alerts.js');
  const inserted = [];
  // A tagged-template stub: no recent alert, capture the INSERT payload.
  const fakeSql = (strings, ...vals) => {
    const q = strings.join('?');
    if (/SELECT 1 FROM sync_runs/.test(q)) return Promise.resolve([]);
    if (/INSERT INTO sync_runs/.test(q)) { inserted.push(vals); return Promise.resolve([]); }
  ...
  await maybeAlert(fakeSql, { source: 'cfb-games-kickoff', subject: 's', body: 'b', detail });

  assert.equal(inserted.length, 1, 'the alert row must be written');
  const summary = JSON.parse(inserted[0].find((v) => typeof v === 'string' && v.startsWith('{')));
  assert.equal(summary.subject, 's');
...
test('alert detail is capped, with the overflow counted not dropped silently', async () => {
  ...
  await maybeAlert(fakeSql, { source: 's', subject: 's', body: 'b', detail: many });
  const summary = JSON.parse(inserted[0].find((v) => typeof v === 'string' && v.startsWith('{')));
...
test('an alert with no detail still writes a bare subject row', async () => {
  const { maybeAlert } = await import('../pollers/alerts.js');
  const inserted = [];
  const fakeSql = (strings, ...vals) => {
    const q = strings.join('?');
    if (/SELECT 1 FROM sync_runs/.test(q)) return Promise.resolve([]);
  ...
  await maybeAlert(fakeSql, { source: 's', subject: 'plain', body: 'b' });
  const summary = JSON.parse(inserted[0].find((v) => typeof v === 'string' && v.startsWith('{')));
  assert.deepEqual(summary, { subject: 'plain' }, 'no empty detail key on ordinary alerts');
});
