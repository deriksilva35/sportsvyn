...
import { buildDriveChart, gamecastState, lastLivePlay, lastActionPlay, byGameClock, downDistanceLabel, spotLabel, showGamecast } from './driveStrip.js';
...
test('the last-play line and the strip render from the fixture: down and distance, the spot, the text', async () => {
  ...
  // POSSESSION LEFT THIS LINE. The strip said "at NE 34 · NE ball"; the score
  // strip above it now carries a volt dot on the row of the team with the
  // ball (POSSESSION DOT ON THE BOARD relay), so the spot stands alone and
  // the sentence is not said twice on one screen.
  assert.match(strip, /class="ds-at">at <b>/, 'the spot still closes the line');
  assert.doesNotMatch(strip, /NE ball/, 'and no longer says whose ball it is');
...
test('the GAMECAST section: drawn for the live fixture, not for the final one; the tabs do not depend on it', () => {
  const { plays } = normalized();
  const last = lastLivePlay(plays);
  const live = gamecastState({ status: 'live', playCount: plays.length, lastPlay: last, liveState: parseBdlProse(GAME.status) });
  const fin = gamecastState({ status: 'final', playCount: plays.length, lastPlay: last, liveState: null });
  assert.equal(live.mode, 'live'); assert.equal(fin.mode, 'final');
  assert.equal(showGamecast({ mode: live.mode, playCount: plays.length }), true, 'live: the strip and the last play');
  assert.equal(showGamecast({ mode: fin.mode, playCount: plays.length }), false, 'final: no kicker over nothing');
  assert.equal(showGamecast({ mode: 'pending', playCount: 0 }), false, 'no plays, no section');
  assert.equal(showGamecast({ mode: 'pending', playCount: 1 }), true, 'the honest-gap strip still draws');
  assert.equal(showGamecast({ mode: 'none', playCount: 5 }), false);
  // the page: the section is gated on showGamecast; the tabs are gated on plays alone
  const page = readFileSync(new URL('../../app/nfl/game/[slug]/page.js', import.meta.url), 'utf8');
  assert.match(page, /\{showGamecast\(\{ mode: stripState\.mode, playCount: gamecast\?\.plays\?\.length \?\? 0 \}\) \? \(\s*<section className="gg-sect" aria-label="Gamecast">/);
  assert.match(page, /gamecast\?\.plays\?\.length \? \{ key: 'drives', label: 'DRIVES' \} : null/, 'DRIVES tab needs only plays');
  assert.match(page, /const quarters = gamecast\?\.plays\?\.length \? scoringFromPlays\(gamecast\.plays\) : scoringByQuarter\(game\)/, 'SCORING needs only plays');
});
