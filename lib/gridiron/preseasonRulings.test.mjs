...
test('RULING 1: the badge renders for PRE and for nothing else', () => {
  ...
  // "PRE W1" and a preseason final still read as a real one. Status gained tz
  // and withDay in v0.7 (it renders the kickoff, which is now the reader's
  // local time and carries the day); the ADJACENCY is what this pins.
...
test('RULING 3: all three odds paths are phase-gated', () => {
  ...
  //
  // THE PREDICATE GAINED A NULL ARM AND RULING 3 IS UNCHANGED. 'PRE' is a
  // non-null value that is not in COMPETITIVE_PHASES, so it is excluded by
  // exactly the same comparison it always was. What the NULL arm admits is a
  // league that carries NO phase at all - EPL, whose 370 scheduled matches all
  // have season_phase NULL. Equality against a set is NULL for a NULL column,
  // so the old predicate silently excluded every one of them: the first EPL
  // tick matched 0 of 20 events and still reported ok. A league with no phases
  // has no preseason to ignore.
  assert.match(oddsJoin,
    /AND \(season_phase IS NULL OR season_phase = ANY\(\$\{COMPETITIVE_PHASES\}::text\[\]\)\)/,
    'oddsJoin candidate query is gated, with NULL-phase leagues admitted');
  assert.ok(!/'PRE'/.test(oddsJoin), 'and PRE is never named as an allowed phase');
});
