...
test('RULING 1: the badge is muted mono, not another result chip', () => {
  // The FINAL chip beside it is solid jade. A second solid chip would read as a
  // second piece of news rather than as a qualifier on the first.
  const rule = css.slice(css.indexOf('.gi-phase-pre {'), css.indexOf('.gi-up {'));
  assert.match(rule, /font-family: var\(--font-jetbrains-mono\)/, 'mono');
  assert.match(rule, /color: var\(--muted\)/, 'muted, not paper');
  assert.match(rule, /border: 1px solid var\(--rule\)/, 'outlined');
  assert.ok(!/background:/.test(rule), 'NOT filled - that is the FINAL chip s job');
});
