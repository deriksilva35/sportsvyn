...
test('EVERY SURFACE THAT STACKS TWO TEAM ROWS ROUTES THROUGH orderFor - no second rule in the tree', () => {
  const SURFACES = [
    ...
    // components/games/LobbyV2.js (the lobby's Tonight strip) was here and is
    // gone: GAMES v3 drops the strip, so the surface it guarded no longer
    // exists. Nothing replaced it on /games - the v3 screen shows the
    // reader's four games and links to /scores for the slate.
  ...
  // THE PICK'EM ROW NO LONGER HAS A CONNECTOR (v2 board, docs/design/mocks/
  // pickem-v2.html): the mock puts the two tap targets flush against each
  // other, so there is no "at"/"v" word between them to take from the rule.
  // The ORDER is the whole rule on that surface now, and orderFor still
  // decides it - asserted above with every other surface. connectorFor itself
  // is unchanged and still serves the scoreboards and the game pages.
  const pickemBoard = strip(src('components/pickem/PickemBoard.js'));
  assert.match(pickemBoard, /orderFor\(contest\?\.sport\)/, "the Pick'em row orders through the rule");
  assert.doesNotMatch(pickemBoard, /connectorFor/, 'and draws no connector at all');
});
