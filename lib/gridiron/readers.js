...
export async function getNearestUpcomingWeek(leagueSlug, seasonYear) {
  const rows = await sql`
    SELECT m.season_phase, m.week, m.kickoff_at
     ...
     LIMIT 1`;
  if (!rows[0]) return null;
  // kickoff_at drives the days/weeks-out liveness cue on the Today lede. Ordering
  // is by kickoff_at, so a PRE-phase game (if the feed ever carries preseason)
  // naturally sorts ahead of REG by date — no phase-priority logic needed.
...
export async function getEditorialBoard(listSlug, leagueSlug) {
  const rows = await sql`
    SELECT ed.edition_number, ed.edition_label,
