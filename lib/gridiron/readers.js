// lib/gridiron/readers.js — read-only readers for the gridiron surfaces
// (/scores, /nfl, /cfb). "DEV" here once meant the Neon DEV branch these were
// built against while the routes were unlinked; they are public now and read
// whatever DATABASE_URL points at. The half that never went stale is the half
// that matters: NO writes.
// Server-component readers, mirror lib/scheduleData.js:
...
export function rowToGame(r) {
  ...
  return {
    ...
    // THE TWO BASEBALL KEYS, MAPPED HERE AND NOT READ OFF `metadata` BY A
    // CALLER. This function consumes `metadata` and does not pass it on, so a
    // caller reaching for g.metadata?.scoring_plays gets undefined on every
    // game and its feature is silently absent forever - which is exactly what
    // the MLB card's diamond and probables did until the shape was printed.
    // Written by lib/mlb/detail.js and the schedule importer's --probables.
    scoringPlays: meta.scoring_plays ?? null,
    probables: meta.probables ?? null,
...
// ---------------------------------------------------------------------------
// BOTH colors or none: a helmet is never drawn half-dressed.
export const teamColors = (primary, secondary) => (primary && secondary ? { primary, secondary } : null);
...
//     Super Bowl slate).
//
//     THIS IS A FALLBACK NOW, not the primary. The Today tab asks
//     getNearestUpcomingWeek() first and only lands here when nothing is
//     scheduled ahead (TodayPage.js). The note that used to sit here - "against
//     2025 data in July 2026 this is the final REG week, the shells demo on
//     2025 season state" - described the DEV-branch era and has been false
//     since the routes went live on the current season.
// ---------------------------------------------------------------------------
