...
export const LEAGUE_LABEL = Object.freeze({ nfl: 'NFL', cfb: 'CFB', epl: 'EPL', mlb: 'MLB' });
...
/** A side with no abbreviation gets one from its name ("Norfolk State" -> NOR);
 *  the rows and the drive strip share it (GO rider). */
...
/**
 * WHAT MAKES A GAME YOURS. A pick on it, a Weekly player in it, alerts set on
 * it - or, since the TEAM FOLLOWING relay (R2), a followed team playing in
 * it. The first three are things the reader did to a game; the fourth is
 * something they did to a team, and it is the most durable of the four.
 */
export function hasStake(s) {
  return Boolean(s && (s.pick || (s.weekly && s.weekly.length) || s.alerts || s.follow));
}
export const mineCount = (games, stake) => games.filter((g) => hasStake(stake?.get(g.id))).length;

/**
 * THE GROUPS, and who leads.
 *
 * TODAY: Live now, then today's upcoming, then today's finals - a reader on
 * today came for what is happening now.
 *
 * ANY OTHER DAY: THAT DAY LEADS (DAY PICK LEADS relay). Picking Sunday to
 * read the slate and getting eighteen live Saturday cards first is the page
 * answering a question nobody asked. The day's own games come first, by
 * status, and the live games collapse to one line at the top - `liveAway`,
 * which the page renders as "<n> live now · Back to today" in live-red. No
 * live CARD renders on a non-today day.
 *
 * Empty groups are omitted. `sport` narrows by league, `mine` to staked
 * matches, and both narrow the collapse count too - a Mine reader is told
 * how many of THEIR games are live, not how many exist.
 *
 * @returns {{ groups: Array, liveAway: {count:number}|null }}
 */
/** A CFB game with a ranked side - the Top 25 filter's only test (R3). */
export function rankedGame(g, extras = null) {
  if (g?.leagueSlug !== 'cfb') return false;
  const r = extras?.get?.(g.id)?.rank ?? null;
  return r != null && (r.home != null || r.away != null);
}

/**
 * R5, THE WHOLE RULING, in one testable place.
 *
 * `rankedToday` decides whether the pill exists at all: the picked day must
 * hold a CFB game with a ranked side, UNDER the league filter in force, so a
 * reader standing on NFL is never offered a filter that would empty the board.
 *
 * `top25` is whether the flag APPLIES. A stale ?top25=1 carried from a ranked
 * day onto an unranked one stops applying rather than emptying the board - and
 * the pill that set it is not on screen to unset.
 */
export function top25State(onPickedDay, extras, { sport = 'all', requested = false } = {}) {
  const rankedToday = (sport === 'all' || sport === 'cfb')
    && onPickedDay.some((g) => rankedGame(g, extras));
  return { rankedToday, top25: Boolean(requested) && rankedToday };
}

/**
 * "N games" plus whatever follows it - the ONE place that sentence is built.
 *
 * It is exported because the count can change after the group is made: the
 * YOURS band lifts games out, and a heading that still said 14 over 12 cards
 * shipped on 20 Sep. A second copy of this template in the band would be the
 * same defect one refactor later, so both callers say it here.
 */
export function gamesSub(n, tail = '') {
  return `${n} game${n === 1 ? '' : 's'}${tail}`;
}

export function groupGames(games, { date, today, tz = ET, sport = 'all', mine = false, top25 = false, stake = null, extras = null, signedIn = false } = {}) {
  // TOP 25 IS A CFB FILTER AND NOTHING ELSE (R3, R4): a game survives it when
  // the ONE AP reader gave either side a number. The ranks arrive on `extras`,
  // so this filter and the badge on the card can never disagree.
  const base = games.filter((g) => (sport === 'all' || g.leagueSlug === sport)
    && (!mine || hasStake(stake?.get(g.id))));
  const isToday = date === today;
  // WHAT THIS DAY WOULD DRAW WITHOUT THE TOP 25 FILTER, which is not the same
  // as what is on the slate: a day that is not today does not draw the live
  // games at all - they collapse to the one-line liveAway link - so a ranked
  // game live RIGHT NOW must not put the pill on tomorrow's board.
  const shown = base.filter((g) => (g.status === 'live' ? isToday : viewerDay(g.kickoffAt, tz) === date));
  const { rankedToday, top25: useTop25 } = top25State(shown, extras, { sport, requested: top25 });
  const keep = useTop25 ? base.filter((g) => rankedGame(g, extras)) : base;
  ...
  const groups = [];
  if (isToday && live.length) groups.push({ key: 'live', title: 'Live now', sub: 'updates every 30s', games: live });
  // THE DAY GROUP'S SUB LEADS WITH A COUNT, so it carries the tail that
  // follows the count and is marked as rebuildable. Anything that takes games
  // OUT of a group after this point - the YOURS band does exactly that - has to
  // restate the number, and it cannot do that by pattern-matching a sentence.
  // The two other subs carry no count and are left alone by construction.
  if (upcoming.length) {
    const tail = signedIn ? ' · your picks lock at kick' : '';
    groups.push({ key: 'day', title: dayTitle, sub: gamesSub(upcoming.length, tail), subTail: tail, games: upcoming });
  }
  if (finals.length) groups.push({ key: 'final', title: 'Final', sub: weekdayOf(date), games: finals });
  return { groups, liveAway: !isToday && live.length ? { count: live.length } : null, rankedToday, top25: useTop25 };
}
...
export function driveStripFor({ play, lastText = null, game }) {
  ...
  return {
    ...
    spot: spotLabel(ytg, abbrOf(offense) || null, abbrOf(defense) || null),
    offenseAbbr: abbrOf(offense) || null,
    pct,
