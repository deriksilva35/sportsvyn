...
async function upsertGame(leagueId, providerId, g) {
  ...
  if (existing) {
    // FINAL IS RECORDED THE FIRST TIME IT IS SEEN, AND NEVER RETRACTED.
    //
    // The provider walks statuses backwards. On 13 Aug it took two finished
    // games from final back to live inside sixty seconds, twice, and the
    // post-whistle detail fetch - which read `status = 'final'` at the instant
    // it ran - missed both on the first pass. One game (TEN at SF) never got
    // its final version at all, and its brief published a score that was two
    // field goals stale.
    //
    // A timestamp cannot be walked backwards. This writes it the first time the
    // feed says final and leaves it alone forever after: the COALESCE keeps the
    // original instant, so a flap to live and back does not move it, and the
    // jsonb || merge means the sibling `detail` keys survive untouched.
    if (g.status === 'final') {
      await sql`
        UPDATE matches
           SET metadata = COALESCE(metadata, '{}'::jsonb) || jsonb_build_object(
                 ...
                 COALESCE(metadata->'detail', '{}'::jsonb) || jsonb_build_object(
                   'final_seen_at',
                   COALESCE(metadata->'detail'->>'final_seen_at', to_char(now() AT TIME ZONE 'UTC', 'YYYY-MM-DD"T"HH24:MI:SS"Z"'))
                 ))
...
export async function toMatchRow(game, { leagueSlug, leagueId, teams, runSummary }) {
  ...
  return {
    ...
    row: {
      ...
      metadata: {
        // WHERE IN THE GAME - the provider's own period short (Q1..Q4/OT/HT)
        // plus the game clock, snapshotted each poll. A FLAT top-level key on
        // purpose: the shallow || merge in upsertGame replaces it wholesale
        // every tick and cannot touch the nested `detail` sibling (the 14 Aug
        // law). Written as null whenever the game is not live, so a stale
        // clock can never outlive the game it described.
        live_state: status === 'live'
          ? { short: g.status?.short ?? null, clock: g.status?.timer ?? null }
          : null,
...
export async function importApiSportsGames({
  ...
  // `date` may be a single day or a LIST of days. The provider indexes its
  // slate by UTC date and one ET football evening spans two of them, so the
  // poller asks for both - see slateDatesForProvider in lib/pollers/
  // preseasonWindow.js for the evidence. A bare string still works, and a null
  // still means "the whole season", so every existing caller is unaffected.
  const dates = date == null ? [null] : (Array.isArray(date) ? date : [date]);
  const seen = new Set();
  const games = [];
  for (const d of dates) {
    const batch = await apiSportsFootball.games({ league: NFL_LEAGUE_ID, season, date: d });
    for (const g of batch ?? []) {
      // Deduped on the PROVIDER'S game id. Two UTC days cannot contain the same
      // kickoff, so in practice this never fires - it is here so that a change
      // to the date window can never turn into double-counted ingests, which
      // would be invisible in the row counts and wrong in the ledger.
      const id = g?.game?.id;
  ...
  runSummary.requests = dates.length;
