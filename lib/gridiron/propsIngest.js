    ...
    for (const key of markets) {
      ...
      for (const [who, e] of collectOverUnder(bookmakers, key)) {
        ...
        if (!e.paired) {
          ...
          // RAW IMPLIED, NOT NULL - and that is the anytime treatment exactly.
          // implied_probability is NOT NULL on this table, and passing null
          // failed the whole EPL leg on first contact. It was also incoherent:
          // anytime rows store decToImplied(price), the book's raw number with
          // its margin still in it, and "same treatment family as anytime" has
          // to mean the same STORAGE, not a different one. What makes it
          // as-offered is that no overround was removed - which the render
          // says, and which the reader derives from the row's own shape.
          upserted += await upsertProp(sql, {
            matchId: row.id, marketType: key,
            selectionLabel: `${who} ${e.singleName}`,
            selectionValue: line == null ? null : `${line}`,
            decimalOdds: dec, impliedPct: decToImplied(dec), books,
          });
          continue;
        }
