...
export async function propsScope(sql, { leagueSlug }) {
  if (leagueSlug === 'cfb') {
    return sql`
      SELECT DISTINCT m.id, m.slug, m.kickoff_at,
  ...
  return sql`
    ...
    SELECT m.id, m.slug, m.kickoff_at, h.name AS home_name, a.name AS away_name
      FROM matches m
      JOIN leagues l ON l.id = m.league_id
...
// Match a vendor event to one of our scoped matches by both team names.
function matchEvent(events, row) {
  const home = normalizeName(row.home_name ?? '');
  const away = normalizeName(row.away_name ?? '');
  return events.find((e) => normalizeName(e.home_team ?? '') === home
    && normalizeName(e.away_team ?? '') === away) ?? null;
}
...
function collectOverUnder(bookmakers, marketKey) {
  ...
  for (const bk of bookmakers ?? []) {
    ...
    for (const [who, sides] of seen) {
      ...
      if (!byPlayer.has(who)) byPlayer.set(who, { over: [], under: [], points: [], books: [] });
...
export async function ingestSportProps(sql, { sport, leagueSlug }) {
  ...
  for (const row of scope) {
    const ev = matchEvent(events, row);
