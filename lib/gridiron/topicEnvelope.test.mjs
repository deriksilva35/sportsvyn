...
const MARK = `topicenv-${process.pid}x${Date.now().toString(36)}`;

// ---------------------------------------------------------------------------
// THE NFL SEASON RECORD, SEEDED.
//
// DEV's NFL anchor season had exactly ONE played game in it - the football
// season has barely started on this database - so the Ravens had no record, and
// two tests that assert a named team produces a POPULATED envelope were red for
// a data gap rather than a defect. CFB was fine, with 934 games behind it, which
// is the tell: the reader works, the database is empty.
//
// SENTINEL ROWS, OUTSIDE THE REAL SCHEDULE. Week 99 of the anchor season, slugs
// carrying a per-run mark, deleted in after() with the teardown asserting
// itself. Week 99 is the isolation that matters: `node --test` runs files in
// parallel, and a weekly-board fixture asking this database for week 1 or week 2
// must not find three games nobody played.
const SEED_WEEK = 99;
let seeded = 0;

before(async () => {
  // A run killed before its own cleanup leaves rows behind. An hour is longer
  // than any file runs, so anything older belongs to nobody.
  await sql`DELETE FROM matches WHERE slug LIKE 'topicenv-%' AND created_at < now() - interval '1 hour'`;
  const anchor = await seasonAnchor('nfl');
  const [lg] = await sql`SELECT id FROM leagues WHERE slug = 'nfl'`;
  const teams = await sql`
    SELECT id, name FROM teams
     WHERE league_id = ${lg.id} AND name IN ('Baltimore Ravens', 'Cleveland Browns', 'Pittsburgh Steelers')`;
  const by = new Map(teams.map((t) => [t.name, t.id]));
  const bal = by.get('Baltimore Ravens');
  const year = anchor.seasonYear ?? new Date().getUTCFullYear();
  // Two wins and a loss: the assertion is wins + losses > 0, and a record that
  // is all wins would not prove the losing side of the UNION ALL runs.
  const games = [
    { opp: by.get('Cleveland Browns'), home: true, hs: 27, as: 17 },
    { opp: by.get('Pittsburgh Steelers'), home: false, hs: 13, as: 24 },
    { opp: by.get('Cleveland Browns'), home: false, hs: 30, as: 21 },
  ];
  for (const [i, g] of games.entries()) {
    await sql`
      INSERT INTO matches (league_id, slug, kickoff_at, status, season_year, season_phase, week,
                           home_team_id, away_team_id, home_score, away_score)
      VALUES (${lg.id}, ${`${MARK}-${i}`}, now() - interval '7 days', 'final',
              ${year}, 'REG', ${SEED_WEEK},
              ${g.home ? bal : g.opp}, ${g.home ? g.opp : bal}, ${g.hs}, ${g.as})`;
    seeded += 1;
  }
});

after(async () => {
  const gone = await sql`DELETE FROM matches WHERE slug LIKE ${`${MARK}-%`} RETURNING id`;
  // THE TEARDOWN ASSERTS ITSELF. A cleanup that quietly removed nothing is how
  // sentinel rows become permanent residents of a shared database.
  assert.equal(gone.length, seeded, 'every seeded match is removed');
  const [{ n }] = await sql`SELECT count(*)::int AS n FROM matches WHERE slug LIKE ${`${MARK}-%`}`;
  assert.equal(n, 0, 'and none is left behind');
});
