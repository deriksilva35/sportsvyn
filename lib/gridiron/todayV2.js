 ...
 *
 * SEVEN FIELDS, ALL OFF COLUMNS getWeekSlate ALREADY SELECTS (readers.js:196
 * takes home_score, away_score and both abbreviations). The first cut kept
 * only status, live_state and kickoff, so a Weekly slot could say "Q3 7:28"
 * but not WHO the player's team was playing, and a final could not say the
 * score it ended on. No new query, no new column - the data was being thrown
 * away one line after it arrived.
 */
export async function weekTeamGames({ week, seasonYear, sport = 'nfl' } = {}) {
  ...
  for (const g of games) {
    for (const t of [g.home, g.away]) {
      ...
      if (!a) continue;
      // ORIENTED TO THE KEYED TEAM, which is the whole reason this is a map
      // per team rather than per game. BUF's entry reads "vs DET, 31-24" and
      // DET's reads "at BUF, 24-31" off the same row - so a slot line can say
      // what happened to THAT player's team without the caller working out
      // which side of the game he was on.
      ...
      out.set(a, {
        ...
        home: isHome,
        // NULL, NEVER 0, BEFORE A GAME. matches.home_score is null until the
        // feed scores it, and a 0-0 printed at 1pm Sunday is a wrong number
        // rather than a low one - the same rule the points on these rows
        // already follow.
