...
test('SINGLE-SIDED O/U stores as-offered, and the fork is on the DATA', () => {
  ...
  assert.match(ING, /impliedPct: null, books/, 'as-offered carries no implied %');
...
test('the /market card notes as-offered from the ROW, not a market list', () => {
  const READS = strip(src('lib/market/reads.js'));
  assert.match(READS, /meta\?\.anytime \|\| r\.implied_probability == null/,
    'a null implied % is what "as offered" means, whatever market produced it');
});
