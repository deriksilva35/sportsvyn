...
export async function syncNfl2025({ log = () => {} } = {}) {
  const summary = { pages: 0, apiStatRows: 0, distinctPlayers: 0, statRowsWritten: 0, skippedNoMatchGame: 0, skippedNoPlayer: 0 };
  ...
  const matchRows = await sql`
    ...
    WHERE l.slug = 'nfl' AND m.season_year = 2025 AND m.external_ids ? 'bdl_game_id'`;
  const matchByBdl = new Map(matchRows.map((r) => [String(r.bdl), r.id]));
  log(`maps: ${teamByBdl.size} teams, ${matchByBdl.size} NFL-2025 matches`);
  ...
  let cursor = null, page = 0;
  do {
    const j = await bdlGet(`/nfl/v1/stats?seasons[]=2025&per_page=100${cursor ? `&cursor=${cursor}` : ''}`);
    page += 1;
