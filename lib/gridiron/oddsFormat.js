...
// THE PROPS PANEL IS NOT FROZEN AT KICKOFF, and the lines strip above it still
// is. Its reader's slate admits live games, because a reader watching one wants
// the last prices the books posted - stamped "pre-kick" by the panel itself,
// never presented as a running line. A FINAL IS STILL OUT: once the game is
// over the price is history and the result belongs to the page, not to a
// market read. Separate from isPreGame so widening one never widens the other.
export function showsProps(status) {
