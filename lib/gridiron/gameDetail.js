...
/**
 * THE BOX SCORE, PURE. nfl_player_game_stats rows (joined to the player's
 * name) -> per team, four groups: passing (cmp/att, yds, td, int), rushing
 * (att, yds, td), receiving (rec, tgt, yds, td), kicking (fg m/a, xp).
 * Each group sorted by yards desc (kicking by fgm desc); a group with no
 * lines is omitted; a team with no groups is omitted; no rows -> null, so
 * the page grows no tab.
 */
export const BOX_GROUPS = [
  { key: 'passing', label: 'PASSING', headings: ['C/ATT', 'YDS', 'TD', 'INT'],
    has: (r) => r.pass_att > 0 || r.pass_cmp > 0, cells: (r) => [`${r.pass_cmp ?? 0}/${r.pass_att ?? 0}`, r.pass_yds ?? 0, r.pass_td ?? 0, r.pass_int ?? 0], sort: (r) => r.pass_yds ?? 0 },
  { key: 'rushing', label: 'RUSHING', headings: ['ATT', 'YDS', 'TD'],
    has: (r) => r.rush_att > 0, cells: (r) => [r.rush_att ?? 0, r.rush_yds ?? 0, r.rush_td ?? 0], sort: (r) => r.rush_yds ?? 0 },
  { key: 'receiving', label: 'RECEIVING', headings: ['REC', 'TGT', 'YDS', 'TD'],
    has: (r) => r.rec > 0 || r.tgt > 0, cells: (r) => [r.rec ?? 0, r.tgt ?? 0, r.rec_yds ?? 0, r.rec_td ?? 0], sort: (r) => r.rec_yds ?? 0 },
  { key: 'kicking', label: 'KICKING', headings: ['FG', 'LONG', 'XP'],
    has: (r) => r.fga > 0 || r.fgm > 0 || r.xp > 0, cells: (r) => [`${r.fgm ?? 0}/${r.fga ?? 0}`, r.fg_long ?? '–', r.xp ?? 0], sort: (r) => (r.fgm ?? 0) * 100 + (r.xp ?? 0) },
];

export function buildBoxScore(rows, game) {
  ...
  for (const t of teams) {
    ...
    const groups = BOX_GROUPS.map((g) => {
      const lines = mine.filter(g.has).sort((a, b) => g.sort(b) - g.sort(a) || String(a.name).localeCompare(String(b.name)));
      return lines.length ? { key: g.key, label: g.label, headings: g.headings, rows: lines.map((r) => ({ name: r.name, position: r.position ?? null, cells: g.cells(r) })) } : null;
    }).filter(Boolean);
    if (groups.length) out.push({ teamId: t.id, abbr: t.abbreviation ?? '', groups });
  }
...
export async function getGamePage(slug) {
  ...
  const [events, lines, statRows] = await Promise.all([
    ...
    // THE BOX SCORE (NFL): nfl_player_game_stats, the table the Weekly settles
    // from, written live by the poller and at final. CFB has none here.
    m.league_slug === 'nfl'
      ? sql`SELECT s.team_id, s.nfl_player_id, p.full_name AS name, p.position,
                   s.pass_cmp, s.pass_att, s.pass_yds, s.pass_td, s.pass_int,
                   s.rush_att, s.rush_yds, s.rush_td, s.tgt, s.rec, s.rec_yds, s.rec_td,
  ...
  return {
    ...
    // short_name IS THE MIDDLE OF resolveAbbr's THREE SOURCES and this query
    // did not select it, so a page-built card fell all the way to the derived
    // form ("PS") while the poller's next update - which has the column - said
    // "Portland State". One card, two names, a tick apart. One column closes
    // it, and it is additive for every other reader of this shape.
    home: { id: m.home_id, name: m.home_name, abbreviation: m.home_abbr, short_name: m.home_short_name, colors: teamColors(m.home_c1, m.home_c2) },
    away: { id: m.away_id, name: m.away_name, abbreviation: m.away_abbr, short_name: m.away_short_name, colors: teamColors(m.away_c1, m.away_c2) },
    events,
    lines,
    boxScore: buildBoxScore(statRows, { home: { id: m.home_id, abbreviation: m.home_abbr }, away: { id: m.away_id, abbreviation: m.away_abbr } }),
  };
}
...
const SCORING_TYPE = [
  [/touchdown/, 'TD'], [/field-goal-good|field_goal_good/, 'FG'], [/safety/, 'SAFETY'],
  [/two-point|2pt/, '2PT'], [/extra-point|point-after|pat/, 'XP'],
];
/**
 * SCORING FROM THE PLAYS TABLE, not match_events - nothing writes NFL
 * match_events yet, and every scoring play is already a plays row with
 * scoring=true and the running score on it. Same quarters shape ScoringPanel
 * renders; team_id is the offense (a defensive score credits the side that
 * had the ball, which is the one limit of this reading).
 */
export function scoringFromPlays(plays) {
  const byQ = new Map();
  let seq = 0;
  for (const p of plays ?? []) {
    if (!p.scoring) continue;
    const t = String(p.playType ?? '');
    const scoring_type = (SCORING_TYPE.find(([re]) => re.test(t)) ?? [null, 'SCORE'])[1];
    const e = {
      seq: seq += 1, quarter: p.period ?? 0, team_id: p.offenseTeamId ?? null, scoring_type,
      description: p.text ?? null, clock: p.clock ?? null, home_score: p.homeScore ?? null, away_score: p.awayScore ?? null,
    };
    if (!byQ.has(e.quarter)) byQ.set(e.quarter, []);
    byQ.get(e.quarter).push(e);
  }
  return [...byQ.entries()].sort((a, b) => a[0] - b[0]).map(([quarter, list]) => ({
    quarter, label: quarter > 4 ? (quarter === 5 ? 'OT' : `OT${quarter - 4}`) : `Q${quarter}`, plays: list,
  }));
}
