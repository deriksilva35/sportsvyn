...
/**
 * The live chip: where in the game, from the poller's snapshot. PURE, and the
 * ONE formatter both the card status row and anything else may use - the
 * numcols law applied to a clock.
 *
 * 'Q1'..'Q4'/'OT' render with the clock when one rides along ('Q4 · 8:12');
 * halftime is 'HALF' (the provider distinguishes it: HT / Halftime); a
 * quarter break ('end of period', observed 13 Aug) is 'END Q' - no clock,
 * there isn't one running. An unrecognized short renders AS ITSELF, uppercase:
 * honest beats invented. Null in, null out - schedulued and final games have
 * no chip (final says FINAL, never a clock).
 *
 * THE CLOCK IS A SNAPSHOT at the poll cadence, and the renderer must treat it
 * that way - no ticking, it updates when the poll does.
 *
 * THE SHORT CODE COMES FROM shortOf() (lib/live/vocabulary.js), NOT FROM
 * liveState.short DIRECTLY - this used to read .short bare, which is why
 * every live card ran with no quarter or clock: the poller that actually
 * writes live_state (services/live-poller/poll.mjs) has only ever written
 * {period, clock}, never a .short key, and this function returned null for
 * every one of those rows without ever noticing. shortOf() reads either
 * shape a row may hold.
 */
export function liveChip(liveState) {
  const s = shortOf(liveState);
  if (!s) return null;
  if (s === 'HT' || s === 'HALFTIME') return 'HALF';
  if (s === 'EOP' || s === 'END OF PERIOD') return 'END Q';
  const clock = liveState?.clock ? String(liveState.clock) : null;
  return clock ? `${s} · ${clock}` : s;
}

/** 1-4 for Q1..Q4, 5 for OT; null when the short names no period. */
export function periodOf(liveState) {
  const s = shortOf(liveState) ?? '';
  if (/^Q[1-4]$/.test(s)) return Number(s[1]);
  if (s === 'OT') return 5;
  if (s === 'HT' || s === 'HALFTIME') return 2;
  return null;
}
...
export function lineScoreGrid(game) {
  ...
  // LIVE GAMES SHOW ONLY THE QUARTERS REACHED: a Q2 game renders '1 2', not
  // four columns of dashes. The period comes from the poller's live_state;
  // a live game without one falls back to the full four (honest, not empty).
  // Final and everything else show all four, plus OT only when scored in.
  const period = game?.status === 'live' ? periodOf(game?.liveState) : null;
  const through = period != null ? Math.min(Math.max(period, 1), QUARTERS) : QUARTERS;
  const columns = [
    ...['1', '2', '3', '4'].slice(0, through),
    // OT can only follow a full regulation - the guard keeps a glitched OT
    // value from stealing a mid-game quarter's column index.
    ...(through === QUARTERS && (hasOt || period === 5) ? ['OT'] : []),
  ];
