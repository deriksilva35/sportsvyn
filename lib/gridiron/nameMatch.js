...
export async function matchPoolIdentities(sql, { log = () => {} } = {}) {
  ...
  // Distinct pool identities — NFL ONLY.
  //
  // A COLLEGE ROW HAS NO NFL IDENTITY, AND GUESSING ONE IS WORSE THAN LEAVING IT
  // DARK. matched_player_id is what the room's PPG, the player sheet and the
  // stat sorts join through, so a college row that resolves to an NFL player
  // does not show a blank cell - it shows ANOTHER MAN'S STATS, under the college
  // player's name, with no way for a reader to tell.
  //
  // MEASURED on the real 927-row college board, 2 Sep 2026, before this scope
  // existed: 6 college players matched an NFL player on normalized name +
  // position - Chris Henry (OSU) onto nfl_players 3522, Chris Johnson (Clem)
  // onto 4539, Daequan Wright (LSU) onto 11181, Sutton Smith (Ark) onto 8433,
  // Marcus Sanders (GaSo) onto 11069, Darius Johnson (Syr) onto 6258 - and 3
  // school defenses landed on NFL clubs whose abbreviation their school code
  // happens to be: Ind -> Indianapolis Colts, MIN -> Minnesota Vikings,
  // Hou -> Houston Texans. Nine wrong identities out of one import.
  //
  // College identity resolves against the CFB tables instead (players.full_name
  // + cfb_player_game_stats), which is a different matcher against a different
  // roster; until it exists, a college row's matched_player_id stays NULL and
  // its stat cells stay honestly empty.
  const identities = await sql`
    SELECT DISTINCT name, position, team FROM sim_player_pool
     WHERE league = 'nfl' ORDER BY position, name`;
  ...
  for (const idn of identities) {
    ...
    // The league predicate rides on the WRITE too, not only the read: without it
    // an NFL identity could still reach a college row that happens to share the
    // (name, position, team) triple - the same class of over-broad write the
    // 2 Sep DST note above describes, one column further out.
