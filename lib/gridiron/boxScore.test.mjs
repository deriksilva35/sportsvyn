...
test('the tab: offered only when boxScore has rows, after DRIVES; the component renders nothing on empty', async () => {
  ...
  assert.match(page, /boxscore: game\.boxScore\?\.length \? <BoxScore boxScore=\{game\.boxScore\} teams=\{teams\} leagueSlug=\{game\.leagueSlug\} \/> : null/);
  install();
  ...
  const box = buildBoxScore(rowsFromFixture(), { away: { id: 101, abbreviation: 'NE' }, home: { id: 131, abbreviation: 'SEA' } });
  const html = renderToStaticMarkup(React.createElement(BoxScore, { boxScore: box, leagueSlug: 'nfl', teams: [{ id: 101, colors: { primary: '#002244', secondary: '#C60C30' } }, { id: 131, colors: { primary: '#002244', secondary: '#69BE28' } }] }));
  assert.equal((html.match(/class="gg-boxhead"/g) ?? []).length, 2, 'one header row per team');
  assert.equal((html.match(/<img class="teammark teammark--headgear gg-hm"[^>]*data-facing="right"/g) ?? []).length, 2, 'NE and SEA: headgear per team, facing right');
  // NO LEAGUE, NO HEADGEAR: the same box score without its league draws discs.
  const bare = renderToStaticMarkup(React.createElement(BoxScore, { boxScore: box, teams: [{ id: 101, colors: { primary: '#002244', secondary: '#C60C30' } }, { id: 131, colors: { primary: '#002244', secondary: '#69BE28' } }] }));
  assert.equal((bare.match(/data-teammark="circle"/g) ?? []).length, 2, 'no league -> two discs');
  assert.doesNotMatch(bare, /headgear/);
  ...
  // TWO SPORTS ON ONE TRACKER NOW, and it is a deliberate widening. MLB's
  // box-score ingest existed and was called by nothing, so
  // mlb_player_game_stats stayed empty on every game ever played - which
  // would have settled every October card and every Run roster to zero and
  // looked healthy doing it. It joins here rather than growing a second
  // cadence, so both sports' pulls are counted against one quota.
  assert.match(poller, /const stats = \(lg\.slug === 'nfl' \|\| lg\.slug === 'mlb'\) \? new StatsTracker\(\) : null/,
    'NFL and MLB, and nothing else');
  assert.match(poller, /import \{ syncMlbGameStats \} from '\.\.\/\.\.\/lib\/mlb\/statsSync\.js'/);
  // THE LEAGUE PICKS ITS OWN SYNC, and the watched-games query is scoped to
  // the league being polled rather than hard-coded to 'nfl' - which it was,
  // and which would have fetched NFL box scores on an MLB tick.
  assert.match(poller, /const syncBox = lg\.slug === 'mlb' \? syncMlbGameStats : syncGameStats/);
  assert.match(poller, /WHERE l\.slug = \$\{lg\.slug\} AND \(m\.status = 'live'/);
