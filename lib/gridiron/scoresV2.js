...
import { baseballStrip } from '../mlb/strip.js';
import { decisionLine, probablesLine } from '../mlb/cardLines.js';
...
import { withYoursBand } from '../scores/yours.js';
...
async function windowGames(today, tz) {
  ...
  const rows = await sql`
    SELECT m.id, m.slug, m.status, m.kickoff_at, m.season_year, m.season_phase, m.week,
     ...
     WHERE l.slug IN ('nfl', 'cfb', 'epl', 'mlb')
...
/**
 * THE MLB FINAL'S FOOT: the winning pitcher and the bat of the night, per the
 * mock. Its own query and not an arm of statLines(), because it reads a
 * different table with no column in common - pass_att means nothing here and
 * total_bases means nothing there.
 *
 * ONE QUERY FOR THE WHOLE BOARD, and the shaping is done in lib/mlb/cardLines.
 * The rows come back whole rather than pre-aggregated in SQL: "bat of the
 * night" is a ranking with three tiebreaks, and a ranking that has to be
 * read to be believed belongs in a tested pure function, not in an ORDER BY
 * nobody can run in isolation.
 */
async function mlbFootLines(finals) {
  const ids = finals.filter((g) => g.leagueSlug === 'mlb').map((g) => g.id);
  if (!ids.length) return { lines: new Map(), hasRows: new Set() };
  const rows = await sql`
    SELECT match_id, player_name, at_bats, hits, home_runs, rbi, total_bases,
           outs_recorded, strikeouts_pitched, earned_runs, wins
      FROM mlb_player_game_stats WHERE match_id = ANY(${ids})`;
  const by = new Map();
  for (const r of rows) {
    if (!by.has(r.match_id)) by.set(r.match_id, []);
    by.get(r.match_id).push(r);
  }
  const lines = new Map();
  for (const [id, rs] of by) {
    const line = decisionLine(rs);
    if (line) lines.set(id, line);
  }
  return { lines, hasRows: new Set(by.keys()) };
}
...
export async function scoresV2({ userId = null, date = null, sport = 'all', mine = false, top25 = false, tz = ET, now = new Date() } = {}) {
  ...
  const ids = games.map((g) => g.id);
  // THE DRIVE STRIP IS FOOTBALL'S, AND IT IS NOW NAMED THAT WAY. This filter
  // used to read `!== 'epl'`, which meant "everything that is not soccer" -
  // true while the only leagues were two gridiron ones and one soccer one, and
  // wrong the moment a third sport arrived: MLB would have been handed the
  // football play reader and drawn a down-and-distance on a baseball card.
  const liveFootball = games
    .filter((g) => g.status === 'live' && (g.leagueSlug === 'nfl' || g.leagueSlug === 'cfb'))
    .map((g) => g.id);
  const finals = games.filter((g) => g.status === 'final');
  const [records, spreads, totals, prev, plays, stats, mlbFoot, probs, stake, ap] = await Promise.all([
    ...
    empty({ lines: new Map(), hasRows: new Set() })(mlbFootLines(finals)),
  ...
  const extras = new Map(games.map((g) => [g.id, {
    ...
    // THE BASEBALL STRIP SITS IN ITS OWN SLOT, not in `drive`. They occupy the
    // same place on the card and describe entirely different things, and a
    // renderer that had to ask which sport a `drive` belonged to would be the
    // shared-vocabulary mistake this build has been avoiding all the way down.
    // Null for every football and soccer card, by construction.
    diamond: g.leagueSlug === 'mlb'
      ? baseballStrip({
        status: g.status,
        liveState: g.liveState,
        scoringPlays: g.scoringPlays ?? [],
      })
      : null,
    stat: stats.lines.get(g.id) ?? null,
    hasStats: stats.hasRows.has(g.id) || mlbFoot.hasRows.has(g.id),
    // THE FINAL'S FOOT AND THE PRE-GAME'S, both the mock's, both MLB-only and
    // null everywhere else - the card asks the sport once rather than asking
    // each line whether it applies.
    mlbFoot: mlbFoot.lines.get(g.id) ?? null,
    probables: g.leagueSlug === 'mlb' ? probablesLine(g.probables) : null,
  ...
  // THE YOURS BAND RIDES ON TOP OF THE GROUPS groupGames ALREADY BUILT, and
  // takes its games out of them. It is applied here rather than inside
  // groupGames because it is not a grouping rule - it reads the reader's
  // stake, which groupGames only ever uses as a yes/no filter for ?mine=1.
  // Signed out it is a no-op, and the ?mine=1 view is left alone: a board
  // already filtered to the reader's games has nothing to lift to the top.
  const banded = mine ? groups : withYoursBand(groups, { games, stake, signedIn: userId != null });
  return {
    ...
    days, groups: banded, liveAway, extras,
  };
}
