...
export async function scoresV2({ userId = null, date = null, sport = 'all', mine = false, tz = ET, now = new Date() } = {}) {
  ...
  const [records, spreads, totals, prev, plays, stats, probs, stake] = await Promise.all([
  ...
  const { groups, liveAway } = groupGames(games, { date: picked, today, tz, sport, mine, stake, signedIn: userId != null });
  return {
    today, date: picked, tz, sport, mine,
