 ...
 */
export async function stakeForMatches(userId, games, { now = new Date(), db = sql, weeklyRows = null } = {}) {
  const out = new Map();
  ...
  const [entries, alerts, weekly] = await Promise.all([
  ...
  for (const g of games) {
    ...
    if (pick || w.length || al) out.set(g.id, { pick, weekly: w, alerts: al });
  }
  return out;
}
...
export async function scoresV2({ userId = null, date = null, sport = 'all', mine = false, top25 = false, tz = ET, now = new Date() } = {}) {
  ...
  const { groups, liveAway } = groupGames(games, { date: picked, today, tz, sport, mine, top25, stake, extras, signedIn: userId != null });
  // R5: the pill exists only when the picked day has a CFB game with a
  // ranked side. A filter that would show nothing never renders.
  const onPickedDay = games.filter((g) => g.status === 'live' || viewerDay(g.kickoffAt, tz) === picked);
  const rankedToday = onPickedDay.some((g) => g.leagueSlug === 'cfb'
    && (extras.get(g.id)?.rank.home != null || extras.get(g.id)?.rank.away != null));
  return {
    today, date: picked, tz, sport, mine, top25, rankedToday, apWeek: ap.week, apSeason: ap.season,
