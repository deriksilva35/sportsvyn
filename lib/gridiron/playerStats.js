...
import { maxOf } from '../util/scoresOf.js';
...
export function careerFrom(seasons, columns) {
  ...
  for (const col of columns) {
    if (col.agg === 'max') {
      // maxOf, NOT map(Number): a season with no value for this column became a
      // 0 and could win the max outright on any column that goes negative.
      out[col.key] = maxOf(seasons.map((s) => s[col.key]));
      continue;
    }
