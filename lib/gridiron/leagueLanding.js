...
/**
 * THE SCREEN EYEBROW. "Sat Sep 13 · Week 2", or just today's date.
 *
 * IT NAMES TODAY, AND THE DAY LEADS. It used to name the week's FIRST kickoff,
 * because the landing handed it allGames[0].kickoffAt - so on Sunday 13 Sep
 * the tab called Today read "Week 1 · Wed Sep 9" and pointed at a past
 * Wednesday. The date a reader wants under a heading called Today is today's.
 * The week number follows it as the context it is.
 *
 * REG-ONLY LANDMARK LAW, unchanged: a week number is a regular-season fact.
 * Preseason and postseason weeks count differently and a bare "Week 3" that
 * silently means preseason week 3 is worse than no week at all. A failed
 * derivation - no week, or a phase that is not REG - renders the date alone,
 * which is always true.
 */
export function landingEyebrow({ week, phase, now = new Date(), tz = 'America/New_York' } = {}) {
  ...
  return [day, wk].filter(Boolean).join(' · ') || null;
}
