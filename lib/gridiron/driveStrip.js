 ...
 */
/**
 * THE GAMECAST SECTION IS FOR A GAME IN PLAY. Plays alone are not enough:
 * on a final game the strip and the last play are withheld and the drive
 * chart lives in the DRIVES tab, so the section would be a kicker over
 * nothing (served 10 Sep on the NE-SEA final). None, pending, between,
 * halftime and live all draw something; final does not.
 */
export function showGamecast({ mode, playCount = 0 }) {
  return playCount > 0 && mode !== 'final' && mode !== 'none';
}
...
export function gamecastState({ status, playCount = 0, lastPlay = null, liveState = null }) {
  ...
  // THE THIRD TEST IS THE ONE THAT ACTUALLY FIRES. The two above look for a
  // `periodLabel` no writer produces and a `clock` literally reading "HALF",
  // and neither shape exists in this codebase - which is why this branch had
  // never run once. shortOf() is the SAME derivation lineScore.js's liveChip
  // and periodOf call - 'HT' maps to "HALF" and period 2 there too. Checking
  // it here is the strip catching up to its own siblings, not a fourth
  // vocabulary. shortOf reads {period, clock} rows (what the poller actually
  // writes) as well as the older {short, clock} shape, so this fires on both.
  const short = shortOf(liveState) ?? '';
  if (/^half/i.test(String(liveState?.periodLabel ?? ''))
      || /^HALF$/i.test(clock)
      || short === 'HT' || short === 'HALFTIME') {
...
/**
 * THE LAST PLAY LINE IS A PLAY, NOT A STOPPAGE. BDL files the two-minute
 * warning, timeouts and end-of-period rows with the current situation on
 * them (down, distance, spot), which is exactly what the STRIP wants - the
 * ball is where it is - but the "Last play" text under it should read the
 * last thing that happened to the ball. Seen live 9 Sep: "Last play:
 * Two-Minute Warning" under a strip that correctly showed 4th & G.
 */
/**
 * GAME ORDER, NOT ARRAY ORDER. playsFor() sorts by drive then play number and
 * puts drive-less rows (kickoffs, stoppages) LAST, so "the last element" is
 * the opening kickoff. Served live 9 Sep: "Last play: A.Borregales kicks 63
 * yards…" in the 4th quarter. The clock is the order: period up, clock down;
 * rows sharing a clock keep their array order.
 */
const clockSeconds = (c) => { const m = /^(\d{1,2}):(\d{2})$/.exec(String(c ?? '')); return m ? Number(m[1]) * 60 + Number(m[2]) : null; };
export function byGameClock(plays) {
  return (plays ?? []).map((p, i) => ({ p, i })).sort((a, b) => {
    const pa = a.p.period ?? 0; const pb = b.p.period ?? 0;
    if (pa !== pb) return pa - pb;
    const ca = clockSeconds(a.p.clock); const cb = clockSeconds(b.p.clock);
    if (ca != null && cb != null && ca !== cb) return cb - ca;
    return a.i - b.i;
  }).map((x) => x.p);
}

export const STOPPAGE_TYPES = new Set(['two-minute-warning', 'timeout', 'official-timeout', 'end-period', 'end-of-half', 'end-of-game', 'injury-timeout']);
export function lastActionPlay(plays) {
  const ordered = byGameClock(plays);
  for (let i = ordered.length - 1; i >= 0; i--) {
    const p = ordered[i];
    if (p?.text && !STOPPAGE_TYPES.has(String(p.playType ?? ''))) return p;
  }
  return null;
}
 ...
 * STOPPAGES ARE SKIPPED HERE TOO: BDL files a timeout or the two-minute
 * warning with the down and distance but start_yards_to_endzone 0, so read
 * as a snap it is "4th & G at the goal line" (served live 9 Sep). The
 * strip's headline is the snap of the last real play - the same play the
 * line under it describes - which is the gamecast mock's convention.
 */
export function lastLivePlay(plays) {
  const ordered = byGameClock(plays);
  for (let i = ordered.length - 1; i >= 0; i--) {
    const p = ordered[i];
    if (STOPPAGE_TYPES.has(String(p.playType ?? ''))) continue;
