...
export function gamecastState({ status, playCount = 0, lastPlay = null, liveState = null }) {
  ...
  // never run once. `short` is the field both live_state writers DO store and
  // both formatters DO read: lineScore.js's liveChip maps 'HT' to "HALF" and
  // periodOf maps it to period 2. Checking it here is the strip catching up to
  // its own siblings, not a fourth vocabulary.
  const short = String(liveState?.short ?? '').toUpperCase();
