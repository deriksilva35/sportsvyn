...
// ------------------------------------------- drive row grammar + disclosure

test('the play row reserves a delta column and truncates the TEXT, not the row', () => {
  // THE DEFECT: .pt wrapped, so a two-line play left the delta beside line one
  // and nothing beside line two, and the wrapped text ran under the card's
  // overflow:hidden and was sliced mid-word. One line, one ellipsis.
  const css = src('components/gridiron/drivestrip.css');
  const pt = css.slice(css.indexOf('.ds-chart .play .pt'), css.indexOf('.ds-chart .play.score'));
  assert.match(pt, /min-width:\s*0/, 'without it a flex child will not shrink past min-content');
  assert.match(pt, /white-space:\s*nowrap/);
  assert.match(pt, /overflow:\s*hidden/);
  assert.match(pt, /text-overflow:\s*ellipsis/);

  const yd = css.slice(css.indexOf('.ds-chart .play .yd'), css.indexOf('.ds-chart .yd.pos'));
  assert.match(yd, /flex:\s*none/, 'the delta column is reserved, never shared');
  assert.match(yd, /width:\s*38px/);
  assert.match(yd, /text-align:\s*right/);
  assert.match(yd, /tabular-nums/, 'digits must not dance between rows as the drive updates');

  const row = css.slice(css.indexOf('.ds-chart .play {'), css.indexOf('.ds-chart .play:last-child'));
  assert.match(row, /align-items:\s*baseline/, 'the delta sits on the text baseline, not floating');
});

test('the narrow case is tuned, not merely tolerated', () => {
  // The app is the primary surface for this component; 375px is the pin.
  const css = src('components/gridiron/drivestrip.css');
  const mq = css.slice(css.indexOf('@media (max-width: 420px)'));
  assert.match(mq, /\.ds-chart \.play \.dn \{ width: 52px/);
  assert.match(mq, /\.ds-chart \.play \{ gap: 7px/);
});

test('completed drives are native <details>; the live drive is not', () => {
  const comp = src('components/gridiron/Gamecast.js');
  assert.match(comp, /<details className="ds-drive" key=\{d\.driveId\}>/);
  assert.match(comp, /<summary className="dh">/);
  // The live drive renders a plain div - no summary, nothing to collapse.
  assert.match(comp, /<div className="ds-drive is-live" key=\{d\.driveId\}>/);
  assert.match(comp, /const isLive = \(expandFirst \? i === 0 : false\)\s*\n?\s*&& \(d\.result == null \|\| d\.result === 'In progress'\)/);
});

test('DEFAULT STATE: every completed drive is CLOSED', () => {
  // A <details> with no `open` attribute is closed. Adding one would defeat
  // the whole point of the disclosure on a game with fifteen drives.
  const comp = src('components/gridiron/Gamecast.js');
  const chart = comp.slice(comp.indexOf('export function DriveChart'));
  assert.doesNotMatch(chart, /<details[^>]*\bopen\b/, 'no drive may ship open');
});

test('the disclosure costs NO client boundary', () => {
  // Native details/summary by ruling - the client package stays deferred.
  // COMMENTS ARE STRIPPED FIRST, the way the win-probability test below does
  // it: the source explains WHY there is no 'use client' here, and a naive
  // grep flags that sentence as the thing it forbids.
  const comp = src('components/gridiron/Gamecast.js')
    .replace(/\/\*[\s\S]*?\*\//g, '')
    .replace(/^\s*\/\/.*$/gm, '');
  assert.doesNotMatch(comp, /'use client'|"use client"/);
  assert.doesNotMatch(comp, /useState|useEffect|onClick/);
});

test('the browser marker is suppressed and ours rotates on open', () => {
  const css = src('components/gridiron/drivestrip.css');
  assert.match(css, /summary\.dh \{ cursor: pointer; list-style: none; \}/);
  assert.match(css, /summary\.dh::-webkit-details-marker \{ display: none; \}/);
  assert.match(css, /details\[open\] > summary\.dh \.dchev \{\s*transform: rotate\(180deg\)/);
});
...
test('the importer stays a library - cadence lives in the poller, not here', () => {
  // THIS TEST USED TO SAY "no poller ships in this relay", and that boundary was
  // correct while only the backfill existed. The poller has since shipped
  // (app/api/cron/plays-live), so the assertion is REPLACED rather than deleted:
  // what still matters is that playsImport.js knows nothing about WHEN to run.
  // Scheduling in the library would give us two places that decide cadence, and
  // the board-scoped cost bound depends on there being exactly one.
  //
  // Comments stripped first: the file's own header discusses the poller relay
  // being "only a scheduler", and a raw grep reads that promise-of-absence as
  // the thing itself - the same prose trap the admin console hit.
  const imp = src('lib/gridiron/playsImport.js')
    .replace(/\/\*[\s\S]*?\*\//g, '').replace(/^\s*\/\/.*$/gm, '');
  assert.doesNotMatch(imp, /setInterval|cron|schedule/i);

  // And the poller that DOES exist must be the board-scoped one, registered once.
  const vercel = JSON.parse(readFileSync(path.join(REPO, 'vercel.json'), 'utf8'));
  const plays = vercel.crons.filter((c) => c.path.includes('plays'));
  assert.equal(plays.length, 1, 'exactly one plays cron');
  assert.equal(plays[0].path, '/api/cron/plays-live');
  const route = readFileSync(path.join(REPO, 'app/api/cron/plays-live/route.js'), 'utf8');
  assert.match(route, /liveBoardGames\(\)/, 'the poller must reach games via the board scope');
});
...
test('the backfill script has NO static imports - they would freeze the wrong database', () => {
  // THE DEFECT THIS PINS, in full: ES imports are hoisted and evaluated before
  // any top-level statement. lib/db.js captures process.env.DATABASE_URL once,
  // at module-evaluation time. A static import at the top of the script
  // therefore connects BEFORE `--prod` reassigns DATABASE_URL - so the run
  // reports "PROD" and writes to dev. It did exactly that: 1,048 rows landed on
  // the dev branch while PROD stayed empty, and the script called it success.
  //
  // Every import in that file must be dynamic and must sit after the flag
  // handling. This test is the only thing standing between us and a silent
  // repeat in the other direction.
  const s = src('scripts/plays-backfill.mjs');
  const code = s.replace(/\/\*[\s\S]*?\*\//g, '').replace(/^\s*\/\/.*$/gm, '');
  assert.doesNotMatch(code, /^\s*import\s/m, 'no static import may appear in this script');
  assert.match(code, /await import\('\.\.\/lib\/db\.js'\)/);
  assert.match(code, /await import\('\.\.\/lib\/gridiron\/playsImport\.js'\)/);
  // The env assignment must precede the first dynamic import.
  assert.ok(
    code.indexOf('process.env.DATABASE_URL = process.env.PROD_DATABASE_URL')
      < code.indexOf('await import('),
    'DATABASE_URL must be settled before anything is imported',
  );
  // And the run must read its own writes back, so a mis-targeted write shows.
  assert.match(code, /read-back on \$\{target\}/);
});
