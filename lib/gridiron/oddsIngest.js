...
export async function ingestSportFutures(sql, { sport, leagueSlug }) {
  ...
  const { events, budget } = await fetchSportOutrights(sportKey);
  const league = (await sql`SELECT id FROM leagues WHERE slug = ${leagueSlug} LIMIT 1`)[0];
  if (!league) return { field: 0, matched: 0, unmatched: 0, unmatchedSample: [], upserted: 0, budget };
  ...
  if (!pcts) return { field: names.length, matched: 0, unmatched: names.length, unmatchedSample: [], upserted: 0, budget };
