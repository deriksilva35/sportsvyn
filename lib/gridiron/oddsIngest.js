...
function collectSides(event, marketKey, aName, bName) {
  ...
  for (const bk of event.bookmakers ?? []) {
    ...
    if (!oa || !ob) continue;
    // A BOOK WITH THE OUTCOME BUT NO PRICE IS NOT A PRICED BOOK. Number(null) is
    // 0 and 0 is finite, so coercing here pushed a decimal odd of ZERO into the
    // consensus median; the de-vig then refused the whole market and the line was
    // dropped because ONE book had a hole in it. Absence is decided before the
    // coercion, the same rule as lib/util/scoresOf.js.
    const pa = numbersOf([oa.price])[0];
    const pb = numbersOf([ob.price])[0];
    if (pa == null || pb == null) continue;
    a.push({ price: pa, point: oa.point });
    b.push({ price: pb, point: ob.point });
