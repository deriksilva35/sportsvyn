...
test('HALFTIME: period 2 with a zeroed clock emits HT, and both consumers read it', async () => {
  const { liveChip, periodOf } = await import('./lineScore.js');
  const ls = toLiveState('live', 2, '00:00');
  assert.deepEqual(ls, { short: 'HT', clock: '00:00' });
  // The round trip that makes the strip's halftime branch reachable.
  assert.equal(liveChip(ls), 'HALF');
  assert.equal(periodOf(ls), 2);
  // And the strip's own detector, transcribed from driveStrip.js:105.
  assert.ok(/^HALF$/i.test(String(liveChip(ls))), 'liveChip renders the token the strip looks for');
  assert.equal(toLiveState('live', 2, '0:00').short, 'HT', 'the unpadded form too');
});
...
test('the halftime branch now FIRES on the real payload', async () => {
  ...
  // clock string - so `short` is the only field that can carry the signal.
...
test('EDGE, RULED: a Q2 play at exactly 00:00 also reads HT - accepted', () => {
  ...
  const ls = toLiveState('live', 2, '00:00');
  assert.equal(ls.short, 'HT', 'no attempt is made to disambiguate; this is deliberate');
  // Every other Q2 clock is untouched - only the zeroed one flips.
  assert.equal(toLiveState('live', 2, '00:01').short, 'Q2');
  assert.equal(toLiveState('live', 2, '15:00').short, 'Q2');
});
...
test('END OF REGULATION IS NOT HALFTIME - period 4 keeps Q4', () => {
  ...
  assert.equal(toLiveState('live', 4, '00:00').short, 'Q4');
  assert.equal(toLiveState('live', 1, '00:00').short, 'Q1');
  assert.equal(toLiveState('live', 3, '00:00').short, 'Q3');
  assert.equal(toLiveState('live', 5, '00:00').short, 'OT');
});
