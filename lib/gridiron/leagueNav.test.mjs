...
const navSwitch = await import('./leagueSwitch.js');
...
test('THE PER-LEAGUE PILL SET', () => {
  ...
  assert.deepEqual(nfl, ['Today', 'Scores', 'Rankings', 'Standings', 'Market', 'Fantasy', 'Wire']);
  assert.deepEqual(cfb, ['Today', 'Scores', 'Rankings', 'Standings', 'Market', 'Wire']);
...
test('A PINNED BOARD HIDES THE LEAGUE CHIPS AND KEEPS THE STATE ONES', () => {
  ...
  // and the landing's own module pins itself. Anchored on the PROP, not on
  // `pinned` being the last one before the slash: v0.7 added initialTz after
  // it, and a test that breaks when a sibling prop is appended is pinning
  // formatting rather than the claim.
  const ls = strip(src('components/league/LeagueScores.js'));
  assert.match(ls, /<Scoreboard[\s\S]*?\n\s*pinned\n[\s\S]*?\/>/,
    'the module must pass pinned to the board');
});
...
test('THE WAY HOME MOVED INSIDE THE SHEET, and it is still one tap', () => {
  // THIS REPLACES "the league title is a link home". The title is the league
  // switcher's trigger now, and the sheet's current-league row carries the link
  // to /{league}. The gesture survived; a split hitbox - left half navigates,
  // right half opens a sheet - would not have been an improvement on either.
  const h = strip(src('components/league/LeagueHeader.js'));
  assert.doesNotMatch(h, /<a className="lgh-h1"/, 'the title is no longer the anchor');
  assert.match(h, /<LeagueSwitcher/);
  // The current row's href IS the way home when you are on the landing.
  const { switcherRows } = navSwitch;
  assert.equal(switcherRows('/nfl', 'nfl').find((r) => r.current).href, '/nfl');
  assert.equal(switcherRows('/cfb', 'cfb').find((r) => r.current).href, '/cfb');
  // and the title keeps its own type inside the trigger.
  assert.match(strip(src('components/league/LeagueSwitcher.js')),
    /<span className="lgh-h1">\{label\}<\/span>/);
  assert.match(src('components/league/league.css'), /\.lgh-h1 \{[^}]*text-decoration: none/s);
});
