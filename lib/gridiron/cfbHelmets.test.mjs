...
test('every CFB surface reads colors through the shared readers, which select both columns', () => {
  ...
  assert.match(gameRow, new RegExp(
    ...
    + String.raw`<Helmet primary=\{t\?\.colors\?\.primary\} secondary=\{t\?\.colors\?\.secondary\} facing="right" size=\{28\} className="gg-hm" \/>${comment}\s*`
    + String.raw`<span className="abbr">`,
  ), 'helmet after the badge, before the abbreviation');
