...
test('every CFB surface reads colors through the shared readers, which select both columns', () => {
  ...
  // R3), so the badge/helmet/abbr order is asserted where it lives.
  const gameRow = src('components/gridiron/GameTeamRow.js');
  assert.match(gameRow, /<RankBadge rank=\{rank\} size="big" \/>\s*\{\/\*[^]*?\*\/\}\s*<Helmet primary=\{t\?\.colors\?\.primary\} secondary=\{t\?\.colors\?\.secondary\} facing="right" size=\{28\} className="gg-hm" \/>\s*<span className="abbr">/, 'helmet after the badge, before the abbreviation');
