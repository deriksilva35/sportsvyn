...
//
// HEADGEAR-WEB: the SVG helmet is gone. CFB has no cutouts yet, so
// headgearFor('cfb', ...) is null and every CFB surface draws TeamMark's disc
// in the same two colours - which is what this file now holds it to.
...
test('every CFB surface reads colors through the shared readers, which select both columns', () => {
  ...
  assert.match(gameRow, new RegExp(
    ...
    + String.raw`<TeamMark primary=\{t\?\.colors\?\.primary\} secondary=\{t\?\.colors\?\.secondary\}[^>]*?size=\{28\}[^>]*?className="gg-hm"[^>]*?leagueSlug=\{leagueSlug\}[^>]*?\/>${comment}\s*`
...
test('all 138 FBS teams carry two colors and every one renders the two-tone disc, not headgear', async () => {
  ...
  for (const t of rows) {
    ...
    const h = html({ primary: t.c1, secondary: t.c2, size: 28, abbr: t.abbreviation, leagueSlug: 'cfb' });
    assert.match(h, /data-teammark="circle"/, `${t.name} is the disc - cfb has no headgear yet`);
    assert.match(h, new RegExp(`<circle[^>]*fill="${t.c1}"`), `${t.name} primary`); assert.match(h, new RegExp(`<rect[^>]*fill="${t.c2}"`), `${t.name} secondary`);
  }
});
