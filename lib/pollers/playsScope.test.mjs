...
test('CFB_PLAYS_ALL: off unless exactly on; on adds every live game with an FBS side, still by join', async () => {
  const { cfbPlaysAll } = await import('./playsScope.js');
  assert.equal(cfbPlaysAll({}), false); assert.equal(cfbPlaysAll({ CFB_PLAYS_ALL: 'off' }), false);
  assert.equal(cfbPlaysAll({ CFB_PLAYS_ALL: '1' }), false, 'only "on" is on');
  assert.equal(cfbPlaysAll({ CFB_PLAYS_ALL: ' ON ' }), true);
  const code = strip(SCOPE);
  assert.match(code, /l\.slug = 'cfb' AND \$\{all\}::boolean AND EXISTS \(/, 'gated by the flag');
  assert.match(code, /t\.id IN \(m\.home_team_id, m\.away_team_id\)/);
  assert.match(code, /t\.metadata->>'classification' = 'fbs'/, 'an FBS side is the bound: FCS v FCS stays out');
  assert.match(SCOPE, /75,000 calls\/month/, 'the budget it was sized against is written down');
});
...
test('NO unbounded CFB query exists anywhere in this poller, and the route holds no query of its own', () => {
  ...
  // EVERY cfb branch is an EXISTS join - the board, or (behind the flag) an
  // FBS side. None is a bare `l.slug = 'cfb'`.
  const cfbBranches = scope.split("l.slug = 'cfb'").slice(1);
  assert.equal(cfbBranches.length, 2);
  for (const b of cfbBranches) assert.match(b.slice(0, 60), /EXISTS \(/, 'each cfb branch is a join, nothing wider');
...
test('one failing game does not abandon the rest of the slate', () => {
  const code = strip(ROUTE);
  // THE CATCH IS PER GAME, in the one function both paths call - main's loop
  // (flag off) and the CFB_PLAYS_ALL pool (flag on).
  const one = code.slice(code.indexOf('const one = async (g) =>'), code.indexOf('const all = cfbPlaysAll();'));
  assert.match(one, /try \{/);
  assert.match(one, /catch \(e\)/);
  assert.match(one, /failed \+= 1/);
  assert.match(code, /for \(const g of due\) \{ started\.push\(g\); await one\(g\); \}/, 'flag off: one at a time, as main');
});
