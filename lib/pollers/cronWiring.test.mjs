...
// A CRON PATH MAY CARRY A QUERY STRING, and one does: /api/cron/power-edition
// is scheduled twice, ?league=cfb on Monday and ?league=nfl on Tuesday, because
// the work is identical and the only difference is one LEAGUE_CONFIG entry.
// Vercel passes the query through to the handler. Both of the checks below ask
// about the ROUTE - does it exist, is it scheduled - and a query string changes
// neither, so it is stripped before either question is asked. Without this the
// guard reported a route that will never run AND a schedule with no route, for
// the same correctly-wired pair.
const routeOf = (p) => String(p).split('?')[0].replace('/api/cron/', '');
const scheduleFor = (name) => vercel.crons.find((c) => routeOf(c.path) === name)?.schedule;

// Routes that share the cron family's HOME AND AUTH (Bearer CRON_SECRET)
// but are invoked on demand, not on a schedule. Each entry needs a reason;
// an unscheduled cron with no entry here is still a bug.
const ON_DEMAND = [
  'push-test',   // owner-device push proof through the Vercel runtime
];

test('every cron route is registered in vercel.json (or declared on-demand)', () => {
  const missing = routeDirs.filter((n) => !scheduleFor(n) && !ON_DEMAND.includes(n));
  assert.deepEqual(missing, [], `route(s) with no schedule (will never run): ${missing.join(', ')}`);
});

test('on-demand routes are never ALSO scheduled', () => {
  const both = ON_DEMAND.filter((n) => scheduleFor(n));
  assert.deepEqual(both, [], `on-demand route(s) with a schedule: ${both.join(', ')}`);
});

test('every vercel.json cron points at a route that exists', () => {
  const paths = vercel.crons.map((c) => c.path);
  const orphans = paths.filter((p) => !routeDirs.includes(routeOf(p)));
  assert.deepEqual(orphans, [], `scheduled path(s) with no route: ${orphans.join(', ')}`);
});

test('no two crons share a path', () => {
  // THE FULL PATH, QUERY INCLUDED. Two schedules on one route with different
  // arguments are two different jobs; two schedules on the identical path are
  // the same job booked twice, which is the thing this catches.
  const paths = vercel.crons.map((c) => c.path);
  assert.equal(new Set(paths).size, paths.length);
});

test('a cron that takes an argument is scheduled once per value it accepts', () => {
  // power-edition is the first route to be parameterised. If a league is added
  // to LEAGUE_CONFIG and not to vercel.json its board is never published, and
  // nothing else in this file would notice - the route exists and IS scheduled,
  // just not for that league.
  const powerPaths = vercel.crons.map((c) => c.path).filter((p) => routeOf(p) === 'power-edition');
  const scheduled = new Set(powerPaths.map((p) => new URL(p, 'https://x').searchParams.get('league')));
  const configured = new Set(
    [...stripComments(readFileSync(path.join(REPO, 'lib/rankings/publishGridironEdition.js'), 'utf8'))
      .matchAll(/^\s{2}(nfl|cfb):\s*\{/gm)].map((m) => m[1]),
  );
  assert.deepEqual([...scheduled].sort(), [...configured].sort(),
    'every league the publisher knows about must have a schedule, and vice versa');
  assert.ok(configured.size >= 2, 'the guard is reading real config, not an empty match');
});
