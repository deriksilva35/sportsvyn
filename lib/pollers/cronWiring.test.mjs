...
const ON_DEMAND = [
  ...
  // Props bill PER GAME and must run against the 100K key. Which key the
  // deployed cron environment carries is not yet established (the droplet's is
  // a 500-credit key), so the route ships with its ingest, guard and tests but
  // WITHOUT a schedule - a registered cron would take that spend decision by
  // simply arriving at 13:00 UTC. Enable by adding
  //   { path: '/api/cron/gridiron-props', schedule: '0 13,17,21,23 * * *' }
  // Four ticks a day, never the 15-minute rhythm: per-game billing at that
  // cadence is ~10,000 credits a day.
  'gridiron-props',
];
