...
const ON_DEMAND = [
  ...
  // EPL PLAYER STATS ARE BLOCKED ON SQUADS, NOT ON CODE. The importer works -
  // it read a real Fulham-Chelsea payload and mapped both sides' scorers - but
  // only 14 of 40 players in that fixture EXIST in our players table, and all
  // 14 are there because they played at the World Cup for a national team. EPL
  // club squads have never been imported.
  //
  // Running it anyway would write stats for the 35% who happen to have a row
  // and silently omit the rest, producing a table that looks populated and is
  // systematically wrong - which is worse than an empty one. Enable by adding
  //   { path: '/api/cron/epl-player-stats', schedule: '30 20,2 * * *' }
  // AFTER an EPL squad import lands (lib/import/squadImport.js is the WC-era
  // tool and is the obvious starting point).
  'epl-player-stats',
];
