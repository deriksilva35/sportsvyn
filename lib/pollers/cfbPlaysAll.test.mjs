...
// OUR THREE GAMES, NOT THE WHOLE SLATE. node --test runs files in parallel,
// and other tests (services/live-poller/winprob.test.mjs, the rider) put live
// NFL sentinels on DEV while this runs - which the route rightly polls. So
// every assertion here is about THESE sentinels: their scope, their CFBD
// calls, their plays, their ledger entries. A slate-wide count is a race.
const ours = (list) => list.filter((g) => Object.values(GAME).includes(g));
const slugOf = (tag) => `${SLUG}-${tag}`;

test('FLAG OFF: main\'s scope - none of ours in scope, no CFBD call for them, no plays', async () => {
  ...
  assert.deepEqual(ours(cfbdAsked), []);
...
test('FLAG ON: the non-board FBS games are polled and their plays WRITTEN; FCS v FCS is never asked', async () => {
  ...
  try {
    const { liveBoardGames } = await import('./playsScope.js');
    const scope = (await liveBoardGames()).map((g) => g.id);
    assert.ok(scope.includes(ids.fbs) && scope.includes(ids.mixed), 'FBS v FBS and FBS v FCS are in scope');
    assert.equal(scope.includes(ids.fcs), false, 'FCS v FCS is not');
    const res = await (await call()).json();
    assert.equal(res.ok, true);
    assert.deepEqual(ours(cfbdAsked).sort(), [GAME.fbs, GAME.mixed].sort(), 'one CFBD call per FBS game, none for FCS v FCS');
    ...
    assert.ok(run.summary.cfbd_calls >= 2, 'our two CFBD calls are counted');
    const mine = (run.summary.games ?? []).filter((g) => [slugOf('fbs'), slugOf('mixed')].includes(g.slug));
    assert.equal(mine.length, 2); assert.ok(mine.every((g) => g.plays === 2 && !g.error));
    // AND THE THROTTLE STILL HOLDS: the next tick inside 90 s asks nothing about ours.
    cfbdAsked = [];
    await (await call()).json();
    assert.deepEqual(ours(cfbdAsked), []);
