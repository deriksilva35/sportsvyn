 ...
 *                   1,328
 *
 * 1,400 covered it with room for a retry, back when a score sweep cost ONE
 * request.
 *
 * RE-PRICED 15 AUG: THE TWO-DATE SWEEP DOUBLED THE SCORE COST. A slate can
 * straddle two UTC dates - a 20:00 ET kickoff is tomorrow in UTC - so
 * slateDatesForProvider now returns [etDay, etDay+1] and a sweep spends
 * `requests = dates.length`, which is 2. That fix is correct and it is why
 * three of six games were invisible on 13 Aug. Nobody re-priced the cap for it.
 *
 * Saturday 15 Aug, seven games, 13:00 to 20:00 ET, priced under the real cost:
 *
 *   score sweeps    1,410   705 sweeps x TWO requests   <- was 705 at one each
 *   detail rounds     294   7 games × 3.5h × one round per 10 min × 2
 *   final fetches      14   7 × 2, once each
 *   daily sync          2
 *   ------------------------
 *                   1,720
 *
 * Against 1,400 that trips at roughly 21:30 ET with two games still playing -
 * the same mid-slate stop the 600 cap would have caused, arrived at from the
 * other direction. The model was calibrated against 14 Aug first: it predicted
 * 674 and the day spent 668, so the arithmetic above is trusted to about 1%.
 *
 * 2,000 covers it with 16% margin. It is 27% of the plan's 7,500/day, and it
 * STILL DOES THE JOB A CAP EXISTS FOR: a runaway 60-second score sweep now tops
 * out at 2,880 a day (1,440 sweeps x 2) and would trip it. Raising the ceiling
 * to match a doubled unit cost is not the same as removing the guard.
 *
 ...
 */
export const DAILY_REQUEST_CAP = 2000;
...
/** The current ET hour, 0-23. DST-aware the same way slateDateEt is. */
export function hourEt(now = new Date()) {
...
/**
 * THE SPORTS-DAY LAW, at the poller: the ET dates whose games this tick may
 * poll. Before 06:00 ET the PRIOR date rides along, because a football
 * Saturday does not end at midnight - DAL@ARI was in Q4 at 00:13 ET on
 * 23 Aug when the single-date window rolled to Sunday and the poller went
 * dark mid-game, freezing a live scoreboard for the night. After 06:00 the
 * prior date drops; a game still 'live' six hours after kickoff is a stuck
 * feed, and isGameHot's MAX_GAME_HOURS bound already refuses to chase it.
 */
export function pollSlateDatesEt(now = new Date()) {
  const today = slateDateEt(now);
  if (hourEt(now) >= 6) return [today];
  const prior = new Date(`${today}T12:00:00Z`);
  prior.setUTCDate(prior.getUTCDate() - 1);
  return [prior.toISOString().slice(0, 10), today];
}
 ...
 * invisible to the poller for the entire evening.
 *
 * THIS IS THE BLIND FALLBACK, kept for the case where we know nothing. It asks
 * for both dates unconditionally, which is two requests per sweep where one
 * would do. Prefer providerDatesForGames below, which derives the exact dates
 * from kickoffs already in our own table and usually returns one.
 */
...
/**
 * HOW LATE THE FEED CAN BE relative to a kickoff we have stored.
 *
 * The lag between a kickoff time and the provider showing the game as live is
 * NOT CONSTANT - it moves with their ingest, and a game can also start a few
 * minutes late. Twenty minutes either side is the tolerance the ruling set: wide
 * enough that a slow feed or a delayed start still falls inside the window we
 * ask for, narrow enough that it does not pull in a date we have no games on.
 */
export const LIVE_LAG_TOLERANCE_MIN = 20;

/**
 * The UTC dates to ask for, DERIVED FROM THE KICKOFFS WE ALREADY HAVE.
 *
 * ============================================================================
 * WHY THIS REPLACES THE TWO-DATE SWEEP
 * ============================================================================
 * The fallback above asks for the ET day and the next one, always, because
 * without looking at the schedule it cannot know which of them holds games. On
 * a three-game Thursday that is one wasted request per sweep. On a ten-game
 * Saturday polled every 60 seconds it is the difference between fitting the
 * daily cap and blowing through it.
 *
 * We already store every kickoff. So the exact set of UTC dates a sweep needs
 * is the set of UTC dates the RELEVANT kickoffs fall on - relevant meaning
 * inside the live window, widened by LIVE_LAG_TOLERANCE_MIN at both ends. On a
 * normal evening that collapses to ONE date. It returns two only when a slate
 * genuinely straddles midnight UTC, which is exactly when two are needed.
 *
 * THE CONVERSION IS EXPLICIT, and that is the whole point of the function. The
 * droplet's cron runs in UTC and the schedule is reasoned about in ET; every
 * bug in this area has come from one of those being assumed. toISOString() is
 * UTC by definition, so slicing its first ten characters is the provider's date
 * key with no timezone inference anywhere.
 *
 * EMPTY IN, EMPTY OUT - and the caller must treat that as "ask for nothing",
 * not as "ask for everything". A sweep with no games in window has no reason to
 * spend a request; sweepDecision already refuses on 'no-games' before this is
 * consulted.
 */
export function providerDatesForGames({
  games = [], now = new Date(), toleranceMin = LIVE_LAG_TOLERANCE_MIN,
} = {}) {
  const ms = now.getTime();
  const tol = toleranceMin * MIN;
  const dates = new Set();

  for (const g of games ?? []) {
    const t = new Date(g?.kickoffAt ?? NaN).getTime();
    if (!Number.isFinite(t)) continue;
    if (g.status === 'postponed' || g.status === 'cancelled') continue;
    // The same window isGameHot uses, widened by the feed-lag tolerance. Widened
    // at BOTH ends: a feed that is slow to show a game live is equally slow to
    // show it final, and dropping the date at the tail loses the final flip.
    const opens = t - PRE_KICKOFF_MIN * MIN - tol;
    const closes = t + MAX_GAME_HOURS * HOUR + POST_FINAL_MIN * MIN + tol;
    if (ms < opens || ms > closes) continue;
    // UTC, explicitly: toISOString is always UTC, so this is the provider's key
    // with nothing inferred from the host's timezone.
...
/**
 * What a sweep will actually cost in provider requests, for budgeting.
 *
 * ONE REQUEST PER DATE PER SWEEP, plus the detail fetches. Exposed so the cap
 * test can price a real slate instead of trusting a number in a comment - see
 * lib/pollers/preseasonBudget.test.mjs, which fails if any scheduled slate this
 * week prices above DAILY_REQUEST_CAP.
 */
export function sweepRequestCost({ dates = [], detailFetches = 0 } = {}) {
  return dates.length + detailFetches;
}
