...
import {
  ...
  providerDatesForGames, LIVE_LAG_TOLERANCE_MIN, sweepRequestCost,
...
test('THE WIDEST DAY, PRICED IN FULL, FITS UNDER THE CAP', () => {
  ...
  // THE DEBT THIS BLOCK PINNED IS PAID. It used to compute 22 Aug at 2,212
  // against a 2,000 cap and then assert `total > DAILY_REQUEST_CAP` - a test
  // asserting the product was broken, which would have passed forever whether
  // or not anyone fixed it. The durable fix landed: sweeps now ask for the UTC
  // dates the kickoffs actually fall on rather than the ET day and the next
  // one, so most sweeps cost ONE request instead of two.
  //
  // THE LIVE PRICING MOVED to lib/pollers/preseasonBudget.test.mjs, which reads
  // the REAL schedule and fails if any scheduled slate exceeds the cap. A
  // hardcoded sweep count for one Saturday cannot do that job - it goes stale
  // the moment the calendar changes, which is exactly how 22 Aug arrived
  // unpriced. Measured there on the real rows: 22 Aug, ten games, 1,502.
  //
  // What stays here is the property that does not depend on a calendar.
  assert.ok(DAILY_REQUEST_CAP < 7500 / 2, 'and it stays a fraction of the plan');
});
...
// ===========================================================================
// KICKOFF-DERIVED DATE TARGETING
// ===========================================================================
// The blind sweep asks for the ET day and the next one on every tick, because
// without looking at the schedule it cannot know which holds games. We already
// store every kickoff, so the exact set of UTC dates is derivable - one request
// most of the evening instead of two.

const g = (iso, status = 'scheduled') => ({ kickoffAt: iso, status });

test('a normal evening collapses to ONE date', () => {
  // Three 23:00-23:30Z kickoffs are all 13 Aug in UTC. The blind sweep would
  // have asked for 13 and 14 Aug; there is nothing on the 14th to ask for.
  const games = [g('2026-08-13T23:00:00Z'), g('2026-08-13T23:30:00Z')];
  assert.deepEqual(
    providerDatesForGames({ games, now: at('2026-08-13T23:10:00Z') }),
    ['2026-08-13'],
  );
});

test('A LATE SATURDAY SLATE CROSSING THE UTC BOUNDARY RETURNS BOTH DATES', () => {
  // THE CASE THE RULING NAMED. 22 Aug ET runs midday to late evening; a 19:30
  // ET kickoff is 23:30Z on the 22nd and a 22:00 ET kickoff is 02:00Z on the
  // 23rd. At 02:10Z the early game is still inside its tail and the late one
  // has just started, so a sweep that asked for only one date would be blind to
  // half the slate - which is the original 13 Aug defect, one day later.
  const games = [
    g('2026-08-22T16:00:00Z'),   // 12:00 ET
    g('2026-08-22T23:30:00Z'),   // 19:30 ET
    g('2026-08-23T02:00:00Z'),   // 22:00 ET, next UTC day
  ];
  const dates = providerDatesForGames({ games, now: at('2026-08-23T02:10:00Z') });
  assert.deepEqual(dates, ['2026-08-22', '2026-08-23'],
    'both sides of the UTC midnight must be requested while both are in window');
  // And the midday game, four and a half hours done, is NOT keeping its date alive.
  assert.equal(dates.includes('2026-08-22'), true, 'the 23:30Z game holds the 22nd');
});

test('the UTC conversion is explicit - an ET-looking date is never emitted', () => {
  // A 20:00 ET kickoff is the NEXT day in UTC. Emitting the ET date here is the
  // exact class of bug this function exists to remove, so it is asserted rather
  // than assumed.
  const games = [g('2026-08-23T00:00:00Z')];   // 20:00 ET on the 22nd
  assert.deepEqual(providerDatesForGames({ games, now: at('2026-08-23T00:05:00Z') }),
    ['2026-08-23'], 'the provider key is UTC, not the ET evening it belongs to');
});

test('FEED LAG TOLERANCE widens the window at BOTH ends', () => {
  const games = [g('2026-08-13T23:00:00Z')];
  const opens = new Date(Date.parse('2026-08-13T23:00:00Z') - (15 + LIVE_LAG_TOLERANCE_MIN) * 60_000);
  // One minute inside the widened opening edge.
  assert.equal(
    providerDatesForGames({ games, now: new Date(opens.getTime() + 60_000) }).length, 1,
    'a slow feed before kickoff must still be asked for',
  );
  // One minute outside it.
  assert.equal(
    providerDatesForGames({ games, now: new Date(opens.getTime() - 60_000) }).length, 0,
    'and the window does not run all day',
  );
});

test('a postponed or cancelled game contributes no date', () => {
  const games = [g('2026-08-13T23:00:00Z', 'postponed'), g('2026-08-13T23:30:00Z', 'cancelled')];
  assert.deepEqual(providerDatesForGames({ games, now: at('2026-08-13T23:10:00Z') }), []);
});

test('EMPTY IN, EMPTY OUT - a sweep with nothing in window asks for nothing', () => {
  // The caller treats [] as "ask for nothing", never as "ask for everything".
  assert.deepEqual(providerDatesForGames({ games: [], now: at('2026-08-13T23:10:00Z') }), []);
  assert.deepEqual(providerDatesForGames({}), []);
  assert.deepEqual(
    providerDatesForGames({ games: [g('nonsense')], now: at('2026-08-13T23:10:00Z') }), []);
});

test('dates come back sorted and unique, so the cost is the count', () => {
  const games = [
    g('2026-08-23T02:00:00Z'), g('2026-08-22T23:30:00Z'), g('2026-08-22T23:45:00Z'),
  ];
  const dates = providerDatesForGames({ games, now: at('2026-08-23T02:10:00Z') });
  assert.deepEqual(dates, ['2026-08-22', '2026-08-23']);
  assert.equal(new Set(dates).size, dates.length, 'no duplicate date is ever requested');
  assert.equal(sweepRequestCost({ dates, detailFetches: 4 }), 6);
});

test('the blind sweep survives as the fallback, unchanged', () => {
  // Still two dates, still ET-day-plus-one. It is what a caller with no games
  // falls back to rather than going dark.
  assert.deepEqual(slateDatesForProvider(at('2026-08-13T23:10:00Z')),
