...
test('pickemBoardView: the Promise.all producers are in destructure order (records, spreads, colors, networks)', async () => {
  ...
  const start = src.indexOf('const [records, spreads, colors, networks] = await Promise.all([');
  assert.ok(start > 0, 'the four-map destructure exists');
