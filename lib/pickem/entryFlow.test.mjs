...
// THREE MORE ARGUED IN, 30 Aug: home_record / away_record / spread_home, so a
// board row can show the two facts a reader actually picks on. Same test as
// the rank badges and the same answer: a win-loss record and the market's
// consensus spread are identical for every reader of the board. They carry no
// pick, no entry and no user id, so they leak nothing - my_side is still the
// only field that differs between two viewers. All three are nullable and
// render as nothing when absent.
// TWO MORE ARGUED IN, 10 Sep: home_colors / away_colors, the helmet's two hex
// strings. Same test, same answer: a team's colors are a fact about the team,
// identical for every reader, carrying no pick, entry or user id. Nullable;
// null draws no helmet.
// THREE MORE KEYS FOR v2, and each passes the same test the others did: none
// is viewer-scoped. A live period and clock, and which network is carrying the
// game, are identical for every reader of the board - they carry no pick, no
// entry and no user id. my_side remains the only field that differs between
// two viewers, which is what this list exists to keep true.
// TWO MORE ARGUED IN, 25 Sep (HEADGEAR-WEB): home_abbr / away_abbr, the
// team's stored abbreviation, so the board can draw its headgear
// (lib/teams/headgear.js). A team's abbreviation is a fact about the team,
// identical for every reader, carrying no pick, entry or user id. Nullable;
// null draws the disc.
const WIRE_KEYS = ['match_id', 'slug', 'kickoff_at', 'home', 'away', 'status',
  ...
  'home_record', 'away_record', 'spread_home', 'home_colors', 'away_colors',
  'period', 'clock', 'network', 'home_abbr', 'away_abbr'].sort();
...
// THE THREE MAPS LAND IN THEIR OWN KEYS. pickemBoardView builds records,
// spreads and colors in one Promise.all and destructures them by position;
// the helmets relay put the color map in the spreads' seat and shipped a board
// with home_colors null and spread_home null on every row. This reads the
// served board against the teams table: a dressed team's row carries exactly
// the two hexes the table holds, and a spread is a number or null, never a
// map.
// And the same guard with no board needed: the producers stand in the order
// the destructure names them. Read from the source, so DEV having no living
// board never turns this into a pass by early return.
test('pickemBoardView: the Promise.all producers are in destructure order (records, spreads, colors, networks, abbrs)', async () => {
  // POSITIONAL, AND NOW FOUR. The helmets relay shipped a board with every
  // colour null because a map was slipped into the wrong seat; the v2 reader
  // adds a fourth (networks) to the same array, so the guard counts four.
  // HEADGEAR-WEB appends a fifth (abbrs), last, so no earlier seat moves.
  const { readFileSync } = await import('node:fs');
  const src = readFileSync(new URL('./entry.js', import.meta.url), 'utf8');
  const start = src.indexOf('const [records, spreads, colors, networks, abbrs] = await Promise.all([');
  assert.ok(start > 0, 'the five-map destructure exists');
  const block = src.slice(start, src.indexOf(']);', start));
  const at = (s) => { const i = block.indexOf(s); assert.ok(i > 0, `${s} is produced in the block`); return i; };
  assert.ok(at('recordMapFor(') < at('getSpreadHome('), 'records before spreads');
  assert.ok(at('getSpreadHome(') < at('teamColorMap('), 'spreads before colors');
  assert.ok(at('teamColorMap(') < at('networksFor('), 'colors before networks');
  assert.ok(at('networksFor(') < at('teamAbbrMap('), 'networks before abbrs');
});

test('records, spreads and colors reach the row they belong to', async () => {
  const { sql } = await import('../db.js');
  const v = await pickemBoardView(null, { sport: 'nfl' });
  if (v.phase === 'preopen' || !v.games?.length) return; // no board to read
  const ids = [...new Set(v.games.flatMap((g) => [g.home_team_id, g.away_team_id]).filter(Boolean))];
  const rows = await sql`SELECT id, color_primary, color_secondary, abbreviation FROM teams WHERE id = ANY(${ids}::int[])`;
  const want = new Map(rows.map((r) => [r.id, r.color_primary && r.color_secondary ? { primary: r.color_primary, secondary: r.color_secondary } : null]));
  const abbr = new Map(rows.map((r) => [r.id, r.abbreviation ?? null]));
  assert.ok([...want.values()].some(Boolean), 'at least one dressed team on the board, or this test proves nothing');
  for (const g of v.games) {
    assert.deepEqual(g.home_colors, want.get(g.home_team_id) ?? null, `${g.slug} home colors`);
    assert.deepEqual(g.away_colors, want.get(g.away_team_id) ?? null, `${g.slug} away colors`);
    assert.equal(g.home_abbr, abbr.get(g.home_team_id) ?? null, `${g.slug} home abbr`);
    assert.equal(g.away_abbr, abbr.get(g.away_team_id) ?? null, `${g.slug} away abbr`);
    assert.ok(g.spread_home == null || typeof g.spread_home === 'number', `${g.slug} spread_home is a number or null, got ${typeof g.spread_home}`);
    assert.ok(g.home_record == null || typeof g.home_record === 'string', `${g.slug} record is a string or null`);
  }
});
...
test('before opens_at the view is preopen with no games; no board at all likewise', async () => {
  // SCOPED TO THIS FILE'S OWN BOARD, and it has to be. currentPickemBoard()
  // takes the latest UNSETTLED board with opens_at <= now, across every
  // pickem contest in the database - so an unscoped read at 2031-10-06,
  // BEFORE this file's October window opens, adopts whatever else is
  // unsettled and older. The builder file (pickem.test.mjs) owns AUGUST 2031
  // for exactly this reason, and that is precisely the board this assertion
  // picked up: August opens before October, so the separate windows protect
  // every OTHER test here and not this one. It passed only while the two
  // files did not overlap, and went red the moment three new pure test files
  // shifted the parallel schedule.
  //
  // The fixtures already carry the isolation key: `sport` holds the league
  // slug, unique per file ('pickemtest2-cfb' here, 'pickemtest-cfb' there),
  // and the teardown on line 111 already deletes by it. Reading by it too
  // makes the assertion true about THIS file's board rather than about the
  // whole database - which is what it always meant.
  const v = await pickemBoardView(uA, { sport: LG, now: new Date('2031-10-06T12:00:00Z') });
  assert.equal(v.phase, 'preopen');
  assert.deepEqual(v.games, []);
  const none = await pickemBoardView(uA, { sport: LG, now: new Date('2020-01-01T00:00:00Z') });
  assert.equal(none.phase, 'preopen');
});

test('bare /pickem never renders a board, only redirects (relay 2c item 6)', () => {
  const t = src('app/pickem/page.js');
  assert.ok(!/notFound/.test(t), 'the bare route itself never 404s - it always has a sport to send you to');
  assert.match(t, /redirect\(`\/pickem\/\$\{sport\}`\)/);
  assert.match(t, /soonestPickemSport/, 'the redirect target is derived, never a hardcoded sport');
});

test('the sport route can never 404 for "no board yet" and holds the sign-in law', () => {
  const t = src('app/pickem/[sport]/page.js');
  // An UNKNOWN sport segment DOES 404 (a real refusal of bad input) - the
  // guarantee is narrower than the old single-route test's: no board yet is
  // never a 404, a nonsense /pickem/xyz correctly is.
  assert.match(t, /if \(!PICKEM_SPORTS\.includes\(sport\)\) notFound\(\);/);
