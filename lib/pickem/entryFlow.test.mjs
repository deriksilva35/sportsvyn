...
const WIRE_KEYS = ['match_id', 'slug', 'kickoff_at', 'home', 'away', 'status',
  ...
  'home_record', 'away_record', 'spread_home', 'home_colors', 'away_colors'].sort();
...
test('pickemBoardView: the Promise.all producers are in destructure order (records, spreads, colors)', async () => {
  const { readFileSync } = await import('node:fs');
  const src = readFileSync(new URL('./entry.js', import.meta.url), 'utf8');
  const start = src.indexOf('const [records, spreads, colors] = await Promise.all([');
  assert.ok(start > 0, 'the three-map destructure exists');
  const block = src.slice(start, src.indexOf(']);', start));
  const at = (s) => { const i = block.indexOf(s); assert.ok(i > 0, `${s} is produced in the block`); return i; };
  assert.ok(at('recordMapFor(') < at('getSpreadHome(') && at('getSpreadHome(') < at('teamColorMap('), 'records, then spreads, then colors');
});
