...
async function liveGames(board) {
  ...
  // live_state RIDES ALONG (v2 reader ruling b). It is a flat top-level key in
  // metadata - {period, clock} with period an INTEGER, written by the live
  // poller - and it is the only source of the clock a live row shows. Selected
  // here rather than in a second query because every caller of this map is
  // already asking about the same matches.
  const rows = await sql`
    SELECT id, status, home_score, away_score, metadata->'live_state' AS live_state
...
export async function pickemBoardView(userId, { sport = null, now = new Date() } = {}) {
  ...
  const [records, spreads, colors, networks] = await Promise.all([
  ...
  const games = gameRows({ board: contest.board, liveById, picks, now, apRanks, records, spreads, colors, networks });
...
export async function pickemCardData(userId, { sport = null, now = new Date() } = {}) {
  ...
  const openRows = contest.board.filter((g) => new Date(g.kickoff_at).getTime() > now.getTime());
  ...
  return {
    total, picked, pickable, pickedOpen, nextKickoff: nextUnlockedKickoff(contest.board, { now }), entered: picked > 0,
    boardNumber: contest.board_number,
