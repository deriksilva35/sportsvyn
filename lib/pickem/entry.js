 ...
 *
 * SPORT-SCOPED WHEN GIVEN (relay 2c items 5/6) - /pickem/nfl and /pickem/cfb
 * each need THEIR OWN current board, and an NFL board opening must never
 * flip which board a CFB reader sees or vice versa. `sport` null keeps the
 * original global behaviour (newest pickem contest of ANY sport) for the
 * callers that still want it.
 *
 * boardNumber IS COMPUTED HERE, ONCE, NOT TYPED ANYWHERE DOWNSTREAM (ruling):
 * 1 + the count of pickem contests OF THE SAME SPORT whose opens_at is
 * earlier than this one's - a CFB board and an NFL board opening in the same
 * week must not share one numbering sequence. Every reader that needs a
 * board number (the /pickem hero, the /games card, push copy) takes it from
 * this one row rather than each carrying its own "Board 1" literal - the
 * exact embarrassment this fixes.
 */
export async function currentPickemBoard({ sport = null, now = new Date() } = {}) {
  ...
  const [{ n }] = await sql`
    SELECT count(*) AS n FROM contests
     WHERE game_type = 'pickem' AND sport = ${contest.sport} AND opens_at < ${contest.opens_at}`;
