...
test('a real contest exists: the next kickoff ON ITS BOARD wins over the schedule derivation (rolling lock)', async () => {
  // ROLLING LOCK: nextLock reads the board's own kickoffs, not locks_at (which
  // is now the last kickoff). The board's kickoff is set deliberately
  // different from the schedule's so the source is provable.
  const REAL_LOCK = '2097-08-30T18:00:00Z';
  const [g1] = await sql`SELECT id FROM matches WHERE slug = 'firstlocktest-g1'`;
  const board = JSON.stringify([{ match_id: g1.id, slug: 'firstlocktest-g1', kickoff_at: REAL_LOCK, home: 'Alpha', away: 'Beta' }]);
  contestId = (await sql`
    INSERT INTO contests (game_type, sport, season_year, week, board, opens_at, locks_at, settles_at)
    VALUES ('pickem', ${LG}, 2097, 99, ${board}::jsonb, ${NOW.toISOString()}, ${REAL_LOCK}, ${REAL_LOCK})
    RETURNING id`)[0].id;
  const label = await firstLockLabel({ sport: LG, now: NOW });
