...
const { lockLabel } = await import('./read.js');
...
test('locks_at is the LAST kickoff of the window (rolling lock); firstKickoff is the earliest', async () => {
  const { plan } = await boardPlan({ leagueSlug: LG, now: new Date('2031-08-27T15:00:00Z') });
  assert.equal(plan.locksAt.toISOString(), '2031-09-01T02:00:00.000Z', 'the join-window close');
  assert.equal(plan.firstKickoff.toISOString(), '2031-08-30T16:00:00.000Z', 'the first game, for the clock and reminders');
...
test('create once, then a double fire is a no-op that says exists', async () => {
  ...
  assert.equal(new Date(rows[0].locks_at).toISOString(), '2031-09-01T02:00:00.000Z', 'the row carries the last kickoff');
...
test('the cron route exists, records every run per sport, and alerts on failure (relay 2c item 5)', () => {
  ...
  // TWO SPORTS, ONE BUILDER, CFB THEN NFL - never a hand-picked leagueSlug.
  assert.match(route, /export const SPORTS = \['cfb', 'nfl'\]/);
  assert.match(route, /ensurePickemBoard\(\{ leagueSlug: sport \}\)/, 'the route runs the one builder, per sport');
...
test('lockLabel derives the exact ghost grammar, noon included', () => {
  // No static fallback constant survives to compare against (relay 2c-fix
  // item 1 deleted it) - these are lockLabel()'s own grammar, asserted
  // directly against known instants.
  assert.equal(lockLabel('2026-08-29T16:00:00Z'), 'Sat Aug 29, noon ET');
