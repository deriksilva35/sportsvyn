...
export function gameRows({
  ...
  apRanks = new Map(), records = new Map(), spreads = new Map(), colors = new Map(),
  networks = new Map(), abbrs = new Map(),
}) {
  ...
  return board.map((g) => {
    ...
    return {
      ...
      // THE HELMET'S TWO COLORS, resolved server-side by team id like the
      // ranks: null when the team has none, and then no helmet is drawn.
      home_colors: colors.get(g.home_team_id) ?? null,
      away_colors: colors.get(g.away_team_id) ?? null,
      // THE STORED ABBREVIATION, so the board can ask for headgear
      // (lib/teams/headgear.js). Not viewer-scoped: the same for every reader.
      home_abbr: abbrs?.get(g.home_team_id) ?? null,
      away_abbr: abbrs?.get(g.away_team_id) ?? null,
      ...
      // THE CLOCK, LIVE ONLY (v2 reader ruling b). live_state is written by
      // the poller and holds {period, clock} with period an INTEGER; shortOf()
      // is the one derivation every other surface reads it through, so a
      // Pick'em row says "Q3" and "HT" exactly as the scoreboard does rather
      // than printing a bare 3. NULL the moment a game is not live: a period
      // beside a final score is a number that stopped being true, the same
      // rule the line already follows.
      period: status === 'live' ? shortOf(live.live_state) : null,
      clock: status === 'live' ? (live.live_state?.clock ?? null) : null,
      // THE PRIMARY BROADCASTER, or null. Null renders as nothing at all -
      // never a dash. See networksFor() in lib/pickem/entry.js.
      network: networks?.get(g.match_id) ?? null,
      kicked,
      my_side: mySide,
      graded,
