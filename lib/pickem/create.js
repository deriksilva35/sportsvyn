...
import { boardCovering, boardNumberFor, plannedBoardNumberFor } from './sequence.js';
...
export async function boardPlan({ leagueSlug = 'cfb', now = new Date() } = {}) {
  if (leagueSlug === 'nfl') return withExistingGuard('nfl', await nflBoardPlan(now));
  ...
  // ROLLING LOCK: firstKickoff keys the board's clock and reminders; locksAt
  // (the contest row) is the LAST kickoff, the join-window close.
  const firstKickoff = new Date(slate[0].kickoff_at);
  const locksAt = new Date(slate[slate.length - 1].kickoff_at);
  const opensAt = new Date(await easternLocalToUtc(tuesdayBefore(firstKickoff.toISOString())));
  const lastKo = new Date(slate[slate.length - 1].kickoff_at);
  return withExistingGuard(leagueSlug, {
    plan: {
      sport: leagueSlug,
...
/**
 * A PLAN IS ONLY A PLAN IF THE BOARD DOES NOT EXIST YET (relay 4 item 2).
 *
 * boardPlan()'s input is `min(kickoff_at) >= now`, which stays true for a
 * board that is ALREADY OPEN right up until its first game starts. So on
 * 7 Sep it dutifully re-planned CFB board 3 - a board with 24 games sitting
 * open in the table - and handed callers a plan whose opensAt was six days
 * in the past. /games and /pickem/cfb both printed "opens Tue Sep 1" off
 * it, for a board that had opened, while the board itself stayed
 * unreachable.
 *
 * The guard is the first kickoff, which is the sequence key: if a contest
 * of this sport already locks on that kickoff, that contest IS the board
 * and there is nothing to plan. Callers get { plan: null, existing, reason:
 * 'exists' } and must render the contest rather than an opening date.
 * ensurePickemBoard() reads it as "already there", which is the answer it
 * wanted anyway.
 */
async function withExistingGuard(sport, result) {
  if (!result?.plan) return result;
  const existing = await boardCovering({ sport, locksAt: result.plan.locksAt })
    .catch(() => null);
  if (!existing) return result;
  return { plan: null, existing, reason: 'exists' };
}
...
export async function ensurePickemBoard({ leagueSlug = 'cfb', now = new Date() } = {}) {
  const { plan, reason, existing: covered } = await boardPlan({ leagueSlug, now });
  // THE ID RIDES ALONG WHEN THE BOARD ALREADY EXISTS. boardPlan's new
  // existing-board guard (relay 4 item 2) answers reason:'exists' before
  // this function's own (season, week) check below ever runs, and that
  // check was where the id came from - so returning the bare reason
  // silently dropped it and every caller asking "which board is it then?"
  // got undefined. Same answer as the local check gives, from one branch
  // earlier.
  if (!plan) {
    return covered
      ? { id: covered.id, created: false, reason }
  ...
  // BOARD NUMBER, FROM THE ONE SEQUENCE KEY (relay 4 item 1). This used to
  // count `opens_at < plan.opensAt`, which numbered two boards 2 the moment
  // a week ran into Monday and both took the same tuesdayBefore(). It is
  // the first kickoff now, same as every other reader.
  const boardNumber = await boardNumberFor({
    sport: plan.sport, locksAt: plan.locksAt, id: r[0].id,
  });
  return { id: r[0].id, created: true, sport: plan.sport, week: plan.week, games: plan.board.length,
    locksAt: plan.locksAt.toISOString(), boardNumber };
}
