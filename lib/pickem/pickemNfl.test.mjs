...
test('nflBoardPlan derives (2026, week 1), all 16 games, no AP rule', async () => {
  ...
  // ROLLING LOCK: the row's locks_at is the LAST kickoff (the join-window
  // close); firstKickoff keys the clock and the reminders.
  assert.equal(plan.firstKickoff.toISOString(), '2026-09-10T00:20:00.000Z', 'first kickoff of the week');
  assert.equal(plan.locksAt.toISOString(), '2026-09-15T00:15:00.000Z', 'last kickoff of the week - the window close');
