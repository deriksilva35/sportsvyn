...
// getTotalsBoard() -> the MAIN goals line per scheduled match, as over/under
// rows for The Board. Main line = the line whose de-vigged over% is closest to
// even (50). Info-only: the page renders a muted dash for model/gap and a
// muted MARKET chip.
export async function getTotalsBoard({ leagueSlug = WC_LEAGUE_SLUG } = {}) {
  ...
  // Group over/under by (match, line); pick the line per match whose over% is
  // nearest 50.
  const pairByKey = new Map();
  for (const r of rows) {
    const key = `${r.match_id}|${r.line}`;
    if (!pairByKey.has(key)) pairByKey.set(key, { match_id: r.match_id });
  ...
  for (const pair of pairByKey.values()) {
    ...
    const dist = Math.abs(pair.over.market_pct - 50);
    const prev = bestByMatch.get(pair.match_id);
    if (!prev || dist < prev.dist) bestByMatch.set(pair.match_id, { dist, pair });
  }

  const shape = (r, selection) => ({
  ...
  const out = [];
  for (const { pair } of bestByMatch.values()) {
    out.push(shape(pair.over, 'over'), shape(pair.under, 'under'));
  }
  out.sort((a, b) => {
    const ta = new Date(a.kickoff_at).getTime();
    const tb = new Date(b.kickoff_at).getTime();
