...
// getBoardUpdatedAt() -> the max fetched_at across the CURRENT scheduled-match
// odds that feed the board. Powers the "Updated Nm ago" stamp (relative time is
// computed server-side; the page is force-dynamic).
export async function getBoardUpdatedAt({ leagueSlug = WC_LEAGUE_SLUG } = {}) {
  const rows = await sql`
