...
/**
 * Apply a reconcile decision (see lib/revenuecat.js reconcileFromRevenueCat).
 * Returns the user_id if a row changed, or null if nothing matched.
 *
 * Same one-statement CTE shape as the webhook writers, for the same reason: the
 * ledger entry and the membership change land together or not at all.
 *
 * THE REVOKE IS SCOPED TO source='apple' IN SQL. That is the guarantee that a
 * user who bought on the web keeps that grant no matter what RevenueCat says
 * about their Apple identity - it is not enforced by the caller's branching, it
 * is in the WHERE clause, so it holds even if the planner is wrong.
 *
 * Reconcile ledger rows use their own event ids (`reconcile:<user>:<iso>`), so
 * they never collide with a RevenueCat delivery id and a reconcile is always
 * distinguishable from a webhook when reading the table.
 */
export async function applyReconcile({ userId, action, expiresAt, productId = null, type, at = new Date() }) {
  const eventId = `reconcile:${userId}:${at.toISOString()}`;

  if (action === 'grant' || action === 'update') {
    const r = await sql`
      WITH claimed AS (
        INSERT INTO revenuecat_events (event_id, type, app_user_id, user_id, product_id, environment)
        VALUES (${eventId}, ${type}, ${String(userId)}, ${userId}, ${productId}, 'RECONCILE')
        ON CONFLICT (event_id) DO NOTHING
        RETURNING event_id
      )
      INSERT INTO memberships
        (user_id, status, kind, tier, expires_at, current_period_end, source, updated_at)
      SELECT ${userId}, 'active', 'pass', 'pass', ${expiresAt}::timestamptz, NULL, 'apple', now()
        FROM claimed
      ON CONFLICT (user_id) DO UPDATE SET
        status             = 'active',
        kind               = 'pass',
