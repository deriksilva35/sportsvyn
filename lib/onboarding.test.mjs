...
test('onboarded_at IS THE GATE - the sheet shows until it is finished', () => {
  assert.equal(needsOnboarding({ onboarded_at: null }), true);
  assert.equal(needsOnboarding({ onboarded_at: new Date() }), false);
});

test('THE HANDLE NO LONGER GATES THIS SHEET (relay item 3)', () => {
  // The handle moved to the first ranked entry (components/handle/
  // HandleGate.js), so this sheet does not write it and must not wait on
  // it. Pinned in BOTH directions, because getting this wrong either way
  // is a real outage: keyed to `handle` it would re-show forever to
  // anybody who finished the sheet without claiming, and it must equally
  // not skip somebody who happens to have a handle already but has never
  // been asked for a contact address.
  assert.equal(needsOnboarding({ handle: null, onboarded_at: new Date() }), false,
    'finished the sheet, no handle -> done, not shown again');
  assert.equal(needsOnboarding({ handle: 'sportsvyn_og', onboarded_at: null }), true,
    'has a handle, never onboarded -> still owed the rest of the sheet');
});

test('EXISTING USERS ARE CAUGHT BY THE SAME RULE - this is the backfill', () => {
  // The trigger is still a column, not a cookie, so every account that has
  // never finished the sheet sees it on next open without a migration or a
  // one-off script - and a second device does not re-prompt somebody who is
  // already done.
  const legacy = { id: 4, handle: null, onboarded_at: null, email: RELAY, contact_email: null, name: 'Old User' };
  assert.equal(needsOnboarding(legacy), true);
  assert.equal(needsOnboarding({ ...legacy, onboarded_at: new Date() }), false);
});
...
test('THE CLAIM UI IS REUSED, NOT REIMPLEMENTED - now from the first-entry gate', () => {
  // A second implementation is a second place for the denylist, the cooldown
  // and the uniqueness rules to drift. The reuse moved with step 1: the
  // sheet no longer claims a handle at all, the first-entry modal does.
  const gate = src('components/handle/HandleGate.js');
  assert.match(gate, /from '@\/components\/daily\/HandleClaim'/);
  assert.equal(/validateHandle/.test(gate), false, 'validation stays in the claim component');

  const sheet = src('components/onboarding/OnboardingSheet.js');
  assert.equal(/HandleClaim/.test(sheet), false, 'the sheet no longer claims a handle');
  assert.equal(/validateHandle/.test(sheet), false);
});

test('THE SHEET RENUMBERS FROM WHAT IS LEFT (relay item 3)', () => {
  const sheet = src('components/onboarding/OnboardingSheet.js');
  // email, name, and push where it can be offered - so 2, or 3 with push.
  assert.match(sheet, /const totalSteps = offerPush \? 3 : 2;/);
  assert.equal(/Step 4 of/.test(sheet), false, 'no step 4 survives the handle leaving');
  for (const n of [1, 2, 3]) {
    assert.ok(sheet.includes(`Step ${n} of {totalSteps}`), `step ${n} is numbered`);
  }
});
