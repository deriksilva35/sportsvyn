...
test('the live board never selects lineups onto the wire', () => {
  ...
  // The read pulls lineup to SCORE it; the returned rows carry only the
  // aggregate - rank / userId / name / total / played - plus the house mark,
  // which is a fact about the ACCOUNT and not about anybody's lineup.
  //
  // THE LAW IS THE KEYS, NOT THEIR FORMATTING. This used to pin the literal
  // one-line return and broke the first time the object gained a field; what
  // it exists to prevent is a lineup reaching the wire, so that is what it
  // asserts now.
  const ret = board.slice(board.indexOf('return {'), board.indexOf('};', board.indexOf('return {')));
  for (const key of ['userId:', 'name:', 'total:', 'played:']) {
    assert.ok(ret.includes(key), `the board row lost ${key}`);
  }
  assert.ok(!/lineup/.test(ret), 'a lineup key on a board row is a leak');
  assert.ok(!/\bmeta\b/.test(ret), 'nor a meta blob, which carries the draft roster');
});
...
test('both locked views label the number honestly - the number will move', () => {
  // THE RULE: a locked view shows a total that is not the ruling, and it has
  // to say so. WHERE it says so moved for the Draft at v2 - the page's
  // score-row caption became the card's own header and strip - so this checks
  // each view where its caption now lives rather than assuming one wording.
  assert.match(src('app/weekly/page.js'), /before drop-worst/,
    'the Weekly still labels its live total on the page');
  const card = src('components/draft/DraftLiveCard.js');
  assert.match(card, /count` : `\$\{rows\.length\} picks in`\} &middot; worst two dropped/,
    "the Draft's card says how many of the eight count, once any of them has played");
  assert.match(card, /the worst\s+two drop/, 'and states the rule under the strip');
  assert.match(card, /graded Tuesday/, 'and when the ruling arrives');
});
