...
// Weekly spends DAYS there - first kickoff to Tuesday settle - and it is
 ...
 */
export function weeklyHomeView({ contest = null, entry = null, now = new Date(), firstKickoff = null } = {}) {
  ...
  // ---- SETTLED -------------------------------------------------------------
  if (contest.settled) {
    // {score, players} - the ceiling's stored shape (relay D1); see
    // lib/weekly/settle.js.
  ...
  if (entry && filled > 0) {
    return { ...base, state: 'building', filled, remaining: SLOTS.length - filled, locksAt: contest.locks_at, firstKickoff };
  }

  // ---- PLAY: no entry, or an entry with nothing in it ----------------------
  return { ...base, state: 'play', locksAt: contest.locks_at, firstKickoff };
}
