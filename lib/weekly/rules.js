...
// then it is not. One lock moment, all six slots. A player who starts someone
// in the week's opening game is carrying that risk knowingly - it is stated in
// the rules, and it is the same trade every real league makes.
//
// THE LAW IS "FIRST KICKOFF", NEVER A WEEKDAY. Thursday is a habit, not a rule,
// and 2026 Week 1 is the exhibit: it opens on WEDNESDAY, Sep 9, 8:20pm ET. Any
// copy, comment or comparison that names a day instead of reading locks_at is
// wrong for that week and for every rescheduled game after it. The stored
// locks_at renders the actual day; nothing hardcodes one.
//
// LOCKS_AT IS SNAPSHOTTED AT BOARD CREATION AND DOES NOT CHASE. If the opening
// game moves after the board opens, the deadline does not move with it. The deadline players planned around IS the deadline; a lock that slides
...
/**
 * THE SLOT LOCKS AT ITS PLAYER'S KICKOFF (rolling lock, R3). saveVerdict is
 * the contest's join-window close; this is the per-slot rule. A slot is
 * locked when the player IN it has kicked off; an empty slot is open until
 * the window closes, but can only take a player whose team has not kicked.
 * `kickoffOf(playerId)` -> ISO or null (null = unknown, treated as open).
 */
export function slotVerdict(playerId, kickoffOf, now = new Date()) {
  if (playerId == null) return { ok: true, empty: true };
  const ko = kickoffOf(playerId);
  const t = new Date(ko ?? NaN).getTime();
  if (!Number.isFinite(t)) return { ok: true, kickoffAt: null };
  if (now.getTime() >= t) return { ok: false, reason: 'slot_locked', kickoffAt: ko };
  return { ok: true, kickoffAt: ko, msLeft: t - now.getTime() };
}

/**
 * Which of `incoming`'s slot writes are legal against `existing`, per slot.
 * A slot whose existing player has kicked cannot change (or clear); a slot
 * cannot take a player who has kicked. Returns the lineup to persist (the
 * accepted writes on top of the existing lineup) and the rejected slots.
 */
export function mergeSlotWrites(existing, incoming, kickoffOf, now = new Date()) {
  const out = { ...(existing ?? {}) };
  const rejected = [];
  for (const slot of SLOTS) {
    const was = existing?.[slot] ?? null;
    const next = incoming?.[slot] ?? null;
    if (was === next) continue;
    const held = slotVerdict(was, kickoffOf, now);
    if (!held.ok) { rejected.push({ slot, playerId: was, kickoffAt: held.kickoffAt, why: 'held' }); continue; }
    const wants = slotVerdict(next, kickoffOf, now);
    if (!wants.ok) { rejected.push({ slot, playerId: next, kickoffAt: wants.kickoffAt, why: 'kicked' }); continue; }
    if (next == null) delete out[slot]; else out[slot] = next;
  }
  return { lineup: out, rejected };
}
