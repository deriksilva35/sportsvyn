...
/**
 * "24/33, 312 yds, 3 TD, 0 INT" per player, for the settled grade rows
 * (relay 2b-fix item 1). Map(nfl_players.id -> string), a player with
 * nothing this formatter covers simply absent.
 *
 * THE DAILY'S OWN FORMATTER, NOT A SECOND ONE. lib/daily/reveal.js's
 * statLine() already renders exactly this shape for the Daily's reveal
 * (and lib/daily/close.js's withLines() already pairs it with
 * toStatLine() the same way) - a Weekly-specific copy would be a second
 * place for "how a stat line reads" to drift.
 *
 * SCOPED TO THE PLAYERS ON SCREEN, not the whole week. A grade card shows
 * at most your six or eight plus the comparison roster's - about a dozen
 * ids - so this reads a dozen rows rather than the ~1,000 weekScores()
 * pulls for the settle itself.
 *
 * READ AT RENDER, NOT STORED AT SETTLE. The board jsonb is written once,
 * at settle, and SETTLED IS FINAL - adding a field to it now would leave
 * every already-settled contest without one. The stat rows behind this
 * are the same rows the settle scored from and do not move.
 */
export async function weekStatLines(season, week, playerIds) {
  const ids = [...new Set((playerIds ?? []).filter((v) => v != null).map(Number))];
  if (!ids.length) return new Map();
  const rows = await sql`
    SELECT s.nfl_player_id AS id, np.position AS pos,
           s.pass_cmp, s.pass_att, s.pass_yds, s.pass_td, s.pass_int,
           s.rush_att, s.rush_yds, s.rush_td,
           s.tgt, s.rec, s.rec_yds, s.rec_td, s.fumbles_lost,
           s.fgm, s.fga, s.fg_long, s.xp
     ...
     WHERE l.slug = 'nfl' AND m.season_year = ${season}
       AND m.season_phase = 'REG' AND m.week = ${week}
       AND s.nfl_player_id = ANY(${ids})`;
  const { toStatLine } = await import('../fantasy/playerStats.js');
  const { statLine } = await import('../daily/reveal.js');
  const out = new Map();
  for (const r of rows) {
    const line = statLine(r.pos, toStatLine(r));
