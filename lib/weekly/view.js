...
/**
 * THE POOL COUNT, RENDERED ONE WAY (relay 2b-fix item 4).
 *
 * Three surfaces state the size of the Weekly's pool and all three mean the
 * identical quantity - the length of the contest's own board snapshot:
 *
 *   the builder's search box   components/weekly/WeeklyRoom.js  board.length
 *   the settled perf box       components/weekly/WeeklyGrade.js board.length
 *   the lobby's "six from N"   lib/games/read.js                SQL
 *                                              jsonb_array_length(board)
 *
 * The lobby counts in SQL rather than loading the board, deliberately - it
 * is a 2,000-plus-player jsonb blob and the lobby drops it everywhere else
 * for exactly that reason - but jsonb_array_length(board) and board.length
 * are the same number by construction, so the SOURCE was never the problem.
 * The RENDERING was: two surfaces called .toLocaleString() and the perf box
 * did not, so the same pool read "2,298" in the builder and "2298" in the
 * grade. One function now, called by all three.
 */
export function poolCountLabel(n) {
...
export function settledView({ contest, entry, board }) {
  ...
  const picks = entry
    ? SLOTS.map((slot) => {
      ...
      return {
        slot, label: slot,
        id: p?.id ?? null, name: p?.name ?? null, pos: p?.pos ?? null, team: p?.team ?? null,
  ...
  // label MIRRORS slot - perfectLineup() (lib/daily/reveal.js) stores the
  // ceiling's own picks with a `slot` field already, from the SAME SLOTS
  // array `picks` above walks; adding `label` here is what lets
  // lib/games/gradePairing.js's pairRows() compare "your slot" against
  // "the ceiling's slot" without knowing which side is which.
  const perfectPicks = (contest?.perfect?.players ?? []).map((p) => ({ ...p, label: p.slot }));
...
export function poolRows(board, slot, query = '') {
  ...
  return (board ?? [])
    ...
    .sort(bySeasonThenCareer);
}

/**
 * THIS SEASON FIRST, NULLS LAST, CAREER AS THE TIEBREAK (ruled).
 *
 * The panel is a list of picks for the week being played, so the number that
 * orders it is what a player has done THIS season - not the career rate frozen
 * into his resume, which put a 120-game veteran above a back in the middle of
 * a career year. A player with no final game yet has no season number and
 * sorts LAST: a blank is not a zero and it is certainly not a lead.
 *
 * THE CAREER RATE IS STILL THE TIEBREAK, which is what keeps week 1 sane. In
 * the first week almost nobody has a season number, so without a second key
 * the whole board would fall back to database order - the 153-quarterbacks-in-
 * insertion-order haystack poolRows exists to prevent. Name breaks a true tie
 * so the order is stable across renders.
 */
function bySeasonThenCareer(a, b) {
  const sa = seasonPpgOf(a);
  const sb = seasonPpgOf(b);
  if (sa !== sb) return sb - sa;
  const ca = ppgOf(a.resume);
  const cb = ppgOf(b.resume);
  if (ca !== cb) return cb - ca;
  return String(a.name ?? '').localeCompare(String(b.name ?? ''));
}

/**
 * A row's season PPG as a number; -1 when it has none.
 *
 * -1, NOT 0, for the same reason ppgOf uses it: a player who has not played
 * is not a player who scored nothing, and a NaN comparison would strand the
 * row wherever it happened to be.
 */
export function seasonPpgOf(row) {
  const v = Number(row?.season?.ppg);
 ...
 *
 * EXPORTED because the house entries rank on it too (lib/house/pickWeekly.js).
 * A second parser for the same string would be a second answer to "how good is
 * this player", and the two would drift the first time the resume changes.
 */
export function ppgOf(resume) {
  if (!resume) return -1;
