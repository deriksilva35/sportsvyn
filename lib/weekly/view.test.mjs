...
const CONTEST = {
  season_year: 2026, week: 1, settled: true, perfect: { score: 120.0, players: [] },
};
...
const { poolRows, seasonPpgOf } = await import('./view.js');
...
test('THIS SEASON SORTS THE PANEL, NULLS LAST, CAREER AS THE TIEBREAK', () => {
  // The rule that replaced a career-only sort: a career rate frozen at board
  // creation is not a fact about the week being played.
  const row = (name, career, ppg) => ({
    id: name, name, pos: 'QB', team: 'KC',
    resume: `${career.toFixed(1)} PPG · 40 g`,
    season: ppg == null ? null : { ppg, gp: 1, line: 'x' },
  });
  const board = [
    row('Career king', 30.0, null),   // best career, no season -> LAST
    row('Season king', 10.0, 31.2),   // worst career, best season -> FIRST
    row('Middle', 20.0, 19.9),
    row('Other blank', 25.0, null),   // no season either, but a better career
  ];
  assert.deepEqual(poolRows(board, 'QB').map((p) => p.name),
    ['Season king', 'Middle', 'Career king', 'Other blank']);
  // THE TIEBREAK IS LOAD-BEARING IN WEEK 1, when almost nobody has a season
  // number: without it the two blanks would fall back to database order, which
  // is the 153-quarterbacks-in-insertion-order haystack this sort exists to
  // prevent. Career king (30.0) leads Other blank (25.0).
});

test('a true tie is broken by name, so the order is stable across renders', () => {
  const same = (name) => ({
    id: name, name, pos: 'WR', team: 'KC', resume: '10.0 PPG · 10 g',
    season: { ppg: 12.0, gp: 1, line: 'x' },
  });
  assert.deepEqual(poolRows([same('Zeta'), same('Alpha'), same('Mid')], 'WR').map((p) => p.name),
    ['Alpha', 'Mid', 'Zeta']);
});

test('seasonPpgOf is -1 for a row with no season, never 0', () => {
  assert.equal(seasonPpgOf({ season: { ppg: 12.4 } }), 12.4);
  assert.equal(seasonPpgOf({ season: null }), -1);
  assert.equal(seasonPpgOf({}), -1);
  assert.equal(seasonPpgOf(null), -1);
  assert.equal(seasonPpgOf({ season: { ppg: 0 } }), 0, 'a real zero is a real zero');
  assert.equal(seasonPpgOf({ season: { ppg: 'x' } }), -1);
});
