 ...
 */
export async function liveBoard(contest, { limit = 10, memberIds = null } = {}) {
  const { scored, playedIds } = await liveScoredBoard(contest);
  // memberIds scopes to a player league (073). null = the global board; an
  // EMPTY league gets an empty board, not the world's - hence the explicit
  // ANY filter rather than a shortcut on falsy.
  const entries = memberIds == null
    ? await sql`
      SELECT e.user_id, e.lineup, e.meta, u.handle, u.is_house
  ...
  const rows = entries.map((e) => {
    ...
    return {
      userId: e.user_id, name: displayName({ id: e.user_id, handle: e.handle }),
      total: v.total, played: v.playedCount,
      ...houseMark({ isHouse: e.is_house === true, handle: e.handle }, 'weekly'),
    };
  })
 ...
 */
export async function weeklyBoardTable(contest, uid = null, { limit = 10, now = new Date(), memberIds = null } = {}) {
  ...
  if (contest.settled) {
    ...
    // liveBoard already marked them; carry the mark rather than re-deriving it.
    rows = (await liveBoard(contest, { limit: 1000, memberIds }))
      .map((r) => ({
        rank: r.rank, userId: r.userId, name: r.name, points: r.total,
