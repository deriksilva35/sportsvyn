...
export async function weeklyBoardPlan({ now = new Date() } = {}) {
  ...
  // ROLLING LOCK: the Weekly is born with locks_at = the week's LAST kickoff
  // (the join-window close); `ko` stays the first kickoff for the Draft's room
  // lock and for reminders.
  return { plan: { season, week, ko, firstKickoff: ko, lastKickoff: last ?? ko, opens, settles, board }, reason: null };
}
...
export async function ensureWeek(season = null, week = null, { sport = 'nfl', opensAt = null, now = new Date() } = {}) {
  ...
  const stmts = [sql`
    ...
    VALUES ('weekly', ${sport}, ${plan.season}, ${plan.week}, ${JSON.stringify(plan.board)}::jsonb,
            ${opens.toISOString()}, ${new Date(plan.lastKickoff).toISOString()}, ${plan.settles.toISOString()})
