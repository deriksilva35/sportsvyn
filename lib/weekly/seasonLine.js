...
export async function seasonStats(season) {
  const totals = await seasonTotals(season);
  const out = new Map();
  for (const [id, t] of totals) {
    out.set(id, {
      gp: t.gp,
      ppg: Math.round(fantasyPoints(t.totals, 'ppr') / t.gp * 10) / 10,
      line: seasonLineFor(t.pos, t.totals),
    });
  }
  return out;
}
...
/**
 * THE SAME SCAN, ONE STEP EARLIER: summed components per player, unscored.
 *
 * TWO SURFACES READ THIS SEASON AND THEY MUST AGREE. The Weekly's panel wants
 * a PPR rate and a prose line; the draft room wants the component totals (its
 * stat tokens print receptions and yards) and a rate in the ROOM'S scoring
 * format, which can be half-PPR or standard. Both are the same sum over the
 * same games - so the sum lives here, once, and each caller scores it with the
 * one scorer in the format it is asking about. Before this, the room had its
 * own query with `const SEASON_YEAR = 2025` hardcoded in it, which is why the
 * Draft showed Kenneth Walker at 11.3 while the Weekly showed 34.1.
 *
 * @param {number|string} season
 * @param {{playerIds?: number[]}} [opts] narrow to these nfl_players ids
 * @returns {Promise<Map<number, {gp: number, pos: string|null, totals: object}>>}
 */
export async function seasonTotals(season, { playerIds = null } = {}) {
  if (season == null) return new Map();
  if (playerIds != null && playerIds.length === 0) return new Map();
  const rows = await sql`
    SELECT s.nfl_player_id                             AS id,
     ...
     WHERE l.slug = 'nfl'
       ...
       AND m.status = 'final'
       ${playerIds == null ? sql`` : sql`AND s.nfl_player_id = ANY(${playerIds})`}
  ...
  for (const r of rows) {
    ...
    out.set(Number(r.id), { gp, pos: r.pos ?? null, totals: toStatLine(r) });
  }
  return out;
}
