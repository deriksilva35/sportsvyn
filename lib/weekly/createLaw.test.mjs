...
test('past the open it creates - derived week, derived lock - then says exists', async () => {
  ...
  // ROLLING LOCK (R1): the Weekly is born with locks_at = the week's LAST
  // kickoff, the join-window close - derived from matches, not a caller number.
  assert.equal(new Date(row.locks_at).toISOString(), '2026-09-22T00:15:00.000Z',
    'week 2 last kickoff - the join-window close');
  ...
  // the Draft keeps the room lock at the FIRST kickoff (untouched until ruled)
  assert.equal(new Date(draftRow.locks_at).toISOString(), '2026-09-18T00:15:00.000Z', 'draft: week 2 first kickoff');
  assert.ok(weeklyRow.locks_at.getTime() > draftRow.locks_at.getTime(), 'weekly closes after the draft room locks');
