...
export async function settleContest(contestId, { now = new Date() } = {}) {
  ...
  for (const e of entries) {
    ...
    const room = isDraft && e.meta?.draftId != null
      ? await roomStandings(Number(e.meta.draftId), scored).catch(() => null)
      : null;
    ...
    await sql`
      UPDATE contest_entries
       ...
       WHERE id = ${e.id}`;
    counted += 1;
    scoredEntries.push({ id: e.id, userId: e.user_id, score: b.baseScore });
  }
  ...
  if (isDraft) {
    if (scoredEntries.length) {
      const top = scoredEntries.reduce((a, b2) => (b2.score > a.score ? b2 : a));
