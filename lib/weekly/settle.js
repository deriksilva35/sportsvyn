...
export async function settleContest(contestId, { now = new Date() } = {}) {
  ...
  // THE DRAFT'S LOCK-TIME BRIDGE, run here as a self-heal before anything is
  // scored. A room that finished after the tab closed would otherwise arrive
  // with no roster and settle as a DNF - failing a player for our missed write
  // rather than for their draft. It only fills blanks; an entry that already
  // has a roster is untouched, so a replay still matches the original.
