...
before(async () => {
  // THE NEWEST REAL SEASON, NOT THE NEWEST ROW. Other test files fabricate
  // finals in far-future seasons - lib/weekly/replay.test.mjs writes a whole
  // 2091 week - and node --test runs files in parallel, so `max(season_year)`
  // picked a fixture out from under this one. A real season has hundreds of
  // players in it; a fixture has a handful.
  const [s] = await sql`
