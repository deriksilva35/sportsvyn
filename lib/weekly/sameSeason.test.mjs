...
before(async () => {
  // THE NEWEST REAL SEASON, NOT THE NEWEST ROW, AND "REAL" IS A DATE BOUND.
  //
  // The first version took max(season_year) and lib/weekly/replay.test.mjs
  // picked it out from under this one: node --test runs files in parallel and
  // replay writes a whole season-2091 week. The second version added
  // "HAVING count(DISTINCT player) >= 200" on the theory that a fixture has a
  // handful of players - AND THAT WAS WRONG. replay does not invent a week, it
  // COPIES a real one: sixteen games and every stat line, which is four
  // hundred-odd distinct players. It sailed past 200 and this test failed
  // again, intermittently, in chunk B.
  //
  // THE BOUND THAT CANNOT BE FOOLED IS THE CALENDAR. A real NFL season cannot
  // be newer than next year; every fabricated season in this suite is 2090+.
  // The 200 stays as a second filter - it excludes a half-loaded real season -
  // but the year is what keeps a fixture out, and it keeps meaning in 2027.
  const [s] = await sql`
    SELECT m.season_year AS y
     ...
     WHERE l.slug = 'nfl' AND m.season_phase = 'REG' AND m.status = 'final'
       AND m.season_year <= extract(year FROM now())::int + 1
     GROUP BY m.season_year
    HAVING count(DISTINCT s.nfl_player_id) >= 200
