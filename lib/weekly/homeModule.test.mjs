...
test('EVERY STATE HAS AN EXACT KEY SET, and a new field has to be argued for', () => {
  ...
  const settled = { ...OPEN, settled: true, perfect: { score: 140 } };
  const cases = [
    // firstKickoff ARGUED IN (rolling lock, R4): before the first kickoff the
    // deadline that matters is the first game, after it the window close.
    [{ contest: OPEN, entry: null, now: during }, ['season', 'week', 'href', 'state', 'locksAt', 'firstKickoff']],
    [{ contest: OPEN, entry: { lineup: SIX }, now: during },
      ['season', 'week', 'href', 'state', 'filled', 'remaining', 'locksAt', 'firstKickoff']],
    [{ contest: OPEN, entry: { lineup: SIX }, now: afterLock },
