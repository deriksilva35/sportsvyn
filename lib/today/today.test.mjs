...
// COMMENTS ARE NOT EVIDENCE. Every assertion below that reads a source file
// reads it stripped, so a rule described in a comment and absent from the code
...
test("'my' SCOPE RESOLUTION IS UNCHANGED - the pin", () => {
  ...
  // THE DEFAULT SET CHANGED BY RULING in the My Sportsvyn Phase 1 relay; the
  // RESOLUTION SEMANTICS this test exists to pin did not. A league id is still
  // not a panel, an unbound panel is still rejected on write, and the defaults
  // still come from the registry rather than from this file.
  assert.deepEqual(my.defaults().map((e) => e.id),
    ['contests', 'pickem', 'fantasy', 'movers', 'today', 'live', 'watch',
     'rankings', 'schedule', 'players']);
  // The retired WC ids must not resolve at all.
  for (const dead of ['goldenboot', 'groups', 'mentioned', 'market', 'form', 'storylines']) {
    assert.equal(my.isValidRead(dead), false, `${dead} is retired`);
  }
});
...
test("'today' scope speaks leagues, and drops ids that stop meaning anything", () => {
  ...
  // 'mlb' USED TO BE THE JUNK EXAMPLE HERE and is now a league: MLB B1 item 6
  // registered it with a band (components/today/LeagueBands.js DiamondBand).
  // 'nba' takes its place as the sport we do not carry.
  for (const junk of ['nba', 'today', '', null, 42]) assert.equal(t.isValidRead(junk), false, String(junk));
  assert.equal(t.isValidRead('mlb'), true);
  assert.deepEqual(t.defaults().map((e) => e.id), ['cfb', 'nfl', 'mlb', 'epl']);
...
test('THE SWITCHER CARRIES ITS OWN STYLESHEET, so both routes dress it', () => {
  // It shipped unstyled on /my - raw text links, no pills - because the rules
  // lived in app/home.css, which `/` imports and `/my` does not. Nothing on DEV
  // could have caught it: /my redirects to /signin when signed out, so every
  // served check of that route compared two redirect stubs.
  //
  // The fix is ownership, not duplication: the component imports its own file,
  // so the styles follow it onto any route. Two pasted blocks in home.css and
  // my.css would be two things to keep in step.
  const sw = src('components/today/ModeSwitch.js');
  assert.match(sw, /import '\.\/modeswitch\.css';/);
  const css = src('components/today/modeswitch.css');
  for (const sel of ['.modeswitch', '.modewrap', '.mode', '.mode.on', '.modesub']) {
    assert.ok(css.includes(sel), `${sel} must live with the component`);
  }
  // And NOT left behind in the page stylesheet, where it would drift.
  const home = src('app/home.css').replace(/\/\*[\s\S]*?\*\//g, '');
  for (const sel of ['.modewrap{', '.mode.on{', '.modesub{']) {
    assert.ok(!home.includes(sel), `${sel} must not be duplicated in home.css`);
  }
});

test('the nav no longer carries a second way into My Sportsvyn', () => {
  // THERE WERE TWO, and the first pass only found one. The account menu had a
  // 'My Sportsvyn' entry AND GlobalHeader rendered a literal MY SPORTSVYN link
  // in both the desktop bar and the mobile drawer. Removing the menu entry left
  // the header link live on every page - caught on PROD, not on DEV, because
  // the account menu is signed-in only and the signed-out page I checked could
  // never have shown it. This pins all three.
  const strip = (f) => src(f).replace(/\/\*[\s\S]*?\*\//g, '')
    .replace(/^\s*\/\/.*$/gm, '').replace(/\{\/\*[\s\S]*?\*\/\}/g, '');
  const nav = strip('lib/nav.js');
  assert.doesNotMatch(nav, /label: 'My Sportsvyn'/);
  assert.match(nav, /label: 'Account'/, 'the rest of the account menu is untouched');
  const header = strip('components/GlobalHeader.js');
  assert.doesNotMatch(header, /MY SPORTSVYN/, 'neither the bar nor the drawer');
  // The switcher is the one remaining way in.
  assert.match(src('lib/today/modes.js'), /label: 'My Sportsvyn'/);
});
...
test('the archive band is OFF by default and the chips carry the default set', () => {
  // 3 -> 4 with MLB (B1 item 6). A chip is only registered WITH a band; the
  // dead "MLB 2027" chip this replaces toggled nothing.
  assert.deepEqual([...DEFAULT_TODAY_LEAGUES], ['cfb', 'nfl', 'mlb', 'epl']);
...
test('A CHIP IS ONLY REGISTERED WITH A BAND, and MLB has one', async () => {
  // THE DEAD "MLB 2027" CHIP IS WHAT THIS RULE IS NAMED AFTER: a control that
  // visibly did nothing, because bandFor() falls through to <ArchiveBand /> on
  // any id it does not know. Every registered league must own a branch there.
  const page = strip(src('app/page.js'));
  for (const id of DEFAULT_TODAY_LEAGUES) {
    assert.match(page, new RegExp(`id === '${id}'`), `${id} has a band branch`);
  }
  assert.match(page, /<DiamondBand /);
  // ITS SLATE IS A DAY, NOT A WEEK. weekSlate groups on matches.week and every
  // MLB row carries week NULL, so weekSlate('mlb') is null on every day of the
  // season and the band would render empty - see lib/today/daySlate.js.
  assert.match(page, /daySlate\('mlb'/);
  assert.doesNotMatch(page, /weekSlate\('mlb'/);
  // AND ITS OWN TIER. The gridiron preference rule is a statement about
  // football in autumn; MLB must neither claim that preference nor be sorted
  // by a rule written for soccer.
  assert.equal(LEAGUES.find((l) => l.id === 'mlb').tier, TIER.DIAMOND);
  assert.notEqual(TIER.DIAMOND, TIER.GRIDIRON);
  assert.notEqual(TIER.DIAMOND, TIER.SOCCER);
});

test('the MLB band names a day and links to the MLB board', () => {
  const bands = strip(src('components/today/LeagueBands.js'));
  // BOUNDED AT THE NEXT export. An unbounded slice runs into EplBand and
  // inherits its table module, which is the one thing this test denies.
  const from = bands.indexOf('export function DiamondBand');
  const dia = bands.slice(from, bands.indexOf('export function ', from + 10));
  assert.match(dia, /title="Today"/, 'not "This week" - baseball has no weeks');
  assert.match(dia, /scoresHref="\/scores\?sport=mlb"/);
  // NO TABLE MODULE. There is no MLB standings reader in this tree, and a
  // module frame over a query that does not exist is a "coming soon".
  assert.doesNotMatch(dia, /<Rows /);
  // AND THE ROW LINKS SOMEWHERE. SlateRow renders a non-link for a league it
  // has no route for, which is how a slate of sixteen dead rows would ship.
  assert.match(src('components/slate/SlateRow.js'), /mlb: '\/mlb\/game'/);
});
