...
test('PRE IS LABELLED, and labelling it is not the same as ranking it', () => {
  ...
  // The row renders the badge - now from the SHARED component, which is where
  // the srow markup moved so /my could speak the same grammar. The law is
  // unchanged and so is the string; only its address moved.
  assert.match(strip(src('components/slate/SlateRow.js')), /isPreseason \? 'PRE · ' : ''/);
...
test('ONE LIVE DECISION-MAKER, not two renderers', () => {
  ...
  // WeekSlate still owns the ORDER and the CAP; the shared row owns the state
  // read. Three surfaces render these games now (front page, /my, the band) and
  // exactly one module decides whether a game is live.
  assert.match(src('components/today/WeekSlate.js'), /import \{ orderSlate, SLATE_ROW_CAP \}/);
  assert.match(src('components/slate/SlateRow.js'), /import \{ rowState \} from '@\/lib\/today\/slateRow'/);
  // /my's game panels read the same one, through the same component.
  assert.match(src('components/my/panels.js'), /import SlateRow from '@\/components\/slate\/SlateRow'/);
  assert.match(src('components/my/WatchBoard.js'), /import SlateRow from '@\/components\/slate\/SlateRow'/);
